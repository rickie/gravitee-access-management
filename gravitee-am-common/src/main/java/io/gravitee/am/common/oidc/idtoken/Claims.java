...
public interface Claims extends io.gravitee.am.common.jwt.Claims {
     ...
     * the Claim Value being the nonce value sent in the Authentication Request. Authorization
     ...
     * not meet the requirements of ISO/IEC 29115 [ISO29115] level 1. Authentication using a
     * long-lived browser cookie, for instance, is one example where the use of "level 0" is
     * appropriate. Authentications with level 0 SHOULD NOT be used to authorize access to any
     * resource of any monetary value. (This corresponds to the OpenID 2.0 PAPE [OpenID.PAPE]
     * nist_auth_level 0.) An absolute URI or an RFC 6711 [RFC6711] registered name SHOULD be used
     * as the acr value; registered names MUST NOT be used with a different meaning than that which
     * is registered. Parties using this claim will need to agree upon the meanings of the values
     ...
     * both password and OTP authentication methods were used. The definition of particular values
     * to be used in the amr Claim is beyond the scope of this specification. Parties using this
     ...
     * be included even when the authorized party is the same as the sole audience. The azp value is
     ...
     * Header. For instance, if the alg is RS256, hash the access_token value with SHA-256, then
     * take the left-most 128 bits and base64url encode them. The at_hash value is a case sensitive
     * string. If the ID Token is issued from the Authorization Endpoint with an access_token value,
     * which is the case for the response_type value code id_token token, this is REQUIRED;
     * otherwise, its inclusion is OPTIONAL.
     */
    String at_hash = "at_hash";

    /**
     * Code hash value. Its value is the base64url encoding of the left-most half of the hash of the
     * octets of the ASCII representation of the code value, where the hash algorithm used is the
     * hash algorithm used in the alg Header Parameter of the ID Token's JOSE Header. For instance,
    ...
    /**
     * State hash value. Its value is the base64url encoding of the left-most half of the hash of
     * the octets of the ASCII representation of the state value, where the hash algorithm used is
     * the hash algorithm used in the alg header parameter of the ID Token's JOSE header. For
