 ...
 *
 * <p>See <a href="https://openid.net/specs/openid-connect-core-1_0.html#ClientAuthentication">9.
 * Client Authentication/a> See <a href="https://tools.ietf.org/html/rfc8705#section-2">2. Mutual
 * TLS for OAuth Client Authentication</a>
 *
...
public interface ClientAuthenticationMethod {
    ...
    /**
     * Clients that have received a client_secret value from the Authorization Server authenticate
     * with the Authorization Server in accordance with Section 2.3.1 of OAuth 2.0 [RFC6749] using
    ...
    /**
     * Clients that have received a client_secret value from the Authorization Server, authenticate
     * with the Authorization Server in accordance with Section 2.3.1 of OAuth 2.0 [RFC6749] by
     * including the Client Credentials in the request body.
     */
    String CLIENT_SECRET_POST = "client_secret_post";

    /**
     * Clients that have received a client_secret value from the Authorization Server create a JWT
     * using an HMAC SHA algorithm, such as HMAC SHA-256. The HMAC (Hash-based Message
     * Authentication Code) is calculated using the octets of the UTF-8 representation of the
     * client_secret as the shared key.
     */
