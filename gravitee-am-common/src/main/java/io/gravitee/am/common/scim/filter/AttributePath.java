...
public class AttributePath {
    /**
     * A regular expression to match the components of an attribute path. Given the path
     * "urn:scim:schemas:core:user:1.0:name.familyName" then group 2 will match
     * "urn:scim:schemas:core:user:1.0", group 3 matches "name" and group 5 matches "familyName".
     */
     ...
     * @param subAttributeName The name of the sub-attribute, or {@code null} if absent.
     */
     ...
     * @param defaultSchema The default schema to assume for attributes that do not have the schema
     *     part of the urn specified. The 'id', 'externalId', and 'meta' attributes will always
     *     assume the SCIM Core schema.
     * @return The parsed attribute path.
     */
    public static AttributePath parse(final String path, final String defaultSchema) {
        ...
        if (attributeSchema != null) {
            ...
            if (attributeName.equalsIgnoreCase(CommonAttribute.ID)
                    || attributeName.equalsIgnoreCase(CommonAttribute.EXTERNAL_ID)
                    || attributeName.equalsIgnoreCase(CommonAttribute.META)) {
                return new AttributePath(Schema.SCHEMA_URI_CORE, attributeName, subAttributeName);
            } else {
                return new AttributePath(defaultSchema, attributeName, subAttributeName);
            }
        }
    }
