...
public class HttpUserProvider implements UserProvider {
    ...
    public Single<User> create(User user) {
        try {
            ...
            LOGGER.error(
                    "An error has occurred while creating the user {}", user.getUsername(), ex);
            return Single.error(
                    new TechnicalManagementException(
                            "An error has occurred while creating the user", ex));
        }
    }
    ...
    public Single<User> update(String id, User updateUser) {
        try {
            ...
            LOGGER.error(
                    "An error has occurred while updating the user {}",
                    updateUser.getUsername(),
                    ex);
            return Single.error(
                    new TechnicalManagementException(
                            "An error has occurred while updating the user", ex));
        }
    }
    ...
    public Completable delete(String id) {
        try {
            ...
            return Completable.error(
                    new TechnicalManagementException(
                            "An error has occurred while deleting the user", ex));
        }
    }
    ...
    private Maybe<User> findByUser(
        ...
        try {
            ...
            return requestHandler
                    ...
                    .map(
                            httpResponse -> {
                                ...
                                return convert(
                                        null,
                                        userAttributes); // on find by user we do not force the
                                                         // username and use the payload as source
                                                         // of truth.
                            })
                    .onErrorResumeNext(
                            ex -> {
            ...
            return Maybe.error(
                    new TechnicalManagementException(
                            "An error has occurred while searching the user", ex));
        }
    }
        ...
        final String usernameValue =
                (username != null)
                        ...
                        : (mappedUserAttributes.get(usernameAttribute) != null)
                                ? String.valueOf(mappedUserAttributes.get(usernameAttribute))
                                : (userAttributes.get(usernameAttribute) != null)
                                        ? String.valueOf(userAttributes.get(usernameAttribute))
                                        : id;
