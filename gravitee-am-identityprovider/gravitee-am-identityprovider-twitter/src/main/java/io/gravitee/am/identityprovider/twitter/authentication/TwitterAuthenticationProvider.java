...
public class TwitterAuthenticationProvider
        extends AbstractSocialAuthenticationProvider<TwitterIdentityProviderConfiguration> {
    ...
    @Autowired private TwitterIdentityProviderConfiguration configuration;
    ...
    protected final Cache<String, String> tokenMemory =
        ...
        try {
            ...
            String authorization =
                    getAuthorizationHeader(
                            "POST",
                            configuration.getRequestTokenUrl(),
                            emptyMap(),
                            parameters,
                            new OAuthCredentials(configuration));

            return getClient()
                    .postAbs(getConfiguration().getRequestTokenUrl())
                    ...
                    .map(
                            httpResponse -> {
                                ...
                                throw new BadCredentialsException(
                                        "Token returned by Twitter mismatch");
                            });
    ...
    protected Maybe<Token> authenticate(Authentication authentication) {
        ...
        String tokenVerifier =
                authentication
                        ...
                        .request()
                        .parameters()
                        .getFirst(configuration.getTokenVerifier());

        if (oauthToken == null
                || oauthToken.isEmpty()
                || tokenMemory.getIfPresent(oauthToken) == null) {
        ...
        Map<String, String> parameters =
                Maps.<String, String>builder().put(OAUTH_VERIFIER, tokenVerifier).build();
        ...
        String authorization =
                getAuthorizationHeader(
                        ...
                        new OAuthCredentials(
                                configuration, oauthToken, tokenMemory.getIfPresent(oauthToken)));
    ...
    protected Maybe<User> profile(Token token, Authentication authentication) {
        Map<String, String> parameters =
                Maps.<String, String>builder().put("include_email", "true").build();
        ...
        return client.getAbs(configuration.getUserProfileUri() + "?include_email=true")
                ...
                .flatMap(
                        httpResponse -> {
                            ...
                            DefaultUser user =
                                    new DefaultUser(jsonObject.getString(TWITTER_SCREEN_NAME));
                            user.setId(jsonObject.getString(TWITTER_ID));

                            Map<String, Object> additionalInfos = new HashMap<>();
                            additionalInfos.putAll(
                                    applyUserMapping(
                                            authentication.getContext(), jsonObject.getMap()));
    ...
    protected Map<String, Object> defaultClaims(Map<String, Object> attributes) {
        ...
        claims.put(
                ...
                TWITTER_BASE_URL + jsonObject.getString(TWITTER_SCREEN_NAME));
