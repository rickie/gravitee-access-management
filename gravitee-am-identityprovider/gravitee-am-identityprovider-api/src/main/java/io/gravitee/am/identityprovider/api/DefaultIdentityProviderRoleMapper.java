...
public class DefaultIdentityProviderRoleMapper implements IdentityProviderRoleMapper {
    public static final Logger LOGGER =
            LoggerFactory.getLogger(DefaultIdentityProviderRoleMapper.class);
                                        ...
                                        .forEach(
                                                u -> {
                                                    if (u.startsWith("{")
                                                            ...
                                                            && templateEngine != null) {
                                                        try {
                                                            if (templateEngine.getValue(
                                                                    u, Boolean.class)) {
                                                                mappedRoles.add(role);
                                                            }
                                                        } catch (Exception e) {
                                                            LOGGER.warn(
                                                                    "Role mapper can't evaluate the expression [{}] as boolean",
                                                                    u);
                                                        }
                                                    } else {
                                                        // user/group have the following syntax
                                                        // userAttribute=userValue
