...
public class DefaultIdentityProviderMapper implements IdentityProviderMapper {
    public static final Logger LOGGER =
            LoggerFactory.getLogger(DefaultIdentityProviderMapper.class);
    ...
    public Map<String, Object> apply(AuthenticationContext context, Map<String, Object> userInfo) {
        ...
        this.mappers.forEach(
                (userClaim, attribute) -> {
                    ...
                    String sanitizedAttr =
                            StringUtils.isEmpty(attribute) ? attribute : attribute.trim();
                    if (sanitizedAttr.startsWith("{")
                            && sanitizedAttr.endsWith("}")
                            && templateEngine != null) {
                        try {
                            additionalInformation.put(
                                    userClaim,
                                    templateEngine.getValue(sanitizedAttr, Object.class));
                        } catch (Exception e) {
                            LOGGER.warn(
                                    "User mapper can't evaluate the expression [{}] as String",
                                    userClaim);
                        }
                    } else {
