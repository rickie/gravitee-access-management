...
public class InlineAuthenticationProvider implements AuthenticationProvider, InitializingBean {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(InlineAuthenticationProvider.class);

    @Autowired private InlineIdentityProviderConfiguration configuration;

    @Autowired private PasswordEncoder passwordEncoder;

    @Autowired private InlineInMemoryUserDetailsManager userDetailsService;
    ...
    public void afterPropertiesSet() {
        if (configuration.getUsers() != null) {
            for (io.gravitee.am.identityprovider.inline.model.User user :
                ...
                .flatMap(
                        inlineUsers -> {
                            ...
                            return passwordValid
                                    ? Maybe.just(
                                            createUser(authentication.getContext(), inlineUsers))
    ...
    private List<String> getUserRoles(
            ...
            io.gravitee.am.identityprovider.inline.model.User inlineUser) {
        if (roleMapper != null) {
    ...
    private User createUser(
        ...
        if (mapper != null && mapper.getMappers() != null && !mapper.getMappers().isEmpty()) {
            ...
            claims.put(
                    StandardClaims.NAME,
                    inlineUser.getFirstname() + " " + inlineUser.getLastname());
