    ...
    protected void doStart() throws Exception {
        ...
        if (configuration.getAutoProvisioning()) {
            ...
            try (InputStream input = this.getClass().getClassLoader().getResourceAsStream(sqlScript);
                ...
                RxJava2Adapter.flowableToFlux(RxJava2Adapter.fluxToFlowable(Flux.just(tableExists(configuration.getProtocol(), configuration.getUsersTable())).flatMap(e->RxJava2Adapter.singleToMono(RxJavaReactorMigrationUtil.<String, Single<Integer>>toJdkFunction(statement -> RxJava2Adapter.fluxToFlowable(query_migrated(statement, new Object[0]).flatMap(RxJavaReactorMigrationUtil.toJdkFunction(Result::getRowsUpdated)))
...
@Override
    public Mono<User> create_migrated(User user) {
       ...
       return connectionPool.create().flatMap(v->RxJava2Adapter.singleToMono((Single<User>)RxJavaReactorMigrationUtil.toJdkFunction((Function<Connection, Single<User>>)cnx -> {
...
private Flux<Result> query_migrated(String sql, Object... args) {
        return connectionPool.create().flux().flatMap(RxJavaReactorMigrationUtil.toJdkFunction(connection ->
