...
public class JdbcUserProvider extends JdbcAbstractProvider<UserProvider> implements UserProvider {
    ...
    public Single<UserProvider> asyncStart() {
        ...
        if (configuration.getAutoProvisioning()) {
            ...
            try (InputStream input =
                ...
                LOGGER.debug(
                        "Found {} statements to execute if SQL schema doesn't exist",
                        sqlStatements.size());
                                    ...
                                    return query(connection, tableExistsStatement, new Object[0])
                                            ...
                                            .flatMapPublisher(
                                                    total -> {
                                                        if (total == 0) {
                                                            LOGGER.debug(
                                                                    "SQL datatable {} doest not exists, initialize it.",
                                                                    configuration.getUsersTable());
    ...
    public Maybe<User> findByEmail(String email) {
        return selectUserByEmail(email).map(result -> createUser(result));
    }
    ...
    public Maybe<User> findByUsername(String username) {
        return selectUserByUsername(username).map(result -> createUser(result));
    }
                ...
                .flatMap(
                        cnx -> {
                            return selectUserByUsername(cnx, user.getUsername())
                                    ...
                                    .doFinally(
                                            () ->
                                                    Completable.fromPublisher(cnx.close())
                                                            .subscribe());
                        });
    }
