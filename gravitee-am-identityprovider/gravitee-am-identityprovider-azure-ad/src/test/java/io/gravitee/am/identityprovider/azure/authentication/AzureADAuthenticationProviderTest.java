...
public class AzureADAuthenticationProviderTest {
    ...
    public void shouldGenerateSignInUrl() throws Exception {
        ...
        assertEquals(
                "https://login.microsoftonline.com/"
                        + TEST_TENANT_ID
                        + "/oauth2/v2.0/authorize?client_id=testClientId&response_type=code&scope=openid profile email&state="
                        + state
    ...
    public void shouldGenerateSignInUrl_withScope() throws Exception {
        ...
        assertEquals(
                "https://login.microsoftonline.com/"
                        + TEST_TENANT_ID
                        + "/oauth2/v2.0/authorize?client_id=testClientId&response_type=code&scope=other_scope other_scope2 openid profile email&state="
                        + state
                        + "&redirect_uri=https://gravitee.io",
                request.getUri());
        assertNull(request.getHeaders());
