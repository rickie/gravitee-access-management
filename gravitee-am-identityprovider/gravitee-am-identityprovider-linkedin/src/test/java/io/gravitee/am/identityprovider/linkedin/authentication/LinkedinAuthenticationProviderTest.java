...
public class LinkedinAuthenticationProviderTest {
    ...
    @Mock private LinkedinIdentityProviderConfiguration configuration;

    @Mock private DefaultIdentityProviderMapper mapper;

    @Mock private DefaultIdentityProviderRoleMapper roleMapper;

    @InjectMocks private LinkedinAuthenticationProvider cut;
    ...
    public void shouldGenerateSignInUrl() {
        ...
        when(configuration.getUserAuthorizationUri())
                .thenReturn("https://www.linkedin.com/oauth/v2/authorization");
        ...
        assertEquals(
                "https://www.linkedin.com/oauth/v2/authorization?client_id=testClientId&redirect_uri=https://gravitee.io&response_type=code&state="
                        + state,
                request.getUri());
    ...
    public void shouldGenerateSignInUrl_withScope() {
        ...
        assertEquals(HttpMethod.GET, request.getMethod());
        assertEquals(
                "https://www.linkedin.com/oauth/v2/authorization?client_id=testClientId&redirect_uri=https://gravitee.io&response_type=code&scope=scope1%20scope2%20scope3&state="
                        + state,
                request.getUri());
        assertNull(request.getHeaders());
    }
    ...
    public void shouldAuthenticate() {
        ...
        when(configuration.getAccessTokenUri())
                .thenReturn("https://www.linkedin.com/oauth/v2/accessToken");
        when(configuration.getUserProfileUri())
                .thenReturn(
        ...
        obs.assertValue(
                user -> {
                    ...
                    assertEquals(
                            "LINKEDINID", user.getAdditionalInformation().get(StandardClaims.SUB));
                    assertEquals(
                            "Doe", user.getAdditionalInformation().get(StandardClaims.FAMILY_NAME));
                    assertEquals(
                            "John", user.getAdditionalInformation().get(StandardClaims.GIVEN_NAME));
                    assertEquals(
                            "not applicable", user.getAdditionalInformation().get("MAIDEN_NAME"));
