...
public class LinkedinAuthenticationProvider
        extends AbstractSocialAuthenticationProvider<LinkedinIdentityProviderConfiguration> {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(LinkedinAuthenticationProvider.class);
    ...
    // r_fullprofile seems to be available only if the application owner belong to LinkedIn partner
    // program
    ...
    @Autowired private LinkedinIdentityProviderConfiguration configuration;
    ...
    protected Maybe<User> profile(Token accessToken, Authentication authentication) {
        return client.getAbs(configuration.getUserProfileUri())
                ...
                .flatMap(
                        user ->
                                requestEmailAddress(accessToken)
                                        .map(
                                                address -> {
                                                    // ask the emailAddress and complete User
                                                    // description using the value
                                                    address.ifPresent(
                                                            value -> {
                                                                ((DefaultUser) user)
                                                                        .setEmail(value);
                    ...
                    .map(
                            httpClientResponse -> {
                                if (httpClientResponse.statusCode() == 200) {
                                    ...
                                    if (payload != null && payload.containsKey("elements")) {
                                        ...
                                        for (int i = 0; i < elements.size(); ++i) {
                                            ...
                                            if (emailPayload != null
                                                    && emailPayload.containsKey("handle~")) {
                                                JsonObject handle =
                                                        emailPayload.getJsonObject("handle~");
                                    ...
                                    LOGGER.warn(
                                            "Unable to retrieve the LinkedIn email address : {}",
                                            httpClientResponse.statusMessage());
                                    return Optional
    ...
    protected Map<String, Object> defaultClaims(Map<String, Object> attributes) {
        ...
        if (json.containsKey(LinkedinUser.LASTNAME)) {
            additionalInformation.put(
                    StandardClaims.FAMILY_NAME, json.getString(LinkedinUser.LASTNAME));
        }
        if (json.containsKey(LinkedinUser.FIRSTNAME)) {
            additionalInformation.put(
                    StandardClaims.GIVEN_NAME, json.getString(LinkedinUser.FIRSTNAME));
        }
        if (json.containsKey(LinkedinUser.PROFILE_URL)) {
            additionalInformation.put(
                    StandardClaims.PROFILE, json.getString(LinkedinUser.PROFILE_URL));
        }
            ...
            if (elements != null && !elements.isEmpty()) {
                ...
                if (imgIdentifiers != null) {
                    for (int i = 0; i < imgIdentifiers.size(); i++) {
                        ...
                        if ("EXTERNAL_URL".equalsIgnoreCase(imgId.getString("identifierType"))
                                && mediaType != null
                                && mediaType.startsWith("image")) {
                            additionalInformation.put(
                                    StandardClaims.PICTURE,
                                    imgIdentifiers.getJsonObject(i).getString("identifier"));
                            break;
                        }
                    }
                }
