...
public class MongoUserProvider extends MongoAbstractProvider implements UserProvider {
    ...
    @Autowired private BinaryToTextEncoder binaryToTextEncoder;
    ...
    public void afterPropertiesSet() {
        ...
        Observable.fromPublisher(
                        usersCollection.createIndex(
                                new Document(configuration.getUsernameField(), 1)))
                .subscribe();
    }
    ...
    public Maybe<User> findByEmail(String email) {
        ...
        return Observable.fromPublisher(usersCollection.find(query).first())
                .firstElement()
                .map(this::convert);
    }
        ...
        return findByUsername(username)
                ...
                .flatMap(
                        isEmpty -> {
                            if (!isEmpty) {
                                return Single.error(
                                        new UserAlreadyExistsException(user.getUsername()));
                            } else {
                                ...
                                if (user.getCredentials() != null) {
                                    if (configuration.isUseDedicatedSalt()) {
                                        ...
                                        document.put(
                                                configuration.getPasswordField(),
                                                passwordEncoder.encode(
                                                        user.getCredentials(), salt));
                                        document.put(
                                                configuration.getPasswordSaltAttribute(),
                                                binaryToTextEncoder.encode(salt));
                                    } else {
                                        document.put(
                                                configuration.getPasswordField(),
                                ...
                                return Single.fromPublisher(usersCollection.insertOne(document))
                                        .flatMap(
                                                success ->
                                                        findById(document.getString(FIELD_ID))
                                                                .toSingle());
                            }
                        });
    }
                        ...
                        oldUser -> {
                            ...
                            if (updateUser.getCredentials() != null) {
                                if (configuration.isUseDedicatedSalt()) {
                                    ...
                                    document.put(
                                            configuration.getPasswordField(),
                                            passwordEncoder.encode(
                                                    updateUser.getCredentials(), salt));
                                    document.put(
                                            configuration.getPasswordSaltAttribute(),
                                            binaryToTextEncoder.encode(salt));
                                } else {
                                ...
                                document.put(
                                        configuration.getPasswordField(), oldUser.getCredentials());
                                if (configuration.isUseDedicatedSalt()
                                        && oldUser.getAdditionalInformation() != null) {
                                    // be sure to include the current salt as we are using the
                                    // MongoDB replaceOne method
                                    document.put(
                                            configuration.getPasswordSaltAttribute(),
                                            oldUser.getAdditionalInformation()
                                                    .get(configuration.getPasswordSaltAttribute()));
                                }
                            }
                            // set additional information
                            ...
                            return Single.fromPublisher(
                                            usersCollection.replaceOne(
                                                    eq(FIELD_ID, oldUser.getId()), document))
        ...
        if (Strings.isNullOrEmpty(password)) {
            return Single.error(
                    new IllegalArgumentException(
                            "Password required for UserProvider.updatePassword"));
        }
                        ...
                        oldUser -> {
                            ...
                            return Single.fromPublisher(
                                            usersCollection.updateOne(
                                                    eq(FIELD_ID, oldUser.getId()), updates))
                                    .flatMap(updateResult -> findById(oldUser.getId()).toSingle());
                        });
    }
                ...
                .flatMapCompletable(
                        idpUser ->
                                Completable.fromPublisher(
                                        usersCollection.deleteOne(eq(FIELD_ID, id))));
    }
