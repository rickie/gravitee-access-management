...
public abstract class MongoAbstractProvider implements InitializingBean {
    ...
    @Autowired private ConnectionProvider commonConnectionProvider;

    @Autowired private IdentityProvider identityProviderEntity;

    @Autowired protected MongoIdentityProviderConfiguration configuration;
    ...
    /**
     * This provider is used to create MongoClient when the main backend is JDBC/R2DBC because in
     * that case the commonConnectionProvider will provide R2DBC ConnectionPool. This is useful if
     * the user want to create a Mongo IDP when the main backend if a RDBMS.
     */
    ...
    public void afterPropertiesSet() {
        if (this.commonConnectionProvider.canHandle(ConnectionProvider.BACKEND_TYPE_MONGO)) {
            this.clientWrapper =
                    this.identityProviderEntity != null && this.identityProviderEntity.isSystem()
                            ? this.commonConnectionProvider.getClientWrapper()
                            : this.commonConnectionProvider.getClientFromConfiguration(
                                    this.configuration);
        } else {
