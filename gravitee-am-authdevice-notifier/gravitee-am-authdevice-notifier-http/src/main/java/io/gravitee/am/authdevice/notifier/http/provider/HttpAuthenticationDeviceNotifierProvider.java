...
public class HttpAuthenticationDeviceNotifierProvider
        implements AuthenticationDeviceNotifierProvider {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(HttpAuthenticationDeviceNotifierProvider.class);
    ...
    @Autowired private HttpAuthenticationDeviceNotifierConfiguration configuration;
    ...
    public Single<ADNotificationResponse> notify(ADNotificationRequest request) {
        ...
        if (!StringUtils.isEmpty(this.configuration.getHeaderValue())) {
            notificationRequest.putHeader(
                    this.configuration.getHeaderName(), this.configuration.getHeaderValue());
        }
    ...
    public Single<Optional<ADUserResponse>> extractUserResponse(ADCallbackContext callbackContext) {
        ...
        if (isEmpty(state) || isEmpty(tid) || isEmpty(validated)) {
            ...
            return Single.just(
                    Optional.of(new ADUserResponse(tid, state, Boolean.valueOf(validated))));
        }
    }
}
