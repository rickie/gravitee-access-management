...
public class CallFactorProvider implements FactorProvider {
    ...
    @Autowired private CallFactorConfiguration configuration;
    ...
    public Completable verify(FactorContext context) {
        ...
        if (provider instanceof MFAResourceProvider) {
            ...
            var challenge =
                    new MFAChallenge(enrolledFactor.getChannel().getTarget(), code, context);
            return mfaProvider.verify(challenge);
        } else {
            return Completable.error(
                    new TechnicalException(
                            "Resource referenced can't be used for MultiFactor Authentication with type Call"));
        }
    }
    ...
    public Completable sendChallenge(FactorContext context) {
        ...
        if (provider instanceof MFAResourceProvider) {
            ...
            MFALink link =
                    new MFALink(MFAType.CALL, enrolledFactor.getChannel().getTarget(), context);
            return mfaProvider.send(link);
        } else {
            return Completable.error(
                    new TechnicalException(
                            "Resource referenced can't be used for MultiFactor Authentication  with type SMS"));
        }
    }
