...
private Mono<Void> generateCodeAndSendEmail_migrated(FactorContext context, EmailSenderProvider provider, EnrolledFactor enrolledFactor) {
        ...
        try {
            ...
            return provider.sendMessage_migrated(emailWrapper.getEmail()).then(Mono.just(enrolledFactor).flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<EnrolledFactor, SingleSource<User>>toJdkFunction(ef ->  {
                                ef.setPrimary(true);
