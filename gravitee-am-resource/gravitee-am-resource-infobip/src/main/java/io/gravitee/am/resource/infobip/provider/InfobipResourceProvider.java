...
public class InfobipResourceProvider implements MFAResourceProvider {
    ...
    @Autowired private InfobipResourceConfiguration configuration;
    ...
    public Completable send(MFALink target) {
        ...
        return Completable.create(
                (emitter) -> {
                    try {
                        TfaStartAuthenticationResponse sendCodeResponse =
                                this.tfaApi.sendTfaPinCodeOverSms(
                                        true,
                        ...
                        boolean isSuccessful =
                                sendCodeResponse.getSmsStatus().equals("MESSAGE_SENT");
                        ...
                        if (!isSuccessful) {
                            ...
                            LOGGER.debug(
                                    "Infobip Verification code asked with ID '{}'",
                                    sendCodeResponse.getPinId());
                            emitter.onComplete();
                        }
    ...
    public Completable verify(MFAChallenge challenge) {
        return Completable.create(
                (emitter) -> {
                    ...
                    try {
                        TfaVerifyPinResponse verifyResponse =
                                this.tfaApi.verifyTfaPhoneNumber(
                                        pinId, new TfaVerifyPinRequest().pin(pin));
