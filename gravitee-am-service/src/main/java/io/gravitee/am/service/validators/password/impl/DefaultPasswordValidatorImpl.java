...
public class DefaultPasswordValidatorImpl implements PasswordValidator {
    ...
    private static final String DEFAULT_PASSWORD_PATTERN_VALUE =
    ...
    private static final InvalidPasswordException INVALID_PASSWORD_VALUE =
            InvalidPasswordException.of(MESSAGE, ERROR_KEY);

    private final Pattern defaultPasswordPattern;

    @Autowired
    public DefaultPasswordValidatorImpl(
            @Value("${user.password.policy.pattern:" + DEFAULT_PASSWORD_PATTERN_VALUE + "}")
                    String pattern) {
