...
@Override
    public Mono<Tag> create_migrated(NewTag newTag, String organizationId, User principal) {
        ...
        return tagRepository.findById_migrated(id, organizationId).hasElement().flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Boolean, SingleSource<Tag>>toJdkFunction(empty -> {
                    if (!empty) {
...
@Override
    public Mono<Tag> update_migrated(String tagId, String organizationId, UpdateTag updateTag, User principal) {
        ...
        return tagRepository.findById_migrated(tagId, organizationId).switchIfEmpty(Mono.error(new TagNotFoundException(tagId))).flatMap(y->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Tag, SingleSource<Tag>>toJdkFunction(oldTag -> {
                    Tag tag = new Tag();
                    tag.setId(tagId);
