...
public class VirtualHostValidatorImpl implements VirtualHostValidator {
    ...
    public Completable validate(VirtualHost virtualHost, List<String> domainRestrictions) {
        ...
        if (!InternetDomainName.isValid(hostWithoutPort)) {
            return Completable.error(
                    new InvalidVirtualHostException("Host [" + hostWithoutPort + "] is invalid"));
        }

        if (!isValidDomainOrSubDomain(hostWithoutPort, domainRestrictions)) {
            return Completable.error(
                    new InvalidVirtualHostException(
                            "Host ["
                                    + hostWithoutPort
        ...
        if (host.contains(":") && split.length < 2) {
            return Completable.error(
                    new InvalidVirtualHostException("Host port for [" + host + "] is invalid"));
        }
        ...
        if (split.length > 1) {
            try {
                ...
                if (port < 0 || port > 65535) {
                    return Completable.error(
                            new InvalidVirtualHostException("Host port [" + port + "] is invalid"));
                }
    ...
    public Completable validateDomainVhosts(Domain domain, List<Domain> domains) {
        ...
        if (domain.isVhostMode()) {
            ...
            // Domain listen on every hosts. Need to check if path overlap (or is overlapped) with
            // all other domain path (including vhost path).
            ...
            if (overlap(sourcePath, otherPath)) {
                return Completable.error(
                        new InvalidVirtualHostException(
                                "Path ["
                                        + sourcePath
                                        + "] overlap path defined in another security domain"));
            }
            ...
            if (overlap(otherPath, sourcePath)) {
                return Completable.error(
                        new InvalidVirtualHostException(
                                "Path ["
                                        + sourcePath
                                        + "] is overlapped by another security domain"));
            }
        }
        return null;
    }
