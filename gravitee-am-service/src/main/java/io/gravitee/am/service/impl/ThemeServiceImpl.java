...
public class ThemeServiceImpl implements ThemeService {
    ...
    @Lazy @Autowired private ThemeRepository themeRepository;

    @Autowired private EventService eventService;

    @Autowired private AuditService auditService;

    @Autowired private ThemeValidator themeValidator;
    ...
    public Maybe<Theme> findByReference(ReferenceType referenceType, String referenceId) {
        return this.themeRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            logger.error(
                                    "An error occurs while trying to find the theme linked to {}/{}",
                                    referenceType,
                                    referenceId,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to find the theme", ex));
                        });
    }
                                        ...
                                        isEmpty -> {
                                            ...
                                            return this.themeRepository
                                                    ...
                                                    .onErrorResumeNext(
                                                            ex -> {
                                                                String msg =
                                                                        "An error occurred while trying to create a theme";
                                                                logger.error(msg, ex);
                                                                return Single.error(
                                                                        new TechnicalManagementException(
                                                                                msg, ex));
    ...
    public Single<Theme> update(Domain domain, Theme updatedTheme, User principal) {
        ...
        return this.themeRepository
                .findById(updatedTheme.getId())
                .switchIfEmpty(
                        Maybe.error(
                                new ThemeNotFoundException(updatedTheme.getId(), domain.getId())))
                .flatMapSingle(
                        existingTheme -> {
                            if (!domain.getId().equals(updatedTheme.getReferenceId())) {
                                return Single.error(
                                        new InvalidThemeException(
                                                "Updated theme is not linked to the domain"));
                            }

                            if (!existingTheme
                                    .getReferenceId()
                                    .equals(updatedTheme.getReferenceId())) {
                                return Single.error(
                                        new InvalidThemeException(
                                                "ReferenceId can not be updated"));
                            }

                            if (!existingTheme
                                    .getReferenceType()
                                    .equals(updatedTheme.getReferenceType())) {
                                return Single.error(
                                        new InvalidThemeException(
                            ...
                            return themeValidator
                                    .validate(updatedTheme)
                                    .andThen(
                                            this.themeRepository
                                                    .update(sanitize(updatedTheme))
                                                    .flatMap(
                                                            newTheme -> {
                                                                Event event =
                                                                        new Event(
                                                    ...
                                                    .onErrorResumeNext(
                                                            ex -> {
                                                                String msg =
                                                                        "An error occurred while trying to update a theme";
                                                                logger.error(msg, ex);
                                                                return Single.error(
                                                                        new TechnicalManagementException(
                                                                                msg, ex));
    ...
    public Completable delete(Domain domain, String themeId, User principal) {
        return this.themeRepository
                ...
                .flatMapCompletable(
                        theme -> {
                            if (!(ReferenceType.DOMAIN.equals(theme.getReferenceType())
                                    && domain.getId().equals(theme.getReferenceId()))) {
                                logger.warn(
                                        "Delete theme '{}' received on wrong domain, delete skipped");
                                return Completable.error(
                                        new InvalidThemeException(
                                                "Theme isn't linked to the domain "
                                                        + domain.getId()));
                            }
                            ...
                            return this.themeRepository
                                            ...
                                            () -> {
                                                Event event =
                                                        new Event(
                                                                ...
                                                                new Payload(
                                                                        theme.getId(),
                                                                        theme.getReferenceType(),
                                                                        theme.getReferenceId(),
                                                                        Action.DELETE));
                                                eventService
                                                        .create(event)
                                                        .ignoreElement()
                                    ...
                                    .onErrorResumeNext(
                                            ex -> {
                                                String msg =
                                                        "An error occurred while trying to delete a theme";
                                                logger.error(msg, ex);
                                                auditService.report(
                                                        AuditBuilder.builder(
                                                                        ThemeAuditBuilder.class)
                                                                ...
                                                                .throwable(ex));
                                                return Completable.error(
                                                        new TechnicalManagementException(msg, ex));
                                            });
                        });
    }
                ...
                .flatMap(
                        theme -> {
                            if (!(ReferenceType.DOMAIN.equals(theme.getReferenceType())
                                    && domain.getId().equals(theme.getReferenceId()))) {
                                return Maybe.error(
                                        new ThemeNotFoundException(themeId, domain.getId()));
                            }
