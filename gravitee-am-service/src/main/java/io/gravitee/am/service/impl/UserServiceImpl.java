...
public class UserServiceImpl extends AbstractUserService implements UserService {
    ...
    public Flowable<User> findByDomain(String domain) {
        ...
        return userRepository
                .findAll(ReferenceType.DOMAIN, domain)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find users by domain {}",
                                    domain,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find users by domain %s",
                                                    domain),
                                            ex));
                        });
    }
    ...
    public Maybe<User> findByDomainAndUsername(String domain, String username) {
        ...
        return userRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a user using its ID: {} for the domain {}",
                                    username,
                                    domain,
                                    ex);
                            return Maybe.error(
                        ...
                        getUserRepository()
                                ...
                                .onErrorResumeNext(
                                        ex -> {
                                            ...
                                            LOGGER.error(
                                                    "An error occurs while trying to update a user",
                                                    ex);
                                            return Single.error(
                                                    new TechnicalManagementException(
                                                            "An error occurs while trying to update a user",
    ...
    public Single<Long> countByDomain(String domain) {
        ...
        return userRepository
                .countByReference(ReferenceType.DOMAIN, domain)
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException) {
                                return Single.error(ex);
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to count users by domain: {}",
                                    domain,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while count users to delete user: %s",
                                                    domain),
                                            ex));
                        });
    }
    ...
    public Single<Long> countByApplication(String domain, String application) {
        ...
        return userRepository
                .countByApplication(domain, application)
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException) {
                                return Single.error(ex);
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to count users by application: {}",
                                    application,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
        ...
        return userRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to get users analytics : {}",
                                    query,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while count users analytics : %s",
                                                    query),
                                            ex));
                        });
    }
    ...
    public Completable deleteByDomain(String domain) {
        ...
        return credentialService
                .deleteByReference(ReferenceType.DOMAIN, domain)
                ...
                .flatMapSingle(
                        oldUser -> {
                            ...
                            if (enrolledFactors == null || enrolledFactors.isEmpty()) {
                                ...
                                // if the Factor already exists, update the target and the security
                                // value
                                var foundFactor =
                                        enrolledFactors.stream()
                                                .filter(
                                                        existingFactor ->
                                                                existingFactor
                                                                        .getFactorId()
                                ...
                                if (foundFactor.isPresent()) {
                                    ...
                                    factorToUpdate.setStatus(
                                            ofNullable(enrolledFactor.getStatus())
                                                    .orElse(factorToUpdate.getStatus()));
                                    factorToUpdate.setChannel(
                                            ofNullable(enrolledFactor.getChannel())
                                                    .orElse(factorToUpdate.getChannel()));
                                    factorToUpdate.setSecurity(
                                            ofNullable(enrolledFactor.getSecurity())
                                                    .orElse(factorToUpdate.getSecurity()));
                                    factorToUpdate.setPrimary(
                                            ofNullable(enrolledFactor.isPrimary())
                                                    .orElse(factorToUpdate.isPrimary()));
                                    // update the factor
                                    enrolledFactors.removeIf(
                                            ef ->
                                                    factorToUpdate
                                                            .getFactorId()
                                                            .equals(ef.getFactorId()));
                            ...
                            if (enrolledFactor.getChannel() != null
                                    && EnrolledFactorChannel.Type.SMS.equals(
                                            enrolledFactor.getChannel().getType())) {
                                // MFA SMS currently used, preserve the phone number into the user
                                // profile if not yet present
                                List<Attribute> phoneNumbers = user.getPhoneNumbers();
                                if (phoneNumbers == null) {
                                    phoneNumbers = new ArrayList<>();
                                    user.setPhoneNumbers(phoneNumbers);
                                }
                                String enrolledPhoneNumber =
                                        enrolledFactor.getChannel().getTarget();
                                if (phoneNumbers.stream()
                                        .noneMatch(p -> p.getValue().equals(enrolledPhoneNumber))) {
                            ...
                            if (enrolledFactor.getChannel() != null
                                    && EnrolledFactorChannel.Type.EMAIL.equals(
                                            enrolledFactor.getChannel().getType())) {
                                // MFA EMAIL currently used, preserve the email into the user
                                // profile if not yet present
                                ...
                                if (email == null) {
                                    ...
                                    // an email is already present but doesn't match the one
                                    // provided as security factor
                                    ...
                                    if (emails.stream()
                                            .noneMatch(p -> p.getValue().equals(enrolledEmail))) {
    ...
    private void removeSensitiveFactorsData(List<EnrolledFactor> enrolledFactors) {
        ...
        enrolledFactors.forEach(enrolledFactor -> enrolledFactor.setSecurity(null));
    }
    ...
    private boolean needToAuditUserFactorsOperation(User newUser, User oldUser) {
        ...
        return newEnrolledFactors.stream()
                .anyMatch(
                        newEnrolledFactor ->
                                oldEnrolledFactors.stream()
                                        .anyMatch(
                                                oldEnrolledFactor -> {
                                                    if (!newEnrolledFactor
                                                            .getFactorId()
                                                            .equals(
                                                                    oldEnrolledFactor
                                                                            .getFactorId())) {
                                                    ...
                                                    // check if enrolled factor was in pending
                                                    // activation
                                                    if (oldEnrolledFactor
                                                            .getStatus()
                                                            .equals(
                                                                    FactorStatus
                                                                            .PENDING_ACTIVATION)) {
                                                    ...
                                                    if (oldEnrolledFactor.getChannel() != null) {
                                                        ...
                                                        if (EnrolledFactorChannel.Type.EMAIL.equals(
                                                                oldEnrolledFactor
                                                                        .getChannel()
                                                                        .getType())) {
                                                            return emailInformationHasChanged(
                                                                    newUser, oldUser);
                                                        }
                                                        // check if phoneNumber has changed
                                                        if (EnrolledFactorChannel.Type.SMS.equals(
                                                                oldEnrolledFactor
                                                                        .getChannel()
                                                                        .getType())) {
                                                            return phoneNumberInformationHasChanged(
                                                                    newUser, oldUser);
                                                        }
                                                    }
                                                    return false;
