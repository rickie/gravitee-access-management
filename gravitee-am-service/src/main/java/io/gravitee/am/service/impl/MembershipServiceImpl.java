...
public class MembershipServiceImpl implements MembershipService {
    ...
    @Lazy @Autowired private MembershipRepository membershipRepository;

    @Autowired private AuditService auditService;

    @Autowired private OrganizationUserService orgUserService;
    ...
    public Maybe<Membership> findById(String id) {
        ...
        return membershipRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find membership by id {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find membership by ID %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Flowable<Membership> findByCriteria(
        ...
        LOGGER.debug(
                "Find memberships by reference type {} and reference id {} and criteria {}",
                referenceType,
                referenceId,
                criteria);
    ...
    public Flowable<Membership> findByReference(String referenceId, ReferenceType referenceType) {
        LOGGER.debug(
                "Find memberships by reference id {} and reference type {}",
                referenceId,
                referenceType);
        return membershipRepository
    ...
    @Override
    public Single<Membership> addOrUpdate(
            String organizationId, Membership membership, User principal) {
                                ...
                                .flatMapSingle(
                                        optMembership -> {
                                            if (!optMembership.isPresent()) {
                                                ...
                                                newMembership.setMemberType(
                                                        membership.getMemberType());
                                                newMembership.setReferenceId(
                                                        membership.getReferenceId());
                                                newMembership.setReferenceType(
                                                        membership.getReferenceType());
                                                newMembership.setRoleId(membership.getRoleId());
                                                newMembership.setCreatedAt(new Date());
                                                newMembership.setUpdatedAt(
                                                ...
                                                return membershipRepository
                                                        ...
                                                        .onErrorResumeNext(
                                                                ex -> {
                                                                    ...
                                                                    LOGGER.error(
                                                                            "An error occurs while trying to update membership {}",
                                                                            oldMembership,
                                                                            ex);
                                                                    return Single.error(
                                                                            new TechnicalManagementException(
                                                                                    String.format(
                                                                                            "An error occurs while trying to update membership %s",
                                                                                            oldMembership),
                                                                                    ex));
                                                                })
                                                        .doOnSuccess(
    ...
    public Single<Membership> setPlatformAdmin(String userId) {
        ...
        return roleService
                .findSystemRole(SystemRole.PLATFORM_ADMIN, ReferenceType.PLATFORM)
                .switchIfEmpty(
                        Single.error(new RoleNotFoundException(SystemRole.PLATFORM_ADMIN.name())))
                .flatMap(
                        role ->
                                findByCriteria(ReferenceType.PLATFORM, Platform.DEFAULT, criteria)
                                        .firstElement()
                                        .switchIfEmpty(
                                                Single.defer(
                                                        () -> {
    ...
    public Single<Map<String, Map<String, Object>>> getMetadata(List<Membership> memberships) {
        ...
        return Single.zip(
                orgUserService
                        .findByIdIn(userIds)
                        .toMap(io.gravitee.am.model.User::getId, this::convert),
                ...
                (users, groups, roles) -> {
                    ...
                    metadata.put(
                            "roles",
                            roles.stream().collect(Collectors.toMap(Role::getId, this::filter)));
                    return metadata;
                });
    }
        ...
        return membershipRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete membership: {}",
                                    membershipId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete membership: %s",
                                                    membershipId),
                                            ex));
                        });
    }
    ...
    @Override
    public Completable addDomainUserRoleIfNecessary(
                                        ...
                                        roleService
                                                ...
                                                .flatMapSingle(
                                                        role -> {
                                                            ...
                                                            domainMembership.setMemberType(
                                                                    newMembership.getMemberType());
                                                            domainMembership.setRoleId(
                                                                    role.getId());
                                                            domainMembership.setReferenceId(
                                                                    domainId);
                                                            domainMembership.setReferenceType(
                                                                    ReferenceType.DOMAIN);
                                                            return this.createInternal(
                                                                    domainMembership, principal);
                                                        })
                                                .toFlowable()))
                .ignoreElements()
                .andThen(
                        addEnvironmentUserRoleIfNecessary(
                                organizationId, environmentId, newMembership, principal));
    }

    @Override
    public Completable addEnvironmentUserRoleIfNecessary(
            String organizationId,
            String environmentId,
            NewMembership newMembership,
            User principal) {
                                                ...
                                                .flatMapSingle(
                                                        role -> {
                                                            ...
                                                            environmentMembership.setMemberId(
                                                                    newMembership.getMemberId());
                                                            environmentMembership.setMemberType(
                                                                    newMembership.getMemberType());
                                                            environmentMembership.setRoleId(
                                                                    role.getId());
                                                            environmentMembership.setReferenceId(
                                                                    environmentId);
                                                            environmentMembership.setReferenceType(
                                                                    ReferenceType.ENVIRONMENT);
                                                            return this.createInternal(
        ...
        return membershipRepository
                ...
                .flatMap(
                        membership1 -> {
                            ...
                            return eventService
                                    .create(event)
                                    .flatMap(__ -> Single.just(membership1));
                        })
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException) {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to create membership {}",
                                    membership,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to create membership %s",
                                                    membership),
                                            ex));
                        })
                .doOnSuccess(
                        membership1 ->
                                auditService.report(
                                        AuditBuilder.builder(MembershipAuditBuilder.class)
                                                .principal(principal)
    ...
    private Completable checkMember(String organizationId, Membership membership) {
        ...
        if (MemberType.USER.equals(membership.getMemberType())) {
            return orgUserService
                    .findById(ReferenceType.ORGANIZATION, organizationId, membership.getMemberId())
                    .ignoreElement();
        } else {
            return groupService
                    .findById(ReferenceType.ORGANIZATION, organizationId, membership.getMemberId())
    ...
    private Completable checkRole(String organizationId, Membership membership) {
        return roleService
                ...
                .flatMap(
                        role -> {
                            // If role is a 'PRIMARY_OWNER' role, need to check if it is already
                            // assigned or not.
                                ...
                                if (membership.getMemberType() == MemberType.GROUP) {
                                    return Maybe.error(
                                            new InvalidRoleException(
                                                    "This role cannot be assigned to a group"));
                                }
                                ...
                                return membershipRepository
                                        ...
                                        .flatMapMaybe(
                                                count ->
                                                        count >= 1
                                                                ? Maybe.error(
                                                                        new SinglePrimaryOwnerException(
                                                                                membership
                                                                                        .getReferenceType()))
                                                                : Maybe.just(role));
                            }
                ...
                .filter(
                        role1 ->
                                role1.getAssignableType().equals(membership.getReferenceType())
                                        &&
                                        // Role can be either a system role, either an organization
                                        // role, either a domain role.
                                        (role1.isSystem()
                                                || (role1.getReferenceType()
