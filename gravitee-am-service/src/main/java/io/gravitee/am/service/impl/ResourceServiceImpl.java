...
public class ResourceServiceImpl implements ResourceService {
    ...
    @Lazy @Autowired private ResourceRepository repository;

    @Lazy @Autowired private AccessPolicyRepository accessPolicyRepository;
    ...
    public Single<Page<Resource>> findByDomain(String domain, int page, int size) {
        ...
        return repository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find resources by domain {}",
                                    domain,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find resources by domain %s",
                                                    domain),
                                            ex));
                        });
    }
    ...
    @Override
    public Single<Page<Resource>> findByDomainAndClient(
            String domain, String client, int page, int size) {
        LOGGER.debug("Listing resource set for domain {} and client {}", domain, client);
        return repository
                .findByDomainAndClient(domain, client, page, size)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find resources by domain {} and client {}",
                                    domain,
                                    client,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find resources by domain %s and client %s",
                                                    domain, client),
                                            ex));
                        });
    }
    ...
    @Override
    public Flowable<Resource> listByDomainAndClientAndUser(
            String domain, String client, String userId) {
    ...
    @Override
    public Flowable<Resource> findByDomainAndClientAndResources(
            String domain, String client, List<String> resourceIds) {
        LOGGER.debug(
                "Getting resource {} for  client {} and resources {}",
                resourceIds,
                client,
                resourceIds);
    ...
    @Override
    public Maybe<Resource> findByDomainAndClientAndUserAndResource(
            String domain, String client, String userId, String resourceId) {
        LOGGER.debug(
                "Getting resource by resource owner {} and client {} and resource {}",
                userId,
                client,
                resourceId);
        return repository.findByDomainAndClientAndUserAndResource(
                domain, client, userId, resourceId);
    }

    @Override
    public Maybe<Resource> findByDomainAndClientResource(
            String domain, String client, String resourceId) {
        LOGGER.debug(
                "Getting resource by domain {} client {} and resource {}",
                domain,
                client,
                resourceId);
    ...
    public Single<Map<String, Map<String, Object>>> getMetadata(List<Resource> resources) {
        ...
        List<String> userIds =
                resources.stream()
                        .filter(resource -> resource.getUserId() != null)
                        .map(Resource::getUserId)
                        .distinct()
                        .collect(Collectors.toList());
        List<String> appIds =
                resources.stream()
                        .filter(resource -> resource.getClientId() != null)
                        .map(Resource::getClientId)
                        .distinct()
                        .collect(Collectors.toList());
        return Single.zip(
                userService.findByIdIn(userIds).toMap(User::getId, this::filter),
                applicationService.findByIdIn(appIds).toMap(Application::getId, this::filter),
                (users, apps) -> {
    ...
    @Override
    public Single<Resource> create(
            NewResource newResource, String domain, String client, String userId) {
                ...
                .flatMap(
                        r -> {
                            ...
                            accessPolicy.setDescription(
                                    "Default deny access policy. Created by Gravitee.io.");
                            accessPolicy.setType(AccessPolicyType.GROOVY);
                            accessPolicy.setCondition(
    ...
    public Single<Resource> update(
            ...
            String resourceId) {
        LOGGER.debug(
                "Updating resource id {} for resource owner {} and client {}",
                resourceId,
                userId,
                client);
    ...
    public Completable delete(String domain, String client, String userId, String resourceId) {
        LOGGER.debug(
                "Deleting resource id {} for resource owner {} and client {}",
                resourceId,
                userId,
                client);
        return findByDomainAndClientAndUserAndResource(domain, client, userId, resourceId)
    ...
    public Completable delete(Resource resource) {
        LOGGER.debug(
                "Deleting resource id {} on domain {}", resource.getId(), resource.getDomain());
        // delete policies and then the resource
        return accessPolicyRepository
                .findByDomainAndResource(resource.getDomain(), resource.getId())
                .flatMapCompletable(
                        accessPolicy -> accessPolicyRepository.delete(accessPolicy.getId()))
                .andThen(repository.delete(resource.getId()));
    }

    @Override
    public Flowable<AccessPolicy> findAccessPolicies(
            String domain, String client, String user, String resource) {
        LOGGER.debug(
                "Find access policies by domain {}, client {}, resource owner {} and resource id {}",
                domain,
                client,
                user,
                resource);
        return findByDomainAndClientAndUserAndResource(domain, client, user, resource)
                ...
                .flatMapPublisher(
                        r -> accessPolicyRepository.findByDomainAndResource(domain, r.getId()))
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error has occurred while trying to find access policies by domain {}, client {}, resource owner {} and resource id {}",
                                    domain,
                                    client,
                                    user,
                                    resource,
                            ...
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to find access policies by domain %s, client %s, resource owner %s and resource id %s",
                                                    domain, client, user, resource),
                                            ex));
                        });
    }
    ...
    public Flowable<AccessPolicy> findAccessPoliciesByResources(List<String> resourceIds) {
        ...
        return accessPolicyRepository
                .findByResources(resourceIds)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error has occurred while trying to find access policies by resource ids {}",
                                    resourceIds,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to find access policies by resource ids %s",
                                                    resourceIds),
                                            ex));
                        });
    }
    ...
    public Single<Long> countAccessPolicyByResource(String resourceId) {
        ...
        return accessPolicyRepository
                .countByResource(resourceId)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error has occurred while trying to count access policies by resource id {}",
                                    resourceId,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to count access policies by resource id %s",
                                                    resourceId),
                                            ex));
                        });
    }
    ...
    @Override
    public Maybe<AccessPolicy> findAccessPolicy(
            String domain, String client, String user, String resource, String accessPolicy) {
        LOGGER.debug(
                "Find access policy by domain {}, client {}, resource owner {}, resource id {} and policy id {}",
                domain,
                client,
                user,
                resource,
        ...
        return findByDomainAndClientAndUserAndResource(domain, client, user, resource)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error has occurred while trying to find access policies by domain {}, client {}, resource owner {} and resource id {} and policy id {}",
                                    domain,
                                    client,
                                    user,
                                    resource,
                            ...
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to find access policies by domain %s, client %s, resource owner %s resource id %s and policy id %s",
                                                    domain, client, user, resource, accessPolicy),
                                            ex));
                        });
    }
    ...
    public Maybe<AccessPolicy> findAccessPolicy(String accessPolicy) {
        ...
        return accessPolicyRepository
                .findById(accessPolicy)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error has occurred while trying to find access policy by id {}",
                                    accessPolicy,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to find access policy by id %s",
                                                    accessPolicy),
                                            ex));
                        });
    }
    ...
    @Override
    public Single<AccessPolicy> createAccessPolicy(
            AccessPolicy accessPolicy, String domain, String client, String user, String resource) {
        LOGGER.debug(
                "Creating access policy for domain {}, client {}, resource owner {} and resource id {}",
                domain,
                client,
                user,
                resource);
        return findByDomainAndClientAndUserAndResource(domain, client, user, resource)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error has occurred while trying to create an access policy for domain {}, client {}, resource owner {} and resource id {}",
                                    domain,
                                    client,
                                    user,
                                    resource,
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to create an access policy for domain %s, client %s, resource owner %s and resource id %s",
                                                    domain, client, user, resource),
                                            ex));
                        });
    }
    ...
    @Override
    public Single<AccessPolicy> updateAccessPolicy(
        ...
        LOGGER.debug(
                "Updating access policy for domain {}, client {}, resource owner {}, resource id {} and policy id {}",
                domain,
                client,
                user,
                resource,
        ...
        return findByDomainAndClientAndUserAndResource(domain, client, user, resource)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error has occurred while trying to update access policy for domain {}, client {}, resource owner {}, resource id {} and policy id {}",
                                    domain,
                                    client,
                                    user,
                                    resource,
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to update access policy for domain %s, client %s, resource owner %s, resource id %s and policy id %s",
                                                    domain, client, user, resource, accessPolicyId),
                                            ex));
                        });
    }

    @Override
    public Completable deleteAccessPolicy(
            String domain, String client, String user, String resource, String accessPolicy) {
        LOGGER.debug(
                "Deleting access policy for domain {}, client {}, resource owner {}, resource id and policy id {}",
                domain,
                client,
                user,
                resource,
        ...
        return findByDomainAndClientAndUserAndResource(domain, client, user, resource)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error has occurred while trying to delete access policy for domain {}, client {}, resource owner {}, resource id {} and policy id {}",
                                    domain,
                                    client,
                                    user,
                                    resource,
                            ...
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred while trying to delete access policy for domain %s, client %s, resource owner %s, resource id %s and policy id %s",
                                                    domain, client, user, resource, accessPolicy),
                                            ex));
                        });
    }
    ...
    private Single<Resource> validateScopes(Resource toValidate) {
        ...
        toValidate.setResourceScopes(
                toValidate.getResourceScopes().stream().distinct().collect(Collectors.toList()));

        return scopeService
                .findByDomainAndKeys(toValidate.getDomain(), toValidate.getResourceScopes())
                .flatMap(
                        scopes -> {
                            if (toValidate.getResourceScopes().size() != scopes.size()) {
                                return Single.error(
                                        new ScopeNotFoundException(
                                                toValidate.getResourceScopes().stream()
