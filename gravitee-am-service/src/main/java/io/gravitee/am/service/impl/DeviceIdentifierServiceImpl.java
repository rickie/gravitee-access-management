...
public class DeviceIdentifierServiceImpl implements DeviceIdentifierService {
    ...
    @Lazy @Autowired private DeviceIdentifierRepository deviceIdentifierRepository;
    ...
    public Maybe<DeviceIdentifier> findById(String id) {
        ...
        return deviceIdentifierRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a device identifier using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a device identifier using its ID: %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Flowable<DeviceIdentifier> findByDomain(String domain) {
        ...
        return deviceIdentifierRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find device identifiers by domain",
                                    ex);
    ...
    @Override
    public Single<DeviceIdentifier> create(
            String domain, NewDeviceIdentifier newDeviceIdentifier, User principal) {
        LOGGER.debug(
                "Create a new device identifier {} for domain {}", newDeviceIdentifier, domain);

        DeviceIdentifier deviceIdentifier = new DeviceIdentifier();
        deviceIdentifier.setId(
                newDeviceIdentifier.getId() == null
                        ? RandomString.generate()
                        : newDeviceIdentifier.getId());
        ...
        return deviceIdentifierRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to create a device identifier",
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to create a device identifier",
                ...
                .doOnSuccess(
                        detection ->
                                auditService.report(
                                        AuditBuilder.builder(DeviceIdentifierAuditBuilder.class)
                                                .principal(principal)
                                                .type(EventType.DEVICE_IDENTIFIER_CREATED)
                                                .deviceIdentifier(detection)))
                .doOnError(
                        throwable ->
                                auditService.report(
                                        AuditBuilder.builder(DeviceIdentifierAuditBuilder.class)
    ...
    @Override
    public Single<DeviceIdentifier> update(
            String domain,
            String id,
        ...
        return deviceIdentifierRepository
                ...
                .flatMapSingle(
                        oldDeviceIdentifier -> {
                            DeviceIdentifier deviceIdentifierToUpdate =
                                    new DeviceIdentifier(oldDeviceIdentifier);
                            deviceIdentifierToUpdate.setName(updateDeviceIdentifier.getName());
                            deviceIdentifierToUpdate.setConfiguration(
                                    updateDeviceIdentifier.getConfiguration());
                            deviceIdentifierToUpdate.setUpdatedAt(new Date());

                            return deviceIdentifierRepository
                                    .update(deviceIdentifierToUpdate)
                                    .flatMap(
                                            detection -> {
                                                // create event for sync process
                                                Event event =
                                                        new Event(
                                                                ...
                                                                new Payload(
                                                                        ...
                                                                        detection
                                                                                .getReferenceType(),
                                                                        detection.getReferenceId(),
                                                                        Action.UPDATE));
                                                return eventService
                                                        .create(event)
                                                        .flatMap(__ -> Single.just(detection));
                                            })
                                    .doOnSuccess(
                                            detection ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                                                                            DeviceIdentifierAuditBuilder
                                                                                    .class)
                                                                    .principal(principal)
                                                                    .type(
                                                                            EventType
                                                                                    .DEVICE_IDENTIFIER_UPDATED)
                                                                    .oldValue(oldDeviceIdentifier)
                                                                    .deviceIdentifier(detection)))
                                    .doOnError(
                                            throwable ->
                                                    auditService.report(
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to update device identifier", ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update device identifier",
                                            ex));
                        });
    }
        ...
        return deviceIdentifierRepository
                                            ...
                                            () ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                                                                    ...
                                                                    .type(
                                                                            EventType
                                                                                    .DEVICE_IDENTIFIER_DELETED)
                                                                    .deviceIdentifier(toDelete)))
                                    .doOnError(
                                            throwable ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete device identifier: {}",
                                    deviceIdentifier,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete device identifier: %s",
                                                    deviceIdentifier),
                                            ex));
                        });
    }
