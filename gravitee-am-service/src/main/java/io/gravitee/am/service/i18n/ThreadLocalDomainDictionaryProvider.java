...
/**
 * DictionaryProvider that allow the usage of a dictionary in an ephemeral context (REST Call). This
 * provider is used to provide the i18n dictionary in order to allow preview in ManagementAPI with
 * right translation.
 *
...
public class ThreadLocalDomainDictionaryProvider implements DynamicDictionaryProvider {
    ...
    public boolean hasDictionaryFor(Locale locale) {
        Map<String, Properties> map = this.propertiesMap.get();
        return map != null
                && (map.containsKey(locale.toString()) || map.containsKey(locale.getLanguage()));
    }
