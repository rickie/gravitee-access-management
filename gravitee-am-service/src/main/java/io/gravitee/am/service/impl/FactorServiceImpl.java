...
@Override
    public Flux<Factor> findByDomain_migrated(String domain) {
        ...
        return factorRepository.findByDomain_migrated(domain).onErrorResume(RxJavaReactorMigrationUtil.toJdkFunction(ex -> {
...
@Override
    public Mono<Factor> create_migrated(String domain, NewFactor newFactor, User principal) {
        ...
        return checkFactorConfiguration_migrated(factor).flatMap(factorRepository::create_migrated).flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Factor, SingleSource<Factor>>toJdkFunction(factor1 -> {
                ...
                }).apply(err))).doOnSuccess(factor1 -> auditService.report(AuditBuilder.builder(FactorAuditBuilder.class).principal(principal).type(EventType.FACTOR_CREATED).factor(factor1))).doOnError(throwable -> auditService.report(AuditBuilder.builder(FactorAuditBuilder.class).principal(principal).type(EventType.FACTOR_CREATED).throwable(throwable)));
    }
        ...
        if (SMS_AM_FACTOR.equalsIgnoreCase(factor.getType())) {
            ...
            for(String code : countryCodes.split(",")) {
                if (!COUNTRY_CODES.contains(code.trim().toUpperCase(Locale.ROOT))) {
                    return Mono.error(new FactorConfigurationException(CONFIG_KEY_COUNTRY_CODES, code));
                }
            }
        }
...
@Override
    public Mono<Factor> update_migrated(String domain, String id, UpdateFactor updateFactor, User principal) {
        ...
        return factorRepository.findById_migrated(id).switchIfEmpty(Mono.error(new FactorNotFoundException(id))).flatMap(y->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Factor, SingleSource<Factor>>toJdkFunction(oldFactor -> {
...
return RxJava2Adapter.monoToSingle(Mono.just(y));
                    ...
                    return RxJava2Adapter.monoToCompletable(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToSingle(factorRepository.delete_migrated(factorId).then(eventService.create_migrated(event)))
                            .toCompletable()
                            .doOnComplete(() -> auditService.report(AuditBuilder.builder(FactorAuditBuilder.class).principal(principal).type(EventType.FACTOR_DELETED).factor(factor)))).doOnError(throwable -> auditService.report(AuditBuilder.builder(FactorAuditBuilder.class).principal(principal).type(EventType.FACTOR_DELETED).throwable(throwable))));
                }).apply(y)))).then())
                .onErrorResumeNext(ex -> {
                    if (ex instanceof AbstractManagementException) {
