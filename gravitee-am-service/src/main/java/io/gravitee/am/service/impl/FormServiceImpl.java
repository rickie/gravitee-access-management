...
@Override
    public Mono<List<Form>> copyFromClient_migrated(String domain, String clientSource, String clientTarget) {
        return findByDomainAndClient_migrated(domain, clientSource).flatMap(e->RxJava2Adapter.singleToMono(RxJavaReactorMigrationUtil.<Form, Single<Form>>toJdkFunction(source -> {
...
@Override
    public Mono<Form> update_migrated(ReferenceType referenceType, String referenceId, String id, UpdateForm updateForm, User principal) {
        ...
        return formRepository.findById_migrated(referenceType, referenceId, id).switchIfEmpty(Mono.error(new FormNotFoundException(id))).flatMap(y->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Form, SingleSource<Form>>toJdkFunction(oldForm -> {
...
private Mono<Form> create0_migrated(ReferenceType referenceType, String referenceId, String client, NewForm newForm, User principal) {
        ...
        return checkFormUniqueness_migrated(referenceType, referenceId, client, newForm.getTemplate().template()).flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Boolean, SingleSource<Form>>toJdkFunction(irrelevant -> {
                    Form form = new Form();
                    form.setId(formId);
