...
public class AlertNotifierServiceImpl implements io.gravitee.am.service.AlertNotifierService {
    ...
    public AlertNotifierServiceImpl(
            @Lazy AlertNotifierRepository alertNotifierRepository,
     ...
     * @return the alert notifier found or an {@link AlertNotifierNotFoundException} exception if it
     *     has not been found.
     */
    @Override
    public Single<AlertNotifier> getById(
        ...
        return this.alertNotifierRepository
                .findById(notifierId)
                .filter(
                        alertNotifier ->
                                alertNotifier.getReferenceType() == referenceType
                                        && alertNotifier.getReferenceId().equals(referenceId))
    ...
    /**
     * Find all the alert notifiers by reference (ex: domain) corresponding to the specified
     * criteria.
     *
     * @param referenceType the reference type.
     * @param referenceId the reference id.
    ...
    public Flowable<AlertNotifier> findByReferenceAndCriteria(
        ...
        LOGGER.debug(
                "Find alert notifier by {} {} and criteria {}",
                referenceType,
                referenceId,
                criteria);
     ...
     * @return the updated alert notifier or a {@link AlertNotifierNotFoundException} if the
     *     notifier has not been found.
     */
    @Override
     ...
     * @param referenceId the reference id.
     * @param notifierId the notifier identifier.
     * @return nothing if the alert notifier has been successfully delete or an {@link
     *     AlertNotifierNotFoundException} exception if it has not been found.
     */
    @Override
    ...
    private Single<AlertNotifier> updateInternal(
            AlertNotifier alertNotifier, User updatedBy, AlertNotifier previous) {
        alertNotifier.setUpdatedAt(new Date());
        return alertNotifierRepository
                .update(alertNotifier)
                .flatMap(
                        updated ->
                                eventService
                                        .create(
                        ...
                        eventService
                                .create(
                                        new Event(
                                                ...
                                                new Payload(
                                                        alertNotifier.getId(),
                                                        alertNotifier.getReferenceType(),
                                                        alertNotifier.getReferenceId(),
                                                        Action.DELETE)))
                                .ignoreElement())
                .doOnComplete(
                        () ->
                                auditService.report(
                                        AuditBuilder.builder(AlertNotifierAuditBuilder.class)
                                                .type(EventType.ALERT_NOTIFIER_DELETED)
