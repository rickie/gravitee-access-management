...
public class JWTConfiguration {
    ...
    protected JWTBuilder jwtBuilder(@Qualifier("managementSecretKey") Key key)
            throws InvalidKeyException {
        SignatureAlgorithm signatureAlgorithm =
                Keys.hmacShaSignatureAlgorithmFor(signingKeySecret.getBytes());
        return new DefaultJWTBuilder(key, signatureAlgorithm.getValue(), kid, issuer);
    }

    @Bean("managementJwtParser")
    protected JWTParser jwtParser(@Qualifier("managementSecretKey") Key key)
            throws InvalidKeyException {
