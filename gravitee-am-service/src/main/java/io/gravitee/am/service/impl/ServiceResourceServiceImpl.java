...
public class ServiceResourceServiceImpl implements ServiceResourceService {
    ...
    public Single<ServiceResource> create(String domain, NewServiceResource newServiceResource, User principal) {
        ...
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(serviceResourceRepository.create(resource)
                .flatMap(resource1 -> {
    ...
    public Single<ServiceResource> update(String domain, String id, UpdateServiceResource updateResource, User principal) {
        ...
        return RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(serviceResourceRepository.findById(id)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.error(new ServiceResourceNotFoundException(id)))))
                .flatMapSingle(oldServiceResource -> {
                    ...
                    return RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(serviceResourceRepository.update(factorToUpdate)
                            .flatMap(resource1 -> {
                                // send sync event to refresh plugins that are using this resource
                                ...
                                return eventService.create(event).flatMap(__ -> Single.just(resource1));
                            })).doOnSuccess(RxJavaReactorMigrationUtil.toJdkConsumer(factor1 -> auditService.report(AuditBuilder.builder(ServiceResourceAuditBuilder.class).principal(principal).type(EventType.RESOURCE_UPDATED).oldValue(oldServiceResource).resource(factor1)))))).doOnError(RxJavaReactorMigrationUtil.toJdkConsumer(throwable -> auditService.report(AuditBuilder.builder(ServiceResourceAuditBuilder.class).principal(principal).type(EventType.RESOURCE_UPDATED).throwable(throwable)))));
                })
                .onErrorResumeNext(ex -> {
                    if (ex instanceof AbstractManagementException) {
    ...
    public Completable delete(String domain, String resourceId, User principal) {
        ...
        return RxJava2Adapter.monoToCompletable(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(serviceResourceRepository.findById(resourceId)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.error(new ServiceResourceNotFoundException(resourceId))))))
                .flatMapSingle(resource ->
                    RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(factorService.findByDomain(domain)
                            .filter(factor -> factor.getConfiguration() != null && factor.getConfiguration().contains("\""+resourceId+"\""))
                            .toList()).flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<List<Factor>, SingleSource<ServiceResource>>toJdkFunction(factors -> {
                                        if (factors.isEmpty()) {
                            ...
                            return RxJava2Adapter.monoToCompletable(RxJava2Adapter.completableToMono(serviceResourceRepository.delete(resourceId)
