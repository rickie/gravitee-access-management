...
public class ServiceResourceServiceImpl implements ServiceResourceService {
    ...
    @Lazy @Autowired private ServiceResourceRepository serviceResourceRepository;
    ...
    public Maybe<ServiceResource> findById(String id) {
        ...
        return serviceResourceRepository
                .findById(id)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a resource using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a resource using its ID: %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Flowable<ServiceResource> findByDomain(String domain) {
        ...
        return serviceResourceRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find resources by domain", ex);
    ...
    @Override
    public Single<ServiceResource> create(
            String domain, NewServiceResource newServiceResource, User principal) {
        LOGGER.debug("Create a new resource {} for domain {}", newServiceResource, domain);
        ServiceResource resource = new ServiceResource();
        resource.setId(
                newServiceResource.getId() == null
                        ? RandomString.generate()
                        : newServiceResource.getId());
        ...
        return serviceResourceRepository
                        ...
                        resource1 -> {
                            ...
                            Event event =
                                    new Event(
                                            ...
                                            new Payload(
                                                    resource1.getId(),
                                                    resource1.getReferenceType(),
                                                    resource1.getReferenceId(),
                                                    Action.CREATE));
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to create a resource",
                                            ex));
                        });
    }

    @Override
    public Single<ServiceResource> update(
            String domain, String id, UpdateServiceResource updateResource, User principal) {
        ...
        return serviceResourceRepository
                ...
                .flatMapSingle(
                        oldServiceResource -> {
                            ServiceResource factorToUpdate =
                                    new ServiceResource(oldServiceResource);
                            ...
                            return serviceResourceRepository
                                    ...
                                    .flatMap(
                                            resource1 -> {
                                                // send sync event to refresh plugins that are using
                                                // this resource
                                                Event event =
                                                        new Event(
                                                                Type.RESOURCE,
                                                ...
                                                return eventService
                                                        .create(event)
                                                        .flatMap(__ -> Single.just(resource1));
                                            });
                        })
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update a resource",
                                            ex));
                        });
    }
        ...
        return serviceResourceRepository
                ...
                .flatMapSingle(
                        resource ->
                                factorService
                                        ...
                                        .filter(
                                                factor ->
                                                        factor.getConfiguration() != null
                                                                && factor.getConfiguration()
                                                                        .contains(
                                                                                "\""
                                                                                        + resourceId
                                                                                        + "\""))
                                        ...
                                        .flatMap(
                                                factors -> {
                                                    if (factors.isEmpty()) {
                                                        ...
                                                        return Single.error(
                                                                new ServiceResourceCurrentlyUsedException(
                                                                        resourceId,
                                                                        factors.get(0).getName(),
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete resource: {}",
                                    resourceId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete resource: %s",
                                                    resourceId),
                                            ex));
                        });
    }
