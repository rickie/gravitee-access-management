...
public class TokenServiceImpl implements TokenService {
        ...
        return applicationService
                ...
                .flatMap(
                        totalAccessTokens -> {
                            ...
                            totalToken.setTotalAccessTokens(
                                    totalAccessTokens.stream().mapToLong(Long::longValue).sum());
                            return Single.just(totalToken);
                        })
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find total tokens by domain: {}",
                                    domain,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find total tokens by domain: %s",
                                                    domain),
                                            ex));
                        });
    }
    ...
    public Single<TotalToken> findTotalTokensByApplication(Application application) {
        ...
        return countByClientId(application)
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find total tokens by application: {}",
                                    application,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find total tokens by application: %s",
                                                    application),
                                            ex));
                        });
    }
        ...
        return applicationService
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to find total tokens",
                                            ex));
                        });
    }
    ...
    public Completable deleteByUserId(String userId) {
        ...
        return accessTokenRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to delete tokens by user {}",
                                    userId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find total tokens by user: %s",
                                                    userId),
                                            ex));
                        });
    }
    ...
    private Single<Long> countByClientId(Application application) {
        ...
        return accessTokenRepository.countByClientId(
