...
public class DeviceServiceImpl implements DeviceService {
    ...
    public Flowable<Device> findByDomainAndUser(String domain, String user) {
        return deviceRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find Devices by {} {}",
                                    domain,
                                    user,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find Devices by %s %s",
                                                    domain, user),
                                            ex));
                        });
    }
    ...
    @Override
    public Single<Boolean> deviceExists(
            String domain, String client, String user, String rememberDevice, String deviceId) {
        return deviceRepository
    ...
    @Override
    public Single<Device> create(
            String domain,
            String client,
            String user,
            String rememberDevice,
            String deviceType,
            Long timeExpirationSeconds,
            String deviceId) {
        long expiresAt =
                System.currentTimeMillis()
                        + Optional.ofNullable(timeExpirationSeconds)
                                        .filter(Objects::nonNull)
                                        .filter(time -> time > 0L)
                                        .orElse(DEFAULT_DEVICE_EXPIRATION_TIME_SECONDS)
                                * 1000L;
    ...
    public Completable delete(String domain, String user, String deviceId, User principal) {
        return deviceRepository
                ...
                .flatMapCompletable(
                        device -> {
                            if (DOMAIN.equals(device.getReferenceType())
                                    && device.getReferenceId().equals(domain)
                                    && device.getUserId().equals(user)) {
                                return deviceRepository
                                        .delete(deviceId)
