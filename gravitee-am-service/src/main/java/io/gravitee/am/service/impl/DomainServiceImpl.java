...
@Override
    public Mono<Domain> findByHrid_migrated(String environmentId, String hrid) {
        ...
        return domainRepository.findByHrid_migrated(ReferenceType.ENVIRONMENT, environmentId, hrid).switchIfEmpty(Mono.error(new DomainNotFoundException(hrid))).onErrorResume(err->RxJava2Adapter.singleToMono(RxJavaReactorMigrationUtil.<Throwable, Single<Domain>>toJdkFunction(ex -> {
...
@Override
    public Flux<Domain> findAllByEnvironment_migrated(String organizationId, String environmentId) {
        ...
        return RxJava2Adapter.flowableToFlux(RxJava2Adapter.fluxToFlowable(environmentService.findById_migrated(environmentId, organizationId).map(RxJavaReactorMigrationUtil.toJdkFunction(Environment::getId)).flatMapMany(RxJavaReactorMigrationUtil.toJdkFunction((String ident) -> RxJava2Adapter.fluxToFlowable(domainRepository.findAllByReferenceId_migrated(ident)))))
                .onErrorResumeNext(ex -> {
...
@Override
    public Mono<List<Domain>> findAll_migrated() {
        ...
        return domainRepository.findAll_migrated().collectList().onErrorResume(err->RxJava2Adapter.singleToMono(RxJavaReactorMigrationUtil.<Throwable, Single<List<Domain>>>toJdkFunction(ex -> {
...
@Override
    public Mono<Domain> create_migrated(String organizationId, String environmentId, NewDomain newDomain, User principal) {
        ...
        return domainRepository.findByHrid_migrated(ReferenceType.ENVIRONMENT, environmentId, hrid).hasElement().flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Boolean, SingleSource<Domain>>toJdkFunction(empty -> {
                    if (!empty) {
                ...
                }).apply(v)))).flatMap(this::createSystemScopes_migrated).flatMap(this::createDefaultCertificate_migrated).flatMap(v->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Domain, SingleSource<Domain>>toJdkFunction(domain -> {
                    if (principal == null) {
...
@Override
    public Mono<Domain> update_migrated(String domainId, Domain domain) {
        ...
        return domainRepository.findById_migrated(domainId).switchIfEmpty(Mono.error(new DomainNotFoundException(domainId))).flatMap(y->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Domain, SingleSource<Domain>>toJdkFunction(__ -> {
...
@Override
    public Mono<Domain> patch_migrated(String domainId, PatchDomain patchDomain, User principal) {
        ...
        return domainRepository.findById_migrated(domainId).switchIfEmpty(Mono.error(new DomainNotFoundException(domainId))).flatMap(y->RxJava2Adapter.singleToMono(Single.wrap(RxJavaReactorMigrationUtil.<Domain, SingleSource<Domain>>toJdkFunction(oldDomain -> {
    ...
    public Mono<Void> delete_migrated(String domainId, User principal) {
        ...
        return RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(domainRepository.findById_migrated(domainId).switchIfEmpty(Mono.error(new DomainNotFoundException(domainId))).flatMap(y->RxJava2Adapter.completableToMono(Completable.wrap(RxJavaReactorMigrationUtil.toJdkFunction((Function<Domain, CompletableSource>)domain -> {
                    ...
                    return RxJava2Adapter.monoToCompletable(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(applicationService.findByDomain_migrated(domainId).flatMap(u->RxJava2Adapter.completableToMono(Completable.wrap(RxJavaReactorMigrationUtil.<Set<Application>, CompletableSource>toJdkFunction(applications -> {
                            ...
                            .doOnComplete(() -> auditService.report(AuditBuilder.builder(DomainAuditBuilder.class).principal(principal).type(EventType.DOMAIN_DELETED).domain(domain)))).doOnError(throwable -> auditService.report(AuditBuilder.builder(DomainAuditBuilder.class).principal(principal).type(EventType.DOMAIN_DELETED).throwable(throwable))));
                }).apply(y)))).then())
                .onErrorResumeNext(ex -> {
                    if (ex instanceof AbstractManagementException) {
