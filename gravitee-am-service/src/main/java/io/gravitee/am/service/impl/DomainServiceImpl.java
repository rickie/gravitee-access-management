...
public class DomainServiceImpl implements DomainService {
     ...
     * characters. Arbitrarily fix the maximum value to 2048 characters that seems enough to display
    ...
    @Autowired private VirtualHostValidator virtualHostValidator;
    ...
    @Autowired private ExtensionGrantService extensionGrantService;
    ...
    @Autowired private AlertTriggerService alertTriggerService;

    @Autowired private AlertNotifierService alertNotifierService;

    @Autowired private AuthenticationDeviceNotifierService authenticationDeviceNotifierService;
    ...
    public Maybe<Domain> findById(String id) {
        ...
        return domainRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurred while trying to find a domain using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurred while trying to find a domain using its ID: %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Single<Domain> findByHrid(String environmentId, String hrid) {
        ...
        return domainRepository
                .findByHrid(ReferenceType.ENVIRONMENT, environmentId, hrid)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error has occurred when trying to find a domain using its hrid: {}",
                                    hrid,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error has occurred when trying to find a domain using its hrid: %s",
                                                    hrid),
                                            ex));
                        });
    }
    ...
    public Flowable<Domain> search(String organizationId, String environmentId, String query) {
        ...
        return environmentService
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error has occurred when trying to search domains with query {} for environmentId {}",
                                    query,
                                    environmentId,
                                    ex);
                        });
    ...
    public Flowable<Domain> findAllByEnvironment(String organizationId, String environmentId) {
        LOGGER.debug(
                "Find all domains of environment {} (organization {})",
                environmentId,
                organizationId);
        ...
        return environmentService
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error has occurred when trying to find domains by environment",
                                    ex);
                        });
    }
        ...
        return domainRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurred while trying to find all domains",
                                            ex));
                        });
    }
    ...
    public Flowable<Domain> findByIdIn(Collection<String> ids) {
        ...
        return domainRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurred while trying to find domains by id in {}",
                                    ids,
                                    ex);
    ...
    @Override
    public Single<Domain> create(
            String organizationId, String environmentId, NewDomain newDomain, User principal) {
        ...
        return domainRepository
                        ...
                        empty -> {
                            if (!empty) {
                                ...
                                return environmentService
                                        ...
                                        .doOnSuccess(
                                                environment -> setDeployMode(domain, environment))
                                        .flatMapCompletable(
                                                environment -> validateDomain(domain, environment))
                                        .andThen(
                                                Single.defer(
                ...
                .flatMap(
                        domain -> {
                            ...
                            return roleService
                                    .findSystemRole(SystemRole.DOMAIN_PRIMARY_OWNER, DOMAIN)
                                    .switchIfEmpty(
                                            Single.error(
                                                    new InvalidRoleException(
                                                            "Cannot assign owner to the domain, owner role does not exist")))
                                    .flatMap(
                                            role -> {
                                                ...
                                                return membershipService
                                                        .addOrUpdate(organizationId, membership)
                                                        .map(__ -> domain);
                                            });
                        })
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurred while trying to create a domain",
                                            ex));
                        })
                .doOnSuccess(
                        domain ->
                                auditService.report(
                                        AuditBuilder.builder(DomainAuditBuilder.class)
                                                .principal(principal)
        ...
        return domainRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurred while trying to update a domain",
                                            ex));
                        });
    }
        ...
        return domainRepository
                            ...
                            return validateDomain(toPatch)
                                    ...
                                    .flatMap(
                                            domain1 -> {
                                                ...
                                                return eventService
                                                        .create(event)
                                                        .flatMap(__ -> Single.just(domain1));
                                            })
                                    .doOnSuccess(
                                            domain1 ->
                                                    auditService.report(
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurred while trying to patch a domain",
                                            ex));
                        });
    }
        ...
        return domainRepository
                ...
                .flatMapCompletable(
                        domain -> {
                            ...
                            return applicationService
                                    ...
                                    .flatMapCompletable(
                                            applications -> {
                                                List<Completable> deleteApplicationsCompletable =
                                                        applications.stream()
                                                                .map(
                                                                        a ->
                                                                                applicationService
                                    ...
                                    .andThen(
                                            identityProviderService
                                                    ...
                                                    .flatMapCompletable(
                                                            identityProvider ->
                                                                    identityProviderService.delete(
                                                                            domainId,
                                                                            identityProvider
                                                                                    .getId())))
                                    ...
                                    .andThen(
                                            membershipService
                                                    .findByReference(domainId, DOMAIN)
                                                    .flatMapCompletable(
                                                            membership ->
                                    ...
                                    .andThen(
                                            alertTriggerService
                                                    .findByDomainAndCriteria(
                                                            domainId, new AlertTriggerCriteria())
                                                    .flatMapCompletable(
                                                            alertTrigger ->
                                                                    alertTriggerService.delete(
                                                                            alertTrigger
                                                                                    .getReferenceType(),
                                                                            alertTrigger
                                                                                    .getReferenceId(),
                                    ...
                                    .andThen(
                                            authenticationDeviceNotifierService
                                                    .findByDomain(domainId)
                                                    .flatMapCompletable(
                                                            authDeviceNotifier ->
                                                                    authenticationDeviceNotifierService
                                    ...
                                    .andThen(
                                            i18nDictionaryService
                                                    ...
                                                    .flatMapCompletable(
                                                            i18nDictionary ->
                                                                    i18nDictionaryService.delete(
                                                                            DOMAIN,
                                                                            domainId,
                                                                            i18nDictionary.getId(),
                                                                            principal)))
                                    // delete theme
                                    // TODO improve this by implement a deleteThemeByReferenceId
                                    // method
                                    .andThen(
                                            themeService
                                    ...
                                    .andThen(
                                            passwordHistoryService.deleteByReference(
                                                    ReferenceType.DOMAIN, domainId))
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurred while trying to delete security domain {}",
                                    domainId,
                                    ex);
    ...
    private Completable validateDomain(Domain domain) {
        if (domain.getReferenceType() != ReferenceType.ENVIRONMENT) {
            return Completable.error(
                    new InvalidDomainException("Domain must be attached to an environment"));
        }
            ...
            if (domain.getOidc().getCibaSettings().getAuthReqExpiry() <= 0) {
                return Completable.error(
                        new InvalidDomainException(
                                "CIBA settings are invalid: auth_req_id expiry must be higher than 0"));
            }

            if (domain.getOidc().getCibaSettings().getTokenReqInterval() <= 0) {
                return Completable.error(
                        new InvalidDomainException(
                                "CIBA settings are invalid: token request interval must be higher than 0"));
            }

            if (domain.getOidc().getCibaSettings().getBindingMessageLength() <= 0) {
                return Completable.error(
                        new InvalidDomainException(
                                "CIBA settings are invalid: maLength of binding_message must be higher than 0"));
            }

            if (domain.getOidc().getCibaSettings().getBindingMessageLength()
                    > CIBA_MAX_BINDING_MESSAGE_LENGTH) {
                return Completable.error(
                        new InvalidDomainException(
                                "CIBA settings are invalid: binding_message length too high"));
            }
        }
        ...
        return domainRepository
                .findByHrid(domain.getReferenceType(), domain.getReferenceId(), domain.getHrid())
                .map(Optional::of)
                .defaultIfEmpty(Optional.empty())
                .flatMapCompletable(
                        optDomain -> {
                            if (optDomain.isPresent()
                                    && !optDomain.get().getId().equals(domain.getId())) {
                                return Completable.error(
                                        new DomainAlreadyExistsException(domain.getName()));
                            } else {
                                // Get environment domain restrictions and validate all data are
                                // correctly defined.
                                return environmentService
                                        .findById(domain.getReferenceId())
                                        .flatMapCompletable(
            ...
            for (String logoutRedirectUri : domain.getOidc().getPostLogoutRedirectUris()) {
                try {
                    ...
                    if (!domain.isRedirectUriWildcardAllowed()
                            && (nonNull(uri.getPath()) && uri.getPath().contains("*")
                    ...
                    throw new InvalidTargetUrlException(
                            "post_logout_redirect_uri : " + logoutRedirectUri + " is malformed");
                }
            }
        }
    }
    ...
    private Completable validateDomain(Domain domain, Environment environment) {
        ...
        return domainValidator
                .validate(domain, environment.getDomainRestrictions())
                .andThen(
                        findAll()
                                .flatMapCompletable(
                                        domains ->
                                                virtualHostValidator.validateDomainVhosts(
                                                        domain, domains)));
    }
    ...
    private void setDeployMode(Domain domain, Environment environment) {
        ...
        if (CollectionUtils.isEmpty(environment.getDomainRestrictions())) {
            ...
            // There are some domain restrictions defined at environment level. Switching to domain
            // vhost mode.

            // Creating one vhost per constraint.
            List<VirtualHost> vhosts =
                    environment.getDomainRestrictions().stream()
                            .map(
                                    domainConstraint -> {
