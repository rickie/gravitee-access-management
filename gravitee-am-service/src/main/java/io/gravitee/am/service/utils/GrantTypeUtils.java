...
public class GrantTypeUtils {
            ...
            Collections.unmodifiableSet(
                    new HashSet<>(
                            Arrays.asList(
                                    ...
                                    JWT_BEARER,
                                    UMA,
                                    CIBA_GRANT_TYPE // , DEVIDE_CODE, SAML2_BEARER
                                    )));
    ...
    public static Single<Application> validateGrantTypes(Application application) {
        ...
        if (application == null) {
            return Single.error(
                    new InvalidClientMetadataException("No application to validate grant"));
        }
        ...
        List<String> formattedClientGrantTypes =
                oAuthSettings.getGrantTypes() == null
                        ? null
                        : oAuthSettings.getGrantTypes().stream()
                                .map(str -> str.split(EXTENSION_GRANT_SEPARATOR)[0])
        ...
        Set<String> grantTypeSet =
                Collections.unmodifiableSet(new HashSet<>(oAuthSettings.getGrantTypes()));
        if (grantTypeSet.contains(REFRESH_TOKEN)) {
            // Hybrid is not managed yet and AM does not support refresh token for
            // client_credentials for now...
            List<String> allowedRefreshTokenGrant =
                    Arrays.asList(
                            AUTHORIZATION_CODE,
                            PASSWORD,
                            JWT_BEARER); // , CLIENT_CREDENTIALS, HYBRID);
            ...
            if (Collections.disjoint(formattedClientGrantTypes, allowedRefreshTokenGrant)) {
                return Single.error(
                        new InvalidClientMetadataException(
                                REFRESH_TOKEN
                                        + " grant type must be associated with one of "
                                        + String.join(", ", allowedRefreshTokenGrant)));
            }
        }
    ...
    public static List<String> getSupportedGrantTypes() {
        return Collections.unmodifiableList(
                SUPPORTED_GRANT_TYPES.stream().sorted().collect(Collectors.toList()));
    }
    ...
    public static boolean isRedirectUriRequired(List<String> grantTypes) {
        List<String> requireRedirectUri =
                Arrays.asList(IMPLICIT); // , HYBRID); Add Hybrid once supported...
     ...
     * Here is the following table lists response_type --> expected grant_type. code :
     * authorization_code id_token : implicit token id_token : implicit code id_token :
     * authorization_code, implicit code token : authorization_code, implicit code id_token token :
    ...
    public static Application completeGrantTypeCorrespondance(Application application) {
        ...
        ApplicationOAuthSettings oAuthSettings = application.getSettings().getOauth();
        Set responseType =
                oAuthSettings.getResponseTypes() != null
                        ? new HashSet<>(oAuthSettings.getResponseTypes())
                        : new HashSet();
        Set grantType =
                oAuthSettings.getGrantTypes() != null
                        ? new HashSet<>(oAuthSettings.getGrantTypes())
                        : new HashSet();
        ...
        // if grant type list has been modified, then update it.
            ...
            oAuthSettings.setGrantTypes(
                    (List<String>) grantType.stream().collect(Collectors.toList()));
        }
    ...
    public static Client completeGrantTypeCorrespondance(Client client) {
        ...
        Set responseType =
                client.getResponseTypes() != null
                        ? new HashSet<>(client.getResponseTypes())
                        : new HashSet();
        Set grantType =
                client.getAuthorizedGrantTypes() != null
                        ? new HashSet<>(client.getAuthorizedGrantTypes())
                        : new HashSet();
        ...
        // If grant_type contains client_credentials, remove refresh_token flow, only for old
        // clients created by the upgrader
        if (AM_V2_VERSION.equals(client.getSoftwareVersion())) {
            if (grantType.contains(CLIENT_CREDENTIALS)
                    && grantType.contains(REFRESH_TOKEN)
                    && grantType.size() == 2) {
