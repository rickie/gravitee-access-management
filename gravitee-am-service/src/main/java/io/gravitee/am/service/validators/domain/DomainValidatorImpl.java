...
public class DomainValidatorImpl implements DomainValidator {
    ...
    @Autowired
    public DomainValidatorImpl(
            PathValidator pathValidator, VirtualHostValidator virtualHostValidator) {
    ...
    public Completable validate(Domain domain, List<String> domainRestrictions) {
        ...
        if (domain.getName().contains("/")) {
            return Completable.error(
                    new InvalidDomainException("Domain name cannot contain '/' character"));
        }

        if (!CollectionUtils.isEmpty(domainRestrictions) && !domain.isVhostMode()) {
            return Completable.error(
                    new InvalidDomainException("Domain can only work in vhost mode"));
        }

        if (domain.isVhostMode()) {
            if (domain.getVhosts() == null || domain.getVhosts().isEmpty()) {
                return Completable.error(
                        new InvalidDomainException("VHost mode requires at least one VHost"));
            }

            // Check at there is only one vhost flagged with override entrypoint.
            long count =
                    domain.getVhosts().stream().filter(VirtualHost::isOverrideEntrypoint).count();
            if (count > 1) {
                return Completable.error(
                        new InvalidDomainException(
                                "Only one vhost can be used to override entrypoint"));
            } else if (count == 0) {
                return Completable.error(
                        new InvalidDomainException(
                                "You must select one vhost to override entrypoint"));
            }

            chain.addAll(
                    domain.getVhosts().stream()
                            .map(vhost -> virtualHostValidator.validate(vhost, domainRestrictions))
                            .collect(Collectors.toList()));
        } else {
            if ("/".equals(domain.getPath())) {
                return Completable.error(
                        new InvalidDomainException("'/' path is not allowed in context-path mode"));
            }
        ...
        if (domain.getWebAuthnSettings() != null
                && domain.getWebAuthnSettings().getCertificates() != null) {
            boolean containsInvalidCertFormat =
                    domain.getWebAuthnSettings().getCertificates().values().stream()
                            .anyMatch(
                                    cert ->
                                            !(cert instanceof String)
                                                    || (((String) cert).startsWith("-----")
                                                            || ((String) cert).endsWith("-----")));
            if (containsInvalidCertFormat) {
                return Completable.error(
                        new InvalidDomainException(
                                "WebAuthnSettings contains certificates with boundaries"));
            }
        }
