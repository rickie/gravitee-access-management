...
public class UserFactorUpdater {
    ...
    public static void updateFactors(
            List<EnrolledFactor> factors, User existingUser, User updatedUser) {
        if (factors != null) {
            factors.stream()
                    .filter(factor -> factor.getChannel() != null)
                    .forEach(
                            factor -> {
                                // Do not manage the SMS channel type because we can't know if it is
                                // a mobile phone number...
                                // in addition, we have to know the country related to the phone
                                // number that is an information provided by the user
                                // during the factor enrollment
                                EnrolledFactorChannel.Type channelType =
                                        factor.getChannel().getType();
                                if (EnrolledFactorChannel.Type.EMAIL.equals(channelType)) {
                                    ...
                                    } else if (existingUser.getEmails() != null
                                            && updatedUser.getEmails() != null) {
                                        List<String> existingEmails =
                                                existingUser.getEmails().stream()
                                                        .map(Attribute::getValue)
                                                        .collect(Collectors.toList());
                                        List<String> updatedEmails =
                                                updatedUser.getEmails().stream()
                                                        .map(Attribute::getValue)
                                                        .collect(Collectors.toList());
                                        List<String> existingRemaining =
                                                new ArrayList<>(existingEmails);
                                        List<String> updatedRemaining =
                                                new ArrayList<>(updatedEmails);
