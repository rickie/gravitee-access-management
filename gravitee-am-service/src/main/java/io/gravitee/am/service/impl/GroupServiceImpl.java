...
public class GroupServiceImpl implements GroupService {
    ...
    @Lazy @Autowired private GroupRepository groupRepository;

    @Autowired private UserService userService;

    @Autowired private OrganizationUserService organizationUserService;
    ...
    @Override
    public Single<Page<Group>> findAll(
        ...
        return groupRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find groups by {} {}",
                                    referenceType,
                                    referenceId,
                                    ex);
                            return Single.error(
    ...
    @Override
    public Maybe<Group> findByName(
            ReferenceType referenceType, String referenceId, String groupName) {
                            ...
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    ...
                                                    groupName, referenceType, referenceId),
                                            ex));
                        });
    }
    ...
    public Flowable<Group> findByMember(String memberId) {
        ...
        return groupRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a groups using member ",
                                    memberId,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a user using member: %s",
                                                    memberId),
                                            ex));
                        });
    }
    ...
    public Single<Group> findById(ReferenceType referenceType, String referenceId, String id) {
        ...
        return groupRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a group using its id {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a group using its id: %s",
                                                    id),
                                            ex));
                        })
    ...
    @Override
    public Single<Page<User>> findMembers(
            ReferenceType referenceType, String referenceId, String groupId, int page, int size) {
                            ...
                            if (group.getMembers() == null || group.getMembers().isEmpty()) {
                                ...
                                List<String> pagedMemberIds =
                                        sortedMembers.subList(
                                                ...
                                                Math.min(sortedMembers.size(), endOffset));
                                CommonUserService service =
                                        (group.getReferenceType() == ReferenceType.ORGANIZATION
                                                ? organizationUserService
                                                : userService);
                                return service.findByIdIn(pagedMemberIds)
                                        .toList()
                                        .map(
                                                users ->
                                                        new Page<>(
                                                                users, page, sortedMembers.size()));
                            }
                        });
    }
    ...
    public Flowable<Group> findByIdIn(List<String> ids) {
        ...
        return groupRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a group using ids {}",
                                    ids,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a group using ids: %s",
                                                    ids),
                                            ex));
                        });
    }
    ...
    public Single<Group> create(
        ...
        LOGGER.debug(
                "Create a new group {} for {} {}", newGroup.getName(), referenceType, referenceId);
                ...
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException) {
                                ...
                                return Single.error(
                                        new TechnicalManagementException(
                                                "An error occurs while trying to create a group",
                                                ex));
                            }
                        })
                .doOnSuccess(
                        group ->
                                auditService.report(
                                        AuditBuilder.builder(GroupAuditBuilder.class)
                                                .principal(principal)
    ...
    @Override
    public Single<Group> create(
            String domain, NewGroup newGroup, io.gravitee.am.identityprovider.api.User principal) {
        ...
        return findById(referenceType, referenceId, id)
                ...
                .flatMapMaybe(
                        existingGroup ->
                                groupRepository
                                        .findByName(
                                                referenceType, referenceId, updateGroup.getName())
                                        .map(Optional::of)
                            ...
                            return setMembers(groupToUpdate)
                                    ...
                                    .flatMap(
                                            group -> {
                                                ...
                                                return eventService
                                                        .create(event)
                                                        .flatMap(__ -> Single.just(group));
                                            })
                                    .doOnSuccess(
                                            group ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                                                                            GroupAuditBuilder.class)
                                                                    .principal(principal)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update a group", ex));
                        });
    }
        ...
        return findById(referenceType, referenceId, groupId)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete group: {}",
                                    groupId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete group: %s",
                                                    groupId),
                                            ex));
                        });
    }
