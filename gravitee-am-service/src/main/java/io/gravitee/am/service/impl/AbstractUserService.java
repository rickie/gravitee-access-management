...
public abstract class AbstractUserService<T extends CommonUserRepository>
    ...
    public Flowable<User> findByIdIn(List<String> ids) {
        ...
        return getUserRepository()
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find users by ids {}",
                                    userIds,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find users by ids %s",
                                                    userIds),
                                            ex));
                        });
    }
    ...
    public Single<Page<User>> findAll(
        ...
        return getUserRepository()
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find users by {} {}",
                                    referenceType,
                                    referenceId,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find users by %s %s",
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    ...
                                                    referenceType, referenceId, query),
                                            ex));
                        });
    }
    ...
    public Single<Page<User>> search(
            ...
            int size) {
        LOGGER.debug(
                "Search users for {} {} with filter {}",
                referenceType,
                referenceId,
                filterCriteria);
        return getUserRepository()
                .search(referenceType, referenceId, filterCriteria, page, size)
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to search users for {} {} and filter {}",
                                    referenceType,
                                    referenceId,
                                    filterCriteria,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find users for %s %s and filter %s",
                                                    referenceType, referenceId, filterCriteria),
                                            ex));
                        });
    }
    ...
    public Flowable<User> search(
            ReferenceType referenceType, String referenceId, FilterCriteria filterCriteria) {
        LOGGER.debug(
                "Search users for {} {} with filter {}",
                referenceType,
                referenceId,
                filterCriteria);
        return getUserRepository()
                .search(referenceType, referenceId, filterCriteria)
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof IllegalArgumentException) {
                                return Flowable.error(
                                        new InvalidParameterException(ex.getMessage()));
                            }
                            LOGGER.error(
                                    "An error occurs while trying to search users for {} {} and filter {}",
                                    referenceType,
    ...
    public Maybe<User> findByUsernameAndSource(
        ...
        LOGGER.debug(
                "Find user by {} {}, username and source: {} {}",
                referenceType,
                referenceId,
                username,
                source);
                            ...
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    ...
                                                    username, referenceType, referenceId, source),
                                            ex));
                        });
    }
    ...
    public Maybe<User> findByExternalIdAndSource(
        ...
        LOGGER.debug(
                "Find user by {} {}, externalId and source: {} {}",
                referenceType,
                referenceId,
                externalId,
                source);
                            ...
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    ...
                                                    externalId, referenceType, referenceId, source),
                                            ex));
                        });
    }
    ...
    public Single<User> create(ReferenceType referenceType, String referenceId, NewUser newUser) {
        ...
        return getUserRepository()
                .findByUsernameAndSource(
                        referenceType, referenceId, newUser.getUsername(), newUser.getSource())
                .isEmpty()
                .flatMap(
                        isEmpty -> {
                            if (!isEmpty) {
                ...
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException) {
                                ...
                                return Single.error(
                                        new TechnicalManagementException(
                                                "An error occurs while trying to create a user",
                                                ex));
                            }
                        });
    }
    ...
    public Single<User> update(
            ReferenceType referenceType, String referenceId, String id, UpdateUser updateUser) {
                ...
                .flatMapSingle(
                        oldUser -> {
                            ...
                            if (generatedDisplayName
                                    && !isNullOrEmpty(updateUser.getFirstName())
                                    && Objects.equals(
                                            updateUser.getDisplayName(),
                                            oldUser.getDisplayName())) {
        ...
        return getUserRepository()
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete user: {}", userId, ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete user: %s",
                                                    userId),
                                            ex));
                        });
    }
        ...
        return groupService
                ...
                .flatMap(
                        groups -> {
                            ...
                            if (user.getDynamicRoles() != null
                                    && !user.getDynamicRoles().isEmpty()) {
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to enhance user {}",
                                    user.getId(),
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to enhance user %s",
                                                    user.getId()),
                                            ex));
                        });
    }
