 ...
 */
@Component
public class AuthenticationDeviceNotifierServiceImpl
        implements AuthenticationDeviceNotifierService {
    /** Logger. */
    private final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationDeviceNotifierServiceImpl.class);

    @Lazy @Autowired private AuthenticationDeviceNotifierRepository adNotifierRepository;
    ...
    public Maybe<AuthenticationDeviceNotifier> findById(String id) {
        ...
        return adNotifierRepository
                .findById(id)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find an authentication device notifier using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find an authentication device notifier using its ID: %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Flowable<AuthenticationDeviceNotifier> findByDomain(String domain) {
        ...
        return adNotifierRepository
                .findByReference(ReferenceType.DOMAIN, domain)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find authentication device notifiers by domain",
                                    ex);
    ...
    @Override
    public Single<AuthenticationDeviceNotifier> create(
            String domain, NewAuthenticationDeviceNotifier newADNotifier, User principal) {
        LOGGER.debug(
                "Create a new authentication device notifier {} for domain {}",
                newADNotifier,
                domain);

        AuthenticationDeviceNotifier notifier = new AuthenticationDeviceNotifier();
        notifier.setId(
                newADNotifier.getId() == null ? RandomString.generate() : newADNotifier.getId());
        ...
        return adNotifierRepository
                ...
                .flatMap(
                        authDeviceNotifier -> {
                            ...
                            Event event =
                                    new Event(
                                            Type.AUTH_DEVICE_NOTIFIER,
                                            new Payload(
                                                    authDeviceNotifier.getId(),
                                                    authDeviceNotifier.getReferenceType(),
                                                    authDeviceNotifier.getReferenceId(),
                                                    Action.CREATE));
                            return eventService
                                    .create(event)
                                    .flatMap(__ -> Single.just(authDeviceNotifier));
                        })
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to create a notifier",
                                            ex));
                        })
                .doOnSuccess(
                        authDeviceNotifier ->
                                auditService.report(
                                        AuditBuilder.builder(AuthDeviceNotifierAuditBuilder.class)
                                                .principal(principal)
    ...
    @Override
    public Single<AuthenticationDeviceNotifier> update(
            String domain,
            String id,
            UpdateAuthenticationDeviceNotifier updateNotifier,
            User principal) {
        ...
        return adNotifierRepository
                ...
                .flatMapSingle(
                        oldNotifier -> {
                            AuthenticationDeviceNotifier notifierToUpdate =
                                    new AuthenticationDeviceNotifier(oldNotifier);
                            ...
                            return adNotifierRepository
                                    ...
                                    .flatMap(
                                            notifier -> {
                                                ...
                                                return eventService
                                                        .create(event)
                                                        .flatMap(__ -> Single.just(notifier));
                                            })
                                    .doOnSuccess(
                                            notifier ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                                                                            AuthDeviceNotifierAuditBuilder
                                                                                    .class)
                                                                    .principal(principal)
                                                                    .type(
                                                                            EventType
                                                                                    .AUTH_DEVICE_NOTIFIER_UPDATED)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to update authentication device notifier",
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update authentication device notifier",
    ...
    public Completable delete(String domainId, String notifierId, User principal) {
        ...
        return adNotifierRepository
                .findById(notifierId)
                .switchIfEmpty(
                        Maybe.error(new AuthenticationDeviceNotifierNotFoundException(notifierId)))
                .flatMapCompletable(
                        notifier -> {
                            // create event for sync process
                            Event event =
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete authentication device notifier: {}",
                                    notifierId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete authentication device notifier: %s",
                                                    notifierId),
                                            ex));
                        });
    }
