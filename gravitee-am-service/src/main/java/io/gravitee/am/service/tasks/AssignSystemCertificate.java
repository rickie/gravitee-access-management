...
public class AssignSystemCertificate extends AbstractTask<AssignSystemCertificateDefinition> {
    ...
    public AssignSystemCertificate(
        ...
        this.taskManager
                                        ...
                                        .flatMapSingle(
                                                cert -> {
                                                    if (cert.isPresent()) {
                                                        ...
                                                        logger.warn(
                                                                "System certificate {} doesn't exist, unable to assigne it to applications of domain {}",
                                                                renewedCertificate,
                                                                domainId);
                ...
                .subscribe(
                        apps -> {
                            if (apps >= 0) {
                                this.logger.info(
                                        "System certificate {} assigned to {} applications of domain {}",
                                        renewedCertificate,
                                        apps,
                                        domainId);
                                this.taskManager
                                ...
                                this.logger.debug(
                                        "Task already executed to assign system certificate for domain {}. (deprecated certificate: {} / new certificate: {})",
                                        domainId,
                                        deprecatedCertificate,
                                        renewedCertificate);
                            }
                        ...
                        error -> {
                            logger.warn(
                                    "System certificate {} can't be assigned to applications of domain {}: {}",
                                    renewedCertificate,
                                    domainId,
                                    error.getMessage());
                            if (rescheduledOnError()) {
                                logger.info(
                                        "Reschedule task {} to assign system certificate {}",
                                        getId(),
                                        renewedCertificate);
                                this.schedule();
                            } else {
                                this.taskManager
                                        .markAsError(this.getId())
                                        .doOnError(
                                                e ->
                                                        logger.warn(
                                                                "Unable to register error status for task {}",
                                                                this.getId(),
                                                                e))
                                        .subscribe();
                            }
