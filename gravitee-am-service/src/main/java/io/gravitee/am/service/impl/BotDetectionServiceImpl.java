...
public class BotDetectionServiceImpl implements BotDetectionService {
    ...
    @Lazy @Autowired private BotDetectionRepository botDetectionRepository;
    ...
    public Maybe<BotDetection> findById(String id) {
        ...
        return botDetectionRepository
                .findById(id)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a bot detection using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a bot detection using its ID: %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Flowable<BotDetection> findByDomain(String domain) {
        ...
        return botDetectionRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find bot detections by domain",
                                    ex);
    ...
    @Override
    public Single<BotDetection> create(
            String domain, NewBotDetection newBotDetection, User principal) {
        LOGGER.debug("Create a new bot detection {} for domain {}", newBotDetection, domain);

        BotDetection botDetection = new BotDetection();
        botDetection.setId(
                newBotDetection.getId() == null
                        ? RandomString.generate()
                        : newBotDetection.getId());
        ...
        return botDetectionRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to create a detection",
                                            ex));
                        });
    }

    @Override
    public Single<BotDetection> update(
            String domain, String id, UpdateBotDetection updateBotDetection, User principal) {
        ...
        return botDetectionRepository
                ...
                .flatMapSingle(
                        oldBotDetection -> {
                            ...
                            botDetectionToUpdate.setConfiguration(
                                    updateBotDetection.getConfiguration());
                            botDetectionToUpdate.setUpdatedAt(new Date());

                            return botDetectionRepository
                                    .update(botDetectionToUpdate)
                                    .flatMap(
                                            detection -> {
                                                // create event for sync process
                                                Event event =
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to update bot detection", ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update bot detection",
                                            ex));
                        });
    }
        ...
        return botDetectionRepository
                                            ...
                                            () ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                                                                            BotDetectionAuditBuilder
                                                                                    .class)
                                                                    .principal(principal)
                                                                    .type(
                                                                            EventType
                                                                                    .BOT_DETECTION_DELETED)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete bot detection: {}",
                                    botDetectionId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete bot detection: %s",
                                                    botDetectionId),
                                            ex));
                        });
    }

    private Function<BotDetection, SingleSource<? extends BotDetection>>
            checkBotDetectionReleasedByApp(String domainId, String botDetectionId) {
        return botDetection ->
                applicationService
    ...
    private Function<BotDetection, SingleSource<? extends BotDetection>>
            checkBotDetectionReleasedByDomain(String domainId, String botDetectionId) {
        return botDetection ->
                domainService
                        .findById(domainId)
                        .flatMapSingle(
                                domain -> {
                                    if (domain.getAccountSettings() != null
                                            && botDetectionId.equals(
                                                    domain.getAccountSettings()
                                                            .getBotDetectionPlugin())) {
                                        throw new BotDetectionUsedException();
                                    }
                                    return Single.just(botDetection);
