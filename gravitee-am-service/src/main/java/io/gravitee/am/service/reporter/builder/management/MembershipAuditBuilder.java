...
public class MembershipAuditBuilder extends ManagementAuditBuilder<MembershipAuditBuilder> {
    ...
    public MembershipAuditBuilder membership(Membership membership) {
        if (membership != null) {
            if (EventType.MEMBERSHIP_CREATED.equals(getType())
                    || EventType.MEMBERSHIP_UPDATED.equals(getType())) {
                setNewValue(membership);
            }

            if (membership.getReferenceType() == ReferenceType.DOMAIN
                    || membership.getReferenceType() == ReferenceType.APPLICATION) {
