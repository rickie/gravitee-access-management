...
public class PasswordHistoryService {
    ...
    public PasswordHistoryService(
            @Lazy PasswordHistoryRepository repository,
            AuditService auditService,
            @Named("argon2IdEncoder") PasswordEncoder passwordEncoder) {
    ...
    /**
     * Checks that a password has not already been used in the allotted history and adds it if it
     * has not. Returns a {@link PasswordHistory} instance on completion.
     *
     ...
     * @param rawPassword unencrypted password provided by the user. Passed separate from the user
     *     object as its password is nulled after creation to avoid leakage.
     * @param principal user performing this action
     * @param passwordSettings domain/application password settings
     * @return Single containing {@link PasswordHistory} or an error if the password was already in
     *     the history.
     */
    public Maybe<PasswordHistory> addPasswordToHistory(
        ...
        if (rawPassword == null
                ...
                || !passwordSettings.isPasswordHistoryEnabled()) {
            LOGGER.debug(
                    "Password history not added for user {} due to null password or settings, or because paswword history is disabled.",
                    user.getUsername());
            return Maybe.empty();
        }
        return repository
                .findUserHistory(referenceType, referenceId, user.getId())
                .toList()
                .flatMap(
                        passwordHistories -> {
                            if (passwordAlreadyUsed(rawPassword, passwordHistories)) {
                                return Single.error(
                                        () ->
                                                PasswordHistoryException.passwordAlreadyInHistory(
                                                        passwordSettings));
                            }
                            ...
                            if (passwordCount >= passwordSettings.getOldPasswords()) {
                                ...
                                return repository
                                        .delete(passwordHistories.get(0).getId())
                                        .andThen(
                                                repository.create(
                                                        getPasswordHistory(
                                                                referenceType,
                                ...
                                return repository.create(
                                        getPasswordHistory(
                                                referenceType, referenceId, user, rawPassword));
                            }
                        })
                .doOnSuccess(
                        a ->
     ...
     * @param passwordSettings domain/application password settings
     * @return Single containing a {@link Boolean} {@code true} if the password is already present
     *     the allotted number of previous passwords.
     */
    public Single<Boolean> passwordAlreadyUsed(
            ReferenceType referenceType,
            String referenceId,
            String userId,
            String password,
        ...
        return repository
                .findUserHistory(referenceType, referenceId, userId)
                .toList()
                .flatMap(
                        passwordHistories ->
                                Single.just(passwordAlreadyUsed(password, passwordHistories)));
    }
     ...
     */
    public Flowable<PasswordHistory> findUserHistory(
     ...
     */
    public Flowable<PasswordHistory> findByReference(
            ReferenceType referenceType, String referenceId) {
        LOGGER.debug(
                "Find password histories by reference id {} and reference type {}",
                referenceId,
                referenceType);
        return repository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "Error finding password histories by reference id {} and reference type {}",
                                    referenceId,
                                    referenceType,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "Error finding password histories by reference id %s and reference type %s",
    ...
    private boolean passwordAlreadyUsed(String password, List<PasswordHistory> passwordHistories) {
        return passwordHistories.stream()
                .anyMatch(
                        passwordHistory ->
                                passwordEncoder.matches(password, passwordHistory.getPassword()));
    }

    private PasswordHistory getPasswordHistory(
            ReferenceType referenceType,
            String referenceId,
            io.gravitee.am.model.User user,
