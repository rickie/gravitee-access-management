...
public class ResponseTypeUtils {
    ...
    private static final Set<String> SUPPORTED_RESPONSE_TYPES =
    ...
    public static List<String> getSupportedResponseTypes() {
        return Collections.unmodifiableList(
                SUPPORTED_RESPONSE_TYPES.stream().sorted().collect(Collectors.toList()));
    }
    ...
    /**
     * Before Dynamic Client Registration feature, there were no response type settings on Access
     * Management UI & API. In order to avoid breaking changes for legacy users, we'll add by
     * default all response type possible according to selected grant_type : authorization_code :
     * add code implicit : add token and id_token
     *
    ...
    public static boolean isImplicitFlow(String responseType) {
        return responseType != null
                && Arrays.stream(responseType.split("\\s"))
                        .allMatch(type -> type.equals(TOKEN) || type.equals(ID_TOKEN));
    }

    public static boolean isHybridFlow(String responseType) {
        return (CODE_ID_TOKEN.equals(responseType)
                || CODE_TOKEN.equals(responseType)
                || CODE_ID_TOKEN_TOKEN.equals(responseType));
    }

    public static boolean requireNonce(String responseType) {
        return responseType != null
                && (isHybridFlow(responseType)
                        || ID_TOKEN.equals(responseType)
                        || ID_TOKEN_TOKEN.equals(responseType));
    }
}
