...
public abstract class AbstractTask<Def extends TaskDefinition> implements Task {
    ...
    @JsonIgnore private TaskScheduler scheduler;
    ...
    public final void schedule() {
        if (this.scheduler != null) {
            ...
            this.scheduler.schedule(
                    this, Instant.now().plus(def.getDelay(), def.getUnit().toChronoUnit()));
        } else {
            logger.warn(
                    "Trying to schedule a {} task before the registration of the TaskScheduler",
                    this.kind());
        }
    }
