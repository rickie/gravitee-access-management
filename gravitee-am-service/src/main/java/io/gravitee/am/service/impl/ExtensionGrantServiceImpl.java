...
public class ExtensionGrantServiceImpl implements ExtensionGrantService {
    ...
    @Lazy @Autowired private ExtensionGrantRepository extensionGrantRepository;
    ...
    public Maybe<ExtensionGrant> findById(String id) {
        ...
        return extensionGrantRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find an extension grant using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find an extension grant using its ID: %s",
                                                    id),
                                            ex));
                        });
    }
    ...
    public Flowable<ExtensionGrant> findByDomain(String domain) {
        ...
        return extensionGrantRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find extension grants by domain",
                                    ex);
    ...
    @Override
    public Single<ExtensionGrant> create(
            String domain, NewExtensionGrant newExtensionGrant, User principal) {
        ...
        return extensionGrantRepository
                ...
                .flatMap(
                        empty -> {
                            if (!empty) {
                                throw new ExtensionGrantAlreadyExistsException(
                                        newExtensionGrant.getName());
                            } else {
                                ...
                                extensionGrant.setIdentityProvider(
                                        newExtensionGrant.getIdentityProvider());
                                ...
                                extensionGrant.setConfiguration(
                                        newExtensionGrant.getConfiguration());
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to create a extension grant", ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to create a extension grant",
                                            ex));
                        })
                .doOnSuccess(
                        extensionGrant ->
                                auditService.report(
                                        AuditBuilder.builder(ExtensionGrantAuditBuilder.class)
                                                .principal(principal)
    ...
    @Override
    public Single<ExtensionGrant> update(
            String domain, String id, UpdateExtensionGrant updateExtensionGrant, User principal) {
        ...
        return extensionGrantRepository
                                                ...
                                                existingTokenGranter -> {
                                                    if (existingTokenGranter.isPresent()
                                                            && !existingTokenGranter
                                                                    ...
                                                                    .equals(id)) {
                                                        throw new ExtensionGrantAlreadyExistsException(
                                                                "Extension grant with the same name already exists");
                                                    }
                                                    return Single.just(tokenGranter);
                                                }))
                .flatMap(
                        oldExtensionGrant -> {
                            ExtensionGrant extensionGrantToUpdate =
                                    new ExtensionGrant(oldExtensionGrant);
                            extensionGrantToUpdate.setName(updateExtensionGrant.getName());
                            extensionGrantToUpdate.setGrantType(
                                    updateExtensionGrant.getGrantType() != null
                                            ? updateExtensionGrant.getGrantType()
                                            : oldExtensionGrant.getGrantType());
                            extensionGrantToUpdate.setIdentityProvider(
                                    updateExtensionGrant.getIdentityProvider());
                            extensionGrantToUpdate.setCreateUser(
                                    updateExtensionGrant.isCreateUser());
                            extensionGrantToUpdate.setUserExists(
                                    updateExtensionGrant.isUserExists());
                            extensionGrantToUpdate.setConfiguration(
                                    updateExtensionGrant.getConfiguration());
                            ...
                            return extensionGrantRepository
                                    ...
                                    .flatMap(
                                            extensionGrant -> {
                                                ...
                                                return eventService
                                                        .create(event)
                                                        .flatMap(__ -> Single.just(extensionGrant));
                                            })
                                    .doOnSuccess(
                                            extensionGrant ->
                                                    auditService.report(
                                                            AuditBuilder.builder(
                                                                            ExtensionGrantAuditBuilder
                                                                                    .class)
                                                                    .principal(principal)
                                                                    .type(
                                                                            EventType
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to update a extension grant", ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update a extension grant",
                                            ex));
                        });
    }
        ...
        return extensionGrantRepository
                                ...
                                applicationService
                                        ...
                                        .flatMap(
                                                applications -> {
                                                    ...
                                                    // backward compatibility, check for old clients
                                                    // configuration
                                                    return Single.zip(
                                                            applicationService
                                                                    .findByDomainAndExtensionGrant(
                                                            ...
                                                            (clients1, extensionGrants) -> {
                                                                ...
                                                                // if clients use this grant_type,
                                                                // check it is the oldest one
                                                                Date minDate =
                                                                        Collections.min(
                                                                                extensionGrants
                                                                                        .stream()
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to extension grant: {}",
                                    extensionGrantId,
                                    ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete extension grant: %s",
                                                    extensionGrantId),
                                            ex));
                        });
    }
