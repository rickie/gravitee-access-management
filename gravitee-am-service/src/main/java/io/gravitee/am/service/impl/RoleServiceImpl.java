...
public class RoleServiceImpl implements RoleService {
    ...
    public Flowable<Role> findAllAssignable(
        ...
        return Flowable.merge(
                        findAllSystem(assignableType),
                        roleRepository.findAll(referenceType, referenceId))
                .filter(
                        role ->
                                assignableType == null
                                        || assignableType == role.getAssignableType())
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                            ...
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    ...
                                                    referenceType, referenceId, assignableType),
                                            ex));
                        });
    }
    ...
    public Single<Role> findById(ReferenceType referenceType, String referenceId, String id) {
        ...
        return roleRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find a role using its ID: {}",
                                    id,
                                    ex);
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find a role using its ID: %s",
                                                    id),
                                            ex));
                        })
    ...
    public Maybe<Role> findSystemRole(SystemRole systemRole, ReferenceType assignableType) {
        ...
        return roleRepository
                .findByNameAndAssignableType(
                        ReferenceType.PLATFORM, Platform.DEFAULT, systemRole.name(), assignableType)
                            ...
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    ...
                                                    systemRole.name(), assignableType),
                                            ex));
                        });
    }

    @Override
    public Flowable<Role> findRolesByName(
            ReferenceType referenceType,
            String referenceId,
            ReferenceType assignableType,
            List<String> roleNames) {
        return roleRepository
                .findByNamesAndAssignableType(referenceType, referenceId, roleNames, assignableType)
                .onErrorResumeNext(
                        ex -> {
                            String joinedRoles =
                                    roleNames.stream().collect(Collectors.joining(", "));
                            LOGGER.error(
                                    "An error occurs while trying to find roles : {}",
                                    joinedRoles,
                                    ex);
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find roles : %s",
                                                    joinedRoles),
                                            ex));
                        });
    }
    ...
    @Override
    public Maybe<Role> findDefaultRole(
            String organizationId, DefaultRole defaultRole, ReferenceType assignableType) {
        LOGGER.debug(
                "Find default role {} of organization {} for the type {}",
                defaultRole.name(),
                organizationId,
                assignableType);
        return roleRepository
    ...
    public Single<Set<Role>> findByIdIn(List<String> ids) {
        ...
        return roleRepository
                .findByIdIn(ids)
                .collect(() -> (Set<Role>) new HashSet<Role>(), Set::add)
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error("An error occurs while trying to find roles by ids", ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to find roles by ids",
                                            ex));
                        });
    }

    @Override
    public Single<Role> create(
        ...
        return checkRoleUniqueness(newRole.getName(), roleId, referenceType, referenceId)
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to create a role", ex));
                        })
                .doOnSuccess(
                        role ->
                                auditService.report(
                                        AuditBuilder.builder(RoleAuditBuilder.class)
                                                .principal(principal)
        ...
        return findById(referenceType, referenceId, id)
                .flatMap(
                        role -> {
                            ...
                            if (role.isDefaultRole()
                                    && !role.getName().equals(updateRole.getName())) {
                            ...
                            return checkRoleUniqueness(
                                    ...
                                    .flatMap(
                                            irrelevant -> {
                                                ...
                                                roleToUpdate.setDescription(
                                                        updateRole.getDescription());
                                                roleToUpdate.setPermissionAcls(
                                                        Permission.unflatten(
                                                                updateRole.getPermissions()));
                                                roleToUpdate.setOauthScopes(
                                                        updateRole.getOauthScopes());
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            return Single.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to update a role", ex));
                        });
    }
        ...
        return roleRepository
                ...
                .onErrorResumeNext(
                        ex -> {
                            ...
                            LOGGER.error(
                                    "An error occurs while trying to delete role: {}", roleId, ex);
                            return Completable.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to delete role: %s",
                                                    roleId),
                                            ex));
                        });
    }
                                                    ...
                                                    Event event =
                                                            new Event(
                                                                    ...
                                                                    new Payload(
                                                                            ...
                                                                            role1
                                                                                    .getReferenceType(),
                                                                            role1.getReferenceId(),
                                                                            Action.CREATE));
                                                    return eventService
                                                            .create(event)
                                                            .flatMap(__ -> Single.just(role1));
                                                })
                                        .onErrorResumeNext(
                                                ex -> {
                                                    if (ex instanceof AbstractManagementException) {
                                                    ...
                                                    return Single.error(
                                                            new TechnicalManagementException(
                                                                    "An error occurs while trying to create a role",
                                                                    ex));
                                                })
                                        .doOnSuccess(
                                                role1 ->
                                ...
                                return roleRepository
                                        ...
                                        .onErrorResumeNext(
                                                ex -> {
                                                    ...
                                                    return Single.error(
                                                            new TechnicalManagementException(
                                                                    "An error occurs while trying to update a role",
                                                                    ex));
                                                })
                                        .doOnSuccess(
                                                role1 ->
    ...
    private Single<Set<Role>> checkRoleUniqueness(
            String roleName, String roleId, ReferenceType referenceType, String referenceId) {
        return roleRepository
    ...
    private Flowable<Role> findAllSystem(ReferenceType assignableType) {
        ...
        return roleRepository
                ...
                .filter(
                        role ->
                                assignableType == null
                                        || role.getAssignableType() == assignableType);
    }
    ...
    private static List<Role> buildSystemRoles() {
        ...
        Map<Permission, Set<Acl>> platformAdminPermissions =
                Permission.allPermissionAcls(ReferenceType.PLATFORM);
        Map<Permission, Set<Acl>> organizationPrimaryOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.ORGANIZATION);
        Map<Permission, Set<Acl>> environmentPrimaryOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.ENVIRONMENT);
        Map<Permission, Set<Acl>> domainPrimaryOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.DOMAIN);
        Map<Permission, Set<Acl>> applicationPrimaryOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.APPLICATION);

        organizationPrimaryOwnerPermissions.put(Permission.ORGANIZATION, Acl.of(READ));
        organizationPrimaryOwnerPermissions.put(
                Permission.ORGANIZATION_SETTINGS, Acl.of(READ, UPDATE));
        ...
        applicationPrimaryOwnerPermissions.put(
                Permission.APPLICATION, Acl.of(READ, UPDATE, DELETE));
        ...
        roles.add(
                buildSystemRole(
                        ...
                        ReferenceType.ORGANIZATION,
                        organizationPrimaryOwnerPermissions));
        roles.add(
                buildSystemRole(
                        SystemRole.ENVIRONMENT_PRIMARY_OWNER.name(),
                        ReferenceType.ENVIRONMENT,
                        environmentPrimaryOwnerPermissions));
        roles.add(
                buildSystemRole(
                        SystemRole.DOMAIN_PRIMARY_OWNER.name(),
                        ReferenceType.DOMAIN,
                        domainPrimaryOwnerPermissions));
        roles.add(
                buildSystemRole(
                        SystemRole.APPLICATION_PRIMARY_OWNER.name(),
                        ReferenceType.APPLICATION,
                        applicationPrimaryOwnerPermissions));
    ...
    private List<Role> buildDefaultRoles(String organizationId) {
        ...
        Map<Permission, Set<Acl>> organizationOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.ORGANIZATION);
        Map<Permission, Set<Acl>> environmentOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.ENVIRONMENT);
        Map<Permission, Set<Acl>> domainOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.DOMAIN);
        Map<Permission, Set<Acl>> applicationOwnerPermissions =
                Permission.allPermissionAcls(ReferenceType.APPLICATION);
        ...
        roles.add(
                buildDefaultRole(
                        ...
                        ReferenceType.ENVIRONMENT,
                        organizationId,
                        environmentOwnerPermissions));
        roles.add(
                buildDefaultRole(
                        DefaultRole.DOMAIN_OWNER.name(),
                        ReferenceType.DOMAIN,
                        organizationId,
                        domainOwnerPermissions));
        roles.add(
                buildDefaultRole(
                        DefaultRole.APPLICATION_OWNER.name(),
                        ReferenceType.APPLICATION,
                        organizationId,
        ...
        roles.add(
                buildDefaultRole(
                        DefaultRole.ENVIRONMENT_USER.name(),
                        ReferenceType.ENVIRONMENT,
                        organizationId,
                        environmentUserPermissions));
        roles.add(
                buildDefaultRole(
                        DefaultRole.DOMAIN_USER.name(),
                        ReferenceType.DOMAIN,
                        organizationId,
                        domainUserPermissions));
        roles.add(
                buildDefaultRole(
                        DefaultRole.APPLICATION_USER.name(),
                        ReferenceType.APPLICATION,
                        organizationId,
    ...
    private static Role buildSystemRole(
            String name, ReferenceType assignableType, Map<Permission, Set<Acl>> permissions) {
