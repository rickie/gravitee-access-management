...
public class AuthenticationFlowContextServiceImpl implements AuthenticationFlowContextService {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationFlowContextServiceImpl.class);

    @Lazy @Autowired private AuthenticationFlowContextRepository authContextRepository;
                ...
                .map(
                        context -> {
                            if (context.getVersion() > 0
                                    && context.getVersion() < expectedVersion) {
                                LOGGER.debug(
                                        "Authentication Flow Context read with version '{}' but '{}' was expected",
                                        context.getVersion(),
                                        expectedVersion);
    ...
    @Override
    public Maybe<AuthenticationFlowContext> removeContext(
            String transactionId, int expectedVersion) {
        return this.loadContext(transactionId, expectedVersion)
                .doFinally(
                        () -> {
                            // fire and forget the deletion, in case of error the
                            // AuthenticationFLowContext TTL will finally delete the entry
                            // we don't want to stop the request processing due to deletion error if
                            // context is successfully loaded
                            clearContext(transactionId)
                                    .subscribe(
                                            () ->
                                                    LOGGER.debug(
                                                            "Deletion of Authentication Flow context '{}' succeeded after loading it",
                                                            transactionId),
                                            (error) ->
                                                    LOGGER.warn(
                                                            "Deletion of Authentication Flow context '{}' failed after loading it",
                                                            transactionId,
                                                            error));
                        });
    }
    ...
    public Single<AuthenticationFlowContext> updateContext(AuthenticationFlowContext authContext) {
        ...
        authContext.setExpireAt(
                new Date(now.plus(this.contextExpiration, ChronoUnit.SECONDS).toEpochMilli()));

        return authContextRepository.create(authContext);
    }

    /** Retry load context with delay from : https://stackoverflow.com/a/25292833 */
                        ...
                        if (throwable instanceof AuthenticationFlowConsistencyException) {
                            if (++retryCount < maxRetries) {
                                ...
                                return Flowable.timer(
                                        retryDelayMillis * (retryCount + 1), TimeUnit.MILLISECONDS);
                            }
                        }
