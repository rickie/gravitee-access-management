...
public class ResourceServiceTest {
    ...
    @Mock private AccessPolicyRepository accessPolicyRepository;
    ...
    @InjectMocks private ResourceService service = new ResourceServiceImpl();
    ...
    @Before
    public void setUp() {
        when(repository.findByDomainAndClientAndUser(DOMAIN_ID, CLIENT_ID, USER_ID))
                .thenReturn(Flowable.just(new Resource().setId(RESOURCE_ID)));
        when(repository.findByDomainAndClientAndUserAndResource(
                        DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
                .thenReturn(Maybe.just(new Resource().setId(RESOURCE_ID)));
        when(scopeService.findByDomainAndKeys(DOMAIN_ID, Arrays.asList("scope")))
                .thenReturn(Single.just(Arrays.asList(new Scope("scope"))));
    }
    ...
    public void delete_nonExistingResource() {
        ...
        TestObserver testObserver =
                service.delete(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID).test();
    ...
    public void update_nonExistingResource() {
        when(repository.findByDomainAndClientAndUserAndResource(
                ...
                .thenReturn(Maybe.empty());
        TestObserver testObserver =
                service.update(new NewResource(), DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)
                        .test();
        testObserver.assertError(ResourceNotFoundException.class);
    }
    ...
    public void update_scopeMissing() {
        NewResource newResource =
                new JsonObject("{\"resource_scopes\":[]}").mapTo(NewResource.class);
        Resource exitingRS = new Resource().setId(RESOURCE_ID).setDomain(DOMAIN_ID);
        when(repository.findByDomainAndClientAndUserAndResource(
                        DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
                .thenReturn(Maybe.just(exitingRS));
        TestObserver<Resource> testObserver =
                service.update(newResource, DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID).test();
    ...
    public void update_scopeNotFound() {
        NewResource newResource =
                new JsonObject("{\"resource_scopes\":[\"scope\"]}").mapTo(NewResource.class);
        Resource exitingRS = new Resource().setId(RESOURCE_ID).setDomain(DOMAIN_ID);
        when(scopeService.findByDomainAndKeys(DOMAIN_ID, Arrays.asList("scope")))
    ...
    public void update_malformedIconUri() {
        NewResource newResource =
                new JsonObject(
                                "{\"resource_scopes\":[\"scope\"],\"icon_uri\":\"badIconUriFormat\"}")
                        .mapTo(NewResource.class);
        Resource exitingRS = new Resource().setId(RESOURCE_ID).setDomain(DOMAIN_ID);
        when(repository.findByDomainAndClientAndUserAndResource(
                        DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
                .thenReturn(Maybe.just(exitingRS));
    ...
    public void update_existingResource() {
        NewResource newResource =
                new JsonObject(
                                "{\"resource_scopes\":[\"scope\"],\"icon_uri\":\"https://gravitee.io/icon\"}")
                        .mapTo(NewResource.class);
        Resource exitingRS = new Resource().setId(RESOURCE_ID).setDomain(DOMAIN_ID);
        when(repository.findByDomainAndClientAndUserAndResource(
                        DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
    ...
    public void create_success() {
        ...
        TestObserver<Resource> testObserver =
                service.create(newResource, DOMAIN_ID, CLIENT_ID, USER_ID).test();
    ...
    @Test
    public void list() {
        TestSubscriber<Resource> testSubscriber =
                service.listByDomainAndClientAndUser(DOMAIN_ID, CLIENT_ID, USER_ID).test();
        testSubscriber
                .assertNoErrors()
                .assertComplete()
    ...
    public void findByDomain_fail() {
        when(repository.findByDomain(DOMAIN_ID, 0, Integer.MAX_VALUE))
                .thenReturn(Single.error(new ArrayIndexOutOfBoundsException()));
    ...
    public void findByDomain_success() {
        when(repository.findByDomain(DOMAIN_ID, 0, Integer.MAX_VALUE))
                .thenReturn(Single.just(new Page<>(Collections.singleton(new Resource()), 0, 1)));
        TestObserver<Set<Resource>> testObserver = service.findByDomain(DOMAIN_ID).test();
        testObserver.awaitTerminalEvent();
    ...
    public void findByDomainAndClient() {
        when(repository.findByDomainAndClient(DOMAIN_ID, CLIENT_ID, 0, Integer.MAX_VALUE))
                .thenReturn(Single.just(new Page<>(Collections.emptyList(), 0, 0)));
        TestObserver testObserver =
                service.findByDomainAndClient(DOMAIN_ID, CLIENT_ID, 0, Integer.MAX_VALUE).test();
        testObserver.assertComplete().assertNoErrors();
        verify(repository, times(1))
                .findByDomainAndClient(DOMAIN_ID, CLIENT_ID, 0, Integer.MAX_VALUE);
    }

    @Test
    public void findByDomainAndClient_technicalFailure() {
        when(repository.findByDomainAndClient(DOMAIN_ID, CLIENT_ID, 0, Integer.MAX_VALUE))
                .thenReturn(Single.error(RuntimeException::new));
        TestObserver testObserver =
                service.findByDomainAndClient(DOMAIN_ID, CLIENT_ID, 0, Integer.MAX_VALUE).test();
    ...
    public void findByDomainAndClientAndResources() {
        when(repository.findByDomainAndClientAndResources(
                        DOMAIN_ID, CLIENT_ID, Collections.emptyList()))
                .thenReturn(Flowable.empty());
        TestSubscriber testSubscriber =
                service.findByDomainAndClientAndResources(
                                DOMAIN_ID, CLIENT_ID, Collections.emptyList())
                        .test();
        testSubscriber.assertComplete().assertNoErrors();
        verify(repository, times(1))
                .findByDomainAndClientAndResources(anyString(), anyString(), anyList());
    }

    @Test
    public void findByDomainAndClientResource() {
        when(repository.findByDomainAndClientAndResources(eq(DOMAIN_ID), eq(CLIENT_ID), anyList()))
                .thenReturn(Flowable.empty());
        TestObserver testObserver =
                service.findByDomainAndClientResource(DOMAIN_ID, CLIENT_ID, RESOURCE_ID).test();
        testObserver.assertComplete().assertNoErrors();
        verify(repository, times(1))
                .findByDomainAndClientAndResources(eq(DOMAIN_ID), eq(CLIENT_ID), anyList());
    }
    ...
    public void update() {
        ...
        Resource toUpdate =
                new Resource().setId(RESOURCE_ID).setDomain(DOMAIN_ID).setUpdatedAt(now);
    ...
    public void delete() {
        ...
        when(accessPolicyRepository.findByDomainAndResource(toDelete.getDomain(), toDelete.getId()))
                .thenReturn(Flowable.empty());
        when(repository.delete(RESOURCE_ID)).thenReturn(Completable.complete());
    ...
    public void findAccessPolicies() {
        ...
        when(accessPolicyRepository.findByDomainAndResource(DOMAIN_ID, RESOURCE_ID))
                .thenReturn(Flowable.just(accessPolicy));
        TestObserver<List<AccessPolicy>> testObserver =
                service.findAccessPolicies(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)
                        .toList()
                        .test();
        testObserver.assertComplete().assertNoErrors();
    ...
    public void findAccessPolicies_resourceNotFound() {
        when(repository.findByDomainAndClientAndUserAndResource(
                        DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
                .thenReturn(Maybe.empty());
        TestSubscriber<AccessPolicy> testSubscriber =
                service.findAccessPolicies(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID).test();
    ...
    public void findAccessPolicies_technicalFailure() {
        when(repository.findByDomainAndClientAndUserAndResource(
                        DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
                .thenReturn(Maybe.error(RuntimeException::new));
        TestSubscriber<AccessPolicy> testSubscriber =
                service.findAccessPolicies(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID).test();
    ...
    public void findAccessPoliciesByResources() {
        ...
        when(accessPolicyRepository.findByResources(resourceIds))
                .thenReturn(Flowable.just(accessPolicy));
        TestObserver<List<AccessPolicy>> testObserver =
                service.findAccessPoliciesByResources(resourceIds).toList().test();
        testObserver.assertComplete().assertNoErrors();
        testObserver.assertValue(accessPolicies -> accessPolicies.size() == 1);
    ...
    public void findAccessPoliciesByResources_technicalFailure() {
        List<String> resourceIds = Collections.singletonList(RESOURCE_ID);
        when(accessPolicyRepository.findByResources(resourceIds))
                .thenReturn(Flowable.error(RuntimeException::new));
        TestSubscriber<AccessPolicy> testObserver =
                service.findAccessPoliciesByResources(resourceIds).test();
        testObserver.assertNotComplete().assertError(TechnicalManagementException.class);
        verify(accessPolicyRepository, times(1)).findByResources(resourceIds);
    }
    ...
    public void countAccessPolicyByResource_technicalFailure() {
        when(accessPolicyRepository.countByResource(RESOURCE_ID))
                .thenReturn(Single.error(RuntimeException::new));
        TestObserver<Long> testObserver = service.countAccessPolicyByResource(RESOURCE_ID).test();
        testObserver.assertNotComplete().assertError(TechnicalManagementException.class);
        verify(accessPolicyRepository, times(1)).countByResource(RESOURCE_ID);
    ...
    public void findAccessPolicy() {
        ...
        TestObserver<AccessPolicy> testObserver =
                service.findAccessPolicy(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID, POLICY_ID)
                        .test();
        testObserver.assertComplete().assertNoErrors();
    ...
    public void findAccessPolicy_byId_technicalFailure() {
        when(accessPolicyRepository.findById(POLICY_ID))
                .thenReturn(Maybe.error(RuntimeException::new));
        TestObserver<AccessPolicy> testObserver = service.findAccessPolicy(POLICY_ID).test();
        testObserver.assertNotComplete().assertError(TechnicalManagementException.class);
        verify(accessPolicyRepository, times(1)).findById(POLICY_ID);
    ...
    public void createAccessPolicy() {
        ...
        TestObserver<AccessPolicy> testObserver =
                service.createAccessPolicy(accessPolicy, DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)
                        .test();
        testObserver.assertComplete().assertNoErrors();
        testObserver.assertValue(accessPolicy1 -> accessPolicy1.getId().equals(POLICY_ID));
        verify(repository, times(1))
    ...
    public void updateAccessPolicy() {
        ...
        TestObserver<AccessPolicy> testObserver =
                service.updateAccessPolicy(
                                accessPolicy, DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID, POLICY_ID)
                        .test();
        testObserver.assertComplete().assertNoErrors();
        testObserver.assertValue(accessPolicy1 -> accessPolicy1.getId().equals(POLICY_ID));
        verify(repository, times(1))
    ...
    public void deleteAccessPolicy() {
        ...
        TestObserver testObserver =
                service.deleteAccessPolicy(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID, POLICY_ID)
                        .test();
        testObserver.assertComplete().assertNoErrors();
        verify(repository, times(1))
                .findByDomainAndClientAndUserAndResource(
    ...
    public void getMetadata_noResources_null() {
        TestObserver<Map<String, Map<String, Object>>> testObserver =
                service.getMetadata(null).test();
        testObserver.assertComplete().assertNoErrors();
    ...
    public void getMetadata_noResources_empty() {
        TestObserver<Map<String, Map<String, Object>>> testObserver =
                service.getMetadata(Collections.emptyList()).test();
        testObserver.assertComplete().assertNoErrors();
        testObserver.assertValue(map -> map.isEmpty());
    }
    ...
    public void getMetadata_resources() {
        ...
        TestObserver<Map<String, Map<String, Object>>> testObserver =
                service.getMetadata(resources).test();
        testObserver.assertComplete().assertNoErrors();
    }
}
