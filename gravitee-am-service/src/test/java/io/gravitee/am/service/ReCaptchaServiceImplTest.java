...
public class ReCaptchaServiceImplTest {
    ...
    @InjectMocks private ReCaptchaServiceImpl reCaptchaService = new ReCaptchaServiceImpl();
    ...
    protected Single<HttpRequest> spyHttpRequest(HttpRequest httpRequest) {
        ...
        when(httpRequest)
                .thenAnswer(
                        invocationOnMock -> {
                            spyHttpRequest.complete(
                                    (HttpRequest) spy(invocationOnMock.callRealMethod()));
