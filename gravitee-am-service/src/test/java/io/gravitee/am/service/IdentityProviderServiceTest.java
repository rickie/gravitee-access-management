...
public class IdentityProviderServiceTest {
    ...
    public void shouldFindById() {
        when(identityProviderRepository.findById("my-identity-provider"))
    ...
    public void shouldFindByDomain() {
        when(identityProviderRepository.findAll(eq(ReferenceType.DOMAIN), eq(DOMAIN)))
                .thenReturn(Flowable.just(new IdentityProvider()));
        TestSubscriber<IdentityProvider> testObserver =
                identityProviderService.findByDomain(DOMAIN).test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldFindAllByType() {
        ...
        when(identityProviderRepository.findAll(ReferenceType.ORGANIZATION))
                .thenReturn(Flowable.just(identityProvider));

        TestSubscriber<IdentityProvider> obs =
                identityProviderService.findAll(ReferenceType.ORGANIZATION).test();
    ...
    public void shouldCreate() {
        ...
        when(identityProviderRepository.create(any(IdentityProvider.class)))
                .thenReturn(Single.just(idp));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver testObserver =
                identityProviderService.create(DOMAIN, newIdentityProvider).test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldUpdate() {
        ...
        when(identityProviderRepository.findById(
                        eq(ReferenceType.DOMAIN), eq(DOMAIN), eq("my-identity-provider")))
                .thenReturn(Maybe.just(new IdentityProvider()));
        when(identityProviderRepository.update(any(IdentityProvider.class)))
                .thenReturn(Single.just(idp));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver testObserver =
                identityProviderService
                        .update(DOMAIN, "my-identity-provider", updateIdentityProvider, false)
                        .test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldUpdate_isUpgrader_isSystem() {
        UpdateIdentityProvider updateIdentityProvider =
                (UpdateIdentityProvider) createIdentityProviders(true, false);
        IdentityProvider identityProvider = (IdentityProvider) createIdentityProviders(false, true);

        when(identityProviderRepository.findById(
                        eq(ReferenceType.DOMAIN), eq(DOMAIN), eq("my-identity-provider")))
                .thenReturn(Maybe.just(identityProvider));
        when(identityProviderRepository.update(any(IdentityProvider.class)))
                .thenReturn(Single.just(identityProvider));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver testObserver =
                identityProviderService
                        .update(DOMAIN, "my-identity-provider", updateIdentityProvider, true)
                        .test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldUpdate_notUpgrader_notSystem() {
        ...
        IdentityProvider identityProvider =
                (IdentityProvider) createIdentityProviders(false, false);
    ...
    public void shouldDelete_notExistingIdentityProvider() {
        ...
        TestObserver testObserver =
                identityProviderService.delete(DOMAIN, "my-identity-provider").test();
    ...
    public void shouldDelete() {
        ...
        when(identityProviderRepository.delete("my-identity-provider"))
