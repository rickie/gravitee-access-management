...
public class FormServiceTest {
    ...
    @InjectMocks private FormService formService = new FormServiceImpl();
    ...
    public void copyFromClient() {
        ...
        when(formRepository.findByClientAndTemplate(
        ...
        when(formRepository.findByClient(ReferenceType.DOMAIN, DOMAIN, sourceUid))
                .thenReturn(Flowable.just(formOne, formTwo));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver<List<Form>> testObserver =
                formService.copyFromClient(DOMAIN, sourceUid, targetUid).test();
    ...
    public void copyFromClient_duplicateFound() {
        ...
        when(formRepository.findByClientAndTemplate(
                        ReferenceType.DOMAIN, DOMAIN, targetUid, "login"))
                .thenReturn(Maybe.just(new Form()));
        when(formRepository.findByClient(ReferenceType.DOMAIN, DOMAIN, sourceUid))
                .thenReturn(Flowable.just(formOne));
    ...
    public void shouldFindAll_TechnicalException() {
        ...
        when(formRepository.findAll(ReferenceType.ORGANIZATION))
    ...
    public void shouldGetDefaultByDomainAndTemplate_TechnicalManagementException() {
        TestObserver<Form> obs =
                formService.getDefaultByDomainAndTemplate("test", "invalid").test();
        obs.awaitTerminalEvent();
