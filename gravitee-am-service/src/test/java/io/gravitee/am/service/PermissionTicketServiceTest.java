...
public class PermissionTicketServiceTest {
    ...
    @Mock PermissionTicketRepository repository;

    @Mock private ResourceService resourceService;

    @InjectMocks PermissionTicketService service = new PermissionTicketServiceImpl();
    ...
    public void create_errorSingleResource_missingResource() {
        ...
        List<PermissionRequest> request =
                Arrays.asList(
                        new PermissionRequest()
                                .setResourceId("one")
                                .setResourceScopes(Arrays.asList("a", "b")));

        when(resourceService.findByDomainAndClientAndResources(
                        DOMAIN_ID, CLIENT_ID, Arrays.asList("one")))
                .thenReturn(Flowable.empty());
        TestObserver<PermissionTicket> testObserver =
                service.create(request, DOMAIN_ID, CLIENT_ID).test();

        testObserver.assertNotComplete();
        testObserver.assertError(
                err ->
                        ((InvalidPermissionRequestException) err)
                                .getOAuth2ErrorCode()
                                .equals("invalid_resource_id"));
        ...
        when(resourceService.findByDomainAndClientAndResources(
                ...
                .thenReturn(
                        Flowable.just(
                                new Resource()
                                        .setId("one")
                                        .setResourceScopes(Arrays.asList("not", "same"))));
        TestObserver<PermissionTicket> testObserver =
                service.create(request, DOMAIN_ID, CLIENT_ID).test();
    ...
    public void create_successSingleResource() {
        ...
        when(repository.create(any()))
                .thenReturn(Single.just(new PermissionTicket().setId("success")));
        ...
        testObserver
                .assertNoErrors()
                .assertComplete()
                .assertValue(permissionTicket -> "success".equals(permissionTicket.getId()));
    ...
    public void create_errorMultipleResource_missingResource() {
        ...
        List<PermissionRequest> request =
                Arrays.asList(
                        ...
                        new PermissionRequest()
                                .setResourceId("two")
                                .setResourceScopes(Arrays.asList("c", "d")));

        // Prepare Resource
        Flowable<Resource> found =
                FlowableRange.just(
                        new Resource()
                                .setId("one")
                                .setResourceScopes(Arrays.asList("not", "same")));
        ...
        when(resourceService.findByDomainAndClientAndResources(
                        DOMAIN_ID, CLIENT_ID, Arrays.asList("one", "two")))
                .thenReturn(found);
        TestObserver<PermissionTicket> testObserver =
                service.create(request, DOMAIN_ID, CLIENT_ID).test();
    ...
    public void create_errorMultipleResource_moreThanOneResourceOwner() {
        ...
        // Prepare Resource
        Flowable<Resource> found =
                Flowable.fromIterable(request)
                        .map(
                                s ->
                                        new Resource()
                                                .setId(s.getResourceId())
                                                .setResourceScopes(s.getResourceScopes())
                                                .setUserId("user_" + s.getResourceId()));
    ...
    public void create_successMultipleResources_withDupliucatedResourceIds() {
        ...
        List<PermissionRequest> request =
                Arrays.asList(
                        new PermissionRequest()
                                .setResourceId("one")
                                .setResourceScopes(new ArrayList<>(Arrays.asList("a", "b"))),
                        new PermissionRequest()
                                .setResourceId("one")
                                .setResourceScopes(new ArrayList<>(Arrays.asList("c"))),
                        new PermissionRequest()
                                .setResourceId("two")
                                .setResourceScopes(new ArrayList<>(Arrays.asList("c", "d"))));
        ...
        ArgumentCaptor<PermissionTicket> permissionTicketArgumentCaptor =
                ArgumentCaptor.forClass(PermissionTicket.class);

        when(resourceService.findByDomainAndClientAndResources(
                        DOMAIN_ID, CLIENT_ID, Arrays.asList("one", "two")))
                .thenReturn(found);
        when(repository.create(permissionTicketArgumentCaptor.capture()))
                .thenReturn(Single.just(new PermissionTicket().setId("success")));
    ...
    private boolean assertMerged(PermissionTicket permissionTicket) {
        return permissionTicket.getPermissionRequest().size() == 2
                && permissionTicket.getPermissionRequest().stream()
                                .filter(
                                        pr ->
                                                (pr.getResourceId().equals("one")
                                                                && pr.getResourceScopes()
                                                                        .containsAll(
                                                                                Arrays.asList(
                                                                                        "a", "b",
                                                                                        "c")))
                                                        || (pr.getResourceId().equals("two")
                                                                && pr.getResourceScopes()
                                                                        .containsAll(
                                                                                Arrays.asList(
                                                                                        "c", "d"))))
