...
public class DeviceIdentifierServiceTest {
    ...
    @Mock private DeviceIdentifierRepository deviceIdentifierRepository;
    ...
    public void shouldFindById() {
        when(deviceIdentifierRepository.findById("device-identifier"))
                .thenReturn(Maybe.just(new DeviceIdentifier()));
    ...
    public void shouldFindByDomain() {
        when(deviceIdentifierRepository.findByReference(ReferenceType.DOMAIN, DOMAIN))
                .thenReturn(Flowable.just(new DeviceIdentifier()));
        TestSubscriber<DeviceIdentifier> testSubscriber =
                deviceIdentifierService.findByDomain(DOMAIN).test();
    ...
    public void shouldCreate() {
        ...
        when(deviceIdentifierRepository.create(any(DeviceIdentifier.class)))
                .thenReturn(Single.just(new DeviceIdentifier()));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver testObserver =
                deviceIdentifierService.create(DOMAIN, newDeviceIdentifier).test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldCreate_technicalException() {
        NewDeviceIdentifier newDeviceIdentifier = Mockito.mock(NewDeviceIdentifier.class);
        when(deviceIdentifierRepository.create(any()))
    ...
    public void shouldUpdate() {
        ...
        when(deviceIdentifierRepository.findById("device-identifier"))
                .thenReturn(Maybe.just(new DeviceIdentifier()));
        when(deviceIdentifierRepository.update(any(DeviceIdentifier.class)))
                .thenReturn(Single.just(new DeviceIdentifier()));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver testObserver =
                deviceIdentifierService
                        .update(DOMAIN, "device-identifier", updateDeviceIdentifier)
                        .test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldDelete_notDeviceIdentifier() {
        ...
        TestObserver testObserver =
                deviceIdentifierService.delete(DOMAIN, "device-identifier").test();
    ...
    public void shouldDelete() {
        ...
        when(deviceIdentifierRepository.findById(detection.getId()))
                .thenReturn(Maybe.just(detection));
        when(deviceIdentifierRepository.delete(detection.getId()))
                .thenReturn(Completable.complete());
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        TestObserver testObserver =
                deviceIdentifierService.delete(DOMAIN, detection.getId()).test();
        testObserver.awaitTerminalEvent();
