...
public class OrganizationUserServiceTest {
    ...
    @InjectMocks private OrganizationUserService userService = new OrganizationUserServiceImpl();
    ...
    @Mock private OrganizationUserRepository userRepository;

    @Mock private EventService eventService;

    @Mock private CredentialService credentialService;

    private static final String ORG = "organization1";
    ...
    public void shouldCreate() {
        ...
        TestObserver testObserver =
                userService.create(ReferenceType.ORGANIZATION, ORG, newUser).test();
    ...
    public void shouldUpdate() {
        ...
        when(userRepository.findById(ReferenceType.ORGANIZATION, ORG, user.getId()))
                .thenReturn(Maybe.just(user));
        when(userRepository.findByUsernameAndSource(
                        eq(ReferenceType.ORGANIZATION), eq(ORG), any(), any()))
                .thenReturn(Maybe.just(user));
        when(userRepository.update(any(User.class))).thenReturn(Single.just(user));
        when(eventService.create(any())).thenReturn(Single.just(new Event()));

        var testObserver =
                userService.update(ReferenceType.ORGANIZATION, ORG, "my-user", updateUser).test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldUpdate_technicalException() {
        UpdateUser updateUser = Mockito.mock(UpdateUser.class);
        when(userRepository.findById(eq(ReferenceType.ORGANIZATION), eq(ORG), eq("my-user")))
                .thenReturn(Maybe.just(new User()));

        TestObserver testObserver = new TestObserver();
        userService
                .update(ReferenceType.ORGANIZATION, ORG, "my-user", updateUser)
    ...
    public void shouldDelete() {
        ...
        when(eventService.create(any())).thenReturn(Single.just(new Event()));
        when(credentialService.findByUserId(
                        user.getReferenceType(), user.getReferenceId(), user.getId()))
    ...
    public void shouldDelete_with_webauthn_credentials() {
        ...
        when(credentialService.delete(credential.getId(), false))
