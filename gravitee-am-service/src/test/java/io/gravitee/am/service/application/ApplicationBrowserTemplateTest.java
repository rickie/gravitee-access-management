...
public class ApplicationBrowserTemplateTest {
    ...
    private ApplicationBrowserTemplate applicationServiceTemplate =
            new ApplicationBrowserTemplate();
    ...
    public void shouldApply_noGrantType() {
        ...
        Assert.assertTrue(
                oAuthSettings.getClientId() != null && !oAuthSettings.getClientId().isEmpty());
        Assert.assertTrue(
                oAuthSettings.getClientSecret() != null
                        && !oAuthSettings.getClientSecret().isEmpty());
        Assert.assertTrue(
                oAuthSettings
                        .getGrantTypes()
                        .containsAll(Arrays.asList(GrantType.AUTHORIZATION_CODE)));
        Assert.assertTrue(oAuthSettings.isForcePKCE());
        Assert.assertTrue(
                oAuthSettings.getTokenEndpointAuthMethod().equals(ClientAuthenticationMethod.NONE));
        Assert.assertTrue(
                oAuthSettings
                        .getResponseTypes()
                        .containsAll(new ArrayList<>(defaultAuthorizationCodeResponseTypes())));
    }
    ...
    public void shouldApply_existingGrantType() {
        ...
        oAuthSettings.setGrantTypes(
                Arrays.asList(
                        GrantType.AUTHORIZATION_CODE, GrantType.IMPLICIT, GrantType.REFRESH_TOKEN));
        ...
        Assert.assertTrue(
                oAuthSettings1.getClientId() != null && !oAuthSettings.getClientId().isEmpty());
        Assert.assertTrue(
                oAuthSettings1.getClientSecret() != null
                        && !oAuthSettings.getClientSecret().isEmpty());
        Assert.assertTrue(
