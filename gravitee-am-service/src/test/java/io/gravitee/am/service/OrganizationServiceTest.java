...
public class OrganizationServiceTest {
    ...
    @Mock private EntrypointService entrypointService;
    ...
    public void before() {
        ...
        cut =
                new OrganizationServiceImpl(
                        organizationRepository, roleService, entrypointService, auditService);
    }
    ...
    public void shouldCreateDefault() {
        ...
        when(organizationRepository.create(
                        argThat(organization -> organization.getId().equals(Organization.DEFAULT))))
                .thenReturn(Single.just(defaultOrganization));
        when(roleService.createDefaultRoles("DEFAULT")).thenReturn(Completable.complete());
        when(entrypointService.createDefaults(defaultOrganization))
                .thenReturn(Flowable.just(new Entrypoint()));
                ...
                .report(
                        argThat(
                                builder -> {
                                    ...
                                    assertEquals(
                                            defaultOrganization.getId(), audit.getReferenceId());
    ...
    public void shouldCreate() {
        ...
        when(organizationRepository.findById(ORGANIZATION_ID)).thenReturn(Maybe.empty());
        when(organizationRepository.create(
                        argThat(organization -> organization.getId().equals(ORGANIZATION_ID))))
        ...
        when(entrypointService.createDefaults(any(Organization.class)))
                .thenReturn(Flowable.just(new Entrypoint()));
        ...
        TestObserver<Organization> obs =
                cut.createOrUpdate(ORGANIZATION_ID, newOrganization, createdBy).test();
        ...
        obs.assertValue(
                organization -> {
                    ...
                    assertEquals(
                            ...
                            organization.getDomainRestrictions());
    ...
    public void shouldCreate_update() {
        ...
        when(organizationRepository.findById(ORGANIZATION_ID))
                .thenReturn(Maybe.just(existingOrganization));
        when(organizationRepository.update(
                        argThat(organization -> organization.getId().equals(ORGANIZATION_ID))))
                .thenAnswer(i -> Single.just(i.getArgument(0)));
    ...
    public void shouldUpdate() {
        ...
        TestObserver<Organization> obs =
                cut.update(ORGANIZATION_ID, patchOrganization, new DefaultUser("username")).test();
