...
public class EnrichAuthFlowPolicy {
    ...
    private static final String GATEWAY_POLICY_ENRICH_AUTH_FLOW_ERROR_KEY =
            "GATEWAY_POLICY_ENRICH_AUTH_FLOW_ERROR";
        ...
        if (configuration.getProperties() != null && !configuration.getProperties().isEmpty()) {
            ...
            AuthenticationFlowContext authContext =
                    (AuthenticationFlowContext)
                            context.getAttribute(ConstantKeys.AUTH_FLOW_CONTEXT_KEY);
            if (authContext == null) {
                // this should never happen because a default GraviteeContext with AuthFlowContext
                // is generated
                // when there are no data into the repository. This case may happen if the
                // GraviteeContextHandler
                // is not register on the route calling this policy in this case log a WARN but
                // continue to process the chain
                                ...
                                error ->
                                        policyChain.failWith(
                                                PolicyResult.failure(
                                                        GATEWAY_POLICY_ENRICH_AUTH_FLOW_ERROR_KEY,
                                                        error.getMessage())));
            }
        } else {
    ...
    private Single<AuthenticationFlowContext> enrichAuthFlowContext(
        ...
        AuthenticationFlowContext authContext =
                (AuthenticationFlowContext)
                        executionContext.getAttribute(ConstantKeys.AUTH_FLOW_CONTEXT_KEY);
