...
public class EnrichAuthFlowPolicyTest {
    ...
    @Mock private EnrichAuthFlowPolicyConfiguration configuration;

    @Mock private AuthenticationFlowContextRepository authContextRepository;
    ...
    public void init() {
        ...
        when(executionContext.getComponent(AuthenticationFlowContextRepository.class))
                .thenReturn(this.authContextRepository);
        when(environment.getProperty("authenticationFlow.expirationTimeOut", Integer.class, 300))
                .thenReturn(300);
    ...
    private void mockGraviteeContext(int version, Map<String, Object> data) {
        ...
        when(executionContext.getAttribute(ConstantKeys.AUTH_FLOW_CONTEXT_KEY))
                .thenReturn(authenticationFlowContext);
    }
        ...
        when(configuration.getProperties())
                .thenReturn(
                        Arrays.asList(
                                ...
                                new Property(
                                        "key-tpl", "{#request.params['" + REQUEST_PARAM + "']}")));
