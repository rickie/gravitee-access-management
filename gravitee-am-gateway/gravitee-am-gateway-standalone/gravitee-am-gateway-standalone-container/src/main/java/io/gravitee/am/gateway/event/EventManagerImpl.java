...
public class EventManagerImpl implements EventManager {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(io.gravitee.common.event.impl.EventManagerImpl.class);

    private ConcurrentMap<ComparableEventType, List<EventListenerWrapper>> listenersMap =
    ...
    public void publishEvent(Event event) {
        ...
        if (event.content() != null) {
            ...
            if (content instanceof Payload
                    && ((Payload) content).getReferenceType() == ReferenceType.DOMAIN) {
                domain = ((Payload) content).getReferenceId();
            } else if (content instanceof Domain) {
                domain = ((Domain) content).getId();
            } else if (content instanceof AuthenticationDetails
                    && ((AuthenticationDetails) content).getDomain() != null) {
        ...
        List<EventListenerWrapper> safeConcurrentListeners =
                Lists.newArrayList(listeners.iterator());
    ...
    @Override
    public <T extends Enum> void subscribeForEvents(
            EventListener<T, ?> eventListener, T... events) {
        for (T event : events) {
            addEventListener(
                    eventListener, (Class<T>) event.getClass(), Arrays.asList(events), null);
        }
    }

    @Override
    public <T extends Enum> void subscribeForEvents(
            EventListener<T, ?> eventListener, Class<T> events) {
        addEventListener(eventListener, events, EnumSet.allOf(events), null);
    }

    @Override
    public <T extends Enum> void subscribeForEvents(
            EventListener<T, ?> eventListener, Class<T> events, String domain) {
        addEventListener(eventListener, events, EnumSet.allOf(events), domain);
    }

    @Override
    public <T extends Enum> void unsubscribeForEvents(
            EventListener<T, ?> eventListener, Class<T> events, String domain) {
        this.listenersMap.remove(new ComparableEventType(events, domain));
    }

    @Override
    public <T extends Enum> void unsubscribeForCrossEvents(
            EventListener<T, ?> eventListener, Class<T> events, String domain) {
        List<EventListenerWrapper> listeners =
                this.listenersMap.get(new ComparableEventType(events, domain));
        if (listeners != null) {
            List<EventListenerWrapper> filteredList =
                    listeners.stream()
                            .filter(
                                    listenerWrapper ->
                                            !eventListener.equals(listenerWrapper.eventListener()))
    ...
    private <T extends Enum> void addEventListener(
            ...
            String domain) {
        LOGGER.info(
                "Register new listener {} for event type {}",
    ...
    private <T extends Enum> List<EventListenerWrapper> getEventListeners(
            Class<T> eventType, String domain) {
        List<EventListenerWrapper> listeners;
    ...
    private class ComparableEventType<T> implements Comparable<ComparableEventType<T>> {
        ...
        public int compareTo(ComparableEventType<T> o) {
            if (domain != null) {
                return (wrappedClass.getCanonicalName() + domain)
                        .compareTo(o.wrappedClass.getCanonicalName() + o.domain);
            }
