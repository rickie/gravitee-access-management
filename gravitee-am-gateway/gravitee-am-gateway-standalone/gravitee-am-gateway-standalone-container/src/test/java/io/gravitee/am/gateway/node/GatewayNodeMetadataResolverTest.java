...
public class GatewayNodeMetadataResolverTest {
    ...
    @Mock private Environment configuration;

    @InjectMocks private GatewayNodeMetadataResolver cut;
    ...
    public void shouldResolveOrganizations() {
        ...
        when(organizationRepository.findByHrids(Arrays.asList("france")))
                .thenReturn(Flowable.just(organization));
    ...
    public void shouldResolveOrganizationAndEnvironments() {
        ...
        assertEquals(singleton("org#1"), metadata.get(Node.META_ORGANIZATIONS));
        assertEquals(
                new HashSet<>(Arrays.asList("env#1", "env#2")),
                metadata.get(Node.META_ENVIRONMENTS));
    }
