...
public class UserInfoEndpointHandlerTest extends RxWebTestBase {
    ...
    public void setUp() throws Exception {
        ...
        when(env.getProperty("legacy.openid.openid_scope_full_profile", boolean.class, false))
                .thenReturn(false);
        userInfoEndpoint =
                new UserInfoEndpoint(
                        userService, jwtService, jweService, openIDDiscoveryService, env);
    ...
    public void shouldNotInvokeUserEndpoint_invalidToken_noClient() throws Exception {
        ...
        router.route()
                .order(-1)
                .handler(createOAuth2AuthHandler(oAuth2AuthProvider(new InvalidClientException())));
    ...
    public void shouldNotInvokeUserEndpoint_unknownToken() throws Exception {
        ...
        router.route()
                .order(-1)
                .handler(createOAuth2AuthHandler(oAuth2AuthProvider(new InvalidTokenException())));
    ...
    public void shouldNotInvokeUserEndpoint_clientOnlyToken() throws Exception {
        ...
        router.route()
                .order(-1)
                .handler(createOAuth2AuthHandler(oAuth2AuthProvider(token, client)));
                ...
                resp ->
                        resp.bodyHandler(
                                body -> {
                                    ...
                                    assertTrue(claims.containsKey(CustomClaims.ROLES));
                                    assertTrue(
                                            ((List) claims.get(CustomClaims.ROLES))
                                                    .containsAll(
                ...
                resp ->
                        resp.bodyHandler(
                                body -> {
                                    ...
                                    assertTrue(claims.containsKey(CustomClaims.GROUPS));
                                    assertTrue(
                                            ((List) claims.get(CustomClaims.GROUPS))
                                                    .containsAll(
    ...
    @Test
    public void shouldInvokeUserEndpoint_scopesRequest_roles_groups_full_profile()
            throws Exception {
        JWT jwt = new JWT();
        jwt.setJti("id-token");
        jwt.setAud("client-id");
    ...
    @Test
    public void shouldInvokeUserEndpoint_scopesRequest_and_claimsRequest_signedResponse()
            throws Exception {
        JWT jwt = new JWT();
        jwt.setJti("id-token");
        jwt.setAud("client-id");
        ...
        when(jweService.encryptUserinfo("signedJwtBearer", client))
                .thenReturn(Single.just("signedJwtBearer"));
        ...
        testRequest(
                ...
                resp -> {
                    assertEquals(
                            MediaType.APPLICATION_JWT, resp.getHeader(HttpHeaders.CONTENT_TYPE));
    ...
    private OAuth2AuthProvider oAuth2AuthProvider() {
        return new OAuth2AuthProvider() {
            ...
            public void decodeToken(
                    ...
                    Handler<AsyncResult<OAuth2AuthResponse>> handler) {
