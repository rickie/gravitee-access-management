...
public class UserInfoEndpointHandlerTest extends RxWebTestBase {
    ...
    public void shouldNotInvokeUserEndpoint_userNotFound() throws Exception {
        ...
        when(userService.findById_migrated(anyString())).thenReturn(Mono.empty());
    ...
    public void shouldInvokeUserEndpoint() throws Exception {
        ...
        when(userService.findById_migrated(anyString())).thenReturn(Mono.just(user));
    ...
    public void shouldInvokeUserEndpoint_scopesRequest_roles_noRole() throws Exception {
        ...
        when(userService.findById_migrated(anyString())).thenReturn(Mono.just(user));
        when(userService.enhance_migrated(user)).thenReturn(Mono.just(user));
    ...
    public void shouldInvokeUserEndpoint_scopesRequest_and_claimsRequest_signedResponse() throws Exception {
        ...
        when(userService.findById_migrated(anyString())).thenReturn(Mono.just(user));
        when(jwtService.encodeUserinfo_migrated(any(),any())).thenReturn(Mono.just("signedJwtBearer"));
        when(jweService.encryptUserinfo_migrated("signedJwtBearer",client)).thenReturn(Mono.just("signedJwtBearer"));
