...
public class AuthorizationCodeServiceTest {
    ...
    public void shouldCreate_noExistingCode() {
        ...
        when(authorizationCodeRepository.create(any()))
                .thenReturn(Single.just(new AuthorizationCode()));

        TestObserver<AuthorizationCode> testObserver =
                authorizationCodeService.create(authorizationRequest, user).test();
    ...
    public void shouldRemove_existingCode() {
        ...
        when(authorizationCodeRepository.findByCode(authorizationCode.getCode()))
                .thenReturn(Maybe.just(authorizationCode));
        when(authorizationCodeRepository.delete(authorizationCode.getId()))
                .thenReturn(Maybe.just(authorizationCode));
        when(accessTokenRepository.findByAuthorizationCode(authorizationCode.getCode()))
                .thenReturn(Observable.empty());

        TestObserver<AuthorizationCode> testObserver =
                authorizationCodeService.remove(authorizationCode.getCode(), client).test();
        testObserver.assertComplete();
        testObserver.assertNoErrors();
    ...
    public void shouldRemove_invalidCode_existingTokens_noRefreshToken() {
        ...
        when(accessTokenRepository.findByAuthorizationCode(anyString()))
                .thenReturn(Observable.fromIterable(tokens));
