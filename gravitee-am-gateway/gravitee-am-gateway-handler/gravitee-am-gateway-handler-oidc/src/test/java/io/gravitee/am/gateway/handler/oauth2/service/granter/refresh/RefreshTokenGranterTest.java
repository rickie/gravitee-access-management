...
public class RefreshTokenGranterTest {
    ...
    public void shouldGenerateAnAccessToken() {
        ...
        when(tokenService.create_migrated(any(), any(), any())).thenReturn(Mono.just(accessToken));
        when(tokenService.refresh_migrated(refreshToken, tokenRequest, client)).thenReturn(Mono.just(new RefreshToken(refreshToken)));

        TestObserver<Token> testObserver = RxJava2Adapter.monoToSingle(granter.grant_migrated(tokenRequest, client)).test();
    ...
    public void shouldNotGenerateAnAccessToken_invalidRequest() {
        ...
        RxJava2Adapter.monoToSingle(granter.grant_migrated(tokenRequest, client)).test().assertError(InvalidRequestException.class);
    }
    ...
    public void shouldNotGenerateAnAccessToken_invalidGrant() {
        ...
        when(tokenService.refresh_migrated(refreshToken, tokenRequest, client)).thenReturn(Mono.error(new InvalidGrantException()));

        RxJava2Adapter.monoToSingle(granter.grant_migrated(tokenRequest, client)).test().assertError(InvalidGrantException.class);
    }
}
