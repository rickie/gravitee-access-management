...
public class ResourceRegistrationEndpointTest {
    ...
    @InjectMocks
    private ResourceRegistrationEndpoint endpoint =
            new ResourceRegistrationEndpoint(domain, service);
    ...
    public void list_anyError() {
        when(service.listByDomainAndClientAndUser(anyString(), anyString(), anyString()))
                .thenReturn(Flowable.error(new RuntimeException()));
        endpoint.handle(context);
        verify(context, times(1)).fail(errCaptor.capture());
        Assert.assertTrue(
                "Error must be propagated", errCaptor.getValue() instanceof RuntimeException);
    }

    @Test
    public void list_noResources() {
        when(service.listByDomainAndClientAndUser(DOMAIN_ID, CLIENT_ID, USER_ID))
    ...
    public void create_invalidResourceBody() {
        when(context.getBodyAsJson())
                .thenReturn(new JsonObject(Json.encode(new Resource().setId(RESOURCE_ID))));
    ...
    public void create_noResource() {
        when(context.getBodyAsJson())
                .thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.create(any(), eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID)))
                .thenReturn(Single.error(new ResourceNotFoundException(RESOURCE_ID)));
        endpoint.create(context);
        verify(context).fail(errCaptor.capture());
    ...
    public void create_withResource() {
        ...
        when(context.getBodyAsJson())
                .thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.create(any(), eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID)))
                .thenReturn(Single.just(new Resource().setId(RESOURCE_ID)));
        ...
        Assert.assertEquals(
                "Location",
                "http://host" + DOMAIN_PATH + "/uma/protection/resource_set/" + RESOURCE_ID,
                strCaptor.getValue());
    }

    @Test
    public void get_noResource() {
        when(service.findByDomainAndClientAndUserAndResource(
    ...
    public void update_invalidResourceBody() {
        when(context.getBodyAsJson())
                .thenReturn(new JsonObject("{\"description\":\"mydescription\"}"));
    ...
    public void update_noResource() {
        when(context.getBodyAsJson())
                .thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.update(any(), eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID), eq(RESOURCE_ID)))
                .thenReturn(Single.error(new ResourceNotFoundException(RESOURCE_ID)));
        endpoint.update(context);
        verify(context).fail(errCaptor.capture());
        Assert.assertTrue(errCaptor.getValue() instanceof ResourceNotFoundException);
    ...
    public void update_withResource() {
        when(context.getBodyAsJson())
                .thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.update(any(), eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID), eq(RESOURCE_ID)))
                .thenReturn(Single.just(new Resource()));
        endpoint.update(context);
        verify(response, times(1)).putHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        verify(response, times(1)).setStatusCode(intCaptor.capture());
        Assert.assertEquals("Should be ok", 200, intCaptor.getValue().intValue());
    ...
    public void delete_noResource() {
        when(service.delete(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID))
                .thenReturn(Completable.error(new ResourceNotFoundException(RESOURCE_ID)));
