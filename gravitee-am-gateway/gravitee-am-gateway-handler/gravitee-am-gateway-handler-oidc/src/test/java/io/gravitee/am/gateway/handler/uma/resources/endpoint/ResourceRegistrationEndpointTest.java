...
public class ResourceRegistrationEndpointTest {
    ...
    public void list_anyError() {
        when(service.listByDomainAndClientAndUser_migrated(anyString(), anyString(), anyString())).thenReturn(Flux.error(new RuntimeException()));
    ...
    public void list_noResources() {
        when(service.listByDomainAndClientAndUser_migrated(DOMAIN_ID, CLIENT_ID, USER_ID)).thenReturn(Flux.empty());
        endpoint.handle(context);
    ...
    public void list_withResources() {
        when(service.listByDomainAndClientAndUser_migrated(DOMAIN_ID, CLIENT_ID, USER_ID)).thenReturn(Flux.just(new Resource().setId(RESOURCE_ID)));
        endpoint.handle(context);
        verify(response, times(1)).putHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        verify(response, times(1)).setStatusCode(intCaptor.capture());
    ...
    public void create_noResource() {
        ...
        when(service.create_migrated(any() , eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID))).thenReturn(Mono.error(new ResourceNotFoundException(RESOURCE_ID)));
        endpoint.create(context);
        verify(context).fail(errCaptor.capture());
    ...
    public void create_withResource() {
        ...
        when(context.getBodyAsJson()).thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.create_migrated(any() , eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID))).thenReturn(Mono.just(new Resource().setId(RESOURCE_ID)));
    ...
    public void get_noResource() {
        when(service.findByDomainAndClientAndUserAndResource_migrated(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)).thenReturn(Mono.empty());
    ...
    public void get_withResource() {
        when(service.findByDomainAndClientAndUserAndResource_migrated(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)).thenReturn(Mono.just(new Resource().setId(RESOURCE_ID)));
        endpoint.get(context);
        verify(response, times(1)).putHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        verify(response, times(1)).setStatusCode(intCaptor.capture());
        Assert.assertEquals("Should be ok",200, intCaptor.getValue().intValue());
    ...
    public void update_noResource() {
        when(context.getBodyAsJson()).thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.update_migrated(any() , eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID), eq(RESOURCE_ID))).thenReturn(Mono.error(new ResourceNotFoundException(RESOURCE_ID)));
        endpoint.update(context);
        verify(context).fail(errCaptor.capture());
        Assert.assertTrue(errCaptor.getValue() instanceof ResourceNotFoundException);
    }
    ...
    public void update_withResource() {
        when(context.getBodyAsJson()).thenReturn(new JsonObject("{\"id\":\"rs_id\",\"resource_scopes\":[\"scope\"]}"));
        when(service.update_migrated(any() , eq(DOMAIN_ID), eq(CLIENT_ID), eq(USER_ID), eq(RESOURCE_ID))).thenReturn(Mono.just(new Resource()));
        endpoint.update(context);
        verify(response, times(1)).putHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        verify(response, times(1)).setStatusCode(intCaptor.capture());
        Assert.assertEquals("Should be ok",200, intCaptor.getValue().intValue());
    ...
    public void delete_noResource() {
        when(service.delete_migrated(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)).thenReturn(Mono.error(new ResourceNotFoundException(RESOURCE_ID)));
    ...
    public void delete_withResource() {
        when(service.delete_migrated(DOMAIN_ID, CLIENT_ID, USER_ID, RESOURCE_ID)).thenReturn(Mono.empty());
        endpoint.delete(context);
        verify(response, times(1)).putHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        verify(response, times(1)).setStatusCode(intCaptor.capture());
        Assert.assertEquals("Should be no content status",204, intCaptor.getValue().intValue());
