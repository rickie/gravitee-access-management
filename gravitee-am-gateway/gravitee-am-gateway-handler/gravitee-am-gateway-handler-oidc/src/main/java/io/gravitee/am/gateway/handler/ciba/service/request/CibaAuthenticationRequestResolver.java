 ...
 */
public class CibaAuthenticationRequestResolver
        extends AbstractRequestResolver<CibaAuthenticationRequest> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(CibaAuthenticationRequestResolver.class);
    ...
    public CibaAuthenticationRequestResolver(
            Domain domain, JWSService jwsService, JWKService jwkService, UserService userService) {
        this.domain = domain;
    ...
    public Single<CibaAuthenticationRequest> resolve(
            CibaAuthenticationRequest authRequest, Client client) {
        return resolveAuthorizedScopes(authRequest, client, null)
                .flatMap(
                        req -> {
                            if (!StringUtils.isEmpty(req.getIdTokenHint())) {
                                return parseJwt(req.getIdTokenHint())
                                        .flatMap(
                                                jwt -> {
                                                    if (jwt instanceof SignedJWT) {
                                                        return validateIdTokenHint(
                                                                authRequest, (SignedJWT) jwt);
                                                    } else {
                                                        return Single.error(
                                                                new InvalidRequestException(
                                                                        "id_token_hint must be signed"));
                                                    }
                                                });
                            } else if (!StringUtils.isEmpty(req.getLoginHintToken())) {
                                return parseJwt(req.getLoginHintToken())
                                        .flatMap(
                                                jwt -> {
                                                    // Specification doesn't specify if this token
                                                    // have to been signed
                                                    if (jwt instanceof SignedJWT) {
                                                        return verifyLoginHintTokenSignature(
                                                                        authRequest,
                                                                        (SignedJWT) jwt,
                                                                        client)
                                                                .flatMap(
                                                                        verifiedJwt ->
                                                                                validateLoginHintToken(
                                                                                        authRequest,
                                                                                        verifiedJwt));
                                                    } else {
                                                        return validateLoginHintToken(
                                                                authRequest, jwt);
                                                    }
                                                });
                            } else {
                                ...
                                criteria.setFilterName(
                                        authRequest.getLoginHint().contains("@")
                                                ? "email"
                                                : "username");
                                ...
                                return userService
                                        .findByDomainAndCriteria(domain.getId(), criteria)
                                        .map(
                                                users -> {
                                                    if (users.size() != 1) {
                                                        LOGGER.warn(
                                                                "login_hint match multiple users or no one");
    ...
    private Single<CibaAuthenticationRequest> validateIdTokenHint(
            CibaAuthenticationRequest authRequest, SignedJWT signedJwt) {
        // id_token_hint is the id_token generate by the OP for the client, so we use the JWKS
        ...
        return jwkService
                ...
                .switchIfEmpty(
                        Single.error(
                                new InvalidRequestException("JWK not found for id_token_hint")))
                .filter(jwk -> jwsService.isValidSignature(signedJwt, jwk))
                .switchIfEmpty(
                        Single.error(
                                new InvalidRequestException("Invalid signature fo id_token_hint")))
                .flatMap(
                        jwk -> {
                            try {
                                Date expirationTime =
                                ...
                                return Single.error(
                                        new InvalidRequestException("id_token_hint expired"));
                            }
                        });
    }

    private Single<JWT> verifyLoginHintTokenSignature(
            CibaAuthenticationRequest authRequest, SignedJWT signedJwt, Client client) {
        // As the login_hint_token is provided by the client, we use the Client JWKS to validate the
        // signature.
        ...
        return jwkService
                ...
                .switchIfEmpty(
                        Single.error(
                                new InvalidRequestException("JWK not found for login_token_hint")))
                .filter(jwk -> jwsService.isValidSignature(signedJwt, jwk))
                .switchIfEmpty(
                        Single.error(
                                new InvalidRequestException(
                                        "Invalid signature fo login_token_hint")))
        ...
        try {
            ...
            */
            ...
            if (!"email".equals(field) && !"username".equals(field)) {
                return Single.error(
                        new InvalidRequestException(
                                "Invalid hint, only email and username are supported"));
            }
                    ...
                    .flatMap(
                            users -> {
                                if (users.size() != 1) {
                                    ...
                                    return Single.error(
                                            new InvalidRequestException("Invalid hint"));
                                }
                                authRequest.setSubject(users.get(0).getId());
