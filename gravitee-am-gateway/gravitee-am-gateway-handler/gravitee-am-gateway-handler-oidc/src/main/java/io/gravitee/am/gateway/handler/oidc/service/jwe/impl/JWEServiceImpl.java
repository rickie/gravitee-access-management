...
public class JWEServiceImpl implements JWEService {
    ...
    public Single<String> encryptIdToken(String signedJwt, Client client) {
        // Return input without encryption if client does not require JWE or algorithm is set to
        // none
        if (client.getIdTokenEncryptedResponseAlg() == null
                || JWEAlgorithm.NONE
                        .getName()
                        .equalsIgnoreCase(client.getIdTokenEncryptedResponseAlg())) {
        ...
        return encrypt(jwe, client)
                .onErrorResumeNext(
                        throwable -> {
                            ...
                            return Single.error(
                                    new ServerErrorException("Unable to encrypt id_token"));
                        });
    }
    ...
    public Single<String> encryptUserinfo(String signedJwt, Client client) {
        // Return input without encryption if client does not require JWE or algorithm is set to
        // none
        if (client.getUserinfoEncryptedResponseAlg() == null
                || JWEAlgorithm.NONE
                        .getName()
                        .equalsIgnoreCase(client.getUserinfoEncryptedResponseAlg())) {
            return Single.just(signedJwt);
        }
        ...
        return encrypt(jwe, client)
                .onErrorResumeNext(
                        throwable -> {
                            ...
                            LOGGER.error(throwable.getMessage(), throwable);
                            return Single.error(
                                    new ServerErrorException("Unable to encrypt userinfo"));
                        });
    }
            ...
            if (parsedJwt instanceof EncryptedJWT) {
                ...
                if (this.domain.useFapiBrazilProfile()
                        && !(isKeyEncCompliantWithFapiBrazil(algorithm.getName())
                                && isContentEncCompliantWithFapiBrazil(
                                        jweObject.getHeader().getEncryptionMethod().getName()))) {
                    return Single.error(
                ...
                return Single.error(
                        new ServerErrorException(
                                "Unable to perform Json Web Decryption, unsupported algorithm: "
                                        + algorithm.getName()));
            } else if (encRequired) {
                return Single.error(
                        new InvalidRequestObjectException("Request Object must be encrypted"));
            } else {
    ...
    private Single<JWT> decrypt(
        ...
        return jwks.flatMapPublisher(jwkset -> Flowable.fromIterable(jwkset.getKeys()))
                ...
                .filter(
                        jwk ->
                                jwk.getUse() == null
                                        || jwk.getUse().equals(KeyUse.ENCRYPTION.getValue()))
                .filter(
                        jwk ->
                                jwe.getHeader().getKeyID() == null
                                        || jwe.getHeader().getKeyID().equals(jwk.getKid()))
    ...
    public Single<String> encryptAuthorization(String signedJwt, Client client) {
        // Return input without encryption if client does not require JWE or algorithm is set to
        // none
        if (client.getAuthorizationEncryptedResponseAlg() == null
                || JWEAlgorithm.NONE
                        .getName()
                        .equals(client.getAuthorizationEncryptedResponseAlg())) {
            return Single.just(signedJwt);
        }
                        ...
                        new JWEHeader.Builder(
                                        ...
                                        EncryptionMethod.parse(
                                                client.getAuthorizationEncryptedResponseEnc()
                                                        ...
                                                        : JWAlgorithmUtils
                                                                .getDefaultAuthorizationResponseEnc()))
                                .contentType("JWT")
                                .build(),
                        new Payload(signedJwt));
        ...
        return encrypt(jwe, client)
                .onErrorResumeNext(
                        throwable -> {
                            ...
                            LOGGER.error(throwable.getMessage(), throwable);
                            return Single.error(
                                    new ServerErrorException("Unable to encrypt authorization"));
                        });
    }
    ...
    private Single<String> encrypt(JWEObject jwe, Client client) {
        ...
        if (RSACryptoProvider.SUPPORTED_ALGORITHMS.contains(algorithm)) {
            return encrypt(
                    ...
                    client,
                    JWKFilter.RSA_KEY_ENCRYPTION(),
                    jwk -> new RSAEncrypter(JWKConverter.convert((RSAKey) jwk)));
        }
        ...
        return Single.error(
                new ServerErrorException(
                        "Unable to perform Json Web Encryption, unsupported algorithm: "
                                + algorithm.getName()));
    }

    private Single<String> encrypt(
            JWEObject jwe,
            Client client,
            Predicate<JWK> filter,
            JWEEncrypterFunction<JWK, JWEEncrypter> function) {
        return jwkService
                .getKeys(client)
                ...
                .switchIfEmpty(
                        Maybe.error(
                                new InvalidClientMetadataException(
                                        "no matching key found to encrypt")))
