...
public class AuthenticationRequestParseRequestObjectHandler implements Handler<RoutingContext> {
    ...
    public AuthenticationRequestParseRequestObjectHandler(
            RequestObjectService requestObjectService, Domain domain) {
        ...
        try {
            ...
            if (aud == null
                    ...
                    || (aud instanceof Collection
                            && (((Collection) aud).isEmpty()
                                    || !((Collection) aud)
                                            .stream().anyMatch(oidcMeta.getIssuer()::equals)))) {
            ...
            return Single.error(
                    new InvalidRequestException("Unable to read claims in the request parameter"));
