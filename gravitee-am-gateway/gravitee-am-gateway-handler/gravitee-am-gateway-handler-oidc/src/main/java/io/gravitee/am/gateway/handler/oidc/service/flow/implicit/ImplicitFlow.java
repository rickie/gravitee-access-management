...
@Override
    protected Mono<AuthorizationResponse> prepareResponse_migrated(AuthorizationRequest authorizationRequest, Client client, User endUser) {
        ...
        if (io.gravitee.am.common.oidc.ResponseType.ID_TOKEN.equals(authorizationRequest.getResponseType())) {
            return RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(idTokenService.create_migrated(oAuth2Request, client, endUser))).map(RxJavaReactorMigrationUtil.toJdkFunction(idToken -> {
            ...
            return RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(tokenService.create_migrated(oAuth2Request, client, endUser))).map(RxJavaReactorMigrationUtil.toJdkFunction(accessToken -> {
