...
public class UserConsentFailureHandler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext context) {
        if (context.failed()) {
            // logout the user
            // but keep the session intact with the original OAuth 2.0 authorization request in
            // order to replay the whole login process
            context.clearUser();
            ...
            if (throwable instanceof PolicyChainException) {
                ...
                handleException(
                        context, policyChainException.key(), policyChainException.getMessage());
            } else {
    ...
    private void handleException(
        ...
        try {
            ...
            logger.error(
                    "An error occurs while redirecting to {}", context.request().absoluteURI(), ex);
            context.fail(503);
        }
    }
