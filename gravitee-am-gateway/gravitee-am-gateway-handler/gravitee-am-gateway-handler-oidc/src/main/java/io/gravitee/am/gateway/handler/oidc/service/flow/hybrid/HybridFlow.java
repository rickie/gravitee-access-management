...
/**
 * When using the Hybrid Flow, some tokens are returned from the Authorization Endpoint and others
 * are returned from the Token Endpoint.
 *
 * <p>The mechanisms for returning tokens in the Hybrid Flow are specified in OAuth 2.0 Multiple
 * Response Type Encoding Practices [OAuth.Responses].
 *
 * <p>The Hybrid Flow follows the following steps:
 *
 * <p>. Client prepares an Authentication Request containing the desired request parameters. .
 * Client sends the request to the Authorization Server. . Authorization Server Authenticates the
 * End-User. . Authorization Server obtains End-User Consent/Authorization. . Authorization Server
 * sends the End-User back to the Client with an Authorization Code and, depending on the Response
 * Type, one or more additional parameters. . Client requests a response using the Authorization
 * Code at the Token Endpoint. . Client receives a response that contains an ID Token and Access
 * Token in the response body. . Client validates the ID Token and retrieves the End-User's Subject
 * Identifier.
 *
 * <p>See <a href="https://openid.net/specs/openid-connect-core-1_0.html#HybridFlowAuth">3.3.
 * Authentication using the Hybrid Flow</a>
 *
...
public class HybridFlow extends AbstractFlow {
    ...
    private static final List<String> RESPONSE_TYPES =
            Arrays.asList(
    ...
    @Override
    protected Single<AuthorizationResponse> prepareResponse(
            AuthorizationRequest authorizationRequest, Client client, User endUser) {
        ...
        return authorizationCodeService
                ...
                .flatMap(
                        code -> {
                            ...
                            oAuth2Request
                                    .getContext()
                                    .put(Claims.s_hash, authorizationRequest.getState());
                            switch (authorizationRequest.getResponseType()) {
                                    // code id_token response type MUST include both an
                                    // Authorization Code and an id_token
                                case ResponseType.CODE_ID_TOKEN:
                                    ...
                                    // others Hybrid Flow response type MUST include at least an
                                    // Access Token, an Access Token Type and optionally an ID Token
                                default:
                                    return tokenService
                                            .create(oAuth2Request, client, endUser)
                                            .map(
