    ...
    public Completable revoke(RevocationTokenRequest request, Client client) {
        ...
        if (request.getHint() != null && request.getHint().equals(TokenTypeHint.REFRESH_TOKEN)) {
            return revokeRefreshToken(token, client)
                    .onErrorResumeNext(
                            throwable -> {
                                // if the token was not issued to the client making the revocation
                                // request
                                ...
                                // purpose of the revocation request, invalidating the particular
                                // token,
                                // is already achieved.
                                // Log the result anyway for posterity.
                                if (throwable instanceof InvalidTokenException) {
                                    logger.debug(
                                            "No refresh token {} found in the token store.", token);
                                }
                    ...
                    .onErrorResumeNext(
                            throwable -> {
                                ...
                                if (throwable instanceof InvalidTokenException) {
                                    logger.debug(
                                            "No access token {} found in the token store.", token);
                ...
                .flatMapCompletable(
                        accessToken -> {
                            ...
                            if (!client.getClientId().equals(tokenClientId)) {
                                logger.debug(
                                        "Revoke FAILED: requesting client = {}, token's client = {}.",
                                        client.getClientId(),
                                        tokenClientId);
                                return Completable.error(
                                        new InvalidGrantException(
                                                "Cannot revoke tokens issued to other clients."));
                            }
