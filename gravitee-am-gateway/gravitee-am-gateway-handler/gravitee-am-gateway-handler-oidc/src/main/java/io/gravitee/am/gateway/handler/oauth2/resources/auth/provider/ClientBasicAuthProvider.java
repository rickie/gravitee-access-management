 ...
 *
 * <p>Clients that have received a client_secret value from the Authorization Server authenticate
 * with the Authorization Server in accordance with Section 2.3.1 of OAuth 2.0 [RFC6749] using the
...
public class ClientBasicAuthProvider implements ClientAuthProvider {
    ...
    public boolean canHandle(Client client, RoutingContext context) {
        if (client != null
                && ClientAuthenticationMethod.CLIENT_SECRET_BASIC.equals(
    ...
    private static String urlDecode(String value) {
        try {
            ...
            // behaviour for customer that are using some special characters like '%', we fall back
            // to the
