...
public abstract class AbstractRequestResolver<R extends OAuth2Request> {
     ...
     * indicating an invalid scope. See <a
     * href="https://tools.ietf.org/html/rfc6749#section-3.3">3.3. Access Token Scope</a>
     *
    ...
    protected Single<R> resolveAuthorizedScopes(R request, Client client, User endUser) {
            ...
            if (requestScopes == null || requestScopes.isEmpty()) {
                ...
                for (String scope : requestScopes) {
                    if (clientScopes.contains(scope)
                            || ParameterizedScopeUtils.isParameterizedScope(
                                    parameterizedScopes, scope)) {
                    ...
                    requestScopes.forEach(
                            scope -> {
                                if (!permissions.contains(scope)
                                        && !clientResolvedScopes.contains(scope)) {
                                    invalidScopes.add(scope);
                                }
                            });
                }
        ...
        // since https://github.com/gravitee-io/issues/issues/3839, enhanced scopes are added only
        // if there are no requested scopes
        // but we introduce legacy behaviour with https://github.com/gravitee-io/issues/issues/7455
