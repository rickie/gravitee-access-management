...
public class AuthenticationRequestServiceImpl implements AuthenticationRequestService {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationRequestServiceImpl.class);

    @Autowired private CibaAuthRequestRepository authRequestRepository;

    @Autowired private Environment environment;

    @Autowired private Domain domain;

    @Autowired private AuthenticationDeviceNotifierManager notifierManager;

    @Autowired private JWTService jwtService;

    @Autowired private ClientSyncService clientService;

    /**
     * How many time (in sec) an auth-request is kept into the DB once it expired. (This retention
     * is useful to manage the expired_token error)
     */
    ...
    public Single<CibaAuthRequest> register(CibaAuthenticationRequest request, Client client) {
        ...
        // as the application has to be informed of an expired request, we add retention time to the
        // ttl
        ...
        LOGGER.debug(
                "Register AuthenticationRequest with auth_req_id '{}' and expiry of '{}' seconds",
                entity.getId(),
                ttl);
    ...
    public Single<CibaAuthRequest> retrieve(Domain domain, String authReqId) {
        ...
        return this.authRequestRepository
                ...
                .flatMap(
                        request -> {
                            if ((request.getExpireAt().getTime() - (requestRetentionInSec * 1000))
                            ...
                            switch (AuthenticationRequestStatus.valueOf(request.getStatus())) {
                                case ONGOING:
                                    ...
                                    // if the client request to often the endpoint, throws a
                                    // SlowDown error
    ...
    public Single<CibaAuthRequest> updateAuthDeviceInformation(CibaAuthRequest request) {
        LOGGER.debug(
                "Update authentication request '{}' with AuthenticationDeviceNotifier information",
                request.getId());
        return this.authRequestRepository
                .findById(request.getId())
                .switchIfEmpty(
                        Single.error(new AuthenticationRequestNotFoundException(request.getId())))
                .flatMap(
                        existingReq -> {
    ...
    public Single<ADNotificationResponse> notify(ADNotificationRequest adRequest) {
        ...
        if (notifier == null) {
            return Single.error(
                    new InvalidRequestException("No authentication device notifier defined"));
        }
                ...
                .flatMap(
                        userResponse -> {
                            ...
                            return this.jwtService
                                    .decode(userResponse.getState())
                                    .flatMap(
                                            jwtState ->
                                                    verifyState(userResponse, jwtState.getAud()))
                                    .flatMap(
                                            verifiedJwtState ->
                                                    updateRequestStatus(
                                                            verifiedJwtState.getJti(), status));
                        })
                .ignoreElement();
    }
    ...
    private Single<JWT> verifyState(ADUserResponse userResponse, String clientId) {
        LOGGER.debug(
                "Prepare verification of state '{}' with client id '{}'",
                userResponse.getState(),
                clientId);
        return this.clientService
                ...
                .flatMap(
                        client ->
                                Single.defer(
                                        () ->
                                                this.jwtService.decodeAndVerify(
                                                        userResponse.getState(), client)))
                .filter(verifiedJwt -> userResponse.getTid().equals(verifiedJwt.getJti()))
                .switchIfEmpty(
                        Single.error(
                                new InvalidRequestException(
                                        "state parameter mismatch with the transaction id")))
                .onErrorResumeNext(
                        (error) -> {
                            LOGGER.debug(
                                    "Verification of state '{}' fails on CIBA callback with client id '{}'",
                                    userResponse.getState(),
                                    clientId,
                                    error);
    ...
    private Single<CibaAuthRequest> updateRequestStatus(String reqExtId, String status) {
        return this.authRequestRepository
                .findByExternalId(reqExtId)
                .switchIfEmpty(Single.error(new InvalidRequestException("Invalid CIBA State")))
                .flatMap(
                        cibaRequest ->
                                this.authRequestRepository.updateStatus(
                                        cibaRequest.getId(), status));
    }
}
