...
public class AuthenticationRequestParametersHandler implements Handler<RoutingContext> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationRequestParametersHandler.class);
    ...
    public AuthenticationRequestParametersHandler(
            Domain domain,
    ...
    public void handle(RoutingContext context) {
        ...
        try {
            ...
            LOGGER.debug(
                    "CIBA Authentication Request parameter validation fails due to : {}",
                    e.getMessage());
            context.fail(e);
        }
    }
    ...
    private void validateAcrValue(
            OpenIDProviderMetadata openIDProviderMetadata, CibaAuthenticationRequest request) {
        if (request.getAcrValues() != null
                && !request.getAcrValues().stream()
                        .anyMatch(openIDProviderMetadata.getAcrValuesSupported()::contains)) {
    ...
    private void validateBindingMessage(CibaAuthenticationRequest request) {
        if (!StringUtils.isEmpty(request.getBindingMessage())
                && request.getBindingMessage().length() > this.bindingMessageMaxLength) {
