...
/**
 * Implementation of the Authorization Code Grant Flow See <a
 * href="https://tools.ietf.org/html/rfc6749#page-24"></a>
 *
...
public class AuthorizationCodeTokenGranter extends AbstractTokenGranter {
    ...
    public AuthorizationCodeTokenGranter(
        ...
        this.exitOnError =
                env.getProperty("authenticationFlow.exitOnError", Boolean.class, Boolean.FALSE);
    }
                                                        ...
                                                        authenticationFlowContextService
                                                                ...
                                                                .map(
                                                                        ctx -> {
                                                                            ...
                                                                            // store only the
                                                                            // AuthenticationFlowContext.data attributes in order to simplify EL templating
                                                                            // and provide an up to
                                                                            // date set of data if
                                                                            // the enrichAuthFlow
                                                                            // Policy ius used
                                                                            // multiple time in a
    ...
    protected Maybe<User> resolveResourceOwner(TokenRequest tokenRequest, Client client) {
        return userAuthenticationManager
                .loadPreAuthenticatedUser(tokenRequest.getSubject(), tokenRequest)
                .onErrorResumeNext(
                        ex -> {
    ...
    private void checkRedirectUris(TokenRequest tokenRequest, AuthorizationCode authorizationCode) {
        ...
        // https://tools.ietf.org/html/rfc6749#section-4.1.3 (4.1.3. Access Token Request); if
        // provided
        // their values MUST be identical
        String redirectUriApprovalParameter =
                authorizationCode.getRequestParameters().getFirst(Parameters.REDIRECT_URI);
    ...
    private void checkPKCE(TokenRequest tokenRequest, AuthorizationCode authorizationCode) {
        ...
        if (codeChallenge != null && codeVerifier == null) {
            logger.debug(
                    "PKCE code_verifier parameter is missing, even if a code_challenge was initially defined");
        ...
        if (codeChallenge != null) {
            ...
            if (CodeChallengeMethod.S256.equalsIgnoreCase(codeChallengeMethod)) {
                try {
                    ...
                    logger.error(
                            "Not able to generate the codeChallenge from the given code verifier according to S256 algorithm");
