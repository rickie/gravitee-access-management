 ...
 *
 * <p>This method of mutual-TLS OAuth client authentication is intended to support client
...
public class ClientSelfSignedAuthProvider implements ClientAuthProvider {
    ...
    public boolean canHandle(Client client, RoutingContext context) {
        ...
        return client != null
                ...
                && ClientAuthenticationMethod.SELF_SIGNED_TLS_CLIENT_AUTH.equals(
    ...
    public void handle(
        ...
        try {
            Optional<X509Certificate> peerCertificate =
                    extractPeerCertificate(context, certificateHeader);
            if (peerCertificate.isPresent()) {
                                        ...
                                        handler.handle(
                                                Future.failedFuture(
                                                        new InvalidClientException(
                                                                "Invalid client: invalid self-signed certificate")));
                                    }
                                },
                                throwable ->
                                        handler.handle(
                ...
                handler.handle(
                        Future.failedFuture(
                                new InvalidClientException(
                                        "Invalid client: missing self-signed certificate")));
            }
        } catch (Exception ex) {
            handler.handle(
                    Future.failedFuture(
                            new InvalidClientException(
                                    "Invalid client: missing or unsupported self-signed certificate")));
        }
    }
}
