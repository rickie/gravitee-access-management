...
/**
 * Implementation of the User Managed Access 2.0 Grant Flow. This flow enable to get an access token
 * named RPT (Requesting Party Token), associated with a Permission Ticket. See <a
 * href="https://docs.kantarainitiative.org/uma/wg/rec-oauth-uma-grant-2.0.html#uma-grant-type">3.3.1
 * Client Request to Authorization Server for RPT</a>
 *
...
public class UMATokenGranter extends AbstractTokenGranter {
    ...
    private static final List<String> CLAIM_TOKEN_FORMAT_SUPPORTED =
            Arrays.asList(TokenType.ID_TOKEN);
    ...
    protected Single<TokenRequest> parseRequest(TokenRequest tokenRequest, Client client) {
        ...
        if (claimToken != null ^ claimTokenFormat != null) {
            return Single.error(
                    UmaException.needInfoBuilder(ticket)
                            .requiredClaims(
                                    Arrays.asList(
                                            new RequiredClaims(CLAIM_TOKEN)
                                                    .setFriendlyName("Requesting party token"),
                                            new RequiredClaims(CLAIM_TOKEN_FORMAT)
                                                    .setFriendlyName(
                                                            "supported claims token format")
                                                    .setClaimTokenFormat(
                                                            CLAIM_TOKEN_FORMAT_SUPPORTED)))
                            .build());
        }

        if (!StringUtils.isEmpty(claimTokenFormat)
                && !CLAIM_TOKEN_FORMAT_SUPPORTED.contains(claimTokenFormat)) {
            return Single.error(
                    UmaException.needInfoBuilder(ticket)
                            .requiredClaims(
                                    Arrays.asList(
    ...
    protected Maybe<User> resolveResourceOwner(TokenRequest tokenRequest, Client client) {
        ...
        return jwtService
                .decodeAndVerify(tokenRequest.getClaimToken(), client)
                .flatMapMaybe(
                        jwt ->
                                userAuthenticationManager.loadPreAuthenticatedUser(
                                        jwt.getSub(), tokenRequest))
                .switchIfEmpty(Maybe.error(UserInvalidException::new))
                .onErrorResumeNext(
                        ex -> {
                            // If user
                            return Maybe.error(
                                    UmaException.needInfoBuilder(tokenRequest.getTicket())
                                            .requiredClaims(
                                                    Arrays.asList(
                                                            new RequiredClaims(CLAIM_TOKEN)
                                                                    .setFriendlyName(
                                                                            "Malformed or expired claim_token")))
                                            .build());
                        });
    }
        ...
        if (tokenRequest.getScopes() != null && !tokenRequest.getScopes().isEmpty()) {
            if (client.getScopeSettings() == null
                    ...
                    || !client.getScopeSettings().stream()
                            ...
                            .containsAll(tokenRequest.getScopes())) {
                // TokenRequest scopes are not null and not empty, already did the check in earlier
                // step.
                return Single.error(
                ...
                .flatMap(
                        permissionRequests -> {
                            ...
                            return resourceService
                                    ...
                                    .flatMap(
                                            resourceSet ->
                                                    this.checkRequestedScopesMatchResource(
                                                            tokenRequest, resourceSet))
                                    .flatMap(
                                            resourceMap ->
                                                    this.resolveScopeRequestAssessment(
                                                            tokenRequest,
                                                            permissionRequests,
                                                            resourceMap))
                                    .flatMap(
                                            resolvedPermissionRequests ->
                                                    this.extendPermissionWithRPT(
                                                            tokenRequest,
                                                            client,
                                                            endUser,
                                                            resolvedPermissionRequests))
     ...
     */
    private Single<Map<String, Resource>> checkRequestedScopesMatchResource(
            TokenRequest tokenRequest, List<Resource> resourceSet) {
        // Return InvalidScopeException if the token request contains unbounded resource set scopes.
        if (tokenRequest.getScopes() != null && !tokenRequest.getScopes().isEmpty()) {
            Set<String> allResourcesScopes =
                    resourceSet.stream()
    ...
    private Single<List<PermissionRequest>> resolveScopeRequestAssessment(
            TokenRequest tokenRequest,
            List<PermissionRequest> requestedPermissions,
            Map<String, Resource> fetchedResources) {
        // If request does not contains additional scopes, just return the permission Ticket
        // resources.
        if (tokenRequest.getScopes() == null || tokenRequest.getScopes().isEmpty()) {
            return Single.just(requestedPermissions);
        }

        // Else return each Permission Requests scopes such as RequestedScopes = PermissionTicket ∪
        // (ClientRegistered ∩ ClientRequested ∩ RSRegistered)
        return Single.just(
                        ...
                        .map(
                                permissionRequest -> {
                                    Set<String> registeredScopes =
                                            new HashSet(
                                                    fetchedResources
                                                            .get(permissionRequest.getResourceId())
                                                            .getResourceScopes());
                                    // RequestedScopes = PermissionTicket ∪ (ClientRequested ∩
                                    // RSRegistered) /!\ ClientRegistered scopes already have been
                                    // checked earlier. (#resolveRequest)
                                    permissionRequest
                                            .getResourceScopes()
                                            .addAll(
    ...
    /**
     * If a valid Requesting Party Token was provided, then we inject previous permissions from this
     * RPT. This permissions must be reviewed by the Permission Policy Management as some of them
     ...
     */
    private Single<List<PermissionRequest>> extendPermissionWithRPT(
            TokenRequest tokenRequest,
            Client client,
            User endUser,
            List<PermissionRequest> requestedPermissions) {
        if (!StringUtils.isEmpty(tokenRequest.getRequestingPartyToken())) {
            return jwtService
                    .decodeAndVerify(tokenRequest.getRequestingPartyToken(), client)
                            ...
                            throwable ->
                                    Single.error(
                                            new InvalidGrantException(
                                                    "Requesting Party Token (rpt) not valid")));
        }
    ...
    private List<PermissionRequest> mergePermissions(JWT rpt, List<PermissionRequest> requested) {
        if (rpt.get("permissions") != null) {
            ...
            Map<String, PermissionRequest> newRequestedPermission =
                    requested.stream()
            ...
            return Stream.concat(
                            newRequestedPermission.entrySet().stream(),
                            rptPermission.entrySet().stream())
                    .collect(
                            Collectors.toMap(
                                    Map.Entry::getKey,
                                    Map.Entry::getValue,
                                    (requestedPermission, fromRpt) -> {
    ...
    private Single<OAuth2Request> createOAuth2Request(
        ...
        // Client may have refresh_token grant, but if request is not made for an end user, then we
        // should not generate refresh.
    ...
    /**
     * Do not call super.resolveRequest(xxx), as scopes have nothing related with oauth2 scopes
     * mindset. While oauth2 imply user approvals (RO grant access to his resources to an
     * application), here the subject is a Requesting Party that request grant access to someone
     * else resources.
     */
    ...
    /**
     * The resource owner works with the authorization server to configure policy conditions
     * (authorization grant rules), which the authorization server executes in the process of
     * issuing access tokens. The authorization process makes use of claims gathered from the
     * requesting party and client in order to satisfy all operative operative policy conditions.
     *
     ...
     * @return
     */
    private Single<OAuth2Request> executePolicies(
        ...
        return resourceService
                .findAccessPoliciesByResources(resourceIds)
                // map to rules
                .map(
                        accessPolicy -> {
                            Rule rule = new DefaultRule(accessPolicy);
                            Optional<PermissionRequest> permission =
                                    permissionRequests.stream()
                                            .filter(
                                                    permissionRequest ->
                                                            permissionRequest
                                ...
                                ((DefaultRule) rule)
                                        .setMetadata(
                                                Collections.singletonMap(
                                                        "permissionRequest", permission.get()));
                            }
                            return rule;
                        })
                .toList()
                .flatMap(
                        rules -> {
                            ...
                            ExecutionContext simpleExecutionContext =
                                    new SimpleExecutionContext(
                                            oAuth2Request, oAuth2Request.getHttpResponse());
                            ExecutionContext executionContext =
                                    executionContextFactory.create(simpleExecutionContext);
                            ...
                            return rulesEngine
                                    .fire(rules, executionContext)
                                    .toSingleDefault(oAuth2Request)
                                    .onErrorResumeNext(
                                            ex ->
                                                    Single.error(
                                                            new InvalidGrantException(
                                                                    "Policy conditions are not met for actual request parameters")));
                        });
    }
}
