...
public class ClientAuthHandlerImpl implements Handler<RoutingContext> {
    ...
    public ClientAuthHandlerImpl(
            ClientSyncService clientSyncService,
            List<ClientAuthProvider> clientAuthProviders,
            Domain domain,
                ...
                handler -> {
                    ...
                    authenticateClient(
                            ...
                            authHandler -> {
                                if (authHandler.failed()) {
                                    ...
                                    if (throwable instanceof InvalidClientException) {
                                        String authenticateHeader =
                                                ((InvalidClientException) throwable)
                                ...
                                // the client might has been upgraded after authentication process,
                                // get the new value
                                ...
                                try {
                                    Optional<X509Certificate> peerCertificate =
                                            extractPeerCertificate(
                                                    routingContext, this.certificateHeader);
                                    if (peerCertificate.isPresent()) {
                                        routingContext.put(
                                                ConstantKeys.PEER_CERTIFICATE_THUMBPRINT,
                                                getThumbprint(peerCertificate.get(), "SHA-256"));
                                    } else if (authenticatedClient
                                                    .isTlsClientCertificateBoundAccessTokens()
                                            || domain.usePlainFapiProfile()) {
                                        routingContext.fail(
                                                new InvalidClientException(
                                                        "Missing or invalid peer certificate"));
                                        return;
                                    }
        ...
        try {
            if (authorization != null) {
                ...
                String clientAuthentication =
                        new String(Base64.getDecoder().decode(authorization.substring(idx + 1)));
