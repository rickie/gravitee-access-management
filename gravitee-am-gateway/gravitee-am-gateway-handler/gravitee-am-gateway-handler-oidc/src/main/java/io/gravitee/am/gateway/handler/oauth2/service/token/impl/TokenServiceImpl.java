...
public class TokenServiceImpl implements TokenService {
    ...
    @Autowired private IntrospectionTokenService introspectionTokenService;
                ...
                .flatMapMaybe(
                        jwt ->
                                accessTokenRepository
                                        ...
                                        .map(accessToken -> convertAccessToken(jwt)));
    }
                ...
                .flatMapMaybe(
                        jwt ->
                                refreshTokenRepository
                                        .findByToken(jwt.getJti())
                                        .map(refreshToken1 -> convertRefreshToken(jwt)));
    }

    @Override
    public Single<Token> introspect(String token) {
        return introspectionTokenService.introspect(token, false).map(this::convertAccessToken);
    }
                        ...
                        executionContext -> {
                            ...
                            JWT accessToken =
                                    createAccessTokenJWT(
                                            oAuth2Request, client, endUser, executionContext);
                            // create JWT refresh token
                            JWT refreshToken =
                                    oAuth2Request.isSupportRefreshToken()
                                            ? createRefreshTokenJWT(
                                                    oAuth2Request, client, endUser, accessToken)
                                            : null;
                            ...
                            return Single.zip(
                                            ...
                                            (encodedAccessToken, optionalEncodedRefreshToken) ->
                                                    convert(
                                                            accessToken,
                                                            encodedAccessToken,
                                                            optionalEncodedRefreshToken.orElse(
                                                                    null),
                                                            oAuth2Request))
                                    .flatMap(
                                            accessToken1 ->
                                                    tokenEnhancer.enhance(
                                                            accessToken1,
                                                            oAuth2Request,
                                                            client,
                                                            endUser,
    ...
    public Single<Token> refresh(String refreshToken, TokenRequest tokenRequest, Client client) {
        ...
        // owner credentials) or refresh token is
                ...
                .flatMap(
                        refreshToken1 -> {
                            if (refreshToken1.getExpireAt().before(new Date())) {
                                return Single.error(
                                        new InvalidGrantException("Refresh token is expired"));
                            }
                            ...
                            if (refreshToken1.getAdditionalInformation().get("permissions")
                                    != null) {
                            ...
                            // if client has disabled refresh token rotation, do not remove the
                            // refresh token
    ...
    private Completable storeTokens(
        ...
        if (refreshToken != null) {
            return persistAccessToken.andThen(
                    tokenManager.storeRefreshToken(convert(refreshToken)));
        }
        return persistAccessToken;
    }

    private io.gravitee.am.repository.oauth2.model.AccessToken convert(
            JWT token, JWT refreshToken, OAuth2Request oAuth2Request) {
        io.gravitee.am.repository.oauth2.model.AccessToken accessToken =
        ...
        accessToken.setAuthorizationCode(
                oAuth2Request.parameters() != null
                        ? oAuth2Request
                                .parameters()
                                .getFirst(io.gravitee.am.common.oauth2.Parameters.CODE)
                        : null);
        // set refresh token
    ...
    private io.gravitee.am.repository.oauth2.model.RefreshToken convert(JWT token) {
        io.gravitee.am.repository.oauth2.model.RefreshToken refreshToken =
    ...
    private Token convert(
        ...
        if (oAuth2Request.getAdditionalParameters() != null
                && !oAuth2Request.getAdditionalParameters().isEmpty()) {
            oAuth2Request
                    .getAdditionalParameters()
                    .toSingleValueMap()
                    .forEach((k, v) -> token.getAdditionalInformation().put(k, v));
        }
        // set refresh token
    ...
    private Token convert(Token token, JWT jwt) {
        ...
        token.setExpiresIn(
                token.getExpireAt() != null
                        ? Long.valueOf(
                                (token.getExpireAt().getTime() - System.currentTimeMillis())
                                        / 1000L)
                        : 0);
    ...
    private JWT createAccessTokenJWT(
            OAuth2Request request, Client client, User user, ExecutionContext executionContext) {
        JWT jwt = createJWT(request, client, user);
        // set exp claim
        jwt.setExp(
                Instant.ofEpochSecond(jwt.getIat())
        ...
        if (cnfValue != null) {
            jwt.setConfirmationMethod(
                    Maps.<String, Object>builder()
                            .put(JWT.CONFIRMATION_METHOD_X509_THUMBPRINT, cnfValue)
                            .build());
        }
        ...
        // set custom claims
        enhanceJWT(
                jwt, client.getTokenCustomClaims(), TokenTypeHint.ACCESS_TOKEN, executionContext);

        return jwt;
    }

    private JWT createRefreshTokenJWT(
            OAuth2Request request, Client client, User user, JWT accessToken) {
        JWT jwt = createJWT(request, client, user);
        // set exp claim
        jwt.setExp(
                Instant.ofEpochSecond(jwt.getIat())
    ...
    private void enhanceJWT(
            JWT jwt,
            List<TokenClaim> customClaims,
                    ...
                    .forEach(
                            tokenClaim -> {
                                try {
                                    ...
                                    Object extValue =
                                            (claimExpression != null)
                                                    ? executionContext
                                                            .getTemplateEngine()
                                                            .getValue(claimExpression, Object.class)
                                                    : null;
                                    if (extValue != null) {
                                    ...
                                    logger.debug(
                                            "An error occurs while parsing expression language : {}",
                                            tokenClaim.getClaimValue(),
                                            ex);
                                }
                            });
        }
    ...
    private ExecutionContext createExecutionContext(
            OAuth2Request request, Client client, User user) {
        ...
        Object authFlowAttributes =
                request.getContext().get(ConstantKeys.AUTH_FLOW_CONTEXT_ATTRIBUTES_KEY);
        if (authFlowAttributes != null) {
            executionContext.setAttribute(
                    ConstantKeys.AUTH_FLOW_CONTEXT_ATTRIBUTES_KEY, authFlowAttributes);
