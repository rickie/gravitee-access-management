...
public class DynamicClientAccessEndpoint extends DynamicClientRegistrationEndpoint {
    ...
    public void read(RoutingContext context) {
        ...
        RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(this.getClient(context)).map(RxJavaReactorMigrationUtil.toJdkFunction(DynamicClientRegistrationResponse::fromClient)).map(RxJavaReactorMigrationUtil.toJdkFunction(response -> {
    ...
    private Maybe<Client> getClient(RoutingContext context) {
        ...
        return RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(this.clientSyncService.findByClientId(clientId)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.error(new ResourceNotFoundException("client not found"))))).map(RxJavaReactorMigrationUtil.toJdkFunction(Client::clone)));
    }
}
