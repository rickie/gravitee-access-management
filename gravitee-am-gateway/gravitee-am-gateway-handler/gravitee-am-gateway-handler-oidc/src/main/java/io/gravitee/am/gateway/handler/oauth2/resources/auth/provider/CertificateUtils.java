...
public class CertificateUtils {
    ...
    public static boolean hasPeerCertificate(RoutingContext routingContext, String certHeader) {
        String certHeaderValue =
                StringUtils.isEmpty(certHeader)
                        ? null
                        : routingContext.request().getHeader(certHeader);
        return routingContext.request().sslSession() != null || certHeaderValue != null;
    }

    /**
     * This method gets the PeerCertificate from the sslSession. If no sslSession is available, the
     * certificate from the HTTP Header using the header name provided as parameter.
     *
     ...
     */
    public static Optional<X509Certificate> extractPeerCertificate(
            RoutingContext routingContext, String certHeader) throws SSLPeerUnverifiedException {
                                ...
                                (X509Certificate)
                                        certificateFactory.generateCertificate(
                                                new ByteArrayInputStream(
                                                        certHeaderValue.getBytes())));
            } catch (CertificateException e) {
                LOGGER.debug(
                        "Peer Certificate header is present but certificate can't be read, try with the sslSession (cause: {})",
                        e.getMessage());
            }
        }
