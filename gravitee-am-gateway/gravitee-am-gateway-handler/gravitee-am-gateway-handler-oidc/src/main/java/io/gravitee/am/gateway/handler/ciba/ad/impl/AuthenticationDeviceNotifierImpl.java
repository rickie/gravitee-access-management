 ...
 * This backend will be responsible for the user authentication and will call AM back to inform
 * about the user choice (ACCEPTED or REJECTED) regarding the authentication request.
 *
 * @author Eric LELEU (eric.leleu at graviteesource.com)
 * @author GraviteeSource Team
 */
public class AuthenticationDeviceNotifierImpl
        implements AuthenticationDeviceNotifier, InitializingBean {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationDeviceNotifierImpl.class);
    ...
    public static final String URL_TO_GET_FROM_DOMAIN_SETTINGS =
            System.getProperty("ciba.ades", "http://localhost:8080/ciba/ades");

    @Autowired private Domain domain;

    @Autowired private Vertx vertx;

    @Autowired private WebClientBuilder clientBuilder;
    ...
    public Single<ADNotificationResponse> notify(ADNotificationRequest request) {
        ...
        return this.client
                .requestAbs(HttpMethod.POST, URL_TO_GET_FROM_DOMAIN_SETTINGS)
                .rxSendForm(formData)
                .doOnError(
                        (error) ->
                                LOGGER.warn(
                                        "Unexpected error during device notification : {}",
                                        error.getMessage(),
                                        error))
                .onErrorResumeNext(
                        Single.error(
                                new DeviceNotificationException(
                                        "Unexpected error during device notification")))
                .map(
                        response -> {
                            if (response.statusCode() != HttpStatusCode.OK_200) {
                                LOGGER.info(
                                        "Device notification fails for tid '{}' with status '{}'",
                            ...
                            if (!request.getTransactionId().equals(result.getString(TRANSACTION_ID))
                                ...
                                LOGGER.warn(
                                        "Device notification response contains invalid tid or state",
                                        request.getTransactionId(),
                                        response.statusCode());
                                throw new DeviceNotificationException(
                            ...
                            if (result.containsKey(RESPONSE_ATTR_DATA)) {
                                notificationResponse.setExtraData(
                                        result.getJsonObject(RESPONSE_ATTR_DATA).getMap());
                            }
