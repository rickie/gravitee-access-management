...
/**
 * Dynamic Client Registration is a protocol that allows OAuth client applications to register with
 * an OAuth server. Specifications are defined by OpenID Foundation and by the IETF as RFC 7591 too.
 * They define how a client may submit a request to register itself and how should be the response.
 *
 * <p>See <a href="https://openid.net/specs/openid-connect-registration-1_0.html">Openid Connect
 * Dynamic Client Registration</a> See <a href="https://tools.ietf.org/html/rfc7591">OAuth 2.0
...
public class DynamicClientRegistrationEndpoint implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext context) {
        ...
        this.extractRequest(context)
                .flatMap(
                        request ->
                                dcrService.create(
                                        request, UriBuilderRequest.resolveProxyRequest(context)))
    ...
    protected Single<DynamicClientRegistrationRequest> extractRequest(RoutingContext context) {
        try {
            ...
            return Single.just(
                    context.getBodyAsJson().mapTo(DynamicClientRegistrationRequest.class));
        } catch (Exception ex) {
            ...
            if (ex instanceof IllegalArgumentException
                    && ex.getMessage().startsWith(JWKSetDeserializer.PARSE_ERROR_MESSAGE)) {
                String sanitizedMessage =
                        ex.getMessage()
                                .substring(
                                        0, ex.getMessage().indexOf(" (through reference chain:"));
