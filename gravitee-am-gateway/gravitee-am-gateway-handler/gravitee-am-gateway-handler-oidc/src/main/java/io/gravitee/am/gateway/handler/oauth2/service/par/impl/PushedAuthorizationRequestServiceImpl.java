...
public class PushedAuthorizationRequestServiceImpl implements PushedAuthorizationRequestService {
    private static Logger LOGGER =
            LoggerFactory.getLogger(PushedAuthorizationRequestServiceImpl.class);
    ...
    @Autowired private PushedAuthorizationRequestRepository parRepository;
    ...
    @Override
    public Single<JWT> readFromURI(
            String requestUri, Client client, OpenIDProviderMetadata oidcMetadata) {
        if (requestUri.startsWith(PAR_URN_PREFIX)) {
            ...
            return parRepository
                    ...
                    .flatMap(
                            (Function<PushedAuthorizationRequest, Single<JWT>>)
                                    req -> {
    ...
    @Override
    public Single<PushedAuthorizationRequestResponse> registerParameters(
            PushedAuthorizationRequest par, Client client) {
        par.setClient(client.getId()); // link parameters to the internal client identifier
        par.setDomain(domain.getId());
        ; // link parameters to the internal domain identifier
        par.setExpireAt(new Date(Instant.now().plusMillis(requestUriValidity).toEpochMilli()));

        Completable registrationValidation =
                Completable.fromAction(
                        () -> {
                            String clientId =
                                    jwtClientAssertion(par)
                                            ? getClientIdFromAssertion(par)
                                            : par.getParameters().getFirst(Parameters.CLIENT_ID);
                                    ...
                                    Single.defer(
                                            () ->
                                                    readRequestObject(client, request)
                                                            .map(
                                                                    jwt ->
                                                                            checkRedirectUriParameter(
                                                                                    jwt, client))))
                            .ignoreElement();
        } else {
            registrationValidation.andThen(
                    Completable.fromAction(() -> checkRedirectUriParameter(par, client)));
        }
    ...
    private boolean jwtClientAssertion(PushedAuthorizationRequest par) {
        return par.getParameters().getFirst(Parameters.CLIENT_ASSERTION) != null
                && JWT_BEARER.equals(
                        par.getParameters().getFirst(Parameters.CLIENT_ASSERTION_TYPE));
    }

    private String getClientIdFromAssertion(PushedAuthorizationRequest par) {
        try {
            return JWTParser.parse(par.getParameters().getFirst(Parameters.CLIENT_ASSERTION))
                    .getJWTClaimsSet()
                    .getSubject();
    ...
    private JWT checkRequestObjectClaims(JWT jwt) {
        try {
            if (jwt.getJWTClaimsSet().getStringClaim(io.gravitee.am.common.oidc.Parameters.REQUEST)
                            != null
                    || jwt.getJWTClaimsSet()
                                    .getStringClaim(
                                            io.gravitee.am.common.oidc.Parameters.REQUEST_URI)
                            != null) {
                throw new InvalidRequestObjectException(
                        "Claims request and request_uri are forbidden");
            }
            return jwt;
        } catch (ParseException e) {
    ...
    private PushedAuthorizationRequest checkRedirectUriParameter(
            PushedAuthorizationRequest request, Client client) {
    ...
    private JWT checkRedirectUriParameter(JWT request, Client client) {
        try {
            String requestedRedirectUri =
                    request.getJWTClaimsSet().getStringClaim(Parameters.REDIRECT_URI);
