...
public class PushedAuthorizationRequestServiceImpl implements PushedAuthorizationRequestService {
    ...
    public Single<JWT> readFromURI(String requestUri, Client client, OpenIDProviderMetadata oidcMetadata) {
        if (requestUri.startsWith(PAR_URN_PREFIX)) {
            ...
            return RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(parRepository.findById(identifier)).switchIfEmpty(RxJava2Adapter.singleToMono(Single.error(new InvalidRequestUriException()))).flatMap(v->RxJava2Adapter.singleToMono((Single<JWT>)RxJavaReactorMigrationUtil.toJdkFunction((Function<PushedAuthorizationRequest, Single<JWT>>)(Function<PushedAuthorizationRequest, Single<JWT>>)req -> {
    ...
    public Single<PushedAuthorizationRequestResponse> registerParameters(PushedAuthorizationRequest par, Client client) {
        ...
        if (request != null) {
            registrationValidation = RxJava2Adapter.monoToCompletable(RxJava2Adapter.completableToMono(registrationValidation).then(RxJava2Adapter.singleToMono(Single.defer(() ->
        ...
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.completableToMono(registrationValidation).then(RxJava2Adapter.singleToMono(Single.defer(() -> parRepository.create(par)))).map(RxJavaReactorMigrationUtil.toJdkFunction(parPersisted -> {
