...
public class PushedAuthorizationRequestServiceImpl implements PushedAuthorizationRequestService {
    ...
    public Single<JWT> readFromURI(String requestUri, Client client, OpenIDProviderMetadata oidcMetadata) {
        if (requestUri.startsWith(PAR_URN_PREFIX)) {
            ...
            return RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(parRepository.findById(identifier)).switchIfEmpty(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.error(new InvalidRequestUriException())))).flatMap(v->RxJava2Adapter.singleToMono((Single<JWT>)RxJavaReactorMigrationUtil.toJdkFunction((Function<PushedAuthorizationRequest, Single<JWT>>)(Function<PushedAuthorizationRequest, Single<JWT>>)req -> {
    ...
    public Single<PushedAuthorizationRequestResponse> registerParameters(PushedAuthorizationRequest par, Client client) {
        ...
        if (request != null) {
            registrationValidation = RxJava2Adapter.monoToCompletable(RxJava2Adapter.completableToMono(registrationValidation).then(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.defer(()->RxJava2Adapter.singleToMono(readRequestObject(client, request).map((com.nimbusds.jwt.JWT jwt)->checkRedirectUriParameter(jwt, client))))))).then());
        } else {
        ...
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.completableToMono(registrationValidation).then(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.defer(()->RxJava2Adapter.singleToMono(parRepository.create(par)))))).map(RxJavaReactorMigrationUtil.toJdkFunction(parPersisted -> {
