...
public class UserConsentEndpoint implements Handler<RoutingContext> {
    ...
    private void fetchConsentInformation(Set<String> requestedConsents, boolean prompt, Client client, io.gravitee.am.model.User user, Handler<AsyncResult<List<Scope>>> handler) {
        ...
        if (prompt) {
            ...
            consentInformation = RxJava2Adapter.monoToSingle(userConsentService.checkConsent_migrated(client, user).flatMap(v->RxJava2Adapter.singleToMono((Single<List<Scope>>)RxJavaReactorMigrationUtil.toJdkFunction((Function<Set<String>, Single<List<Scope>>>)approvedConsent -> {
        ...
        RxJava2Adapter.singleToMono(consentInformation).subscribe(scopes -> handler.handle(Future.succeededFuture(scopes)), error -> handler.handle(Future.failedFuture(error)));
    }
