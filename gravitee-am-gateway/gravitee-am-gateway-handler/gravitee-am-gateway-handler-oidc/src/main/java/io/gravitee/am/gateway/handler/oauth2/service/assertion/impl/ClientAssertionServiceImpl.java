...
/**
 * Client assertion as described for <a
 * href="https://tools.ietf.org/html/rfc7521#section-4.2">oauth2 assertion framework</a> and <a
 * href="https://openid.net/specs/openid-connect-core-1_0.html#ClientAuthentication">openid client
 * authentication specs</a>
 *
...
public class ClientAssertionServiceImpl implements ClientAssertionService {
    ...
    private static final InvalidClientException NOT_VALID =
            new InvalidClientException("assertion is not valid");
    ...
    public Maybe<Client> assertClient(String assertionType, String assertion, String basePath) {
        ...
        InvalidClientException unsupportedAssertionType =
                new InvalidClientException("Unknown or unsupported assertion_type");
    ...
    /**
     * This method will parse the JWT bearer then ensure that all requested claims are set as
     * required <a href="https://tools.ietf.org/html/rfc7523#section-3">here</a>
     *
    ...
    private Maybe<JWT> validateJWT(String assertion, String basePath) {
        try {
            ...
            if (discovery == null || discovery.getTokenEndpoint() == null) {
                return Maybe.error(
                        new ServerErrorException("Unable to retrieve discovery token endpoint."));
            }

            // OIDC specifies that "The Audience SHOULD be the URL of the Authorization Server's
            // Token Endpoint."
            // https://openid.net/specs/openid-connect-core-1_0.html#ClientAuthentication
            // BUT the PAR specification specify the usage of the Issuer value, Token endpoint or
            // PAR endpoint.
            ...
            if (aud.stream().filter(discovery.getTokenEndpoint()::equals).count() == 0
                    && (discovery.getIssuer() != null
                            && aud.stream().filter(discovery.getIssuer()::equals).count() == 0)
                    && (discovery.getParEndpoint() != null
                            && aud.stream().filter(discovery.getParEndpoint()::equals).count()
                                    == 0)) {
                return Maybe.error(NOT_VALID);
            }
            ...
            if (this.domain.usePlainFapiProfile()
                ...
                return Maybe.error(
                        new InvalidClientException("JWT Assertion must be signed with PS256"));
            }
    ...
    private Maybe<Client> validateSignatureWithPublicKey(JWT jwt) {
        try {
            ...
            return this.clientSyncService
                    .findByClientId(clientId)
                    .switchIfEmpty(
                            Maybe.error(new InvalidClientException("Missing or invalid client")))
                    .flatMap(
                            client -> {
                                if (client.getTokenEndpointAuthMethod() == null
                                    ...
                                    return this.getClientJwkSet(client)
                                            .switchIfEmpty(
                                                    Maybe.error(
                                                            new InvalidClientException(
                                                                    "No jwk keys available on client")))
                                            .flatMap(
                                                    jwkSet ->
                                                            jwkService.getKey(
                                                                    jwkSet,
                                            ...
                                            .flatMap(
                                                    jwk -> {
                                                        if (jwsService.isValidSignature(
                                                                signedJWT, jwk)) {
                                                            return Maybe.just(client);
                                                        }
                                                        return Maybe.error(
                                                                new InvalidClientException(
                                                                        "Unable to validate client, assertion signature is not valid."));
                                                    });
                                } else {
                                    return Maybe.error(
                                            new InvalidClientException(
    ...
    private Maybe<Client> validateSignatureWithHMAC(JWT jwt) {
        try {
            ...
            if (algorithm instanceof JWSAlgorithm) {
                ...
                if (jwsAlgorithm != JWSAlgorithm.HS256
            ...
            return this.clientSyncService
                    ...
                    .flatMap(
                            client -> {
                                try {
                                    // Ensure to validate JWT using client_secret_key only if client
                                    // is authorized to use this auth method
                                    if (client.getTokenEndpointAuthMethod() == null
                                            || ClientAuthenticationMethod.CLIENT_SECRET_JWT
                                                    .equalsIgnoreCase(
                                                            client.getTokenEndpointAuthMethod())) {
                                        JWSVerifier verifier =
                                                new MACVerifier(client.getClientSecret());
