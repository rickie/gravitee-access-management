...
public class UserConsentServiceImpl implements UserConsentService {
    ...
    public Single<Set<String>> checkConsent(Client client, io.gravitee.am.model.User user) {
        return scopeApprovalService
                ...
                .filter(
                        approval -> {
                            Date today = new Date();
                            return (approval.getExpiresAt().after(today)
                                    && approval.getStatus()
    ...
    public Single<List<ScopeApproval>> saveConsent(
        ...
        approvals.forEach(
                a ->
                        a.setExpiresAt(
                                computeExpiry(scopeApprovals, a.getScope(), parameterizedScopes)));
        // save consent
    ...
    private Date computeExpiry(
        ...
        if (scopeApprovals != null) {
            ...
            if (isParameterizedScope) {
                ...
                if (scopeApprovals.containsKey(parameterizedScope)
                    ...
                    expiresAt.add(
                            Calendar.SECOND,
                            scopeApprovals.get(parameterizedScope).getScopeApproval());
                    return expiresAt.getTime();
                }
            }
        }
        ...
        Scope domainScope =
                scopeService.findByKey(isParameterizedScope ? getScopeBase(scope) : scope);
