...
public class OAuth2Provider extends AbstractService<ProtocolProvider> implements ProtocolProvider {
    ...
    @Autowired private Flow flow;

    @Autowired private UserConsentService userConsentService;

    @Autowired private TokenGranter tokenGranter;

    @Autowired private IntrospectionService introspectionService;

    @Autowired private RevocationTokenService revocationTokenService;
    ...
    @Autowired private AuthenticationFlowHandler authenticationFlowHandler;
    ...
    @Autowired private PushedAuthorizationRequestService parService;
    ...
    private void initRouter() {
        ...
        oauth2Router
                ...
                .handler(
                        new RiskAssessmentHandler(
                                deviceService, userActivityService, vertx.eventBus(), objectMapper))
        ...
        Handler<RoutingContext> userConsentPrepareContextHandler =
                new UserConsentPrepareContextHandler(clientSyncService);
        oauth2Router
                ...
                .handler(
                        new UserConsentEndpoint(
                                userConsentService, thymeleafTemplateEngine, domain));
        oauth2Router
    ...
    private void sessionAndCookieHandler(Router router) {
        ...
        router.route("/authorize").handler(sessionHandler).handler(ssoSessionHandler);

        router.route("/consent").handler(sessionHandler).handler(ssoSessionHandler);
    }
