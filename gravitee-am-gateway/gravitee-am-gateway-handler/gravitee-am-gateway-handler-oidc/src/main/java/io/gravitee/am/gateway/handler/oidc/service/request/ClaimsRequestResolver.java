...
public final class ClaimsRequestResolver {
    ...
    /**
     * @param claims the claims parameter value is represented in an OAuth 2.0 request as UTF-8
     *     encoded JSON
    ...
    public ClaimsRequest resolve(String claims) throws ClaimsRequestSyntaxException {
        try {
            // The claims parameter value is represented in an OAuth 2.0 request as UTF-8 encoded
            // JSON
            JsonObject claimsValue = new JsonObject(claims);
            ClaimsRequest claimsRequest = new ClaimsRequest();
            // set userinfo parameter
            claimsRequest.setUserInfoClaims(
                    resolveClaimsRequest(claimsValue, ClaimsRequest.USERINFO));
            // set id_token parameter
            claimsRequest.setIdTokenClaims(
                    resolveClaimsRequest(claimsValue, ClaimsRequest.ID_TOKEN));
     ...
     */
    private Map<String, ClaimRequest> resolveClaimsRequest(
            JsonObject claimsRequest, String parameterName) {
        ...
        if (claimsParameterValue != null) {
            ...
            claimsParameterValue
                    ...
                    .forEachRemaining(
                            entry ->
                                    resolveIndividualClaimsRequests(
                                            entry.getKey(), entry.getValue(), claimsParameter));
    ...
    /**
     * The userinfo and id_token members of the claims request both are JSON objects with the names
     * of the individual Claims being requested as the member names. The member values MUST be one
     * of the following: - null - JSON Object
     *
     ...
     */
    private void resolveIndividualClaimsRequests(
            String claimName, Object claimValue, Map<String, ClaimRequest> claimsParameter) {
