...
public abstract class AbstractFlow implements Flow {
    ...
    @Override
    public Single<AuthorizationResponse> run(
        ...
        return prepareResponse(authorizationRequest, client, endUser)
                .flatMap(
                        response ->
                                processResponse(response, authorizationRequest, client, endUser));
    }
    ...
    private Single<AuthorizationResponse> processResponse(
        ...
        // Create JWT Response
        JWTAuthorizationResponse jwtAuthorizationResponse =
                JWTAuthorizationResponse.from(authorizationResponse);
        jwtAuthorizationResponse.setIss(
                openIDDiscoveryService.getIssuer(authorizationRequest.getOrigin()));
        ...
        jwtAuthorizationResponse.setExp(
                Instant.now().plusSeconds(codeValidityInSec).getEpochSecond());
        ...
        return jwtService
                ...
                .map(
                        token -> {
                            jwtAuthorizationResponse.setResponseType(
                                    authorizationRequest.getResponseType());
                            jwtAuthorizationResponse.setResponseMode(
