...
public class JWKConverter {
    ...
    public static com.nimbusds.jose.jwk.RSAKey convert(io.gravitee.am.model.jose.RSAKey rsaKey) {
        try {
            if (!rsaKey.isPrivate()) {
                // Base64URL n, Base64URL e, KeyUse use, Set<KeyOperation> ops, Algorithm alg,
                // String kid, URI x5u, Base64URL x5t, Base64URL x5t256, List<Base64> x5c, KeyStore
                // ks
                return new com.nimbusds.jose.jwk.RSAKey(
                        new Base64URL(rsaKey.getN()),
                        new Base64URL(rsaKey.getE()),
                        rsaKey.getUse() != null
                                ? com.nimbusds.jose.jwk.KeyUse.parse(rsaKey.getUse())
                                : null,
                        rsaKey.getKeyOps() != null
                                ? KeyOperation.parse(new ArrayList<>(rsaKey.getKeyOps()))
                                : null,
    ...
    public static com.nimbusds.jose.jwk.ECKey convert(io.gravitee.am.model.jose.ECKey ecKey) {
        try {
            if (!ecKey.isPrivate()) {
                // Curve crv, Base64URL x, Base64URL y, KeyUse use, Set<KeyOperation> ops, Algorithm
                // alg, String kid, URI x5u, Base64URL x5t, Base64URL x5t256, List<Base64> x5c,
                // KeyStore ks
                return new com.nimbusds.jose.jwk.ECKey(
                        ...
                        ecKey.getUse() != null
                                ? com.nimbusds.jose.jwk.KeyUse.parse(ecKey.getUse())
                                : null,
                        ecKey.getKeyOps() != null
                                ? KeyOperation.parse(
                                        ecKey.getKeyOps().stream().collect(Collectors.toList()))
                                : null,
    ...
    public static OctetKeyPair convert(io.gravitee.am.model.jose.OKPKey okpKey) {
        try {
            if (!okpKey.isPrivate()) {
                // Curve crv, Base64URL x, KeyUse use, Set<KeyOperation> ops, Algorithm alg, String
                // kid, URI x5u, Base64URL x5t, Base64URL x5t256, List<Base64> x5c, KeyStore ks
                return new OctetKeyPair(
                        Curve.parse(okpKey.getCrv()),
                        new Base64URL(okpKey.getX()),
                        okpKey.getUse() != null
                                ? com.nimbusds.jose.jwk.KeyUse.parse(okpKey.getUse())
                                : null,
                        okpKey.getKeyOps() != null
                                ? KeyOperation.parse(
                                        okpKey.getKeyOps().stream().collect(Collectors.toList()))
                                : null,
    ...
    public static OctetSequenceKey convert(io.gravitee.am.model.jose.OCTKey octKey) {
        try {
            // Base64URL k, KeyUse use, Set<KeyOperation> ops, Algorithm alg, String kid, URI x5u,
            // Base64URL x5t, Base64URL x5t256, List<Base64> x5c, KeyStore ks
            return new OctetSequenceKey(
                    new Base64URL(octKey.getK()),
                    octKey.getUse() != null
                            ? com.nimbusds.jose.jwk.KeyUse.parse(octKey.getUse())
                            : null,
                    octKey.getKeyOps() != null
                            ? KeyOperation.parse(
                                    octKey.getKeyOps().stream().collect(Collectors.toList()))
                            : null,
    ...
    public static JWK convert(com.nimbusds.jose.jwk.RSAKey jwk) {
        ...
        rsaKey.setP(
                jwk.getFirstPrimeFactor() != null ? jwk.getFirstPrimeFactor().toString() : null);
        rsaKey.setQ(
                jwk.getSecondPrimeFactor() != null ? jwk.getSecondPrimeFactor().toString() : null);
        rsaKey.setDp(
                jwk.getFirstFactorCRTExponent() != null
                        ? jwk.getFirstFactorCRTExponent().toString()
                        : null);
        rsaKey.setDq(
                jwk.getSecondFactorCRTExponent() != null
                        ? jwk.getSecondFactorCRTExponent().toString()
                        : null);
        rsaKey.setQi(
                jwk.getFirstCRTCoefficient() != null
                        ? jwk.getFirstCRTCoefficient().toString()
                        : null);
        return rsaKey;
    }
