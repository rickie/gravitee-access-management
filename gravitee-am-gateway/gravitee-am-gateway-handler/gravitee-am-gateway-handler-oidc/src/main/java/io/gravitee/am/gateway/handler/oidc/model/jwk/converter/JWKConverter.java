...
public class JWKConverter {
    ...
    public static io.gravitee.am.gateway.handler.oidc.model.jwk.JWKSet convert(JWKSet jwkSet) {
        ...
        io.gravitee.am.gateway.handler.oidc.model.jwk.JWKSet result =
                new io.gravitee.am.gateway.handler.oidc.model.jwk.JWKSet();
        result.setKeys(
                jwkSet.getKeys().stream().map(JWKConverter::convert).collect(Collectors.toList()));
        return result;
    }

    public static io.gravitee.am.gateway.handler.oidc.model.jwk.JWK convert(
            io.gravitee.am.model.jose.JWK jwk) {
