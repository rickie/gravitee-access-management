...
public class DynamicClientRegistrationServiceImpl implements DynamicClientRegistrationService {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(DynamicClientRegistrationServiceImpl.class);
    public static final int FIVE_MINUTES_IN_SEC = 300;
    public static final String FAPI_OPENBANKING_BRAZIL_DIRECTORY_JWKS_URI =
            "openid.fapi.openbanking.brazil.directory.jwks_uri";
    public static final String OPENID_DCR_ACCESS_TOKEN_VALIDITY =
            "openid.dcr.access_token.validity";
    public static final String OPENID_DCR_REFRESH_TOKEN_VALIDITY =
            "openid.dcr.refresh_token.validity";
    ...
    public Single<Client> create(DynamicClientRegistrationRequest request, String basePath) {
        // If Dynamic client registration from template is enabled and request contains a
        // software_id
        if (domain.isDynamicClientRegistrationTemplateEnabled()
                && request.getSoftwareId() != null
                && request.getSoftwareId().isPresent()) {
    ...
    @Override
    public Single<Client> patch(
            Client toPatch, DynamicClientRegistrationRequest request, String basePath) {
    ...
    @Override
    public Single<Client> update(
            Client toUpdate, DynamicClientRegistrationRequest request, String basePath) {
    ...
    private Single<Client> createClientFromRequest(
            DynamicClientRegistrationRequest request, String basePath) {
    ...
    private Single<Client> applyAccessTokenValidity(Client client) {
        client.setAccessTokenValiditySeconds(
                environment.getProperty(
                        OPENID_DCR_ACCESS_TOKEN_VALIDITY,
                        Integer.class,
                        domain.useFapiBrazilProfile()
                                ? FAPI_OPENBANKING_BRAZIL_DEFAULT_ACCESS_TOKEN_VALIDITY
                                : Client.DEFAULT_ACCESS_TOKEN_VALIDITY_SECONDS));
        client.setRefreshTokenValiditySeconds(
                environment.getProperty(
                        OPENID_DCR_REFRESH_TOKEN_VALIDITY,
                        Integer.class,
                        Client.DEFAULT_REFRESH_TOKEN_VALIDITY_SECONDS));
        client.setIdTokenValiditySeconds(
                environment.getProperty(
                        OPENID_DCR_ID_TOKEN_VALIDITY,
                        Integer.class,
                        Client.DEFAULT_ID_TOKEN_VALIDITY_SECONDS));
     ...
     * </pre>
     */
    private Single<Client> createClientFromTemplate(
            DynamicClientRegistrationRequest request, String basePath) {
        return clientService
                ...
                .switchIfEmpty(
                        Maybe.error(
                                new InvalidClientMetadataException(
                                        "No template found for software_id "
                                                + request.getSoftwareId().get())))
    ...
    private Single<Client> sanitizeTemplate(Client template) {
        if (!template.isTemplate()) {
            return Single.error(
                    new InvalidClientMetadataException(
                            "Client behind software_id is not a template"));
        }
    ...
    private Single<Client> copyForms(String sourceId, Client client) {
        return formService
                .copyFromClient(domain.getId(), sourceId, client.getId())
    ...
    /**
     * Identity provider is not part of dynamic client registration but needed on the client. So we
     * set the first identity provider available on the domain.
     *
    ...
    /**
     * Certificate provider is not part of dynamic client registration but needed on the client. So
     * we set the first certificate provider available on the domain.
     *
     * @param client App to create
     * @return
     */
    ...
    private Single<Client> applyRegistrationAccessToken(String basePath, Client client) {
        ...
        OpenIDProviderMetadata openIDProviderMetadata =
                openIDDiscoveryService.getConfiguration(basePath);
        ...
        jwt.setExp(
                Date.from(new Date().toInstant().plusSeconds(3600 * 24 * 365 * 2)).getTime()
                        / 1000l);
        ...
        return jwtService
                ...
                .map(
                        token -> {
                            ...
                            client.setRegistrationClientUri(
                                    openIDProviderMetadata.getRegistrationEndpoint()
                                            + "/"
                                            + client.getClientId());
                            return client;
                        });
     ...
     *
     * <p>https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata
     *
    ...
    private Single<DynamicClientRegistrationRequest> validateClientPatchRequest(
            DynamicClientRegistrationRequest request) {
    ...
    private Single<DynamicClientRegistrationRequest> enforceWithSoftwareStatement(
            DynamicClientRegistrationRequest request) {
        if (this.domain.useFapiBrazilProfile()) {
            if (request.getSoftwareStatement() == null
                    || request.getSoftwareStatement().isEmpty()) {
                return Single.error(
            ...
            if (Strings.isNullOrEmpty(directoryJwksUri)) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "No jwks_uri for OpenBanking Directory, unable to validate software_statement"));
            }
            ...
            try {
                ...
                if (jwt instanceof SignedJWT) {
                    ...
                    if (isSignAlgCompliantWithFapi(
                        ...
                        return jwkService
                                .getKeys(directoryJwksUri)
                                .flatMap(
                                        jwk ->
                                                jwkService.getKey(
                                                        jwk, signedJWT.getHeader().getKeyID()))
                                .switchIfEmpty(
                                        Single.error(
                                                new TechnicalManagementException(
                                                        "Invalid jwks_uri for OpenBanking Directory")))
                                .filter(jwk -> jwsService.isValidSignature(signedJWT, jwk))
                                .switchIfEmpty(
                                        Single.error(
                                                new InvalidClientMetadataException(
                                                        "Invalid signature for software_statement")))
                                .map(
                                        __ -> {
                                            LOGGER.debug(
                                            ...
                                            if (iat == null
                                                ...
                                                throw new InvalidClientMetadataException(
                                                        "software_statement older than 5 minutes");
                                            }

                                            if (request.getJwks() != null
                                                    && !request.getJwks().isEmpty()) {
                                                throw new InvalidClientMetadataException(
                                                        "jwks is forbidden, prefer jwks_uri");
                                            }

                                            if (request.getJwksUri() == null
                                                    || request.getJwksUri().isEmpty()) {
                                                throw new InvalidClientMetadataException(
                                                        "jwks_uri is required");
                                            }

                                            if (!request.getJwksUri()
                                                    .get()
                                                    .equals(
                                                            softwareStatement.getAsString(
                                                                    "software_jwks_uri"))) {
                                                throw new InvalidClientMetadataException(
                                            ...
                                            if (software_redirect_uris != null) {
                                                if (request.getRedirectUris() == null
                                                        || request.getRedirectUris().isEmpty()) {
                                                    throw new InvalidClientMetadataException(
                                                    ...
                                                    redirectUris.forEach(
                                                            uri -> {
                                                                if (!((JSONArray)
                                                                        ...
                                                                        .contains(uri)) {
                                                                    throw new InvalidClientMetadataException(
                                                                            "redirect_uris contains unknown uri from software_statement");
                                                                }
                                                            });
                                                } else if (software_redirect_uris instanceof String
                                                        && (redirectUris.size() > 1
                                                                || !software_redirect_uris.equals(
                                                                        redirectUris.get(0)))) {
                                                    throw new InvalidClientMetadataException(
                                                            "redirect_uris contains unknown uri from software_statement");
                                                }
                                            ...
                                            if (request.getTokenEndpointAuthMethod() != null
                                                ...
                                                if (ClientAuthenticationMethod.TLS_CLIENT_AUTH
                                                                .equals(
                                                                        request.getTokenEndpointAuthMethod()
                                                                                .get())
                                                        && (request.getTlsClientAuthSubjectDn()
                                                                        == null
                                                                || request.getTlsClientAuthSubjectDn()
                                                                        .isEmpty())) {
                                                    throw new InvalidClientMetadataException(
                                                            "tls_client_auth_subject_dn is required with tls_client_auth as client authentication method");
                                                }
                                            }
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "software_statement isn't signed or doesn't use PS256"));

            } catch (ParseException pe) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "signature of software_statement is invalid"));
            }
        }
     ...
     */
    private Single<DynamicClientRegistrationRequest> validateRedirectUri(
            DynamicClientRegistrationRequest request, boolean isPatch) {

        // Except for patching a client, redirect_uris metadata is required but may be null or
        // empty.
    ...
    private Single<DynamicClientRegistrationRequest> validateResponseType(
            DynamicClientRegistrationRequest request) {
        // if response_type provided, they must be valid.
        if (request.getResponseTypes() != null) {
            if (!ResponseTypeUtils.isSupportedResponseType(
                    request.getResponseTypes().orElse(Collections.emptyList()))) {
    ...
    private Single<DynamicClientRegistrationRequest> validateGrantType(
            DynamicClientRegistrationRequest request) {
        // if grant_type provided, they must be valid.
        if (request.getGrantTypes() != null) {
            if (!GrantTypeUtils.isSupportedGrantType(
                    request.getGrantTypes().orElse(Collections.emptyList()))) {
                return Single.error(
                        new InvalidClientMetadataException("Missing or invalid grant type."));
            }
        }
        return Single.just(request);
    }

    private Single<DynamicClientRegistrationRequest> validateSubjectType(
            DynamicClientRegistrationRequest request) {
    ...
    private Single<DynamicClientRegistrationRequest> validateUserinfoSigningAlgorithm(
            DynamicClientRegistrationRequest request) {
        // if userinfo_signed_response_alg is provided, it must be valid.
        if (request.getUserinfoSignedResponseAlg() != null
                && request.getUserinfoSignedResponseAlg().isPresent()) {
            if (!JWAlgorithmUtils.isValidUserinfoSigningAlg(
                    request.getUserinfoSignedResponseAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
    ...
    private Single<DynamicClientRegistrationRequest> validateUserinfoEncryptionAlgorithm(
            DynamicClientRegistrationRequest request) {
        if (request.getUserinfoEncryptedResponseEnc() != null
                && request.getUserinfoEncryptedResponseAlg() == null) {
            return Single.error(
                    new InvalidClientMetadataException(
        ...
        if (request.getUserinfoEncryptedResponseAlg() != null
                && request.getUserinfoEncryptedResponseAlg().isPresent()) {
            if (!JWAlgorithmUtils.isValidUserinfoResponseAlg(
                    request.getUserinfoEncryptedResponseAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Unsupported userinfo_encrypted_response_alg value"));
            }
            if (request.getUserinfoEncryptedResponseEnc() != null
                    && request.getUserinfoEncryptedResponseEnc().isPresent()) {
                if (!JWAlgorithmUtils.isValidUserinfoResponseEnc(
                        request.getUserinfoEncryptedResponseEnc().get())) {
                    return Single.error(
                            new InvalidClientMetadataException(
                                    "Unsupported userinfo_encrypted_response_enc value"));
                }
            } else {
                // Apply default value if userinfo_encrypted_response_alg is informed and not
                // userinfo_encrypted_response_enc.
                request.setUserinfoEncryptedResponseEnc(
                        Optional.of(JWAlgorithmUtils.getDefaultUserinfoResponseEnc()));
            }
        }
        return Single.just(request);
    }

    private Single<DynamicClientRegistrationRequest> validateRequestObjectSigningAlgorithm(
            DynamicClientRegistrationRequest request) {
        // if userinfo_signed_response_alg is provided, it must be valid.
        if (request.getRequestObjectSigningAlg() != null
                && request.getRequestObjectSigningAlg().isPresent()) {
            if (!JWAlgorithmUtils.isValidRequestObjectSigningAlg(
                    request.getRequestObjectSigningAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
            ...
            if (this.domain.usePlainFapiProfile()
                    && !isSignAlgCompliantWithFapi(request.getRequestObjectSigningAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "request_object_signing_alg shall be PS256"));
            }
        }
    ...
    private Single<DynamicClientRegistrationRequest> validateRequestObjectEncryptionAlgorithm(
            DynamicClientRegistrationRequest request) {
        if (request.getRequestObjectEncryptionEnc() != null
                && request.getRequestObjectEncryptionAlg() == null) {
            return Single.error(
                    new InvalidClientMetadataException(
        ...
        // if userinfo_encrypted_response_alg is provided, it must be valid.
        if (request.getRequestObjectEncryptionAlg() != null
                && request.getRequestObjectEncryptionAlg().isPresent()) {
            if (!domain.useFapiBrazilProfile()
                    && !JWAlgorithmUtils.isValidRequestObjectAlg(
                            request.getRequestObjectEncryptionAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Unsupported request_object_encryption_alg value"));
            }

            if (request.getRequestObjectEncryptionEnc() != null
                    && request.getRequestObjectEncryptionEnc().isPresent()) {
                if (!JWAlgorithmUtils.isValidRequestObjectEnc(
                        request.getRequestObjectEncryptionEnc().get())) {
                    return Single.error(
                            new InvalidClientMetadataException(
                                    "Unsupported request_object_encryption_enc value"));
                }
            } else {
                // Apply default value if request_object_encryption_alg is informed and not
                // request_object_encryption_enc.
                request.setRequestObjectEncryptionEnc(
                        Optional.of(JWAlgorithmUtils.getDefaultRequestObjectEnc()));
            }
        }
        ...
        if (domain.useFapiBrazilProfile()) {
            ...
            if (requestModeByValue
                    && ((request.getRequestObjectEncryptionEnc() == null
                                    || request.getRequestObjectEncryptionEnc().isEmpty())
                            || (request.getRequestObjectEncryptionAlg() == null
                                    || request.getRequestObjectEncryptionAlg().isEmpty())
                            || !(isKeyEncCompliantWithFapiBrazil(
                                            request.getRequestObjectEncryptionAlg().get())
                                    && isContentEncCompliantWithFapiBrazil(
                                            request.getRequestObjectEncryptionEnc().get())))) {
                throw new InvalidClientMetadataException(
                        "Request object must be encrypted using RSA-OAEP with A256GCM");
            }
        }
    ...
    private Single<DynamicClientRegistrationRequest> validateIdTokenSigningAlgorithm(
            DynamicClientRegistrationRequest request) {
        // if userinfo_signed_response_alg is provided, it must be valid.
        if (request.getIdTokenSignedResponseAlg() != null
                && request.getIdTokenSignedResponseAlg().isPresent()) {
            if (!JWAlgorithmUtils.isValidIdTokenSigningAlg(
                    request.getIdTokenSignedResponseAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
    ...
    private Single<DynamicClientRegistrationRequest> validateIdTokenEncryptionAlgorithm(
            DynamicClientRegistrationRequest request) {
        if (request.getIdTokenEncryptedResponseEnc() != null
                && request.getIdTokenEncryptedResponseAlg() == null) {
            return Single.error(
                    new InvalidClientMetadataException(
        ...
        if (request.getIdTokenEncryptedResponseAlg() != null
                && request.getIdTokenEncryptedResponseAlg().isPresent()) {
            if (!JWAlgorithmUtils.isValidIdTokenResponseAlg(
                    request.getIdTokenEncryptedResponseAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Unsupported id_token_encrypted_response_alg value"));
            }
            if (request.getIdTokenEncryptedResponseEnc() != null
                    && request.getIdTokenEncryptedResponseEnc().isPresent()) {
                if (!JWAlgorithmUtils.isValidIdTokenResponseEnc(
                        request.getIdTokenEncryptedResponseEnc().get())) {
                    return Single.error(
                            new InvalidClientMetadataException(
                                    "Unsupported id_token_encrypted_response_enc value"));
                }
            } else {
                // Apply default value if id_token_encrypted_response_alg is informed and not
                // id_token_encrypted_response_enc.
                request.setIdTokenEncryptedResponseEnc(
                        Optional.of(JWAlgorithmUtils.getDefaultIdTokenResponseEnc()));
            }
        }
        return Single.just(request);
    }

    private Single<DynamicClientRegistrationRequest> validateRequestUri(
            DynamicClientRegistrationRequest request) {
        ...
        if (request.getRequestUris() != null && request.getRequestUris().isPresent()) {
            try {
                ...
                return Single.error(
                        new InvalidClientMetadataException("request_uris: " + err.getMessage()));
            }
        }
        return Single.just(request);
    }

    private Single<DynamicClientRegistrationRequest> validateSectorIdentifierUri(
            DynamicClientRegistrationRequest request) {
        // if sector_identifier_uri is provided, then retrieve content and validate redirect_uris
        // among this list.
        if (request.getSectorIdentifierUri() != null
                && request.getSectorIdentifierUri().isPresent()) {
            ...
            try {
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "sector_identifier_uri: " + err.getMessage()));
            }

            if (!uri.getScheme().equalsIgnoreCase("https")) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Scheme must be https for sector_identifier_uri : "
                                        + request.getSectorIdentifierUri().get()));
            }
            ...
            return client.getAbs(uri.toString())
                    ...
                    .onErrorResumeNext(
                            Single.error(
                                    new InvalidClientMetadataException(
                                            "Unable to parse sector_identifier_uri : "
                                                    + uri.toString())))
                    ...
                    .flatMap(
                            allowedRedirectUris ->
                                    Observable.fromIterable(request.getRedirectUris().get())
                                            .filter(
                                                    redirectUri ->
                                            ...
                                            .collect(
                                                    ...
                                                    (list, missingRedirectUri) ->
                                                            list.add(missingRedirectUri))
                                            .flatMap(
                                                    missing -> {
                                                        if (!missing.isEmpty()) {
                                                            return Single.error(
    ...
    private Single<DynamicClientRegistrationRequest> validateJKWs(
            DynamicClientRegistrationRequest request) {
        // The jwks_uri and jwks parameters MUST NOT be used together.
        if (request.getJwks() != null
                && request.getJwks().isPresent()
                && request.getJwksUri() != null
                && request.getJwksUri().isPresent()) {
            return Single.error(
                    new InvalidClientMetadataException(
                            "The jwks_uri and jwks parameters MUST NOT be used together."));
        }
    ...
    /**
     * Remove non allowed scopes (if feature is enabled) and then apply default scopes. The scopes
     * validations are done later (validateMetadata) on the process.
     *
     * @param request DynamicClientRegistrationRequest
     * @return DynamicClientRegistrationRequest
     */
    private Single<DynamicClientRegistrationRequest> validateScopes(
        ...
        if ((request.getScope() == null
                        || !request.getScope().isPresent()
                        || request.getScope().get().isEmpty())
                && hasDefaultScopes) {
    ...
    private Single<DynamicClientRegistrationRequest> validateAuthorizationSigningAlgorithm(
            DynamicClientRegistrationRequest request) {
        // Signing an authorization response is required
        // As per
        // https://bitbucket.org/openid/fapi/src/master/Financial_API_JWT_Secured_Authorization_Response_Mode.md#markdown-header-5-client-metadata
        // If unspecified, the default algorithm to use for signing authorization responses is
        // RS256. The algorithm none is not allowed.
        if (request.getAuthorizationSignedResponseAlg() == null
                || !request.getAuthorizationSignedResponseAlg().isPresent()) {
            request.setAuthorizationSignedResponseAlg(Optional.of(JWSAlgorithm.RS256.getName()));
        }

        if (!JWAlgorithmUtils.isValidAuthorizationSigningAlg(
                request.getAuthorizationSignedResponseAlg().get())) {
            return Single.error(
                    new InvalidClientMetadataException(
                            "Unsupported authorization signing algorithm"));
        }

        return Single.just(request);
    }

    private Single<DynamicClientRegistrationRequest> validateAuthorizationEncryptionAlgorithm(
            DynamicClientRegistrationRequest request) {
        if ((request.getAuthorizationEncryptedResponseEnc() != null
                        && request.getAuthorizationEncryptedResponseEnc().isPresent())
                && (request.getAuthorizationEncryptedResponseAlg() == null
                        || !request.getAuthorizationEncryptedResponseAlg().isPresent())) {
            return Single.error(
                    new InvalidClientMetadataException(
        ...
        if (request.getAuthorizationEncryptedResponseAlg() != null
                && request.getAuthorizationEncryptedResponseAlg().isPresent()) {
            if (!JWAlgorithmUtils.isValidAuthorizationResponseAlg(
                    request.getAuthorizationEncryptedResponseAlg().get())) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Unsupported authorization_encrypted_response_alg value"));
            }

            if (request.getAuthorizationEncryptedResponseEnc() != null
                    && request.getAuthorizationEncryptedResponseEnc().isPresent()) {
                if (!JWAlgorithmUtils.isValidAuthorizationResponseEnc(
                        request.getAuthorizationEncryptedResponseEnc().get())) {
                    return Single.error(
                            new InvalidClientMetadataException(
                                    "Unsupported authorization_encrypted_response_enc value"));
                }
            } else {
                // Apply default value if authorization_encrypted_response_alg is informed and not
                // authorization_encrypted_response_enc.
                request.setAuthorizationEncryptedResponseEnc(
                        Optional.of(JWAlgorithmUtils.getDefaultAuthorizationResponseEnc()));
            }
        }
        return Single.just(request);
    }
    ...
    /**
     * A client using the "tls_client_auth" authentication method MUST use exactly one of the below
     * metadata parameters to indicate the certificate subject value that the authorization server
     * is to expect when authenticating the respective client. <a
     * href="https://tools.ietf.org/html/rfc8705#section-2.1.2">Client Registration Metadata</a>
     *
     * @param request DynamicClientRegistrationRequest
     * @return DynamicClientRegistrationRequest
     */
    private Single<DynamicClientRegistrationRequest> validateTlsClientAuth(
            DynamicClientRegistrationRequest request) {
        if (request.getTokenEndpointAuthMethod() != null
                && request.getTokenEndpointAuthMethod().isPresent()
                && ClientAuthenticationMethod.TLS_CLIENT_AUTH.equalsIgnoreCase(
                        request.getTokenEndpointAuthMethod().get())) {

            if ((request.getTlsClientAuthSubjectDn() == null
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "Missing TLS parameter for tls_client_auth."));
            }
            ...
            if (request.getTlsClientAuthSubjectDn() != null
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "The tls_client_auth must use exactly one of the TLS parameters."));
            } else if (request.getTlsClientAuthSanDns() != null
                    && request.getTlsClientAuthSanDns().isPresent()
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "The tls_client_auth must use exactly one of the TLS parameters."));
            } else if (request.getTlsClientAuthSanIp() != null
                    && request.getTlsClientAuthSanIp().isPresent()
                    && ((request.getTlsClientAuthSubjectDn() != null
                                    && request.getTlsClientAuthSubjectDn().isPresent())
                            || (request.getTlsClientAuthSanDns() != null
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "The tls_client_auth must use exactly one of the TLS parameters."));
            } else if (request.getTlsClientAuthSanEmail() != null
                    && request.getTlsClientAuthSanEmail().isPresent()
                    && ((request.getTlsClientAuthSubjectDn() != null
                                    && request.getTlsClientAuthSubjectDn().isPresent())
                            || (request.getTlsClientAuthSanDns() != null
                ...
                return Single.error(
                        new InvalidClientMetadataException(
                                "The tls_client_auth must use exactly one of the TLS parameters."));
            } else if (request.getTlsClientAuthSanUri() != null
                    && request.getTlsClientAuthSanUri().isPresent()
                    && ((request.getTlsClientAuthSubjectDn() != null
                                    && request.getTlsClientAuthSubjectDn().isPresent())
                            || (request.getTlsClientAuthSanDns() != null
            ...
            // because only TLS parameter is authorized, we force the missing options to empty to
            // avoid
            // remaining values during an update as the null value for a field do not reset the
            // field...
            if (request.getTlsClientAuthSubjectDn() != null
                    && request.getTlsClientAuthSubjectDn().isPresent()) {
     ...
     * authentication using self-signed certificates. As a prerequisite, the client registers its
     * X.509 certificates (using "jwks" defined in [RFC7591]) or a reference to a trusted source for
     * its X.509 certificates (using "jwks_uri" from [RFC7591]) with the authorization server. <a
     * href="https://tools.ietf.org/html/rfc8705#section-2.2.2">Client Registration Metadata</a>
     *
     * @param request DynamicClientRegistrationRequest
     * @return DynamicClientRegistrationRequest
     */
    private Single<DynamicClientRegistrationRequest> validateSelfSignedClientAuth(
            DynamicClientRegistrationRequest request) {
        if (request.getTokenEndpointAuthMethod() != null
                && request.getTokenEndpointAuthMethod().isPresent()
                && ClientAuthenticationMethod.SELF_SIGNED_TLS_CLIENT_AUTH.equalsIgnoreCase(
                        request.getTokenEndpointAuthMethod().get())) {
            if ((request.getJwks() == null || !request.getJwks().isPresent())
                    && (request.getJwksUri() == null || !request.getJwksUri().isPresent())) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "The self_signed_tls_client_auth requires at least a jwks or a valid jwks_uri."));
            }
        }
        return Single.just(request);
    }
    ...
    private Single<DynamicClientRegistrationRequest> validateCibaSettings(
        ...
        if (domain.useCiba() && useCibaGrantType) {
            if (request.getBackchannelTokenDeliveryMode() == null
                    || request.getBackchannelTokenDeliveryMode().isEmpty()) {
                return Single.error(
                        new InvalidClientMetadataException(
            ...
            if (!CIBADeliveryMode.SUPPORTED_DELIVERY_MODES.contains(deliveryMode)) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Unsupported backchannel_token_delivery_mode"));
            }

            if ((deliveryMode.equals(CIBADeliveryMode.PING)
                            || deliveryMode.equals(CIBADeliveryMode.PUSH))
                    && ((request.getBackchannelClientNotificationEndpoint() == null
                                    || request.getBackchannelClientNotificationEndpoint().isEmpty())
                            || (!request.getBackchannelClientNotificationEndpoint()
                                    .get()
                                    .startsWith("https")))) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Missing or Invalid backchannel_client_notification_endpoint"));
            }

            if (request.getBackchannelUserCodeParameter() != null
                    && request.getBackchannelUserCodeParameter().isPresent()
                    && request.getBackchannelUserCodeParameter().get()) {
                return Single.error(
                        new InvalidClientMetadataException(
                                "Unsupported backchannel_user_code_parameter"));
            }
        }
