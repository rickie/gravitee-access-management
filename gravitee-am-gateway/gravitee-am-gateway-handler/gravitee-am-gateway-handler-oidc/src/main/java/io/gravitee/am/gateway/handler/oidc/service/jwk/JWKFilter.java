...
public class JWKFilter {
    ...
    public static Predicate<JWK> RSA_KEY_ENCRYPTION() {
        return jwk ->
                jwk != null
                        ...
                        && ((io.gravitee.am.model.jose.RSAKey) jwk).getN() != null
                        && // Filter weak keys
                        new BigInteger(((io.gravitee.am.model.jose.RSAKey) jwk).getN().getBytes())
                                        .bitLength()
                                >= 2048
                        && // Filter weak keys
    ...
    public static Predicate<io.gravitee.am.model.jose.JWK> OCT_KEY_ENCRYPTION() {
        return jwk ->
                jwk != null
                        ...
                        && (KeyUse.ENCRYPTION.getValue().equals(jwk.getUse())
                                || jwk.getUse() == null);
    }

    /**
     * @return Filter to retrieve AES keys, with same size as the algorithm, expected use for
     *     encryption.
     */
    public static Predicate<io.gravitee.am.model.jose.JWK> OCT_KEY_ENCRYPTION(
        ...
        return jwk ->
                jwk != null
                        ...
                        && new Base64URL(((io.gravitee.am.model.jose.OCTKey) jwk).getK())
                                                .decode()
                                                .length
                                        * 8
    ...
    public static Predicate<io.gravitee.am.model.jose.JWK> OCT_KEY_ENCRYPTION(
        ...
        return jwk -> {
            ...
            } else if (JWEAlgorithm.A192KW.equals(algorithm)
                    || JWEAlgorithm.A192GCMKW.equals(algorithm)) {
                expectedKeySize = 24; // 192/8
            } else if (JWEAlgorithm.A256KW.equals(algorithm)
                    || JWEAlgorithm.A256GCMKW.equals(algorithm)) {
    ...
    public static Predicate<io.gravitee.am.model.jose.JWK> CURVE_KEY_ENCRYPTION() {
        return jwk ->
                jwk != null
                        && (KeyType.EC.getKeyType().equals(jwk.getKty())
                                || isOctetKeyPairEncryption(jwk))
                        && (KeyUse.ENCRYPTION.getValue().equals(jwk.getUse())
                                || jwk.getUse() == null);
    }
