...
/**
 * This handler is responsible to initialize the authorization transaction id when it begin. It
 * basically uses the request transaction id and put it in the current session if not already
 * present. This allows to keep the same transaction identifier during all the authorization steps
 * until the last one as been achieved (or fails).
 *
