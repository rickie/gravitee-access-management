 ...
 *
 * <p>Clients that have registered a public key sign a JWT using that key.
 *
 * <p>The Client authenticates in accordance with JSON Web Token (JWT) Profile for OAuth 2.0 Client
 * Authentication and Authorization Grants [OAuth.JWT] and Assertion Framework for OAuth 2.0 Client
 * Authentication and Authorization Grants [OAuth.Assertions].
 *
 * <p>The JWT MUST contain the following REQUIRED Claim Values and MAY contain the following
 * OPTIONAL Claim Values: iss REQUIRED. Issuer. This MUST contain the client_id of the OAuth Client.
 * sub REQUIRED. Subject. This MUST contain the client_id of the OAuth Client. aud REQUIRED.
 * Audience. The aud (audience) Claim. Value that identifies the Authorization Server as an intended
 * audience. The Authorization Server MUST verify that it is an intended audience for the token. The
 * Audience SHOULD be the URL of the Authorization Server's Token Endpoint. jti REQUIRED. JWT ID. A
 * unique identifier for the token, which can be used to prevent reuse of the token. These tokens
 * MUST only be used once, unless conditions for reuse were negotiated between the parties; any such
 * negotiation is beyond the scope of this specification. exp REQUIRED. Expiration time on or after
 * which the ID Token MUST NOT be accepted for processing. iat OPTIONAL. Time at which the JWT was
 * issued.
 *
...
public class ClientAssertionAuthProvider implements ClientAuthProvider {
    ...
    public boolean canHandle(Client client, RoutingContext context) {
        if (client != null
                && (ClientAuthenticationMethod.PRIVATE_KEY_JWT.equals(
                        ...
                        || ClientAuthenticationMethod.CLIENT_SECRET_JWT.equals(
                                client.getTokenEndpointAuthMethod()))) {
            return true;
        }
        ...
        if ((client == null
                        ...
                        || client.getTokenEndpointAuthMethod().isEmpty())
                && getClientAssertion(context.request()) != null
                && getClientAssertionType(context.request()) != null) {
            return true;
        }
        return false;
    }
    ...
    public void handle(
        ...
        clientAssertionService
                ...
                .flatMap(
                        client1 -> {
                            // clientId is optional, but if provided we must ensure it is the same
                            // than the logged client.
                            if (clientId != null && !clientId.equals(client1.getClientId())) {
                                return Maybe.error(
                                        new InvalidClientException(
                                                "client_id parameter does not match with assertion"));
                            }
                ...
                .subscribe(
                        ...
                        throwable -> {
                            if (throwable instanceof InvalidClientException) {
                                logger.debug(
                                        "Failed to authenticate client with assertion method",
                                        throwable);
                                ...
                                handler.handle(
                                        Future.failedFuture(
                                                new InvalidClientException(
                                                        "Invalid client: Failed to authenticate client with assertion method",
                                                        throwable)));
                            }
                        },
                        () ->
