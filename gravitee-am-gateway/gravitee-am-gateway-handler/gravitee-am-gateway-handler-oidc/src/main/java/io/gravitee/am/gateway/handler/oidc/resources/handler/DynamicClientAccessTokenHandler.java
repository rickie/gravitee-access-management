...
/**
 * The Client Configuration Endpoint is an OAuth 2.0 Protected Resource that MAY be provisioned by
 * the server for a specific Client to be able to view and update its registered information. The
 * Client MUST use its Registration Access Token in all calls to this endpoint as an OAuth 2.0
 * Bearer Token unless the token has at least a DCR_ADMIN scope
 *
...
public class DynamicClientAccessTokenHandler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext context) {
        ...
        if (rawToken == null || !rawToken.equals(client.getRegistrationAccessToken())) {
            context.fail(
                    new ClientRegistrationForbiddenException(
                            "Non matching registration_access_token"));
            return;
        }
        ...
        if (!isRequestPathClientIdMatching(token, clientIdPathParameter)) {
            context.fail(
                    new ClientRegistrationForbiddenException(
                            "Not allowed to access to : " + clientIdPathParameter));
            return;
        }
