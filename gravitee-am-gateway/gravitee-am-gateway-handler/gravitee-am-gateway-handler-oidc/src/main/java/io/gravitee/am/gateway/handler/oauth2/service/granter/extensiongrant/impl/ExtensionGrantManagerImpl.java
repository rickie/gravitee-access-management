 ...
 */
public class ExtensionGrantManagerImpl extends AbstractService
        implements ExtensionGrantManager,
                InitializingBean,
                EventListener<ExtensionGrantEvent, Payload> {
    ...
    private ConcurrentMap<String, ExtensionGrantGranter> extensionGrantGranters =
    ...
    @Autowired private ExtensionGrantPluginManager extensionGrantPluginManager;

    @Autowired private ExtensionGrantRepository extensionGrantRepository;
    ...
    public void afterPropertiesSet() {
        ...
        extensionGrantRepository
                ...
                .subscribe(
                        extensionGrant -> {
                            // backward compatibility, get the oldest extension grant to set the
                            // good one for the old clients
                            minDate =
                                    minDate == null
                                            ? extensionGrant.getCreatedAt()
                                            : minDate.after(extensionGrant.getCreatedAt())
                                                    ? extensionGrant.getCreatedAt()
                                                    : minDate;
                        ...
                        error ->
                                logger.error(
                                        "Unable to initialize extension grants for domain {}",
                                        domain.getName(),
                                        error));

        logger.info(
                "Register event listener for extension grant events for domain {}",
                domain.getName());
    ...
    protected void doStop() throws Exception {
        ...
        logger.info(
                "Dispose event listener for extension grant events for domain {}",
                domain.getName());
    ...
    private void updateExtensionGrant(
        ...
        extensionGrantRepository
                ...
                .subscribe(
                        ...
                        error ->
                                logger.error(
                                        "Unable to {} extension grant for domain {}",
                                        eventType,
                                        domain.getName(),
                                        error),
                        () ->
                                logger.error(
                                        "No extension grant found with id {}", extensionGrantId));
    }

    private void removeExtensionGrant(String extensionGrantId) {
        logger.info(
                "Domain {} has received extension grant event, delete extension grant {}",
                domain.getName(),
                extensionGrantId);
        ...
        if (!extensionGrants.isEmpty()) {
            minDate =
                    Collections.min(
                            extensionGrants.values().stream()
                                    .map(ExtensionGrant::getCreatedAt)
                                    .collect(Collectors.toList()));
            extensionGrantGranters
                    .values()
                    .forEach(extensionGrantGranter -> extensionGrantGranter.setMinDate(minDate));
        }
    }
        ...
        try {
            if (needDeployment(extensionGrant)) {
                ...
                if (extensionGrant.getIdentityProvider() != null) {
                    logger.info(
                            "\tLooking for extension grant identity provider: {}",
                    ...
                    if (authenticationProvider != null) {
                        logger.info(
                                "\tExtension grant identity provider: {}, loaded",
                                extensionGrant.getIdentityProvider());
                    }
                }

                var providerConfiguration =
                        new ExtensionGrantProviderConfiguration(
                                extensionGrant, authenticationProvider);
                var extensionGrantProvider =
                        extensionGrantPluginManager.create(providerConfiguration);
                ExtensionGrantGranter extensionGrantGranter =
                        new ExtensionGrantGranter(
                                ...
                                identityProviderManager,
                                userService);
                // backward compatibility, set min date to the extension grant granter to choose the
                // good one for the old clients
                extensionGrantGranter.setMinDate(minDate);
                ((CompositeTokenGranter) tokenGranter)
                        .addTokenGranter(extensionGrant.getId(), extensionGrantGranter);
                ...
                logger.info(
                        "Extension grant {} already loaded for domain {}",
            ...
            logger.error(
                    "An error occurs while initializing the extension grant : {}",
     ...
     * @return true if the ExtensionGrant has never been deployed or if the deployed version is not
     *     up to date
     */
