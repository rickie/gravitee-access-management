...
/**
 * The Client sends the UserInfo Request using either HTTP GET or HTTP POST. The Access Token
 * obtained from an OpenID Connect Authentication Request MUST be sent as a Bearer Token, per
 * Section 2 of OAuth 2.0 Bearer Token Usage [RFC6750]. It is RECOMMENDED that the request use the
 * HTTP GET method and the Access Token be sent using the Authorization header field.
 *
 * <p>See <a href="http://openid.net/specs/openid-connect-core-1_0.html#UserInfo">5.3.1. UserInfo
 * Request</a>
 *
 ...
 *
 * <p>See <a href="http://openid.net/specs/openid-connect-core-1_0.html#UserInfo">5.3. UserInfo
 * Endpoint</a>
 *
...
public class UserInfoEndpoint implements Handler<RoutingContext> {
    ...
    public UserInfoEndpoint(
        ...
        this.legacyOpenidScope =
                environment.getProperty(
                        "legacy.openid.openid_scope_full_profile", boolean.class, false);
    }
    ...
    public void handle(RoutingContext context) {
        ...
        userService
                .findById(subject)
                .switchIfEmpty(
                        Maybe.error(new InvalidTokenException("No user found for this token")))
                            ...
                            if (!expectSignedOrEncryptedUserInfo(client)) {
                                ...
                                jwt.setIss(
                                        openIDDiscoveryService.getIssuer(
                                                UriBuilderRequest.resolveProxyRequest(context)));
                                ...
                                return jwtService
                                        .encodeUserinfo(
                                                jwt,
                                                client) // Sign if needed, else return unsigned JWT
                                        .flatMap(
                                                userinfo ->
    ...
    private Map<String, Object> processClaims(User user, JWT accessToken) {
        ...
        // 2. process the request using the claims values (If present, the listed Claims are being
        // requested to be added to any Claims that are being requested using scope values.
        // If not present, the Claims being requested from the UserInfo Endpoint are only those
        // requested using scope values.)
        if (accessToken.getClaimsRequestParameter() != null) {
            requestForSpecificClaims =
                    processClaimsRequest(
                            (String) accessToken.getClaimsRequestParameter(),
                            fullProfileClaims,
                            requestedClaims);
        }
    ...
    private boolean processScopesRequest(
        ...
        scopesClaimKeys.stream()
                ...
                .forEach(
                        scopeClaim ->
                                requestedClaims.putIfAbsent(
                                        scopeClaim, fullProfileClaims.get(scopeClaim)));
                ...
                .map(
                        user1 -> {
                            ...
                            if (user.getRolesPermissions() != null
                                    && !user.getRolesPermissions().isEmpty()) {
                                userClaims.putIfAbsent(
                                        CustomClaims.ROLES,
    ...
    private boolean expectSignedOrEncryptedUserInfo(Client client) {
        return client.getUserinfoSignedResponseAlg() != null
