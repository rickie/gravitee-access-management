...
public class IDTokenServiceImpl implements IDTokenService {
    ...
    public Single<User> extractUser(String idToken, Client client) {
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(jwtService.decodeAndVerify(idToken, client)).flatMap(v->RxJava2Adapter.singleToMono((Single<User>)RxJavaReactorMigrationUtil.toJdkFunction((Function<JWT, Single<User>>)jwt -> {
                    return RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(userService.findById(jwt.getSub())).switchIfEmpty(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.error(new UserNotFoundException(jwt.getSub()))))).map(RxJavaReactorMigrationUtil.toJdkFunction(user -> {
