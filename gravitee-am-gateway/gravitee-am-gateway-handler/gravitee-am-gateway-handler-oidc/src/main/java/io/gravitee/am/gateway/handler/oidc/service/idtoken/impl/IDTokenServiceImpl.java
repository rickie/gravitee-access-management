...
public class IDTokenServiceImpl implements IDTokenService {
        ...
        return Single.fromCallable(
                        () ->
                                executionContext != null
                                        ? executionContext
                                        : createExecution(oAuth2Request, client, user))
                .flatMap(
                        executionContext1 -> {
                            // create JWT ID Token
                            IDToken idToken =
                                    createIDTokenJWT(oAuth2Request, client, user, executionContext);
                                    ...
                                    .flatMapSingle(
                                            certificateProvider -> {
                                                ...
                                                if (oAuth2Request.getContext() != null
                                                        && !oAuth2Request.getContext().isEmpty()) {
                                                    oAuth2Request
                                                            .getContext()
                                                            .forEach(
                                                ...
                                                return jwtService.encode(
                                                        idToken, certificateProvider);
                                            })
                                    .flatMap(
                                            signedIdToken -> {
                                                if (client.getIdTokenEncryptedResponseAlg()
                                                        != null) {
                                                    return jweService.encryptIdToken(
                                                            signedIdToken, client);
                                                }
                ...
                .flatMap(
                        jwt ->
                                userService
                                        ...
                                        .switchIfEmpty(
                                                Single.error(
                                                        new UserNotFoundException(jwt.getSub())))
                                        .map(
                                                user -> {
    ...
    /**
     * Create ID Token. The ID Token is represented as a JSON Web Token (JWT) and contains some
     * required claims
     *
     ...
     * @return an ID Token
     */
    private IDToken createIDTokenJWT(
            OAuth2Request oAuth2Request,
            Client client,
            User user,
            ExecutionContext executionContext) {
        ...
        idToken.setExp(
                Instant.ofEpochSecond(idToken.getIat())
                        .plusSeconds(client.getIdTokenValiditySeconds())
                        .getEpochSecond());
        ...
        // we decided to always provide this claim during the Financial-grand API conformance
        // implementation
        // since this claim was return by default in some cases even if conditions that require it
        // were missing
        ...
        String nonce =
                oAuth2Request.parameters() != null
                        ? oAuth2Request.parameters().getFirst(Parameters.NONCE)
                        : null;
        ...
        if (!oAuth2Request.isClientOnly()
            ...
            // to be sure that this sub value coming from the IDP will not override the one provided
            // by AM
            ...
            if (oAuth2Request.getScopes() != null) {
                requestForSpecificClaims =
                        processScopesRequest(
                                oAuth2Request.getScopes(), userClaims, fullProfileClaims, idToken);
            }

            // 2. process the request using the claims values (If present, the listed Claims are
            // being requested to be added to the default Claims in the ID Token)
            if (oAuth2Request.parameters() != null
                    && oAuth2Request.parameters().getFirst(Parameters.CLAIMS) != null) {
                requestForSpecificClaims =
                        processClaimsRequest(
                                oAuth2Request.parameters().getFirst(Parameters.CLAIMS),
                                fullProfileClaims,
                                idToken);
            }
    ...
    /**
     * For OpenID Connect, scopes can be used to request that specific sets of information be made
    ...
    private boolean processScopesRequest(
        ...
        if (scopes.contains(FULL_PROFILE.getKey())
                || (legacyOpenidScope
                        && scopes.size() == 1
                        && scopes.contains(Scope.OPENID.getKey()))) {
        ...
        List<String> scopesClaims =
                scopes.stream()
                        ...
                        .filter(
                                scope ->
                                        Scope.exists(scope)
                                                && !Scope.valueOf(scope).getClaims().isEmpty())
                        .map(Scope::valueOf)
                        .map(Scope::getClaims)
                        .flatMap(List::stream)
                                    ...
                                    if (fullProfileClaims.containsKey(key)) {
                                        ...
                                        if (claimRequest.getValues() != null) {
                                            idToken.addAdditionalClaim(
                                                    key, claimRequest.getValues());
                                        } else if (claimRequest.getValue() != null) {
                                            idToken.addAdditionalClaim(
                                                    key, claimRequest.getValue());
                                        }
                                    }
                                });
                return true;
    ...
    private void enhanceIDToken(
            JWT jwt, List<TokenClaim> customClaims, ExecutionContext executionContext) {
