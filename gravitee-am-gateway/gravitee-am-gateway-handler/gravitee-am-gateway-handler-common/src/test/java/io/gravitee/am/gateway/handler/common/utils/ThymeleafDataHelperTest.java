...
public class ThymeleafDataHelperTest {
    ...
    public void shouldGetAuthenticatedUser() {
        ...
        io.vertx.reactivex.ext.auth.User authenticatedUser =
                mock(io.vertx.reactivex.ext.auth.User.class);
        given(routingContext.user()).willReturn(authenticatedUser);
        io.gravitee.am.gateway.handler.common.vertx.web.auth.user.User delegateUser =
                mock(io.gravitee.am.gateway.handler.common.vertx.web.auth.user.User.class);
