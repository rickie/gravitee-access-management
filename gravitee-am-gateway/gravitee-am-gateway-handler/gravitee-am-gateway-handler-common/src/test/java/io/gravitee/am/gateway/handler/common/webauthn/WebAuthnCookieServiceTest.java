...
public class WebAuthnCookieServiceTest {
    ...
    @InjectMocks private WebAuthnCookieService webAuthnCookieService = new WebAuthnCookieService();
    ...
    public void shouldGenerateRememberDeviceCookieValue_nominal_case() {
        ...
        when(jwtService.encode(any(), eq(certificateProvider)))
                .thenReturn(Single.just("cookieValue"));

        TestObserver<String> testObserver =
                webAuthnCookieService.generateRememberDeviceCookieValue(user).test();
    ...
    public void shouldVerifyRememberDeviceCookieValue_nominal_case() {
        when(jwtService.decodeAndVerify(anyString(), eq(certificateProvider)))
                .thenReturn(Single.just(new JWT()));
        TestObserver<Void> testObserver =
                webAuthnCookieService.verifyRememberDeviceCookieValue("cookieValue").test();
        testObserver.awaitTerminalEvent();
        testObserver.assertNoErrors();
        testObserver.assertComplete();
    }
    ...
    public void shouldVerifyRememberDeviceCookieValue_error() {
        when(jwtService.decodeAndVerify(anyString(), eq(certificateProvider)))
                .thenReturn(Single.error(new IllegalArgumentException("invalid-token")));
        TestObserver<Void> testObserver =
                webAuthnCookieService.verifyRememberDeviceCookieValue("cookieValue").test();
        testObserver.awaitTerminalEvent();
