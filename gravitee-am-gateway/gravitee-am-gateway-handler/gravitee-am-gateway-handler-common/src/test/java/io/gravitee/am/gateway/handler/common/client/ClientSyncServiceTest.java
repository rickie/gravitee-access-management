...
public class ClientSyncServiceTest {
    ...
    @InjectMocks private ClientSyncService clientSyncService = new ClientSyncServiceImpl();
    ...
    public void findById_clientFound() {
        when(clientManager.get("aa"))
                .thenReturn(
                        clientSet.stream().filter(c -> c.getId().equals("aa")).findFirst().get());
