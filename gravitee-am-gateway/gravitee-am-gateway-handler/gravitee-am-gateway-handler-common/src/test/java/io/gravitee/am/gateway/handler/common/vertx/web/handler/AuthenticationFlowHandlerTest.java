...
public class AuthenticationFlowHandlerTest extends RxWebTestBase {
    ...
    @Mock private FactorManager factorManager;
    ...
    public void setUp() throws Exception {
        ...
        steps.add(
                new MFAEnrollStep(
                        RedirectHandler.create("/mfa/enroll"), ruleEngine, factorManager));
        steps.add(
                new MFAChallengeStep(
                        RedirectHandler.create("/mfa/challenge"), ruleEngine, factorManager));
        steps.add(
                new MFARecoveryCodeStep(
                        RedirectHandler.create("/mfa/recovery_code"), ruleEngine, factorManager));
        AuthenticationFlowChainHandler authenticationFlowChainHandler =
                new AuthenticationFlowChainHandler(steps);
                ...
                .handler(
                        rc -> {
                            ...
                            mfaSettings.setAdaptiveAuthenticationRule(
                                    "{context.attributes['geoip']['country_iso_code'] == 'FR'");
                            rc.put(
                                    ConstantKeys.GEOIP_KEY,
                                    new JsonObject().put("country_iso_code", "US").getMap());
                            // set user
                            io.gravitee.am.model.User endUser = new io.gravitee.am.model.User();
                            rc.getDelegate().setUser(new User(endUser));
                            rc.next();
        ...
        router.route()
                ...
                .handler(
                        rc -> {
                            ...
                            rc.put(
                                    ConstantKeys.GEOIP_KEY,
                                    new JsonObject().put("country_iso_code", "FR").getMap());
                            // set user
                            io.gravitee.am.model.User endUser = new io.gravitee.am.model.User();
                            rc.getDelegate().setUser(new User(endUser));
                            rc.next();
    ...
    @Test
    public void shouldRedirectToMFAChallengePage_device_remembered_but_no_matching_active_factor()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
    ...
    @Test
    public void
            shouldRedirectToMFAChallengePage_device_remembered_but_active_factor_is_recovery_code()
                    throws Exception {
        router.route()
                .order(-1)
                .handler(
    ...
    @Test
    public void shouldRedirectToChallenge_user_device_known_and_step_up_active_strongly_auth()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
    ...
    @Test
    public void shouldContinue_user_device_known_and_step_up_active_not_strongly_auth()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
    ...
    @Test
    public void shouldRedirectToMFAChallengePage_adaptiveMFA_with_step_up_true_strong_auth_true()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
                        rc -> {
                            ...
                            rc.session().put(DEVICE_ALREADY_EXISTS_KEY, true);
                            mfaSettings.setStepUpAuthenticationRule(
                                    "{#request.params['scope'][0].contains('write')}");
                            mfaSettings.setAdaptiveAuthenticationRule(
                                    "{#context.attributes['geoip']['country_iso_code'] == 'FR'}");
                            rc.put(
                                    ConstantKeys.GEOIP_KEY,
    ...
    @Test
    public void shouldContinue_adaptiveMFA_with_step_up_false_strong_auth_true_device_known()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
                            ...
                            rc.session()
                                    .put(
                                            ...
                                            new AssessmentMessageResult()
                                                    .setDevices(
                                                            new AssessmentResult<Double>()
                                                                    .setAssessment(Assessment.SAFE)
                                                                    .setResult(0D)));
                            rc.session().put(DEVICE_ALREADY_EXISTS_KEY, false);
                            client.setMfaSettings(mfaSettings);
                            // set user
    ...
    @Test
    public void shouldRedirectToMFAChallengePage_rememberDevice_with_risk_assessment_but_no_device()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
    ...
    @Test
    public void shouldRedirectToMFAChallengePage_adaptiveMFA_3_factor_is_pending()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
    ...
    @Test
    public void shouldRedirectToMFAChallengePage_adaptiveMFA_3_alternate_factor_id()
            throws Exception {
        router.route()
                .order(-1)
                .handler(
        ...
        router.route()
                ...
                .handler(
                        rc -> {
                            ...
                            factorRecovery.setSecurity(
                                    new EnrolledFactorSecurity(
                                            FactorSecurityType.RECOVERY_CODE, null));
