...
public class PolicyChainHandlerTest {
    ...
    @Mock private FlowManager flowManager;

    @Mock private PolicyChainProcessorFactory policyChainProcessorFactory;
    ...
    @Mock private io.vertx.core.http.HttpServerRequest delegateRequest;

    @Mock private io.vertx.core.http.HttpServerResponse delegateResponse;

    @Mock private Policy policy;

    @Mock private Processor<ExecutionContext> processor;

    @Test
    public void shouldNotInvoke_noPolicies() {
        when(flowManager.findByExtensionPoint(
                        eq(ExtensionPoint.PRE_CONSENT), eq(null), any(ExecutionPredicate.class)))
        ...
        PolicyChainHandlerImpl policyChainHandler =
                new PolicyChainHandlerImpl(
                        flowManager,
    ...
    public void shouldInvoke_onePolicy() {
        when(flowManager.findByExtensionPoint(
                        eq(ExtensionPoint.PRE_CONSENT), eq(null), any(ExecutionPredicate.class)))
                .thenReturn(Single.just(Collections.singletonList(policy)));
        when(delegateRequest.method()).thenReturn(HttpMethod.GET);
        when(request.getDelegate()).thenReturn(delegateRequest);
        when(request.getDelegate().response()).thenReturn(delegateResponse);
        when(routingContext.request()).thenReturn(request);
        ...
        when(policyChainProcessorFactory.create(
    ...
    public void shouldInvoke_manyPolicies() {
        when(flowManager.findByExtensionPoint(
                        eq(ExtensionPoint.PRE_CONSENT), eq(null), any(ExecutionPredicate.class)))
                .thenReturn(Single.just(Arrays.asList(policy, policy)));
        when(delegateRequest.method()).thenReturn(HttpMethod.GET);
        when(request.getDelegate()).thenReturn(delegateRequest);
        when(request.getDelegate().response()).thenReturn(delegateResponse);
        when(routingContext.request()).thenReturn(request);
        ...
        when(processor.handler(any())).thenReturn(processor);
        when(processor.errorHandler(any())).thenReturn(processor);
        when(policyChainProcessorFactory.create(Arrays.asList(policy, policy), executionContext))
                .thenReturn(processor);
