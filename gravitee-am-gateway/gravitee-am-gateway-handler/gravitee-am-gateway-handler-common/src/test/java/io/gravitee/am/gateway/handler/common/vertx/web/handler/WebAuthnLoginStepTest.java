...
public class WebAuthnLoginStepTest {
    ...
    private static final Handler<RoutingContext> redirectHandler =
            RedirectHandler.create("/webauthn/login");

    @Mock private RoutingContext routingContext;

    @Mock private HttpServerRequest httpServerRequest;

    @Mock private WebAuthnCookieService webAuthnCookieService;
    ...
    public void mustDoNext_failedToDecodeCookie() {
        ...
        when(client.getLoginSettings()).thenReturn(loginSettings);
        when(routingContext.get(CLIENT_CONTEXT_KEY)).thenReturn(client);
        when(httpServerRequest.getCookie("cookieName"))
                .thenReturn(Cookie.cookie("cookieName", "cookieValue"));
        when(webAuthnCookieService.verifyRememberDeviceCookieValue("cookieValue"))
                .thenReturn(Completable.error(new SignatureException("Invalid payload")));
