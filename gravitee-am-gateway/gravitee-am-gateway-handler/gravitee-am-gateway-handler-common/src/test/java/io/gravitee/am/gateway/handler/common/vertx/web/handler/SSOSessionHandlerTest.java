...
public class SSOSessionHandlerTest extends RxWebTestBase {
    ...
    public void setUp() throws Exception {
        super.setUp();
        when(authenticationFlowContextService.clearContext_migrated(any())).thenReturn(Mono.empty());
        when(jwtService.encode_migrated(any(JWT.class), (CertificateProvider) eq(null))).thenReturn(Mono.just("token"));
    ...
    public void shouldInvoke_sameClient() throws Exception {
        ...
        when(clientSyncService.findById_migrated(anyString())).thenReturn(Mono.empty());
        when(clientSyncService.findByClientId_migrated(client.getClientId())).thenReturn(Mono.just(client));
    ...
    public void shouldInvoke_differentClient_sameIdp() throws Exception {
        ...
        when(clientSyncService.findById_migrated(anyString())).thenReturn(Mono.empty()).thenReturn(Mono.empty());
        when(clientSyncService.findByClientId_migrated(anyString())).thenAnswer(
                invocation -> {
                    ...
                    if (argument.equals("test-client")) {
                        ...
                        return Mono.just(requestedClient);
                    }
