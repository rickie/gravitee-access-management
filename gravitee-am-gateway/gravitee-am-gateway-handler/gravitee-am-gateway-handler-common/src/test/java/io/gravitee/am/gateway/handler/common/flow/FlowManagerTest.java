...
public class FlowManagerTest {
    ...
    @Mock private PolicyPluginManager policyPluginManager;

    @Mock private EventManager eventManager;

    @InjectMocks private FlowManagerImpl flowManager = new FlowManagerImpl();

    @Test
    public void shouldNoFindByExtensionPoint_noFlows() {
        when(domain.getId()).thenReturn("domain-id");
        when(flowService.findAll(ReferenceType.DOMAIN, domain.getId()))
                .thenReturn(Flowable.empty());
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs =
                flowManager.findByExtensionPoint(ExtensionPoint.PRE_CONSENT, null, null).test();
    ...
    public void shouldNotFindByExtensionPoint_onePolicy_differentStep() {
        ...
        when(domain.getId()).thenReturn("domain-id");
        when(policyPluginManager.create(
    ...
    public void shouldNotFindByExtensionPoint_applicationPolicy_wrongClient() {
        ...
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs =
                flowManager.findByExtensionPoint(ExtensionPoint.PRE_CONSENT, client, null).test();
        obs.awaitTerminalEvent();
        obs.assertValue(
                policies -> {
                    Assert.assertTrue(policies.isEmpty());
    ...
    public void shouldFindByExtensionPoint_twoFlows_inherit_false() {
        ...
        when(domain.getId()).thenReturn("domain-id");
        when(policyPluginManager.create(
                        domainStep.getPolicy(),
                        domainStep.getCondition(),
                        domainStep.getConfiguration()))
                .thenReturn(domainPolicy);
        when(policyPluginManager.create(
                        appStep.getPolicy(), appStep.getCondition(), appStep.getConfiguration()))
                .thenReturn(appPolicy);
        when(flowService.findAll(ReferenceType.DOMAIN, domain.getId()))
                .thenReturn(Flowable.just(domainFlow, appFlow));
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs =
                flowManager.findByExtensionPoint(ExtensionPoint.PRE_CONSENT, client, null).test();
        obs.awaitTerminalEvent();
        ...
        TestObserver<List<Policy>> obs =
                flowManager
                        .findByExtensionPoint(
                                ExtensionPoint.PRE_CONSENT, client, ExecutionPredicate.alwaysTrue())
                        .test();
        obs.awaitTerminalEvent();
        obs.assertValue(
                policies -> {
