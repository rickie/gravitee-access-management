...
public class FlowManagerTest {
    ...
    public void shouldNoFindByExtensionPoint_noFlows() {
        ...
        when(flowService.findAll_migrated(ReferenceType.DOMAIN, domain.getId())).thenReturn(Flux.empty());
    ...
    public void shouldNotFindByExtensionPoint_flowDisabled() {
        ...
        when(domain.getId()).thenReturn("domain-id");
        when(flowService.findAll_migrated(ReferenceType.DOMAIN, domain.getId())).thenReturn(Flux.just(flow));
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs = RxJava2Adapter.monoToSingle(flowManager.findByExtensionPoint_migrated(ExtensionPoint.PRE_CONSENT, null, null)).test();
        obs.awaitTerminalEvent();
        obs.assertValue(policies -> {
    ...
    public void shouldFindByExtensionPoint_twoFlows_inherit_false() {
        ...
        when(flowService.findAll_migrated(ReferenceType.DOMAIN, domain.getId())).thenReturn(Flux.just(domainFlow, appFlow));
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs = RxJava2Adapter.monoToSingle(flowManager.findByExtensionPoint_migrated(ExtensionPoint.PRE_CONSENT, client, null)).test();
        obs.awaitTerminalEvent();
        obs.assertValue(policies -> {
