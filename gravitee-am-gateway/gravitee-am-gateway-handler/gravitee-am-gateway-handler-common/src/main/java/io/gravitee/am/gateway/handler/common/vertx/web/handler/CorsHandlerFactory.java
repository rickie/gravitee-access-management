...
public class CorsHandlerFactory implements FactoryBean<CorsHandler> {
        ...
        return CorsHandler.newInstance(
                        io.vertx.ext.web.handler.CorsHandler.create(
                                        environment.getProperty(
                                                "http.cors.allow-origin", String.class, "*"))
                                .allowedHeaders(
                                ...
                                .allowedMethods(
                                        getHttpMethodPropertiesAsList(
                                                "http.cors.allow-methods",
                                                "GET, POST, PUT, PATCH, DELETE"))
                                .maxAgeSeconds(
                                        environment.getProperty(
                                                "http.cors.max-age", Integer.class, 86400)))
                .allowCredentials(
                        environment.getProperty(
                                "http.cors.allow-credentials", Boolean.class, false));
    }
    ...
    private Set<HttpMethod> getHttpMethodPropertiesAsList(String propertyKey, String defaultValue) {
        ...
        return asList(property.replaceAll("\\s+", "").split(",")).stream()
