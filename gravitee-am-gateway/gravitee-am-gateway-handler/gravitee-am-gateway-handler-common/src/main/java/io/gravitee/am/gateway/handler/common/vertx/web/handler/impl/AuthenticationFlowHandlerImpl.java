...
public class AuthenticationFlowHandlerImpl implements AuthenticationFlowHandler {
    ...
    public Handler<RoutingContext> create() {
        ...
        steps.add(
                new SPNEGOStep(
                        RedirectHandler.create("/login/SSO/SPNEGO"), identityProviderManager));
        steps.add(
                new FormIdentifierFirstLoginStep(
                        RedirectHandler.create("/login/identifier"), domain));
        steps.add(
                new WebAuthnLoginStep(
                        RedirectHandler.create("/webauthn/login"), domain, webAuthnCookieService));
        steps.add(new FormLoginStep(RedirectHandler.create("/login")));
        steps.add(
                new WebAuthnRegisterStep(
                        domain,
                        RedirectHandler.create("/webauthn/register"),
                        factorManager,
                        credentialService));
        steps.add(
                new MFAEnrollStep(
