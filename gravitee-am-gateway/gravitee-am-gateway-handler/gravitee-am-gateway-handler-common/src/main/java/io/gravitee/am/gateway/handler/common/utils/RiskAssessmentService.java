...
public class RiskAssessmentService {
    ...
    public Maybe<AssessmentMessage> computeRiskAssessment(
        ...
        if (riskAssessmentSettings.isEnabled()) {
            var assessmentMessage =
                    Single.just(
                            new AssessmentMessage()
                                    .setSettings(riskAssessmentSettings)
            ...
            return assessmentMessage
                    ...
                    .doOnSuccess(
                            processedMessage ->
                                    consumeRiskAssessmentResult(processedMessage, resultConsumer))
                    .doOnError(
                            throwable ->
                                    logger.error(
    ...
    private Function<AssessmentMessage, Single<AssessmentMessage>> buildDeviceAssessmentMessage(
            AuthenticationDetails authDetails, Domain domain, User user) {
        return assessmentMessage -> {
            var deviceAssessment =
                    ofNullable(riskAssessmentSettings.getDeviceAssessment())
                            .orElse(new AssessmentSettings());
            if (deviceAssessment.isEnabled()) {
                logger.debug("Decorating assessment with devices");
                return deviceService
                        .findByDomainAndUser(domain.getId(), user.getId())
    ...
    private Function<AssessmentMessage, Single<AssessmentMessage>> buildIpReputationMessage(
        ...
        return assessmentMessage -> {
            var ipReputationAssessment =
                    ofNullable(riskAssessmentSettings.getIpReputationAssessment())
                            .orElse(new AssessmentSettings());
    ...
    private Function<AssessmentMessage, Single<AssessmentMessage>> buildGeoVelocityMessage(
        ...
        return assessmentMessage -> {
            var geoVelocityAssessment =
                    ofNullable(riskAssessmentSettings.getGeoVelocityAssessment())
                            .orElse(new AssessmentSettings());
            if (geoVelocityAssessment.isEnabled()) {
                return userActivityService
                        .findByDomainAndTypeAndUserAndLimit(
                                domain.getId(), UserActivity.Type.LOGIN, user.getId(), 2)
                        .toList()
                        .flatMap(
                                activityList -> {
                                    assessmentMessage
    ...
    private void consumeRiskAssessmentResult(
            AssessmentMessage message, Consumer<AssessmentMessageResult> resultConsumer)
