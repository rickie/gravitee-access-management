 ...
 */
public class FactorManagerImpl extends AbstractService
        implements FactorManager, InitializingBean, EventListener<FactorEvent, Payload> {
    ...
    @Autowired private FactorPluginManager factorPluginManager;
    ...
    public void afterPropertiesSet() {
        ...
        factorService
                ...
                .subscribe(
                        ...
                        error ->
                                logger.error(
                                        "Unable to initialize factors for domain {}",
                                        domain.getName(),
                                        error));
    }
    ...
    private void updateFactor(String factorId, FactorEvent factorEvent) {
        ...
        factorService
                ...
                .subscribe(
                        ...
                        error ->
                                logger.error(
                                        "Unable to load factor for domain {}",
                                        domain.getName(),
                                        error),
    ...
    private void removeFactor(String factorId) {
        logger.info(
                "Domain {} has received form event, remove factor {}", domain.getName(), factorId);
    ...
    private void updateFactor(Factor factor) {
        try {
            if (needDeployment(factor)) {
                var factorProviderConfig =
                        new ProviderConfiguration(factor.getType(), factor.getConfiguration());
                ...
                logger.info(
                        "Factor {} already loaded for domain {}",
                        factor.getName(),
                        domain.getName());
            }
        } catch (Exception ex) {
    ...
    /**
     * @param factor
     * @return true if the Factor has never been deployed or if the deployed version is not up to
     *     date
     */
