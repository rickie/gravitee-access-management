...
public class UserAuthenticationManagerImpl implements UserAuthenticationManager {
    ...
    @Override
    public Single<User> authenticate(
            Client client, Authentication authentication, boolean preAuthenticated) {
        ...
        return Observable.fromIterable(applicationIdentityProviders)
                .filter(
                        appIdp ->
                                selectionRuleMatches(
                                        appIdp.getSelectionRule(), authentication.copy()))
                .switchIfEmpty(
                        Observable.error(() -> getInternalAuthenticationServiceException(client)))
                .concatMapMaybe(
                        appIdp ->
                                authenticate0(
                                        client,
                ...
                .takeUntil(
                        userAuthentication ->
                                userAuthentication.getUser() != null
                                        || userAuthentication.getLastException()
                                                instanceof AccountLockedException)
                .lastOrError()
                .flatMap(
                        userAuthentication -> {
                            io.gravitee.am.identityprovider.api.User user =
                            ...
                            if (user == null) {
                                ...
                                if (lastException != null) {
                                    if (lastException instanceof BadCredentialsException) {
                                        ...
                                        // if an IdP return UsernameNotFoundException, convert it as
                                        // BadCredentials in order to avoid helping attackers
                                        return Single.error(
                                                new BadCredentialsException(
                                                        "The credentials you entered are invalid",
                                                        lastException));
                                        ...
                                        return Single.error(lastException);
                                            ...
                                            instanceof NegotiateContinueException) {
                                        return Single.error(lastException);
                                    } else {
                                        logger.error(
                                                "An error occurs during user authentication",
                                                lastException);
                                        return Single.error(
                                    ...
                                    // if an IdP return null user, throw BadCredentials in order to
                                    // avoid helping attackers
                                    return Single.error(
                                            new BadCredentialsException(
                                                    "The credentials you entered are invalid"));
                                }
                        ...
                        user -> {
                            ...
                            eventManager.publishEvent(
                                    ...
                                    new AuthenticationDetails(
                                            authentication, domain, client, user));
                        })
                .doOnError(
                        throwable -> {
                            ...
                            eventManager.publishEvent(
                                    ...
                                    new AuthenticationDetails(
                                            authentication, domain, client, throwable));
                        });
    }
    ...
    private Single<User> checkAccountPasswordExpiry(Client client, User connectedUser) {
        if (passwordService.checkAccountPasswordExpiry(connectedUser, client, domain)) {
            return Single.error(
                    new AccountPasswordExpiredException("Account's password is expired "));
        }
        return Single.just(connectedUser);
    }

    private List<ApplicationIdentityProvider> getApplicationIdentityProviders(
        ...
        return client.getIdentityProviders().stream()
                .filter(
                        appIdp -> {
    ...
    public Single<User> connect(
            io.gravitee.am.identityprovider.api.User user, boolean afterAuthentication) {
        return userAuthenticationService.connect(user, afterAuthentication);
    }

    @Override
    public Single<User> connectPreAuthenticatedUser(
            Client client, String subject, Authentication authentication) {
        return userAuthenticationService
                .connectPreAuthenticatedUser(subject)
                .doOnSuccess(
                        user ->
                                eventManager.publishEvent(
                                        AuthenticationEvent.SUCCESS,
    ...
    private Maybe<UserAuthentication> authenticate0(
            Client client,
    ...
    private Maybe<UserAuthentication> loadUserByUsername0(
            Client client,
            Authentication authentication,
            String authProvider,
            boolean preAuthenticated) {
                        ...
                        Maybe.error(
                                new BadCredentialsException(
                                        "Unable to load authentication provider "
                                                + authProvider
                                                + ", an error occurred during the initialization stage")))
                .flatMap(
    ...
    private InternalAuthenticationServiceException getInternalAuthenticationServiceException(
            Client client) {
        return new InternalAuthenticationServiceException(
                "No identity provider found for client : " + client.getClientId());
    }
    ...
    private Completable postAuthentication(
        ...
        return loginAttemptService
                ...
                .flatMapCompletable(
                        optLoginAttempt -> {
                            if (optLoginAttempt.isPresent()
                                    && optLoginAttempt
                                            .get()
                                            .isAccountLocked(
                                                    accountSettings.getMaxLoginAttempts())) {
                                Map<String, String> details = new HashMap<>();
                                details.put("attempt_id", optLoginAttempt.get().getId());
                                return Completable.error(
                                        new AccountLockedException(
                                                "User " + username + " is locked", details));
                            }
                            ...
                            if (userAuthentication.getLastException()
                                    instanceof BadCredentialsException) {
                                // do not execute login attempt feature for non existing users
                                // normally the IdP should respond with Maybe.empty() or
                                // UsernameNotFoundException
                                // but we can't control custom IdP that's why we have to check user
                                // existence
                                return userService
                                        .findByDomainAndUsernameAndSource(
                                                criteria.domain(),
                                                criteria.username(),
                                                criteria.identityProvider())
                                        .flatMapCompletable(
                                                user ->
                                                        loginAttemptService
                                                                .loginFailed(
                                                                        criteria, accountSettings)
                                                                .flatMapCompletable(
                                                                        loginAttempt -> {
                                                                            if (loginAttempt
                                                                                    .isAccountLocked(
    ...
    private class UserAuthentication {
        ...
        public UserAuthentication(
                io.gravitee.am.identityprovider.api.User user, Throwable lastException) {
            this.user = user;
