...
public class AlertEventProcessor extends AbstractService {
    ...
    @Autowired private AlertEventProducer eventProducer;
    ...
    @Autowired private RiskAssessmentService riskAssessmentHelper;

    private final EventListener<AuthenticationEvent, AuthenticationDetails>
            authenticationEventListener = this::onAuthenticationEvent;
    ...
    protected void doStart() throws Exception {
        ...
        try {
            ...
            logger.warn(
                    "The domain [{}] seems not attached to any environment or organization. Alert events may not be accurate.",
                    domain.getName());
        }

        logger.info("Register event listener for all events for domain {}", domain.getName());
        eventManager.subscribeForEvents(
                authenticationEventListener, AuthenticationEvent.class, domain.getId());
    }
    ...
    private void onAuthenticationEvent(
        ...
        DefaultEvent.Builder eventBuilder =
                io.gravitee.alert.api.event.Event.at(System.currentTimeMillis())
                        ...
                        .property(
                                PROPERTY_USER, authenticationDetails.getPrincipal().getPrincipal())
        ...
        if (SUCCESS.equals(eventItem.type())) {
            this.riskAssessmentHelper
                    .computeRiskAssessment(
                            authenticationDetails, sendAssessmentMessageResult(eventBuilder))
                    .switchIfEmpty(
                            Maybe.defer(
                                    () -> {
    ...
    private Consumer<AssessmentMessageResult> sendAssessmentMessageResult(
            DefaultEvent.Builder eventBuilder) {
        return result -> {
            Map.of(
                            PROPERTY_RISK_ASSESSMENT + "." + PROPERTY_UNKNOWN_DEVICES,
                                    result.getDevices().getAssessment(),
                            PROPERTY_RISK_ASSESSMENT + "." + PROPERTY_IP_REPUTATION,
                                    result.getIpReputation().getAssessment(),
                            PROPERTY_RISK_ASSESSMENT + "." + PROPERTY_GEO_VELOCITY,
                                    result.getGeoVelocity().getAssessment())
                    .forEach(
                            (property, assessment) ->
                                    eventBuilder.property(property, assessment.name()));
            sendEvent(eventBuilder.build());
        };
    }
