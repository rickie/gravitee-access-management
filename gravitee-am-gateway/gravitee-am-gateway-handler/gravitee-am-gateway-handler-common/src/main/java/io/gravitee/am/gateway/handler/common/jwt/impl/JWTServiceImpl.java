...
public class JWTServiceImpl implements JWTService {
    ...
    public Single<JWT> decodeAndVerify(String jwt, CertificateProvider certificateProvider) {
        return decode(certificateProvider, jwt).map(JWT::new);
    }
        ...
        return Single.create(
                emitter -> {
                    try {
                        String json =
                                new String(
                                        Base64.getDecoder().decode(jwt.split("\\.")[1]), "UTF-8");
                        emitter.onSuccess(objectMapper.readValue(json, JWT.class));
                    } catch (Exception ex) {
                        logger.debug("Failed to decode JWT", ex);
                        emitter.onError(
                                new InvalidTokenException("The access token is invalid", ex));
                    }
                });
    }
        ...
        return Single.create(
                emitter -> {
                    try {
                        ...
                        emitter.onError(
                                new InvalidTokenException("The JWT token couldn't be signed", ex));
                    }
                });
    }

    private Single<Map<String, Object>> decode(
            CertificateProvider certificateProvider, String payload) {
        return Single.create(
                emitter -> {
                    try {
                        Map<String, Object> decodedPayload =
                                certificateProvider.getJwtParser().parse(payload);
