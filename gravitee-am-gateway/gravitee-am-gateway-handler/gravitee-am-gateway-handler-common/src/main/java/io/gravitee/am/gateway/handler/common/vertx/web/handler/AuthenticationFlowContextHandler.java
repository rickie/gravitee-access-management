...
public class AuthenticationFlowContextHandler implements Handler<RoutingContext> {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationFlowContextHandler.class);

    private AuthenticationFlowContextService authenticationFlowContextService;

    private final boolean exitOnError;

    public AuthenticationFlowContextHandler(
            AuthenticationFlowContextService authenticationFlowContextService, Environment env) {
    ...
    public void handle(RoutingContext context) {
        ...
        if (session != null && !session.isDestroyed()) {
            ...
            authenticationFlowContextService
                    .loadContext(transactionId, version)
                    .subscribe(
                            ctx -> {
                                // store the AuthenticationFlowContext in order to provide all
                                // related information about this context
                                context.put(ConstantKeys.AUTH_FLOW_CONTEXT_KEY, ctx);
                                // store only the AuthenticationFlowContext.data attributes in order
                                // to simplify EL templating
                                // and provide an up to date set of data if the enrichAuthFlow
                                // Policy is used multiple time in a step
                                // {#context.attributes['authFlow']['entry']}
                                context.put(
                                        ConstantKeys.AUTH_FLOW_CONTEXT_ATTRIBUTES_KEY,
                                        ctx.getData());
                                context.next();
                            },
                            error -> {
