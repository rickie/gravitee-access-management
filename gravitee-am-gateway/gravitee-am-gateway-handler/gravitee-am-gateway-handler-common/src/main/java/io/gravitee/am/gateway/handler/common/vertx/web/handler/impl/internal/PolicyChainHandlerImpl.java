...
public class PolicyChainHandlerImpl implements Handler<RoutingContext> {
        ...
        prepareContext(
                ...
                contextHandler -> {
                    if (contextHandler.failed()) {
                        logger.error(
                                "An error occurs while preparing execution context",
                                contextHandler.cause());
                    ...
                    resolve(
                            ...
                            handler -> {
                                if (handler.failed()) {
                                    logger.error(
                                            "An error occurs while resolving policies",
                                            handler.cause());
                                ...
                                executePolicyChain(
                                        ...
                                        policyChainHandler -> {
                                            if (policyChainHandler.failed()) {
                                                logger.debug(
                                                        "An error occurs while executing the policy chain",
                                                        policyChainHandler.cause());
                                                context.fail(policyChainHandler.cause());
                                                return;
                                            }
                                            // update context attributes
                                            ExecutionContext processedExecutionContext =
                                                    policyChainHandler.result();
                                            processedExecutionContext
                                                    .getAttributes()
                                                    .forEach(
                                                            (k, v) -> {
                                                                if (ConstantKeys
    ...
    private void resolve(
            ExecutionContext executionContext, Handler<AsyncResult<List<Policy>>> handler) {
        flowManager
                .findByExtensionPoint(
                        extensionPoint,
                        (Client) executionContext.getAttribute(ConstantKeys.CLIENT_CONTEXT_KEY),
                        ExecutionPredicate.from(executionContext))
                .subscribe(
    ...
    private void prepareContext(
            RoutingContext routingContext, Handler<AsyncResult<ExecutionContext>> handler) {
        try {
            ...
            ExecutionContext simpleExecutionContext =
                    new SimpleExecutionContext(serverRequest, serverResponse);
    ...
    private void executePolicyChain(
        ...
        policyChainProcessorFactory
                ...
                .handler(
                        executionContext1 ->
                                handler.handle(Future.succeededFuture(executionContext1)))
                .errorHandler(
                        processorFailure ->
                                handler.handle(
                                        Future.failedFuture(
