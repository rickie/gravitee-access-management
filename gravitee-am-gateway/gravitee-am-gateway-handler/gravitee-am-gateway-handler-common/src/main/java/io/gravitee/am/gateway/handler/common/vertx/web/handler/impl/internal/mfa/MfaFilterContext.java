...
public class MfaFilterContext {
    ...
    public MfaFilterContext(
            RoutingContext routingContext, Client client, FactorManager factorManager) {
    ...
    private boolean isEnrollSkipped() {
        ...
        if (FALSE.equals(forceEnrollment) && nonNull(endUser.getMfaEnrollmentSkippedAt())) {
            Date now = new Date();
            long skipTime =
                    ofNullable(enrollmentSettings.getSkipTimeSeconds())
                                    .orElse(DEFAULT_ENROLLMENT_SKIP_TIME_SECONDS)
                            * 1000L;
