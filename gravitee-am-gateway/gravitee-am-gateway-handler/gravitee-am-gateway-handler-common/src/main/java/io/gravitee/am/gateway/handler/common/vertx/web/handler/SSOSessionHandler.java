...
public class SSOSessionHandler implements Handler<RoutingContext> {
    ...
    private Single<Optional<Client>> getClient(String clientId) {
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(clientSyncService.findById(clientId)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.defer(() -> clientSyncService.findByClientId(clientId))))).map(RxJavaReactorMigrationUtil.toJdkFunction(Optional::ofNullable)).defaultIfEmpty(Optional.empty()).single());
    }
}
