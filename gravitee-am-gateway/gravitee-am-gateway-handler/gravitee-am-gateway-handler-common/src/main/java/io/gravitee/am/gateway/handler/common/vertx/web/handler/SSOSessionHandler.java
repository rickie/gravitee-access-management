...
/**
 * SSO Session Handler to check if the user stored in the HTTP session is still "valid" upon the
 * incoming request
 *
 * <p>- a user is invalid if he is disabled - a user is invalid if he is not on a shared identity
 * provider
 *
...
public class SSOSessionHandler implements Handler<RoutingContext> {
                ...
                h -> {
                    if (h.failed()) {
                        ...
                        if (cause instanceof AccountStatusException) {
                            ...
                            // clear AuthenticationFlowContext. data of this context have a TTL so
                            // we can fire and forget in case on error.
                            ...
                            context.fail(
                                    new HttpException(
                                            403, "Invalid request for the current SSO context"));
                            return;
                        }
                    }
                    context.next();
    ...
    private void checkAccountStatus(
        ...
        // if user has reset its password, check the last login date to make sure that the current
        ...
        // if user has been sign out in a REST manner way, check the last login date to make sure
        // that the current session is not compromised
        ...
        Single.zip(
                        ...
                        (optRequestedClient, optUserClient) -> {
                            ...
                            throw new InvalidRequestException(
                                    "User is not on a shared identity provider");
                        })
                .subscribe(
