...
public class AdaptiveMfaFilter extends MfaContextHolder implements Supplier<Boolean> {
    ...
    public AdaptiveMfaFilter(MfaFilterContext context, RuleEngine ruleEngine) {
        super(context);
    ...
    public Boolean get() {
        ...
        if (context.isMfaSkipped()
                && !context.hasEndUserAlreadyEnrolled()
                && !context.userHasMatchingFactors()) {
            return false;
        }
        ...
        if (context.userHasMatchingActivatedFactors()
            ...
            context.setAmfaRuleTrue(
                    ruleEngine.evaluate(
                            context.getAmfaRule(),
                            context.getEvaluableContext(),
                            Boolean.class,
                            false));
        }
