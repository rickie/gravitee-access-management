...
/**
 * See <a href="https://tools.ietf.org/html/rfc6750">The OAuth 2.0 Authorization Framework: Bearer
 * Token Usage</a>
 *
 * <p>Default implementation with the Authorization Request Header Field
 *
 * <p>When sending the access token in the "Authorization" request header field defined by HTTP/1.1
 * [RFC2617], the client uses the "Bearer" authentication scheme to transmit the access token.
 *
 * <p>See <a href="https://tools.ietf.org/html/rfc6750#section-2.1>2.1. Authorization Request Header
 * Field</a>
 *
 * <p>When a request fails, the resource server responds using the appropriate HTTP status code
 * (typically, 400, 401, 403, or 405) and includes one of the following error codes in the response:
 *
 * <p>invalid_request The request is missing a required parameter, includes an unsupported parameter
 * or parameter value, repeats the same parameter, uses more than one method for including an access
 * token, or is otherwise malformed. The resource server SHOULD respond with the HTTP 400 (Bad
 * Request) status code.
 *
 * <p>invalid_token The access token provided is expired, revoked, malformed, or invalid for other
 * reasons. The resource SHOULD respond with the HTTP 401 (Unauthorized) status code. The client MAY
 * request a new access token and retry the protected resource request.
 *
 * <p>insufficient_scope The request requires higher privileges than provided by the access token.
 * The resource server SHOULD respond with the HTTP 403 (Forbidden) status code and MAY include the
 * "scope" attribute with the scope necessary to access the protected resource.
 *
 * <p>If the request lacks any authentication information (e.g., the client was unaware that
 ...
 * <p>For example:
 *
 * <p>HTTP/1.1 401 Unauthorized WWW-Authenticate: Bearer realm="example"
 *
 * <p>See <a href="https://tools.ietf.org/html/rfc6750#section-3.1">3.1. Error Codes</a>
 *
