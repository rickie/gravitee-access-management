...
public class UserAuthenticationServiceImpl implements UserAuthenticationService {
    ...
    public Single<User> connect(io.gravitee.am.identityprovider.api.User principal, boolean afterAuthentication) {
        ...
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(saveOrUpdate(principal, afterAuthentication)).flatMap(user->RxJava2Adapter.completableToMono(checkAccountStatus(user)).then(RxJava2Adapter.singleToMono(Single.wrap(Single.defer(()->userService.enhance(user)))))));
    }
    ...
    public Maybe<User> loadPreAuthenticatedUser(String subject, Request request) {
        ...
        return RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(userService
                .findById(subject)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.error(new UserNotFoundException(subject))))).flatMap(z->RxJava2Adapter.maybeToMono(identityProviderManager.get(z.getSource()).flatMap((io.gravitee.am.identityprovider.api.AuthenticationProvider authenticationProvider)->{
...
})).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.defer(()->userService.enhance(z).toMaybe()))))));
    }
