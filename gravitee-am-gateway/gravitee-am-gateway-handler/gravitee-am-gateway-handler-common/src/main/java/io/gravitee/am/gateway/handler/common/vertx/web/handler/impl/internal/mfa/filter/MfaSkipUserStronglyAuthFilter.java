...
public class MfaSkipUserStronglyAuthFilter extends MfaContextHolder implements Supplier<Boolean> {
    ...
    public Boolean get() {
        ...
        // We need to check whether the AMFA, Device and Step Up rule is false since we don't know
        // of other MFA return False
        else if (
        ...
        !mfaSkipped && context.isAmfaActive() && !context.isAmfaRuleTrue()
                ...
                !context.isDeviceRiskAssessmentEnabled()
                        && !mfaSkipped
                        && context.getRememberDeviceSettings().isActive()
                        && !context.deviceAlreadyExists()
                ||
                // Or that Step up authentication is active and user is strongly auth or mfa is
                // skipped
                context.isStepUpActive()
                        && context.isStepUpRuleTrue()
                        && (userStronglyAuth || mfaSkipped)) {
            return false;
        } else if (
        // We need to make sure we come from a place where the user is not trying to challenge a new
        // device
        ...
        context.userHasMatchingActivatedFactors()
                ...
                && context.isAmfaActive()
                && context.isAmfaRuleTrue()) {
            return true;
        }
        // We check then if StepUp is not active and of user is strongly auth or mfa is skipped to
        // skip MFA
