...
/**
 * WebAuthn cookie service used mainly to determine if the device is already enrolled or not, and
 * prompt the user for enrollment by using HTTP cookie
 *
...
public class WebAuthnCookieService implements InitializingBean {
    ...
    public Completable verifyRememberDeviceCookieValue(String cookieValue) {
        return jwtService
                .decodeAndVerify(cookieValue, certificateProvider)
                .ignoreElement()
                .onErrorResumeNext(
                        throwable -> {
                            LOGGER.error(
                                    "An error has occurred when parsing WebAuthn cookie {}",
                                    cookieValue,
                                    throwable);
