 ...
 */
public class AuditReporterManagerImpl extends AbstractService
    ...
    private final ConcurrentMap<String, io.gravitee.am.reporter.api.provider.Reporter>
            reporterPlugins = new ConcurrentHashMap<>();
        ...
        deployment.subscribe(
                id -> {
                    ...
                    reporterRepository
                            ...
                            .subscribe(
                                    tupleReportersContext -> {
                                        if (!tupleReportersContext.getT1().isEmpty()) {
                                            tupleReportersContext
                                                    .getT1()
                                                    .forEach(
                                                            reporter -> {
                                                                startReporterProvider(
                                                                        reporter,
                                                                        tupleReportersContext
                                                                                .getT2());
                                                            });
                                            logger.info(
                                                    "Reporters loaded for domain {}",
                                                    domain.getName());
                                        } else {
    ...
    protected void doStop() throws Exception {
        ...
        if (deploymentId != null) {
            vertx.undeploy(
                    deploymentId,
                    event -> {
                        for (io.gravitee.am.reporter.api.provider.Reporter reporter :
                                reporterPlugins.values()) {
                            try {
        ...
        reporterRepository
                ...
                .subscribe(
                        tupleReporterContext -> {
                            updateReporterProvider(
                                    tupleReporterContext.getT1(), tupleReporterContext.getT2());
                            logger.info(
                                    "Reporter {} {}d for domain {}",
                                    reporterId,
                                    eventType,
                                    domain.getName());
                        },
    ...
    private void removeReporter(String reporterId) {
        logger.info(
                "Domain {} has received reporter event, delete reporter {}",
                domain.getName(),
                reporterId);
    ...
    private void startReporterProvider(Reporter reporter, GraviteeContext context) {
        if (reporter.isEnabled()) {
            if (needDeployment(reporter)) {
                logger.info(
                        "\tInitializing reporter: {} [{}]", reporter.getName(), reporter.getType());
                var providerConfiguration = new ReporterProviderConfiguration(reporter, context);
                io.gravitee.am.reporter.api.provider.Reporter reporterProvider =
                        reporterPluginManager.create(providerConfiguration);

                if (reporterProvider != null) {
                    try {
                        logger.info("Starting reporter: {}", reporter.getName());
                        io.gravitee.am.reporter.api.provider.Reporter eventBusReporter =
                                new EventBusReporterWrapper(
                                        vertx, domain.getId(), reporterProvider);
                ...
                logger.info(
                        "Reporter {} already up to date for Domain {}",
                        reporter.getId(),
                        domain.getName());
            }
        } else {
    ...
    private void stopReporterProvider(
            String reporterId, io.gravitee.am.reporter.api.provider.Reporter reporter) {
        if (reporter != null) {
            try {
                reporter.stop();
            } catch (Exception ex) {
    ...
    public io.gravitee.am.reporter.api.provider.Reporter getReporter() {
        return reporterPlugins.values().stream()
