...
public class AuditReporterManagerImpl extends AbstractService implements AuditReporterManager, EventListener<ReporterEvent, Payload>, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        deployment.subscribe(id -> {
            ...
            // Start reporters
            RxJava2Adapter.monoToSingle(RxJava2Adapter.flowableToFlux(reporterRepository.findByDomain(domain.getId())).collectList().flatMap(reporters->RxJava2Adapter.singleToMono(environmentService.findById(domain.getReferenceId()).map((io.gravitee.am.model.Environment env)->new GraviteeContext(env.getOrganizationId(), env.getId(), domain.getId()))).map(RxJavaReactorMigrationUtil.toJdkFunction((io.gravitee.am.common.utils.GraviteeContext ctx)->Tuples.of(reporters, ctx)))).subscribeOn(reactor.core.scheduler.Schedulers.boundedElastic()))
