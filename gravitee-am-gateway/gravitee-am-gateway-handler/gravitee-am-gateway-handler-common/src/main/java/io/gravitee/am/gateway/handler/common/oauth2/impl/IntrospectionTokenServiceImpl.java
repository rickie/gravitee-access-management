...
public class IntrospectionTokenServiceImpl implements IntrospectionTokenService {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(IntrospectionTokenServiceImpl.class);
    ...
    public Single<JWT> introspect(String token, boolean offlineVerification) {
        return jwtService
                .decode(token)
                .flatMapMaybe(
                        jwt -> clientService.findByDomainAndClientId(jwt.getDomain(), jwt.getAud()))
                .switchIfEmpty(
                        Maybe.error(
                                new InvalidTokenException(
                                        "Invalid or unknown client for this token")))
                .flatMapSingle(client -> jwtService.decodeAndVerify(token, client))
                .flatMap(
                        jwt -> {
                            // Just check the JWT signature and JWT validity if offline verification
                            // option is enabled
                            // or if the token has just been created (could not be in database so
                            // far because of async database storing process delay)
                            if (offlineVerification
                                    || Instant.now()
                                            .isBefore(
                                                    Instant.ofEpochSecond(
                                                            jwt.getIat()
                                                                    + OFFLINE_VERIFICATION_TIMER_SECONDS))) {
                ...
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof JWTException) {
                                LOGGER.debug(
                                        "An error occurs while decoding JWT access token : {}",
                                        token,
                                        ex);
