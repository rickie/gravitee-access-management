...
public class ErrorEndpoint implements Handler<RoutingContext> {
        ...
        resolveClient(
                ...
                handler -> {
                    if (handler.failed()) {
                        ...
                        logger.debug(
                                "An error occurs while fetching client {}",
                                clientId,
                                handler.cause());
                        renderErrorPage(routingContext, null);
                        return;
    ...
    private void renderErrorPage(RoutingContext routingContext, Client client) {
            ...
            try {
                errorDescription =
                        java.net.URLDecoder.decode(
                                request.getParam(ERROR_DESCRIPTION_PARAM),
                            ...
                            .map(
                                    jwt -> {
                                        ...
                                        result.put(
                                                ...
                                                (String) jwt.get(ERROR_DESCRIPTION_PARAM));
                                        return result;
                                    });
        }
        ...
        singlePageRendering.subscribe(
                ...
                // single contains an error due to JWT decoding, return the default error page
    ...
    private void resolveClient(String clientId, Handler<AsyncResult<Client>> handler) {
        clientSyncService
                .findByDomainAndClientId(domain.getId(), clientId)
                .subscribe(
