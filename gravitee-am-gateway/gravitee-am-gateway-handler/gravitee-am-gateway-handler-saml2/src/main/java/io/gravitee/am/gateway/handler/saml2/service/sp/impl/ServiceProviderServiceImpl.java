...
public class ServiceProviderServiceImpl implements ServiceProviderService {
    ...
    public Single<Metadata> metadata(String providerId, String idpUrl) {
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(identityProviderManager.get(providerId)).switchIfEmpty(RxJava2Adapter.singleToMono(Single.wrap(Single.error(new IdentityProviderNotFoundException(providerId))))))).map(RxJavaReactorMigrationUtil.toJdkFunction(authenticationProvider -> {
