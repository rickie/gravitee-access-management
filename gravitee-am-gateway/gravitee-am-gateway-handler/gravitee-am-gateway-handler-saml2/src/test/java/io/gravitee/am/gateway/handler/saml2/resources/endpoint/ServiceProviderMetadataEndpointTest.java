...
public class ServiceProviderMetadataEndpointTest extends RxWebTestBase {
    ...
    @InjectMocks
    private ServiceProviderMetadataEndpoint spMetadataEndpoint =
            new ServiceProviderMetadataEndpoint(serviceProviderService);

    @Override
    public void setUp() throws Exception {
        super.setUp();
        router.route(HttpMethod.GET, "/sp/metadata/:providerId").handler(spMetadataEndpoint);
    }

    @Test
    public void shouldNotInvokeEndpoint_invalidProvider() throws Exception {
        when(serviceProviderService.metadata(eq("unknown-provider"), anyString()))
                .thenReturn(
                        Single.error(new IdentityProviderNotFoundException("unknown-provider")));
    ...
    public void shouldNotInvokeEndpoint_invalidMetadata() throws Exception {
        when(serviceProviderService.metadata(eq("unknown-provider"), anyString()))
                .thenReturn(
                        Single.error(
                                new IdentityProviderMetadataNotFoundException("unknown-provider")));
    ...
    public void shouldInvokeEndpoint() throws Exception {
        ...
        when(serviceProviderService.metadata(eq("provider-id"), anyString()))
