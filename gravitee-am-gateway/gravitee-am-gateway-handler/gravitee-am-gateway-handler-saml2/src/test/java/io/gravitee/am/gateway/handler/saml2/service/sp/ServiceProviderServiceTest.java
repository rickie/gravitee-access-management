...
public class ServiceProviderServiceTest {
    ...
    public void shouldNotGetMetadata_idp_not_found() {
        ...
        TestObserver testObserver =
                serviceProviderService.metadata("provider-id", "https://idp.example.com").test();
    ...
    public void shouldNotGetMetadata_null_metadata() {
        ...
        when(identityProviderManager.get("provider-id"))
                .thenReturn(Maybe.just(authenticationProvider));
        TestObserver testObserver =
                serviceProviderService.metadata("provider-id", "https://idp.example.com").test();
        testObserver.awaitTerminalEvent();
    ...
    public void shouldGetMetadata() {
        ...
        when(identityProviderManager.get("provider-id"))
                .thenReturn(Maybe.just(authenticationProvider));
        TestObserver<Metadata> testObserver =
                serviceProviderService.metadata("provider-id", "https://idp.example.com").test();
        testObserver.awaitTerminalEvent();
