 ...
 */
public class MFAChallengeAlternativesEndpoint extends AbstractEndpoint
        implements Handler<RoutingContext> {

    private static final Logger logger =
            LoggerFactory.getLogger(MFAChallengeAlternativesEndpoint.class);
    private final FactorManager factorManager;

    public MFAChallengeAlternativesEndpoint(
            TemplateEngine templateEngine, FactorManager factorManager) {
    ...
    private void get(RoutingContext routingContext) {
        if (routingContext.user() == null) {
            ...
            routingContext.fail(
                    new HttpException(
                            401,
                            "User must be authenticated to access MFA challenge alternatives page."));
            return;
        }
        ...
        if (endUser.getFactors() == null || endUser.getFactors().size() <= 1) {
            logger.warn(
                    "User must have at least two enrolled factors to access MFA challenge alternatives page.");
            routingContext.fail(
                    new HttpException(
                            400,
                            "User must have at least two enrolled factors to access MFA challenge alternatives page."));
        ...
        this.renderPage(
                ...
                client,
                logger,
                "Unable to render MFA challenge alternatives page");
    }
    ...
    private void post(RoutingContext routingContext) {
        if (routingContext.user() == null) {
            ...
            routingContext.fail(
                    new HttpException(
                            401,
                            "User must be authenticated to submit MFA challenge alternate choice."));
            return;
        }
        ...
        if (factorId == null) {
            ...
            routingContext.fail(
                    new HttpException(
                            400,
                            "No factor id in form - did you forget to include factor id value ?"));
            return;
        }
    ...
    private List<Factor> getEnabledFactors(Client client, io.gravitee.am.model.User endUser) {
        ...
        return endUser.getFactors().stream()
                ...
                .map(
                        enrolledFactor ->
                                new Factor(
                                        factorManager.getFactor(enrolledFactor.getFactorId()),
                                        enrolledFactor))
    ...
    private static class Factor {
        ...
        public Factor(io.gravitee.am.model.Factor factor, EnrolledFactor enrolledFactor) {
            ...
            this.target =
                    enrolledFactor.getChannel() != null
