 ...
 */
public class DeviceIdentifierManagerImpl extends AbstractService
        implements DeviceIdentifierManager,
                InitializingBean,
                EventListener<DeviceIdentifierEvent, Payload> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeviceIdentifierManagerImpl.class);
    private final ConcurrentMap<String, DeviceIdentifierProvider> providers =
            new ConcurrentHashMap<>();
    private final ConcurrentMap<String, DeviceIdentifier> deviceIdentifiers =
            new ConcurrentHashMap<>();

    @Autowired private DeviceIdentifierService deviceIdentifierService;

    @Autowired private Domain domain;

    @Autowired private EventManager eventManager;

    @Autowired private DeviceIdentifierPluginManager deviceIdentifierPluginManager;
        ...
        deviceIdentifierService
                ...
                .subscribe(
                        remeberDevice -> {
                            ...
                            LOGGER.info(
                                    "Device identifier {} loaded for domain {}",
                                    remeberDevice.getName(),
                                    domain.getName());
                        },
                        error ->
                                LOGGER.error(
                                        "Unable to initialize device identifiers for domain {}",
                                        domain.getName(),
                                        error));
    }
    ...
    protected void doStart() throws Exception {
        ...
        LOGGER.info(
                "Register event listener for device identifiers events for domain {}",
                domain.getName());
    ...
    protected void doStop() throws Exception {
        ...
        LOGGER.info(
                "Dispose event listener for Device identifier events for domain {}",
                domain.getName());
    ...
    private void updateDeviceIdentifier(String deviceIdentifierId, DeviceIdentifierEvent event) {
        ...
        deviceIdentifierService
                ...
                .subscribe(
                        ...
                        error ->
                                LOGGER.error(
                                        "Unable to load Device identifier for domain {}",
                                        domain.getName(),
                                        error),
                        () ->
                                LOGGER.error(
                                        "No Device identifier found with id {}",
                                        deviceIdentifierId));
    }

    private void removeBotDetection(String pluginId) {
        LOGGER.info(
                "Domain {} has received event, remove Device identifier {}",
                domain.getName(),
                pluginId);
    ...
    private void updateDeviceIdentifier(DeviceIdentifier detection) {
        try {
            if (needDeployment(detection)) {
                ...
                LOGGER.info(
                        "Device identifier {} already loaded for domain {}",
                        detection.getName(),
                        domain.getName());
            }
        } catch (Exception ex) {
            ...
            LOGGER.error(
                    "Unable to create Device identifier provider for domain {}",
                    domain.getName(),
                    ex);
        }
    }
    ...
    public Map<String, ?> getTemplateVariables(Client client) {
        ...
        var rememberDeviceSettings =
                ofNullable(mfaSettings).orElse(new MFASettings()).getRememberDevice();
        variables.put(
                REMEMBER_DEVICE_IS_ACTIVE,
                nonNull(rememberDeviceSettings)
                        && !isNullOrEmpty(rememberDeviceSettings.getDeviceIdentifierId())
                        && rememberDeviceSettings.isActive());
        if (TRUE.equals(variables.get(REMEMBER_DEVICE_IS_ACTIVE))) {
            var rememberDevice =
                    this.deviceIdentifiers.get(rememberDeviceSettings.getDeviceIdentifierId());
     ...
     * @return true if the BotDetection has never been deployed or if the deployed version is not up
     *     to date
     */
