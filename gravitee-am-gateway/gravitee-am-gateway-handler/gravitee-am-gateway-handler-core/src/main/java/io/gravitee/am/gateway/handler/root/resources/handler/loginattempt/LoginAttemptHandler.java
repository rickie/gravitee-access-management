...
public class LoginAttemptHandler implements Handler<RoutingContext> {
    ...
    private Maybe<Optional<LoginAttempt>> getLoginAttempt(
            Client client, String username, AccountSettings accountSettings) {
        return ofNullable(client.getIdentityProviders()).orElse(new TreeSet<>()).stream()
                ...
                .filter(Objects::nonNull)
                .filter(not(IdentityProvider::isExternal))
    ...
    private Maybe<Optional<LoginAttempt>> getLoginAttempt(
            AccountSettings accountSettings, LoginAttemptCriteria criteria) {
