...
/**
 * Post Login WebAuthn Remember Device Cookie handler We remove the cookie if the end-user sign-in
 * via password or social provider It means that a user did not succeed to sign-in via passwordless
 * (the device has been deleted or non-functional) In this case we don't want to redirect the users
 * directly to the webauthn login page
 *
    ...
    public void handle(RoutingContext routingContext) {
        ...
        if (cookie != null) {
            routingContext
                    .response()
                    .addCookie(
                            Cookie.cookie(webAuthnCookieService.getRememberDeviceCookieName(), "")
                                    .setMaxAge(0));
        }
        routingContext.next();
    }
