...
public class ForgotPasswordEndpoint extends AbstractEndpoint implements Handler<RoutingContext> {
    ...
    public ForgotPasswordEndpoint(
            TemplateEngine engine, Domain domain, BotDetectionManager botDetectionManager) {
        super(engine);
        this.domain = domain;
    ...
    public void handle(RoutingContext routingContext) {
        ...
        if (settings != null && settings.isResetPasswordCustomForm()) {
            ...
            // otherwise display custom form (ConfirmIdentity is disabled or an identity
            // confirmation is required)
            if (settings.isResetPasswordConfirmIdentity()
                    && !FORGOT_PASSWORD_CONFIRM.equals(warning)) {
                routingContext.put(
                ...
                routingContext.put(
                        ConstantKeys.FORGOT_PASSWORD_FIELDS_KEY,
                        settings.getResetPasswordCustomFormFields());
            }
        } else {
            routingContext.put(
                    ConstantKeys.FORGOT_PASSWORD_FIELDS_KEY,
        ...
        this.renderPage(
                routingContext, data, client, logger, "Unable to render forgot password page");
    }
