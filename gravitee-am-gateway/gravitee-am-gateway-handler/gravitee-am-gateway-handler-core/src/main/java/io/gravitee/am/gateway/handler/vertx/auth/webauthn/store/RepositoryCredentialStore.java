...
public class RepositoryCredentialStore {
    ...
    public Future<List<Authenticator>> fetch(Authenticator query) {
        ...
        Single<List<Credential>> fetchCredentials =
                query.getUserName() != null
                        ...
                        : credentialService
                                .findByCredentialId(
                                        ReferenceType.DOMAIN, domain.getId(), query.getCredID())
                                .toList();
                ...
                .flatMap(
                        credentials -> {
                            if (credentials.isEmpty() && query.getUserName() != null) {
                                // If, when initiating an authentication ceremony, there is no
                                // account matching the provided username,
                                // continue the ceremony by invoking navigator.credentials.get()
                                // using a syntactically valid
                                // PublicKeyCredentialRequestOptions object that is populated with
                                // plausible imaginary values.
                                // Prevent 14.6.2. Username Enumeration
                                                ...
                                                if (deviceType == 1) {
                                                    if (i < 2) {
                                                        if (initialValue.length() > 27) {
                                                            initialValue =
                                                                    initialValue.substring(0, 27);
                                                        }
                                                        authenticator.setCredID(initialValue);
                                                    } else {
                                                        authenticator.setCredID(
                                                                createCredID(
                                                                        md, initialValue, part1));
                                                    }
                                                } else {
                                                    if (i < 2) {
                                                        authenticator.setCredID(
    ...
    public Future<Void> store(Authenticator authenticator) {
        ...
        credentialService
                .findByCredentialId(ReferenceType.DOMAIN, domain.getId(), authenticator.getCredID())
                .toList()
                .flatMapCompletable(
                        credentials -> {
    ...
    private Single<String> generateCredID(String username, String claim) {
        return Single.create(
                emitter -> {
                    String credID =
                            jwtBuilder.sign(new JWT(Collections.singletonMap(claim, username)))
                                    .split("\\.")[2];
