...
public class LoginCallbackParseHandler implements Handler<RoutingContext> {
    ...
    public LoginCallbackParseHandler(
                                ...
                                parseSocialProvider(
                                        ...
                                        socialProviderHandler -> {
                                            ...
                                            AuthenticationProvider authenticationProvider =
                                                    socialProviderHandler.result();
                                            context.put(
                                                    ConstantKeys.PROVIDER_CONTEXT_KEY,
        ...
        final String state =
                context.request().getParam(Parameters.STATE) != null
                        ...
                        : (context.request().getParam(RELAY_STATE_PARAM_KEY) != null
                                ? context.request().getParam(RELAY_STATE_PARAM_KEY)
                                : context.get(Parameters.STATE));
    ...
    private void parseClient(RoutingContext context, Handler<AsyncResult<Client>> handler) {
        ...
        if (context.get(ConstantKeys.PARAM_CONTEXT_KEY) == null
                || !(context.get(ConstantKeys.PARAM_CONTEXT_KEY) instanceof MultiMap)) {
            logger.error("Unable to restore initial client query params login callback");
            handler.handle(
                    Future.failedFuture(
                            new InvalidRequestException("Missing client query parameters")));
            return;
        }
    ...
    private void parseSocialProvider(
        ...
        identityProviderManager
                ...
                .subscribe(
                        authenticationProvider ->
                                handler.handle(Future.succeededFuture(authenticationProvider)),
                        ex -> {
                            logger.error(
                                    "An error occurs while getting identity provider {}",
                                    providerId,
                                    ex);
