...
public class LoginNegotiateAuthenticationHandler implements Handler<RoutingContext> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(LoginNegotiateAuthenticationHandler.class);

    private final UserAuthProvider authProvider;
    private final ThymeleafTemplateEngine engine;

    public LoginNegotiateAuthenticationHandler(
            UserAuthProvider authProvider, ThymeleafTemplateEngine engine) {
    ...
    public void handle(RoutingContext context) {
        String authHeader =
                context.request().getHeader(io.vertx.core.http.HttpHeaders.AUTHORIZATION);
        boolean hasNegotiateToken =
                (authHeader != null && authHeader.trim().startsWith(AUTH_NEGOTIATE_KEY));
        if (hasNegotiateToken) {
            ...
            JsonObject authInfo =
                    new JsonObject()
                            ...
                            .put(
                                    PASSWORD_PARAM_KEY,
                                    authHeader.replaceFirst(AUTH_NEGOTIATE_KEY, "").trim())
            ...
            authProvider.authenticate(
                    ...
                    res -> {
                        if (res.failed()) {
                            LOGGER.debug(
                                    "SPNEGO token is invalid, continue flow to display login form");
            ...
            // Responds with an 401 response with the "WWW-Authenticate: Negotiate" HTTP Response
            // Header.
            // This tells the web browser that it needs to check with the local OS regarding what
            ...
            engine.render(
                    ...
                    res -> {
                        if (res.succeeded()) {
                            context.response()
                                    .putHeader(HttpHeaders.WWW_AUTHENTICATE, AUTH_NEGOTIATE_KEY);
                            context.response()
