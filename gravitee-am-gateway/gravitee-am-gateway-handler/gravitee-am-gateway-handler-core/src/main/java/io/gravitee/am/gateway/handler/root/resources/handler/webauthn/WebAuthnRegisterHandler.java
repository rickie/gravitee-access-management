...
public class WebAuthnRegisterHandler extends WebAuthnHandler {
    ...
    public WebAuthnRegisterHandler(
                        ...
                        updateCredential(
                                ...
                                credentialHandler -> {
                                    if (credentialHandler.failed()) {
                                        logger.error(
                                                "An error has occurred while updating credential for user {}",
                                                username,
                                    ...
                                    if (isEnrollingFido2Factor(ctx)) {
                                        enrollFido2Factor(
                                                ctx,
                                                authenticatedUser,
                                                createEnrolledFactor(
                                                        session.get(ENROLLED_FACTOR_ID_KEY),
                                                        credentialId));
                                    } else {
                                        manageFido2FactorEnrollment(
                                                ctx, client, credentialId, authenticatedUser);
                                    }
                                });
                    } else {
