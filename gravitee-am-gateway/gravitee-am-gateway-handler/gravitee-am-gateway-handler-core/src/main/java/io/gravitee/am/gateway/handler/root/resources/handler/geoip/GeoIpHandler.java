...
public class GeoIpHandler implements Handler<RoutingContext> {
    ...
    public GeoIpHandler(UserActivityService userActivityService, EventBus eventBus) {
    ...
    public void handle(RoutingContext routingContext) {
        ...
        var adaptiveRule =
                client.map(Client::getMfaSettings)
                        ...
                        .orElse("");
        if ((!adaptiveRule.isEmpty() || userActivityService.canSaveUserActivity())
                && isNull(routingContext.data().get(GEOIP_KEY))) {
