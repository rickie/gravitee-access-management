...
public class WebAuthnRegisterEndpoint extends WebAuthnHandler {
    ...
    public WebAuthnRegisterEndpoint(
            TemplateEngine templateEngine, Domain domain, FactorManager factorManager) {
        ...
        try {
            ...
            String skipAction =
                    UriBuilderRequest.resolveProxyRequest(
                            routingContext.request(),
                            routingContext.request().path(),
                            queryParams.set("skipWebAuthN", "true"),
                            true);
            ...
            if (domain.getWebAuthnSettings() != null
                    && domain.getWebAuthnSettings().getAuthenticatorAttachment() != null) {
                routingContext.put(
                        ConstantKeys.PARAM_AUTHENTICATOR_ATTACHMENT_KEY,
                        domain.getWebAuthnSettings().getAuthenticatorAttachment().getValue());
            }
            ...
            this.renderPage(
                    ...
                    client,
                    logger,
                    "Unable to render WebAuthn register page");
        } catch (Exception ex) {
