...
public class DataConsentHandler implements Handler<RoutingContext> {
    ...
    public static final List<String> CONSENT_KEYS =
            List.of(USER_CONSENT_IP_LOCATION, USER_CONSENT_USER_AGENT);

    private static final String DATA_CONSENT_ON = "on";

    public DataConsentHandler(Environment environment) {
        IMPLICIT_IP_CONSENT =
                environment.getProperty(CONFIG_KEY_IMPLICIT_CONSENT_IP, boolean.class, false);
        IMPLICIT_USER_AGENT_CONSENT =
                environment.getProperty(
                        CONFIG_KEY_IMPLICIT_CONSENT_USER_AGENT, boolean.class, false);
    }
    ...
    public void handle(RoutingContext context) {
        ...
        if (context.session() != null) {
            ...
            CONSENT_KEYS.forEach(
                    key ->
                            ofNullable(request.params())
    ...
    private boolean isContentTypeJson(MultiMap headers) {
        return nonNull(headers) && APPLICATION_JSON.equalsIgnoreCase(headers.get(CONTENT_TYPE));
    }
}
