...
public class MFAChallengeEndpoint extends AbstractEndpoint implements Handler<RoutingContext> {
    ...
    private void saveFactor(User user, Single<EnrolledFactor> enrolledFactor, Handler<AsyncResult<User>> handler) {
        RxJava2Adapter.singleToMono(enrolledFactor).flatMap(factor->userService.addFactor_migrated(user.getId(), factor, new DefaultUser(user))).subscribe(user1 -> handler.handle(Future.succeededFuture(user1)), error -> handler.handle(Future.failedFuture(error)));
    }
