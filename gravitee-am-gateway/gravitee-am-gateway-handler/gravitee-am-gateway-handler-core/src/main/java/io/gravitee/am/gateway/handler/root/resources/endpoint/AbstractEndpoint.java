...
public abstract class AbstractEndpoint {
    ...
    protected final String getTemplateFileName(Client client) {
        return getTemplateSuffix()
                + Optional.ofNullable(client)
                        .map(c -> FormManager.TEMPLATE_NAME_SEPARATOR + c.getId())
                        .orElse("");
    }

    protected final void copyValue(
            HttpServerRequest request, RoutingContext routingContext, String paramKey) {
    ...
    protected void renderPage(
            Map<String, Object> data, Client client, Handler<AsyncResult<Buffer>> handler) {
    ...
    protected void addUserActivityTemplateVariables(
        ...
        if (userActivityService.canSaveUserActivity()) {
            ...
            final String retentionTime =
                    time
                            ...
                            + (time > 1
                                    ? retentionUnit
                                    : retentionUnit.substring(0, retentionUnit.length() - 1));
    ...
    protected void addUserActivityConsentTemplateVariables(RoutingContext routingContext) {
        ...
        if (session != null) {
            ...
            routingContext.put(
                    USER_CONSENT_USER_AGENT, TRUE.equals(session.get(USER_CONSENT_USER_AGENT)));
        }
    }
}
