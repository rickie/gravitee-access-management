...
public class LoginCallbackFailureHandler extends LoginAbstractHandler {
    ...
    private void redirect(RoutingContext context, Throwable throwable) {
        try {
            ...
            if (context.user() != null) {
                // clear AuthenticationFlowContext. data of this context have a TTL so we can fire
                // and forget in case on error.
            ...
            var elContext =
                    new SimpleAuthenticationContext(
                            new VertxHttpServerRequest(
                                    context.request().getDelegate(), originalParams),
                            context.data());
            ...
            // redirect to the SP redirect_uri to avoid an infinite loop between AM and the external
            // IdP
            long externalIdentities =
                    Optional.ofNullable(client.getIdentityProviders()).stream()
            ...
            // if client hasn't activated hide login form but has external IdP with matching
            // selection rule
            // redirect to the SP redirect_uri to avoid an infinite loop between AM and the external
            // IdP
            long selectedExternalIdp =
                    Optional.ofNullable(client.getIdentityProviders()).stream()
                            .flatMap(Collection::stream)
                            .filter(
                                    aidp ->
    ...
    private void redirectToSP(
        ...
        query.put(
                ConstantKeys.ERROR_DESCRIPTION_PARAM_KEY,
        ...
        boolean fragment =
                originalParams != null
                        ...
                        && (isImplicitFlow(originalParams.get(Parameters.RESPONSE_TYPE))
                                || isHybridFlow(originalParams.get(Parameters.RESPONSE_TYPE)));
