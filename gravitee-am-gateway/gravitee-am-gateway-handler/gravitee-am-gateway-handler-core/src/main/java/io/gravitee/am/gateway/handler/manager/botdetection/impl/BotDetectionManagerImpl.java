 ...
 */
public class BotDetectionManagerImpl extends AbstractService
        implements BotDetectionManager,
                InitializingBean,
                EventListener<BotDetectionEvent, Payload> {
    ...
    @Autowired private BotDetectionService botDetectionService;

    @Autowired private Domain domain;

    @Autowired private EventManager eventManager;

    @Autowired private BotDetectionPluginManager botDetectionPluginManager;
        ...
        botDetectionService
                ...
                .subscribe(
                        detection -> {
                            ...
                            LOGGER.info(
                                    "Bot detection {} loaded for domain {}",
                                    detection.getName(),
                                    domain.getName());
                        },
                        error ->
                                LOGGER.error(
                                        "Unable to initialize bot detections for domain {}",
                                        domain.getName(),
                                        error));
    }
    ...
    protected void doStart() throws Exception {
        ...
        LOGGER.info(
                "Register event listener for bot detections events for domain {}",
                domain.getName());
    ...
    protected void doStop() throws Exception {
        ...
        LOGGER.info(
                "Dispose event listener for bot detection events for domain {}", domain.getName());
    ...
    public Single<Boolean> validate(BotDetectionContext context) {
        ...
        if (botDetectionProvider != null) {
            ...
            LOGGER.error(
                    "No BotDetectionProvider matches the pluginId '{}'", context.getPluginId());
    ...
    public Map<String, Object> getTemplateVariables(Domain domain, Client client) {
        ...
        variables.put(
                TEMPLATE_KEY_BOT_DETECTION_ENABLED,
                accountSettings != null
                        && accountSettings.isUseBotDetection()
                        && accountSettings.getBotDetectionPlugin() != null);
        if (accountSettings != null && accountSettings.isUseBotDetection()) {
            if (accountSettings.getBotDetectionPlugin() == null) {
                ...
                try {
                    variables.put(
                            TEMPLATE_KEY_BOT_DETECTION_CONFIGURATION,
                            objectMapper.readValue(botDetection.getConfiguration(), HashMap.class));
                } catch (JsonProcessingException e) {
                    LOGGER.error(
                            "Unable to read the configuration for bot detection '{}'",
                            botDetection.getId(),
                            e);
    ...
    private void updateBotDetection(String pluginId, BotDetectionEvent event) {
        ...
        botDetectionService
                ...
                .subscribe(
                        ...
                        error ->
                                LOGGER.error(
                                        "Unable to load bot detection for domain {}",
                                        domain.getName(),
                                        error),
    ...
    private void updateBotDetection(BotDetection detection) {
        try {
            if (needDeployment(detection)) {
                ...
                LOGGER.info(
                        "Bot detection {} already loaded for domain {}",
                        detection.getName(),
                        domain.getName());
            }
        } catch (Exception ex) {
            ...
            LOGGER.error(
                    "Unable to create bot detection provider for domain {}", domain.getName(), ex);
        }
    }
    ...
    private void stopBotDetectionProvider(BotDetectionProvider previousProvider) {
        try {
            ...
            LOGGER.error(
                    "Unable to stop bot detection provider for domain {}", domain.getName(), e);
        }
    }
