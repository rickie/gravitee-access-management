...
public abstract class WebAuthnEndpoint extends AbstractEndpoint implements Handler<RoutingContext> {
    ...
    protected void checkUser(Client client, String username, Request request, Handler<AsyncResult<User>> handler) {
        userAuthenticationManager.loadUserByUsername_migrated(client, username, request).subscribe(user -> handler.handle(Future.succeededFuture(user)), error -> handler.handle(Future.failedFuture(error)), () -> handler.handle(Future.failedFuture(new UsernameNotFoundException(username))));
    }
