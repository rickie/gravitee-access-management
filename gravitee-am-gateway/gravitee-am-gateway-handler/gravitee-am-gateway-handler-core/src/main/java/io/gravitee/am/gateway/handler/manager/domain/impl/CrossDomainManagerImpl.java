...
public class CrossDomainManagerImpl extends AbstractService implements CrossDomainManager, InitializingBean, EventListener<DomainEvent, Domain> {
    ...
    public void afterPropertiesSet() throws Exception {
        if (domain.isMaster()) {
            ...
            RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(domainRepository.findAllByReferenceId_migrated(domain.getReferenceId()).filter(RxJavaReactorMigrationUtil.toJdkPredicate(d -> !domain.getId().equals(d.getId()))).collectList())).subscribe(RxJavaReactorMigrationUtil.toJdkConsumer(domains -> domains.forEach(clientManager::deployCrossDomain)));
        }
    }
