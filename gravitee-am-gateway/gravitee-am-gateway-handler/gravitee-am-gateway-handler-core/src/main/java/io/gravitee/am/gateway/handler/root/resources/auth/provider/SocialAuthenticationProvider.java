...
public class SocialAuthenticationProvider implements UserAuthProvider {
    private static final Logger logger =
            LoggerFactory.getLogger(SocialAuthenticationProvider.class);
    ...
    public void authenticate(
        ...
        authenticationContext.set(
                Parameters.REDIRECT_URI, authInfo.getString(Parameters.REDIRECT_URI));
        ...
        if (canSaveIp(context)) {
            endUserAuthentication
                    .getContext()
                    .set(Claims.ip_address, RequestUtils.remoteAddress(context.request()));
        }
        if (canSaveUserAgent(context)) {
            endUserAuthentication
                    .getContext()
                    .set(Claims.user_agent, RequestUtils.userAgent(context.request()));
        }
        ...
        authenticationProvider
                ...
                .switchIfEmpty(
                        Maybe.error(
                                new BadCredentialsException(
                                        "Unable to authenticate social provider, authentication provider has returned empty value")))
                ...
                .flatMap(
                        user -> {
                            ...
                            var idToken =
                                    ofNullable(
                                            endUserAuthentication.getContext().get(ID_TOKEN_KEY));

                            accessToken.ifPresentOrElse(
                                    at -> {
                                        // If isStoreOriginalToken, we add both the access_token and
                                        // id_token in profile since they are present
                                        additionalInformation.put(
                                                OIDC_PROVIDER_ID_ACCESS_TOKEN_KEY, at);
                                        idToken.ifPresent(
                                                it ->
                                                        additionalInformation.put(
                                                                OIDC_PROVIDER_ID_TOKEN_KEY, it));
                                    },
                                    () -> {
                                        // We remove both otherwise
                                        additionalInformation.remove(
                                                OIDC_PROVIDER_ID_ACCESS_TOKEN_KEY);
                                        additionalInformation.remove(OIDC_PROVIDER_ID_TOKEN_KEY);
                                    });

                            // If id_token is present and SSO is enabled we add the id_token in
                            // profile
                            if (client.isSingleSignOut() && idToken.isPresent()) {
                                logger.debug(
                                        "Single SignOut enable for client '{}' store the id_token coming from the provider {} as additional information",
                                        client.getId(),
                                        authProvider);
                                additionalInformation.put(
                                        OIDC_PROVIDER_ID_TOKEN_KEY, idToken.get());
                            } else if (accessToken.isEmpty()) {
                                // unless isStoreOriginalToken is enabled (e.g access_token
                                // isPresent) we can remove id_token from the profile
                                additionalInformation.remove(OIDC_PROVIDER_ID_TOKEN_KEY);
                            }
                ...
                .subscribe(
                        user -> {
                            ...
                            eventManager.publishEvent(
                                    ...
                                    new AuthenticationDetails(
                                            endUserAuthentication, domain, client, user));
                        ...
                        error -> {
                            ...
                            eventManager.publishEvent(
                                    ...
                                    new AuthenticationDetails(
                                            endUserAuthentication, domain, client, error));
    ...
    private Single<io.gravitee.am.identityprovider.api.User> checkDomainWhitelist(
        ...
        if (nonNull(domainWhitelist) && !domainWhitelist.isEmpty()) {
            ...
            if (!isUsernameWhitelisted(domainWhitelist, endUser.getUsername())
                ...
                return Single.error(
                        new LoginCallbackFailedException("could not authenticate user"));
            }
        }
