...
public class VertxSecurityDomainHandler extends AbstractService<VertxSecurityDomainHandler> {
    ...
    @Autowired private ProtocolPluginManager protocolPluginManager;
    ...
    protected void doStop() throws Exception {
        logger.info(
                "Security domain ["
                        + domain.getName()
                        + "] handler is now stopping, closing context...");
    ...
    private void startSecurityDomainProtocols() {
        ...
        PROTOCOLS.forEach(
                protocol -> {
                    try {
                        var providerConfig =
                                new ProtocolProviderConfiguration(protocol, applicationContext);
        ...
        protocolProviders.forEach(
                protocolProvider -> {
                    try {
                        ...
                        logger.error(
                                "\t An error occurs while stopping {} protocol",
                                protocolProvider.path(),
                                e);
                    }
                });
    }
        ...
        components.forEach(
                componentClass -> {
                    ...
                    try {
                        ...
                        logger.error(
                                "An error occurs while stopping component {}",
    ...
    private void sendNotFound(RoutingContext context) {
        ...
        String message =
                environment.getProperty(
                        "http.domain.errors[404].message", "No endpoint matches the request URI.");
        serverResponse
                .headers()
