...
/**
 * Checks a password against a user's history of passwords and returns either a 200 ok or 400
 * failure if the password is already in the history.
 */
public class PasswordHistoryHandler implements Handler<RoutingContext> {
    ...
    public PasswordHistoryHandler(
            PasswordHistoryService passwordHistoryService, UserService userService, Domain domain) {
        ...
        userService
                ...
                .flatMapSingle(
                        userToken -> {
                            ...
                            return passwordHistoryService.passwordAlreadyUsed(
                                    ReferenceType.DOMAIN,
                                    domain.getId(),
                                    user.getId(),
                                    password,
                                    getPasswordSettings(context));
                        })
                .doOnError(
                        throwable ->
                                context.fail(HttpStatusCode.INTERNAL_SERVER_ERROR_500, throwable))
                .subscribe(
                        passwordUsed -> {
