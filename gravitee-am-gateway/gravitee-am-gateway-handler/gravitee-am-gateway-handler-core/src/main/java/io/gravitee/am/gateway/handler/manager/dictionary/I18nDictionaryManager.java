 ...
 */
public class I18nDictionaryManager extends AbstractService
        implements InitializingBean, EventListener<I18nDictionaryEvent, Payload> {
    ...
    @Autowired private I18nDictionaryRepository i18nDictionaryRepository;

    @Autowired private Domain domain;

    @Autowired private EventManager eventManager;

    @Autowired private GraviteeMessageResolver graviteeMessageResolver;

    @Override
    public void afterPropertiesSet() throws Exception {
        logger.info("Initializing i18n dictionaries for domain {}", domain.getName());
        i18nDictionaryRepository
                .findAll(ReferenceType.DOMAIN, domain.getId())
                .subscribe(
                        i18nDictionary -> {
                            update(i18nDictionary);
                            logger.info(
                                    "I18n dictionary {} loaded for domain {}",
                                    i18nDictionary.getId(),
                                    domain.getName());
                        },
                        error ->
                                logger.error(
                                        "Unable to initialize i18n dictionaries for domain {}",
                                        domain.getName(),
                                        error));
    }
    ...
    protected void doStart() throws Exception {
        ...
        logger.info(
                "Register event listener for i18n dictionary events for domain {}",
                domain.getName());
    ...
    protected void doStop() throws Exception {
        ...
        logger.info(
                "Dispose event listener for i18n dictionary events for domain {}",
                domain.getName());
    ...
    private void update(String id, I18nDictionaryEvent event) {
        ...
        i18nDictionaryRepository
                ...
                .subscribe(
                        ...
                        error ->
                                logger.error(
                                        "Unable to {} i18n dictionary for domain {}",
                                        eventType,
                                        domain.getName(),
                                        error),
    ...
    private void remove(String id) {
        logger.info(
                "Domain {} has received i18n dictionary event, delete i18n dictionary {}",
                domain.getName(),
                id);
