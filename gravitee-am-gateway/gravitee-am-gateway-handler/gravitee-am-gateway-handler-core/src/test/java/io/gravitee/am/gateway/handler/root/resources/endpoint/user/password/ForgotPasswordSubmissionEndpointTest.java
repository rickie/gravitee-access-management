...
public class ForgotPasswordSubmissionEndpointTest extends RxWebTestBase {
    ...
    public void shouldInvokeForgotPasswordEndpoint() throws Exception {
        ...
        when(userService.forgotPassword_migrated(argThat(p -> p.getEmail().equals("email@test.com")), eq(client), any(User.class))).thenReturn(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(Mono.empty())));
    ...
    public void shouldCompleteWhenUserNotFoundException() throws Exception {
        ...
        when(userService.forgotPassword_migrated(argThat(p -> p.getEmail().equals("email@test.com")), eq(client), any(User.class))).thenReturn(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(Mono.error(new UserNotFoundException("email@test.com")))));
    ...
    public void shouldFailWhenEmailFormatInvalidException() throws Exception {
        ...
        when(userService.forgotPassword_migrated(argThat(p -> p.getEmail().equals("email.test.com")), eq(client), any(User.class))).thenReturn(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(Mono.error(new EmailFormatInvalidException("email.test.com")))));
    ...
    public void shouldCompleteWithWarningWhen_EnforceIdentityException_TooManyResult() throws Exception {
        ...
        when(userService.forgotPassword_migrated(argThat(p -> p.getEmail().equals("email@test.com")), eq(client), any(User.class))).thenReturn(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(Mono.error(new EnforceUserIdentityException()))));
    ...
    private void postEmail(HttpClientRequest httpClientRequest, String email) {
