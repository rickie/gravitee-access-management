...
public class DataHandlerConsentImplicitTest {
    ...
    private void setNoImplicitConsent() {
        ...
        Mockito.when(
                        mockEnv.getProperty(
                                DataConsentHandler.CONFIG_KEY_IMPLICIT_CONSENT_IP,
                                boolean.class,
                                false))
                .thenReturn(true);
        Mockito.when(
                        mockEnv.getProperty(
                                DataConsentHandler.CONFIG_KEY_IMPLICIT_CONSENT_USER_AGENT,
                                boolean.class,
                                false))
                .thenReturn(true);
    }
    ...
    @Test
    public void
            must_have_user_ip_location_with_default_implicit_value_when_no_request_param_body() {
        routingContext.request().headers().set("Content-type", APPLICATION_JSON);
        routingContext.setBody(
                new Buffer(new JsonObject(Map.of(USER_CONSENT_IP_LOCATION, "on")).toBuffer()));
        handler.handle(routingContext);
        assertTrue(routingContext.session().get(USER_CONSENT_IP_LOCATION));
        assertTrue(routingContext.session().get(USER_CONSENT_USER_AGENT));
    }
    ...
    public void must_have_user_agent_with_default_implicit_value_when_no_request_param_body() {
        routingContext.request().headers().set("Content-type", APPLICATION_JSON);
        routingContext.setBody(
                new Buffer(new JsonObject(Map.of(USER_CONSENT_USER_AGENT, "on")).toBuffer()));
        handler.handle(routingContext);
        assertTrue(routingContext.session().get(USER_CONSENT_IP_LOCATION));
        assertTrue(routingContext.session().get(USER_CONSENT_USER_AGENT));
    }
