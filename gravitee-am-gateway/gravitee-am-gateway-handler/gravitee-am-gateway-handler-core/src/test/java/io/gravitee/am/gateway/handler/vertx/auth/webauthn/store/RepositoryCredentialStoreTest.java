...
public class RepositoryCredentialStoreTest {
    ...
    public void shouldFetchAuthenticator_byUsername_emptyList() {
        ...
        when(credentialService.findByUsername_migrated(ReferenceType.DOMAIN, domain.getId(), query.getUserName())).thenReturn(Flux.empty());
    ...
    public void shouldFetchAuthenticator_byUsername_emptyList_different_users() {
        ...
        when(domain.getId()).thenReturn("domain-id");
        when(credentialService.findByUsername_migrated(ReferenceType.DOMAIN, domain.getId(), query.getUserName())).thenReturn(Flux.empty());
        when(credentialService.findByUsername_migrated(ReferenceType.DOMAIN, domain.getId(), query2.getUserName())).thenReturn(Flux.empty());
    ...
    public void shouldFetchAuthenticator_byUsername_withValues() {
        ...
        when(domain.getId()).thenReturn("domain-id");
        when(credentialService.findByUsername_migrated(ReferenceType.DOMAIN, domain.getId(), query.getUserName())).thenReturn(Flux.just(credential));
        List<Authenticator> authenticators = repositoryCredentialStore.fetch(query).result();
