...
public class ResetPasswordSubmissionEndpointTest extends RxWebTestBase {
    ...
    public void shouldInvokeResetPasswordEndpoint() throws Exception {
        ...
        when(userService.resetPassword_migrated(eq(client), eq(user), any())).thenReturn(Mono.just(new ResetPasswordResponse()));
    ...
    public void shouldInvokeResetPasswordEndpoint_redirectUri() throws Exception {
        ...
        when(userService.resetPassword_migrated(eq(client), eq(user), any())).thenReturn(Mono.just(resetPasswordResponse));
