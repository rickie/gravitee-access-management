...
public class ResetPasswordSubmissionEndpointTest extends RxWebTestBase {
    ...
    public void setUp() throws Exception {
        ...
        when(environment.getProperty(
                        eq(ResetPasswordSubmissionEndpoint.GATEWAY_ENDPOINT_RESET_PWD_KEEP_PARAMS),
                        any(),
                        eq(true)))
                .thenReturn(true);

        ResetPasswordSubmissionEndpoint resetPasswordSubmissionEndpoint =
                new ResetPasswordSubmissionEndpoint(userService, environment);
    ...
    public void shouldInvokeResetPasswordEndpoint() throws Exception {
        ...
        when(userService.resetPassword(eq(client), eq(user), any()))
                .thenReturn(Single.just(new ResetPasswordResponse()));

        testRequest(
                HttpMethod.POST,
                "/resetPassword?client_id=client-id",
                this::postPassword,
                resp -> {
                    ...
                    assertTrue(
                            location.endsWith(
                                    "/resetPassword?client_id=client-id&success=reset_password_completed"));
                },
    ...
    public void shouldInvokeResetPasswordEndpoint_redirectUri() throws Exception {
        ...
        when(userService.resetPassword(eq(client), eq(user), any()))
                .thenReturn(Single.just(resetPasswordResponse));
