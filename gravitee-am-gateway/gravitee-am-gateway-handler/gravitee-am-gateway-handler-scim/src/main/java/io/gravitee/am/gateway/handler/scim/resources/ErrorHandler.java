...
/**
 * The SCIM protocol uses the HTTP response status codes defined in Section 6 of [RFC7231] to
 * indicate operation success or failure. In addition to returning an HTTP response code,
 * implementers MUST return the errors in the body of the response in a JSON format, using the
 * attributes described below. Error responses are identified using the following "schema" URI:
 * "urn:ietf:params:scim:api:messages:2.0:Error". The following attributes are defined for a SCIM
 * error response using a JSON body:
 *
 * <p>status The HTTP status code (see Section 6 of [RFC7231]) expressed as a JSON string. REQUIRED.
 *
 * <p>scimType A SCIM detail error keyword. See Table 9. OPTIONAL.
 *
 * <p>detail A detailed human-readable message. OPTIONAL.
 *
 * <p>See <a href="https://tools.ietf.org/html/rfc7644#section-3.12">3.12. HTTP Status and Error
 * Response Handling</a>
 *
    ...
    public void handle(RoutingContext routingContext) {
        if (routingContext.failed()) {
            ...
            if (throwable instanceof AbstractManagementException) {
                ...
                handleException(
                        routingContext,
                        scimException.getHttpStatusCode(),
                        scimException.getMessage(),
                        scimException.getScimType());
            } else if (throwable instanceof HttpException) {
                if (401 == ((HttpException) throwable).getStatusCode()) {
                    UnauthorizedException unauthorizedException = new UnauthorizedException();
                    handleException(
                            routingContext,
                            unauthorizedException.getHttpStatusCode(),
