...
public class SCIMProvider extends AbstractService<ProtocolProvider> implements ProtocolProvider {
    ...
    @Autowired private ServiceProviderConfigService serviceProviderConfigService;
    ...
    protected void doStart() throws Exception {
        ...
        if (isSCIMEnabled()) {
            ...
            // Declare SCIM routes
            // see <a href="https://tools.ietf.org/html/rfc7644#section-3.2">3.2. SCIM Endpoints and
            // HTTP Methods</a>

            // Service Provider configuration
            ServiceProviderConfigurationEndpointHandler
                    serviceProviderConfigurationEndpointHandler =
                            ServiceProviderConfigurationEndpointHandler.create(
                                    serviceProviderConfigService);
            serviceProviderConfigurationEndpointHandler.setObjectMapper(objectMapper);
            scimRouter
                    .get("/ServiceProviderConfig")
                    .handler(serviceProviderConfigurationEndpointHandler);

            // SCIM resources routes are OAuth 2.0 secured
            OAuth2AuthHandler oAuth2AuthHandler =
                    OAuth2AuthHandler.create(oAuth2AuthProvider, "scim");
