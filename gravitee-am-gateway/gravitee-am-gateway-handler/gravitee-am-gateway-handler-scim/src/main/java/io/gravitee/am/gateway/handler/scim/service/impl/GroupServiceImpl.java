...
public class GroupServiceImpl implements GroupService {
    ...
    public Single<Group> update(String groupId, Group group, String baseUrl) {
        ...
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(groupRepository.findById(groupId)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.error(new GroupNotFoundException(groupId)))))
                .flatMapSingle(existingGroup -> RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(groupRepository.findByName(ReferenceType.DOMAIN, domain.getId(), group.getDisplayName())
                        .map(group1 -> {
    ...
    public Single<Group> patch(String groupId, PatchOp patchOp, String baseUrl) {
        ...
        return RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(get(groupId, baseUrl)).switchIfEmpty(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.error(new GroupNotFoundException(groupId))))).flatMap(v->RxJava2Adapter.singleToMono((Single<Group>)RxJavaReactorMigrationUtil.toJdkFunction((Function<Group, Single<Group>>)group -> {
    ...
    public Completable delete(String groupId) {
        ...
        return RxJava2Adapter.monoToCompletable(RxJava2Adapter.maybeToMono(groupRepository.findById(groupId)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.error(new GroupNotFoundException(groupId)))).flatMap(user->RxJava2Adapter.completableToMono(groupRepository.delete(groupId))).then())
                .onErrorResumeNext(ex -> {
                    if (ex instanceof AbstractManagementException) {
