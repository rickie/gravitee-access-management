...
public class GroupServiceImpl implements GroupService {
        ...
        return groupRepository
                ...
                .flatMap(
                        groupPage -> {
                            ...
                            if (size <= 0) {
                                return Single.just(
                                        new ListResponse<Group>(
                                                null,
                                                groupPage.getCurrentPage() + 1,
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to find groups by domain {}",
                                    domain,
                                    ex);
                            return Single.error(
                                    new TechnicalManagementException(
                                            String.format(
                                                    "An error occurs while trying to find groups by domain %s",
                                                    domain),
                                            ex));
                        });
    }
    ...
    public Single<Group> create(
            Group group, String baseUrl, io.gravitee.am.identityprovider.api.User principal) {
        LOGGER.debug(
                "Create a new group {} for domain {}", group.getDisplayName(), domain.getName());
                ...
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof SCIMException) {
                                ...
                                return Single.error(
                                        new TechnicalManagementException(
                                                "An error occurs while trying to router a group",
                                                ex));
                            }
                        })
                .doOnError(
                ...
                .flatMapSingle(
                        existingGroup ->
                                groupRepository
                                        ...
                                        .flatMap(
                                                group1 -> {
                                                    io.gravitee.am.model.Group groupToUpdate =
                                                            convert(group1);
                                                    // set immutable attribute
                                                    groupToUpdate.setId(existingGroup.getId());
                                                    groupToUpdate.setReferenceType(
                                                            existingGroup.getReferenceType());
                                                    groupToUpdate.setReferenceId(
                                                            existingGroup.getReferenceId());
                                                    groupToUpdate.setCreatedAt(
                                                            existingGroup.getCreatedAt());
                                                    groupToUpdate.setUpdatedAt(new Date());
                                                    groupToUpdate.setRoles(
                                                            existingGroup.getRoles());
                                                    groupToUpdate.setDescription(
                                                            existingGroup.getDescription());
                ...
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException
                                    || ex instanceof SCIMException) {
                                return Single.error(ex);
                            } else {
                                LOGGER.error("An error occurs while trying to update a group", ex);
                                return Single.error(
                                        new TechnicalManagementException(
                                                "An error occurs while trying to update a group",
                                                ex));
                            }
                        });
    }
        ...
        return get(groupId, baseUrl)
                ...
                .flatMap(
                        group -> {
                            ...
                            return update(
                                    groupId,
                                    objectMapper.treeToValue(node, Group.class),
                                    baseUrl,
                                    principal);
                        })
                .onErrorResumeNext(
                        ex -> {
                            if (ex instanceof AbstractManagementException) {
                                ...
                                LOGGER.error(
                                        "An error has occurred when trying to delete group: {}",
                                        groupId,
                                        ex);
                                return Single.error(
                                        new TechnicalManagementException(
                                                String.format(
                                                        "An error has occurred when trying to delete group: %s",
                                                        groupId),
                                                ex));
                            }
                        });
    ...
    private Single<Group> setMembers(Group group, String baseUrl) {
        ...
        if (members != null && !members.isEmpty()) {
            List<String> memberIds =
                    group.getMembers().stream().map(Member::getValue).collect(Collectors.toList());
                                ...
                                String display =
                                        (user.getDisplayName() != null)
                                                ...
                                                : (user.getFirstName() != null)
                                                        ...
                                                        : user.getUsername();
                                String usersBaseUrl =
                                        baseUrl.substring(0, baseUrl.lastIndexOf("/Groups"))
                                                .concat("/Users");
