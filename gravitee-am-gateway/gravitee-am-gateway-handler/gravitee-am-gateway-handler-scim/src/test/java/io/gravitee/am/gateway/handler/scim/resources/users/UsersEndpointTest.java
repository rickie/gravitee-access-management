...
public class UsersEndpointTest extends RxWebTestBase {
    ...
    public void setUp() throws Exception {
        ...
        router.get("/Users").handler(usersEndpoint::list).failureHandler(new ErrorHandler());
    }

    @Test
    public void shouldListUsers() throws Exception {
        when(userService.list(eq(null), eq(0), eq(100), anyString()))
                .thenReturn(Single.just(new ListResponse<>()));
        testRequest(HttpMethod.GET, "/Users", req -> {}, 200, "OK", "UserObject");
    }
    ...
    public void shouldListUsers_validFilter() throws Exception {
        when(userService.list(any(Filter.class), eq(0), eq(100), anyString()))
                .thenReturn(Single.just(new ListResponse<>()));
        testRequest(
                HttpMethod.GET,
