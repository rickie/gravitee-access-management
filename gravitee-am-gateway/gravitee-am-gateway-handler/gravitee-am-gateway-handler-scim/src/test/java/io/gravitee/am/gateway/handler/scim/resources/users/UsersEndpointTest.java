...
public class UsersEndpointTest extends RxWebTestBase {
    ...
    public void shouldListUsers() throws Exception {
        when(userService.list_migrated(eq(null), eq(0), eq(100), anyString())).thenReturn(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.just(new ListResponse<>()))));
        testRequest(
                HttpMethod.GET,
                "/Users",
                req -> {},
    ...
    public void shouldListUsers_validFilter() throws Exception {
        when(userService.list_migrated(any(Filter.class), eq(0), eq(100), anyString())).thenReturn(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.just(new ListResponse<>()))));
        testRequest(
                HttpMethod.GET,
