...
public class CreateUserEndpointHandlerTest extends RxWebTestBase {
    ...
    private final UsersEndpoint usersEndpoint =
            new UsersEndpoint(domain, userService, objectMapper);
    ...
    public void shouldNotInvokeSCIMCreateUserEndpoint_invalid_password() throws Exception {
        ...
        when(userService.create(any(), eq(null), any(), any(), any()))
                ...
                .thenReturn(
                        Single.error(
                                new InvalidValueException(
                                        "User provider [unknown-idp] can not be found.")));
    ...
    public void shouldReturn409WhenUsernameAlreadyExists() throws Exception {
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create(any(), eq(null), any(), any(), any()))
                .thenReturn(Single.error(new UniquenessException("Username already exists")));
    ...
    public void shouldUseASelectedIdp() throws Exception {
        ...
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create(any(), eq("123456"), any(), any(), any()))
                .thenReturn(Single.just(getUser()));
    ...
    public void shouldAcceptCustomGraviteeUser() throws Exception {
        ...
        when(domain.getScim()).thenReturn(scimSettings);
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create(any(), any(), any(), any(), any()))
                .thenReturn(Single.just(getUser()));
