...
public class CreateUserEndpointHandlerTest extends RxWebTestBase {
    ...
    public void shouldNotInvokeSCIMCreateUserEndpoint_invalid_password() throws Exception {
        ...
        when(userService.create_migrated(any(), any())).thenReturn(Mono.error(new InvalidValueException("Field [password] is invalid")));
    ...
    public void shouldInvokeSCIMCreateUserEndpoint_valid_password() throws Exception {
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create_migrated(any(), any())).thenReturn(Mono.just(getUser()));
    ...
    public void shouldNotInvokeSCIMCreateUserEndpoint_invalid_identity_provider() throws Exception {
        ...
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create_migrated(any(), any())).thenReturn(Mono.error(new InvalidValueException("User provider [unknown-idp] can not be found.")));
    ...
    public void shouldNotInvokeSCIMCreateUserEndpoint_invalid_roles() throws Exception {
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create_migrated(any(), any())).thenReturn(Mono.error(new InvalidValueException("Role [role-1] can not be found.")));
    ...
    public void shouldReturn409WhenUsernameAlreadyExists() throws Exception {
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create_migrated(any(), any())).thenReturn(Mono.error(new UniquenessException("Username already exists")));
    ...
    public void shouldReturn400WhenInvalidUserException() throws Exception {
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create_migrated(any(), any())).thenReturn(Mono.error(new InvalidUserException("Invalid user infos")));
    ...
    public void shouldReturn400WhenEmailFormatInvalidException() throws Exception {
        router.route("/Users").handler(usersEndpoint::create);
        when(userService.create_migrated(any(), any())).thenReturn(Mono.error(new EmailFormatInvalidException("Invalid email")));
