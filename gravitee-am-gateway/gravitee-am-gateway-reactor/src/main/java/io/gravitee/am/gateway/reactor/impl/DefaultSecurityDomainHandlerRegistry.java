...
public class DefaultSecurityDomainHandlerRegistry implements SecurityDomainHandlerRegistry {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(DefaultSecurityDomainHandlerRegistry.class);
    private final ConcurrentMap<String, VertxSecurityDomainHandler> handlers =
            new ConcurrentHashMap<>();

    @Autowired private SecurityDomainRouterFactory securityDomainRouterFactory;
    ...
    public void create(Domain domain) {
        ...
        if (domain.isVhostMode()) {
            logger.info(
                    "Register a new domain [{}] on vhosts [{}]",
                    domain.getId(),
                    domain.getVhosts());
        } else {
            logger.info(
                    "Register a new domain [{}] on path [{}]", domain.getId(), domain.getPath());
        }
