...
/**
 * Domain router for a particular vhost. Vhost is optional, meaning routing is made only on domain
 * path. If vhost is defined, routing is made on both vhost and path (matching condition must be
 * true). If multiple vhosts have to be defined for the same domain, it is necessary to defined
 * multiple {@link VHostRouter}s.
 *
...
public class VHostRouter implements Router {
    ...
    public static io.vertx.reactivex.ext.web.Router router(
            Domain domain, VirtualHost vhost, io.vertx.reactivex.ext.web.Router delegate) {

        return io.vertx.reactivex.ext.web.Router.newInstance(
                new VHostRouter(domain, vhost, delegate));
    }

    public static io.vertx.reactivex.ext.web.Router router(
            Domain domain, io.vertx.reactivex.ext.web.Router delegate) {
