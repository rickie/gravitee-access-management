...
public class CertificatePluginManager
        extends ProviderPluginManager<
                Certificate, CertificateProvider, CertificateProviderConfiguration>
    ...
    public CertificateProvider create(CertificateProviderConfiguration providerConfig) {
        ...
        if (certificate != null) {
            Class<? extends CertificateConfiguration> configurationClass =
                    certificate.configuration();
            var certificateConfiguration =
                    certificateConfigurationFactory.create(
            ...
            return createProvider(
                    ...
                    List.of(
                            new CertificateConfigurationBeanFactoryPostProcessor(
                                    certificateConfiguration),
                            new CertificateMetadataBeanFactoryPostProcessor(certificateMetadata)));
        } else {
            logger.error(
                    "No certificate provider is registered for type {}", providerConfig.getType());
            throw new IllegalStateException(
                    "No certificate provider is registered for type " + providerConfig.getType());
        }
    }

    private static class CertificateMetadataBeanFactoryPostProcessor
            extends NamedBeanFactoryPostProcessor<CertificateMetadata> {
        private CertificateMetadataBeanFactoryPostProcessor(CertificateMetadata metadata) {
            super("metadata", metadata);
        }
    }

    private static class CertificateConfigurationBeanFactoryPostProcessor
            extends NamedBeanFactoryPostProcessor<CertificateConfiguration> {
        private CertificateConfigurationBeanFactoryPostProcessor(
