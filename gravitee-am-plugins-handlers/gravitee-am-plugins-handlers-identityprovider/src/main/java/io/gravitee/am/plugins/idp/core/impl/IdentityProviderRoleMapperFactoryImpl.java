...
public class IdentityProviderRoleMapperFactoryImpl implements IdentityProviderRoleMapperFactory {
    ...
    private final Logger logger =
            LoggerFactory.getLogger(IdentityProviderRoleMapperFactoryImpl.class);

    @Override
    public <T extends IdentityProviderRoleMapper> T create(
            Class<T> clazz, Map<String, String[]> roles) {
        if (clazz != null) {
            logger.debug(
                    "Create a new instance of identity provider roles for class: {}",
                    clazz.getName());
            try {
