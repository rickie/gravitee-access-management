    ...
    public BotDetectionProvider create(ProviderConfiguration providerConfiguration) {
        ...
        if (botDetection != null) {
            ...
            var botDetectionConfiguration =
                    botDetectionConfigurationFactory.create(
            ...
            return createProvider(
                    ...
                    List.of(
                            new BotDetectionConfigurationBeanFactoryPostProcessor(
                                    botDetectionConfiguration)));
        } else {
            logger.error(
                    "No bot detection is registered for type {}", providerConfiguration.getType());
            throw new IllegalStateException(
                    "No bot detection is registered for type " + providerConfiguration.getType());
        }
    }

    private static class BotDetectionConfigurationBeanFactoryPostProcessor
            extends NamedBeanFactoryPostProcessor<BotDetectionConfiguration> {
