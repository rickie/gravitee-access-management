...
public class AuthenticationDeviceNotifierPluginManager
        extends ProviderPluginManager<
                ...
                AuthenticationDeviceNotifierProvider,
                ProviderConfiguration>
        implements AmPluginManager<AuthenticationDeviceNotifier> {

    private static final Logger logger =
            LoggerFactory.getLogger(AuthenticationDeviceNotifierPluginManager.class);
    private final ConfigurationFactory<AuthenticationDeviceNotifierConfiguration>
            authDeviceNotifierConfigurationFactory;

    public AuthenticationDeviceNotifierPluginManager(
            PluginContextFactory pluginContextFactory,
            ConfigurationFactory<AuthenticationDeviceNotifierConfiguration>
                    authDeviceNotifierConfigurationFactory) {
    ...
    public AuthenticationDeviceNotifierProvider create(ProviderConfiguration providerConfig) {
        logger.debug(
                "Looking for an authentication device notifier for [{}]", providerConfig.getType());
        ...
        if (authDeviceNotifier != null) {
            ...
            var botDetectionConfiguration =
                    authDeviceNotifierConfigurationFactory.create(
            ...
            return createProvider(
                    ...
                    List.of(
                            new AuthenticationDeviceNotifierConfigurationBeanFactoryPostProcessor(
                                    botDetectionConfiguration)));
        } else {
            logger.error(
                    "No authentication device notifier is registered for type {}",
    ...
    private static class AuthenticationDeviceNotifierConfigurationBeanFactoryPostProcessor
        ...
        private AuthenticationDeviceNotifierConfigurationBeanFactoryPostProcessor(
                AuthenticationDeviceNotifierConfiguration configuration) {
