...
public class ReporterPluginManager
        extends ProviderPluginManager<
                Reporter,
                io.gravitee.am.reporter.api.provider.Reporter,
                ReporterProviderConfiguration>
    ...
    @Override
    public io.gravitee.am.reporter.api.provider.Reporter create(
            ReporterProviderConfiguration providerConfiguration) {
        ...
        if (reporter != null) {
            ...
            var reporterConfiguration =
                    reporterConfigurationFactory.create(
                            configurationClass, providerConfiguration.getConfiguration());
            if (providerConfiguration.getGraviteeContext() != null) {
                return createProvider(
                        plugins.get(reporter),
                        reporter.auditReporter(),
                        List.of(
                                new ReporterConfigurationBeanFactoryPostProcessor(
                                        reporterConfiguration),
                                new GraviteeContextBeanFactoryPostProcessor(
                                        providerConfiguration.getGraviteeContext())));
            }
            return createProvider(
                    plugins.get(reporter),
                    reporter.auditReporter(),
            ...
            logger.error(
                    "No reporter provider is registered for type {}",
    ...
    private static class GraviteeContextBeanFactoryPostProcessor
            extends NamedBeanFactoryPostProcessor<GraviteeContext> {
        private GraviteeContextBeanFactoryPostProcessor(GraviteeContext context) {
            super("graviteeContext", context);
        }
    }

    private static class ReporterConfigurationBeanFactoryPostProcessor
            extends NamedBeanFactoryPostProcessor<ReporterConfiguration> {
