...
public class PolicyPluginManagerImpl implements PolicyPluginManager {
    ...
    @Autowired private ConfigurablePluginManager<PolicyPlugin> pluginManager;

    @Autowired private ConfigurationFactory<PolicyConfiguration> policyConfigurationFactory;

    @Autowired private PluginClassLoaderFactory pluginClassLoaderFactory;
        ...
        if (policyPlugin != null) {
            try {
                ...
                PluginClassLoader pluginClassLoader =
                        pluginClassLoaderFactory.getOrCreateClassLoader(policyPlugin);
                Class<? extends PolicyConfiguration> configurationClass =
                        (Class<? extends PolicyConfiguration>)
                                ClassUtils.forName(
                                        policyPlugin.configuration().getName(), pluginClassLoader);
                var policyConfiguration =
                        policyConfigurationFactory.create(configurationClass, configuration);
                ...
                // Prepare metadata
                Class<?> policyClass =
                        ClassUtils.forName(policyPlugin.policy().getName(), pluginClassLoader);
                ...
                if (policyPlugin.context() != null) {
                    Class<? extends PolicyContext> policyContextClass =
                            (Class<? extends PolicyContext>)
                                    ClassUtils.forName(
                                            policyPlugin.context().getName(), pluginClassLoader);
                    ...
                    if (context instanceof PolicyContextProviderAware) {
                        ((PolicyContextProviderAware) context)
                                .setPolicyContextProvider(
                ...
                if (constr != null) {
                    try {
                        ...
                        if (policyInst != null) {
                            return PolicyImpl.target(policyInst)
                                    .condition(condition)
    ...
    private Constructor<?> lookingForConstructor(Class<?> policyClass) {
        ...
        if (constructor == null) {
            ...
            if (policyConstructors.isEmpty()) {
                ...
                logger.info(
                        "Too much constructors to instantiate policy {}", policyClass.getName());
            }
