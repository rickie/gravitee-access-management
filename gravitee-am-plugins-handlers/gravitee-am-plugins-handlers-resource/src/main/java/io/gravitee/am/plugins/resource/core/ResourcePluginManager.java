    ...
    public ResourceProvider create(ProviderConfiguration providerConfiguration) {
        ...
        if (resource != null) {
            ...
            ResourceConfiguration resourceConfiguration =
                    resourceConfigurationFactory.create(
                            configurationClass, providerConfiguration.getConfiguration());
            return createProvider(
                    plugins.get(resource),
                    resource.resourceProvider(),
                    List.of(
                            new ResourceConfigurationBeanFactoryPostProcessor(
                                    resourceConfiguration)));
        } else {
            logger.error("No resource is registered for type {}", providerConfiguration.getType());
            throw new IllegalStateException(
                    "No resource is registered for type " + providerConfiguration.getType());
        }
    }

    private class ResourceConfigurationBeanFactoryPostProcessor
            extends NamedBeanFactoryPostProcessor<ResourceConfiguration> {
