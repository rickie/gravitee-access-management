...
public class JdbcAuditReporter extends AbstractService implements AuditReporter, InitializingBean {
    ...
    private static final List<String> auditColumns =
            List.of(
                    COL_ID,
                    COL_TRANSACTION_ID,
                    COL_TYPE,
    ...
    private static final List<String> entityColumns =
            List.of(
                    COL_AUDIT_ID,
                    COL_AUDIT_FIELD,
                    COL_ID,
    ...
    private static final List<String> outcomesColumns =
            List.of(COL_AUDIT_ID, COL_STATUS, COL_MESSAGE);

    private static final List<String> accessPointColumns =
            List.of(
                    COL_AUDIT_ID,
                    COL_ID,
                    COL_ALTERNATIVE_ID,
    ...
    @Autowired private ReactiveTransactionManager tm;

    @Autowired private DialectHelper dialectHelper;

    @Autowired private ConnectionFactory connectionFactory;

    @Autowired protected MappingR2dbcConverter rowMapper;
    ...
    public Single<Page<Audit>> search(
        ...
        SearchQuery searchQuery =
                dialectHelper.buildSearchQuery(referenceType, referenceId, criteria);

        DatabaseClient.GenericExecuteSpec query =
                template.getDatabaseClient()
                        .sql(searchQuery.getQuery() + dialectHelper.buildPagingClause(page, size));
        DatabaseClient.GenericExecuteSpec count =
                template.getDatabaseClient().sql(searchQuery.getCount());
        ...
        return fluxToFlowable(
                        query.map(row -> rowMapper.read(AuditJdbc.class, row))
                                .all()
                                .map(this::convert)
                ...
                .flatMap(
                        content ->
                                monoToSingle(total)
                                        .map(value -> new Page<Audit>(content, page, value)))
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve reports for referenceType {} and referenceId {}",
                                        referenceType,
                                        referenceId,
                                        error));
    }
    ...
    public Single<Map<Object, Object>> aggregate(
            ...
            Type analyticsType) {
        LOGGER.debug(
                "aggregate on ({}, {}) with type {}", referenceType, referenceType, analyticsType);

        switch (analyticsType) {
            case DATE_HISTO:
                return executeHistogramAggregation(referenceType, referenceId, criteria);
            case GROUP_BY:
                SearchQuery groupByQuery =
                        dialectHelper.buildGroupByQuery(referenceType, referenceId, criteria);
    ...
    protected Single<Map<Object, Object>> executeHistogramAggregation(
        ...
        return dialectHelper
                .buildAndProcessHistogram(
                        template.getDatabaseClient(), referenceType, referenceId, criteria)
                .map(
                        stats -> {
                            ...
                            stats.forEach(
                                    slotValue -> {
                                        Long timestamp =
                                                ((Number) slotValue.get("slot")).longValue();
                                        Long attempts =
                                                ((Number) slotValue.get("attempts")).longValue();
                                        if (((String) slotValue.get(COL_STATUS))
    ...
    private Single<Map<Object, Object>> executeCount(SearchQuery searchQuery) {
        ...
        return monoToSingle(
                        count.map(row -> row.get(0, Long.class))
                                .first()
    ...
    private Single<Map<Object, Object>> executeGroupBy(
            SearchQuery searchQuery, AuditReportableCriteria criteria) {
        if (!ready) {
            LOGGER.debug("Reporter not yet bootstrapped");
            return Single.just(Collections.emptyMap());
        }
        DatabaseClient.GenericExecuteSpec groupBy =
                template.getDatabaseClient().sql(searchQuery.getQuery());
        for (Map.Entry<String, Object> bind : searchQuery.getBindings().entrySet()) {
            groupBy = groupBy.bind(bind.getKey(), bind.getValue());
        }
        return monoToSingle(
                groupBy.map(row -> Map.of(row.get(convertFieldName(criteria)), row.get("counter")))
                        .all()
                        .reduce(
                                new HashMap<>(),
                                (acc, value) -> {
    ...
    private String convertFieldName(AuditReportableCriteria criteria) {
        if (criteria.field().contains(".")) {
            return CaseFormat.UPPER_CAMEL.to(
                    CaseFormat.LOWER_UNDERSCORE, criteria.field().split("\\.")[1]);
        } else {
    ...
    public Maybe<Audit> findById(ReferenceType referenceType, String referenceId, String id) {
        ...
        Mono<Audit> auditMono =
                template.select(AuditJdbc.class)
                        .from(this.auditsTable)
                        .matching(
                                Query.query(
        ...
        return monoToMaybe(auditMono)
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve the Report with referenceType {}, referenceId {} and id {}",
                                        referenceType,
                                        referenceId,
                                        id,
                                        error));
    ...
    private Mono<Audit> fillWithActor(Audit audit) {
        return template.select(AuditEntityJdbc.class)
                .from(this.auditEntitiesTable)
                .matching(
                        Query.query(
                                where(COL_AUDIT_ID)
                                        .is(audit.getId())
    ...
    private Mono<Audit> fillWithAccessPoint(Audit audit) {
        return template.select(AuditAccessPointJdbc.class)
                .from(this.auditAccessPointsTable)
                .matching(Query.query(where(COL_AUDIT_ID).is(audit.getId())))
                .first()
                .map(entity -> fillWith(audit, entity))
                .switchIfEmpty(Mono.just(audit));
    }
    ...
    private Mono<Audit> fillWithOutcomes(Audit audit) {
        return template.select(AuditOutcomeJdbc.class)
                .from(auditOutcomesTable)
                .matching(Query.query(where(COL_AUDIT_ID).is(audit.getId())))
                .first()
                .map(entity -> fillWith(audit, entity))
                .switchIfEmpty(Mono.just(audit));
        ...
        audit.setTimestamp(
                entity.getTimestamp() == null
                        ...
                        : Instant.ofEpochSecond(
                                entity.getTimestamp().toEpochSecond(ZoneOffset.UTC)));
    ...
    private Audit fillWith(Audit audit, AuditEntityJdbc entity) {
        ...
        auditEntity.setReferenceType(
                entity.getReferenceType() == null
                        ? null
                        : ReferenceType.valueOf(entity.getReferenceType()));
        auditEntity.setAttributes(
                JSONMapper.toCollectionOfBean(
                        entity.getAttributes(), new TypeReference<Map<String, Object>>() {}));
    ...
    private Mono insertReport(Audit audit) {
        ...
        DatabaseClient.GenericExecuteSpec insertSpec =
                template.getDatabaseClient().sql(INSERT_AUDIT_STATEMENT);
        insertSpec = addQuotedField(insertSpec, COL_ID, audit.getId(), String.class);
        insertSpec =
                addQuotedField(
                        insertSpec, COL_TRANSACTION_ID, audit.getTransactionId(), String.class);
        ...
        insertSpec =
                addQuotedField(
                        ...
                        String.class);
        insertSpec =
                addQuotedField(insertSpec, COL_REFERENCE_ID, audit.getReferenceId(), String.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_TIMESTAMP,
                        LocalDateTime.ofInstant(
                                audit.timestamp(), ZoneId.of(ZoneOffset.UTC.getId())),
                        LocalDateTime.class);
        ...
        if (target != null) {
            insertAction =
                    insertAction.then(prepateInsertEntity(audit, target, AUDIT_FIELD_TARGET));
        }

        AuditOutcome outcome = audit.getOutcome();
        if (outcome != null) {
            DatabaseClient.GenericExecuteSpec insertOutcomeSpec =
                    template.getDatabaseClient().sql(INSERT_OUTCOMES_STATEMENT);

            insertOutcomeSpec =
                    addQuotedField(insertOutcomeSpec, COL_AUDIT_ID, audit.getId(), String.class);
            insertOutcomeSpec =
                    addQuotedField(
                            insertOutcomeSpec, COL_STATUS, outcome.getStatus(), String.class);
            insertOutcomeSpec =
                    addQuotedField(
                            insertOutcomeSpec, COL_MESSAGE, outcome.getMessage(), String.class);
        ...
        if (accessPoint != null) {
            DatabaseClient.GenericExecuteSpec insertAccessPointSpec =
                    template.getDatabaseClient().sql(INSERT_ACCESSPOINT_STATEMENT);

            insertAccessPointSpec =
                    addQuotedField(
                            insertAccessPointSpec, COL_AUDIT_ID, audit.getId(), String.class);
            insertAccessPointSpec =
                    addQuotedField(
                            insertAccessPointSpec, COL_ID, accessPoint.getId(), String.class);
            insertAccessPointSpec =
                    addQuotedField(
                            insertAccessPointSpec,
                            COL_ALTERNATIVE_ID,
                            accessPoint.getAlternativeId(),
                            String.class);
            insertAccessPointSpec =
                    addQuotedField(
                            insertAccessPointSpec,
                            COL_DISPLAY_NAME,
                            accessPoint.getDisplayName(),
                            String.class);
            insertAccessPointSpec =
                    addQuotedField(
                            insertAccessPointSpec,
    ...
    private Mono<Integer> prepateInsertEntity(Audit audit, AuditEntity entity, String field) {
        DatabaseClient.GenericExecuteSpec insertEntitySpec =
                template.getDatabaseClient().sql(INSERT_ENTITY_STATEMENT);
        insertEntitySpec =
                addQuotedField(insertEntitySpec, COL_AUDIT_ID, audit.getId(), String.class);
        ...
        insertEntitySpec =
                addQuotedField(
                        ...
                        String.class);
        insertEntitySpec =
                addQuotedField(insertEntitySpec, COL_TYPE, entity.getType(), String.class);
        insertEntitySpec =
                addQuotedField(
                        insertEntitySpec, COL_DISPLAY_NAME, entity.getDisplayName(), String.class);
        insertEntitySpec =
                addQuotedField(
                        insertEntitySpec,
                        COL_REFERENCE_TYPE,
                        entity.getReferenceType() == null ? null : entity.getReferenceType().name(),
                        String.class);
        insertEntitySpec =
                addQuotedField(
                        insertEntitySpec, COL_REFERENCE_ID, entity.getReferenceId(), String.class);
        insertEntitySpec =
                addQuotedField(
                        insertEntitySpec,
                        COL_ATTRIBUTES,
                        JSONMapper.toJson(entity.getAttributes()),
                        String.class);
        return insertEntitySpec.fetch().rowsUpdated();
    }

    protected <T> DatabaseClient.GenericExecuteSpec addQuotedField(
            DatabaseClient.GenericExecuteSpec spec, String name, Object value, Class<T> type) {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        auditAccessPointsTable =
                pattern.matcher("reporter_audits_access_points___").replaceAll(tableSuffix);
        ...
        this.INSERT_ENTITY_STATEMENT =
                createInsertStatement(this.auditEntitiesTable, entityColumns);
        this.INSERT_OUTCOMES_STATEMENT =
                createInsertStatement(this.auditOutcomesTable, outcomesColumns);
        this.INSERT_ACCESSPOINT_STATEMENT =
                createInsertStatement(this.auditAccessPointsTable, accessPointColumns);
        ...
        if (environment.getProperty(REPORTER_AUTO_PROVISIONING, Boolean.class, true)) {
            ...
            Function<Connection, Mono<Integer>> resultFunction =
                    connection -> {
                        Statement doesTableExist =
                                connection.createStatement(dialectHelper.tableExists(auditsTable));
                        return flowableToFlux(
                                        Flowable.fromPublisher(doesTableExist.execute())
                                                ...
                                                .flatMapPublisher(
                                                        total -> {
                                                            if (total == 0) {
                                                                LOGGER.debug(
                                                                        "SQL datatable {} doest not exists, initialize all audit tables for the reporter.",
                                                                        auditsTable);
                                                                try (InputStream input =
                                                                    ...
                                                                    LOGGER.error(
                                                                            "Unable to initialize the reporter schema",
                                                                            e);
            ...
            disposable =
                    bulkProcessor
                            ...
                            .doOnError(
                                    error ->
                                            LOGGER.error(
                                                    "An error occurs while indexing data into report_audits_{} table of {} database",
    ...
    protected void doStop() throws Exception {
        ...
        try {
            ...
            if (this.connectionFactory != null
                    && this.connectionFactory instanceof ConnectionPool) {
