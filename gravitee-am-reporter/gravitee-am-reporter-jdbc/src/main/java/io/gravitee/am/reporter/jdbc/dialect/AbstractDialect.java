...
public abstract class AbstractDialect implements DialectHelper {
    ...
    @Override
    public SearchQuery buildGroupByQuery(
        ...
        String field =
                "a."
                        + CaseFormat.UPPER_CAMEL.to(
                                ...
                                criteria.field()); // default to audits table
    ...
    protected String groupByConcatQueryParts(
        ...
        String query =
                "SELECT "
                        ...
                        + " "
                        + " LIMIT "
                        + (criteria.size() == null ? 50 : +criteria.size())
                        + " ";
        return query;
    }
    ...
    public SearchQuery buildSearchQuery(
        ...
        String query =
                "SELECT DISTINCT a.* " + queryBuilder.toString() + whereClauseBuilder.toString();
        String count =
    ...
    protected void processQuery(
            ...
            boolean joinBasedOnField,
            boolean joinOnOutcomes) {
        ...
        whereClauseBuilder =
                whereClauseBuilder.append(
                        " WHERE a.reference_type = :refType AND a.reference_id = :refId");
        ...
        if (criteria.status() != null && !criteria.status().isEmpty()) {
            queryBuilder =
                    queryBuilder.append(
                            " INNER JOIN " + auditOutcomesTable + " o ON a.id = o.audit_id ");
            outcomeJoin = true;
        ...
        if (criteria.user() != null && !criteria.user().isEmpty()) {
            queryBuilder =
                    queryBuilder.append(
                            " INNER JOIN " + auditEntitiesTable + " e ON a.id = e.audit_id ");
            entitiesJoin = true;
        ...
        if (criteria.from() != 0 && criteria.to() != 0) {
            ...
            bindings.put(
                    ...
                    LocalDateTime.ofInstant(
                            Instant.ofEpochMilli(criteria.from()),
                            ZoneId.of(ZoneOffset.UTC.getId())));
            bindings.put(
                    "to",
                    LocalDateTime.ofInstant(
                            Instant.ofEpochMilli(criteria.to()),
                            ZoneId.of(ZoneOffset.UTC.getId())));
        ...
        if (criteria.accessPointId() != null && !criteria.accessPointId().isEmpty()) {
            queryBuilder =
                    queryBuilder.append(
                            " INNER JOIN " + auditAccessPointsTable + " p ON a.id = p.audit_id ");
            entitiesJoin = true;
    ...
    @Override
    public Single<List<Map<String, Object>>> buildAndProcessHistogram(
