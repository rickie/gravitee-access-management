    ...
    public SearchQuery buildHistogramQuery(
        ...
        String startDate =
                dateTimeFormatter.format(
                        LocalDateTime.ofInstant(
                                Instant.ofEpochMilli(criteria.from()).truncatedTo(unit),
                                ZoneId.of(ZoneOffset.UTC.getId())));
        String endDate =
                dateTimeFormatter.format(
                        LocalDateTime.ofInstant(
                                Instant.ofEpochMilli(criteria.to()).truncatedTo(unit),
                                ZoneId.of(ZoneOffset.UTC.getId())));
        ...
        String query =
                "WITH time_buckets AS (\n"
                        ...
                        + "                (bucket + '"
                        + criteria.interval()
                        + " milliseconds'::interval) AS endDate\n"
