...
public class MySQLDialect extends AbstractDialect {
    ...
    public Single<List<Map<String, Object>>> buildAndProcessHistogram(
        ...
        return fluxToFlowable(
                        Flux.fromIterable(intervals.keySet())
                                .flatMap(
                                        slot -> {
                                            String beginSlot =
                                                    dateTimeFormatter.format(
                                                            LocalDateTime.ofInstant(
                                                                    Instant.ofEpochMilli(slot),
                                                                    ZoneId.of(
                                                                            ZoneOffset.UTC
                                                                                    .getId())));
                                            String endSlot =
                                            ...
                                            String query =
                                                    " SELECT "
                                                            + slot
                                                            + " as slot, o.status, COUNT(o.status) as attempts "
                                                            + queryBuilder
                                                            + whereClauseBuilder
                                                            + " AND '"
                                                            + beginSlot
                                            ...
                                            for (Map.Entry<String, Object> bind :
                                                    bindings.entrySet()) {
