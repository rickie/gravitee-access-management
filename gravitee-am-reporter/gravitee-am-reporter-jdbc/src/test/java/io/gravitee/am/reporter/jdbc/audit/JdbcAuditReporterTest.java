...
@ContextConfiguration(
        classes = {DatabaseUrlProvider.class, JdbcReporterJUnitConfiguration.class},
...
public class JdbcAuditReporterTest {
    ...
    public void testReporter_aggregationHistogram() {
        ...
        for (int i = 0; i < loop; ++i) {
            Audit reportable =
                    buildRandomAudit(ReferenceType.DOMAIN, "testReporter_aggregationHistogram");
        ...
        TestObserver<Map<Object, Object>> test =
                auditReporter
                        .aggregate(
                                ...
                                "testReporter_aggregationHistogram",
                                criteria,
                                Type.DATE_HISTO)
                        .test();
        ...
        test.assertValue(
                map ->
                        map.get(failureKey) != null
                                && ((List<Long>) map.get(failureKey))
                                                .stream().distinct().filter(i -> i != 0).count()
                                        == 1);
        test.assertValue(
                map ->
                        map.get(failureKey) != null
        ...
        test.assertValue(
                map ->
                        map.get(successKey) != null
                                && ((List<Long>) map.get(successKey))
                                                .stream().distinct().filter(i -> i != 0).count()
                                        == 1);
        test.assertValue(
                map ->
    ...
    public void testReporter_aggregationGroupBy() {
        ...
        for (int i = 0; i < loop; ++i) {
            Audit reportable =
                    buildRandomAudit(ReferenceType.DOMAIN, "testReporter_aggregationGroupBy");
            if (i % 2 == 0) {
        ...
        AuditReportableCriteria criteria =
                new AuditReportableCriteria.Builder().user(MY_USER).field("outcome.status").build();
        TestObserver<Map<Object, Object>> test =
                auditReporter
                        .aggregate(
                                ReferenceType.DOMAIN,
                                "testReporter_aggregationGroupBy",
                                criteria,
                                Type.GROUP_BY)
                        .test();
        test.awaitTerminalEvent();
        ...
        test.assertValue(
                map -> map.size() == ((expectedFailure > 0 && expectedSuccess > 0) ? 2 : 1));
        ...
        test.assertValue(
                map ->
                        expectedFailure == 0
                                || ((Number) map.get("FAILURE")).intValue() == expectedFailure);
        test.assertValue(
                map ->
                        expectedSuccess == 0
                                || ((Number) map.get("SUCCESS")).intValue() == expectedSuccess);
    }
    ...
    public void testReporter_aggregationCount() {
        ...
        for (int i = 0; i < loop; ++i) {
            Audit reportable =
                    buildRandomAudit(ReferenceType.DOMAIN, "testReporter_aggregationCount");
            if (i % 2 == 0) {
                if (random.nextBoolean()) {
                    reportable.getActor().setAlternativeId(MY_USER);
                } else {
        ...
        AuditReportableCriteria criteria =
                new AuditReportableCriteria.Builder().user(MY_USER).build();
        TestObserver<Map<Object, Object>> test =
                auditReporter
                        .aggregate(
                                ReferenceType.DOMAIN,
                                "testReporter_aggregationCount",
                                criteria,
                                Type.COUNT)
                        .test();
        test.awaitTerminalEvent();
        ...
        test.assertValue(
                map ->
                        map.get("data") != null
                                && ((Long) map.get("data")).intValue() == expectedResult);
    }
    ...
    public void testReporter_aggregationCount_accessPointId() {
        ...
        AuditReportableCriteria criteria =
                new AuditReportableCriteria.Builder().accessPointId(accessPointId).build();
        TestObserver<Map<Object, Object>> test =
                auditReporter
                        .aggregate(
                                ReferenceType.DOMAIN,
    ...
    public void testReporter_searchUser() {
        ...
        AuditReportableCriteria criteria =
                new AuditReportableCriteria.Builder().user(MY_USER).build();
        TestObserver<Page<Audit>> test =
                auditReporter
                        .search(ReferenceType.DOMAIN, "testReporter_searchUser", criteria, 0, 20)
                        .test();
        test.awaitTerminalEvent();
        test.assertNoErrors();
        int expectedResult = acc;
        ...
        test.assertValue(
                page ->
                        page.getData().stream().map(Audit::getId).distinct().count()
                                == expectedResult);
    }
    ...
    public void testReporter_searchTypes() {
        ...
        AuditReportableCriteria criteria =
                new AuditReportableCriteria.Builder().types(types).build();
        TestObserver<Page<Audit>> test =
                auditReporter
                        .search(ReferenceType.DOMAIN, "testReporter_searchUser", criteria, 0, 20)
                        .test();
    ...
    public void testReporter_search() {
        ...
        TestObserver<Page<Audit>> test =
                auditReporter
                        .search(
                                ReferenceType.DOMAIN,
                                "testReporter_search",
                                new AuditReportableCriteria.Builder().build(),
                                0,
                                20)
                        .test();
        test.awaitTerminalEvent();
        ...
        test.assertValue(
                page -> page.getData().stream().map(Audit::getId).distinct().count() == loop);
    ...
    public void testReporter_findById() {
        ...
        TestObserver<Audit> test =
                auditReporter
                        .findById(audit.getReferenceType(), audit.getReferenceId(), audit.getId())
                        .test();
        test.awaitTerminalEvent();
        test.assertNoErrors();
    ...
    protected void assertReportEqualsTo(Audit audit, TestObserver<Audit> test) {
        ...
        test.assertValue(
                a -> a.getTarget().getAlternativeId().equals(audit.getTarget().getAlternativeId()));

        test.assertValue(a -> a.getActor() != null);
        test.assertValue(
                a -> a.getActor().getAlternativeId().equals(audit.getActor().getAlternativeId()));
