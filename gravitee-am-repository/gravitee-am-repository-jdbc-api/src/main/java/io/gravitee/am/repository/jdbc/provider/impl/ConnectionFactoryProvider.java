...
public class ConnectionFactoryProvider {
    ...
    public static ConnectionFactory createClient(R2DBCConnectionConfiguration configuration) {
        LOGGER.info(
                "Initializing connection pool for database server {} on host {}",
        ...
        builder.option(PoolingConnectionFactoryProvider.ACQUIRE_RETRY, 1)
                ...
                .option(
                        ...
                        Duration.of(30000l, ChronoUnit.MILLIS))
                .option(
                        PoolingConnectionFactoryProvider.MAX_LIFE_TIME,
                        Duration.of(30000l, ChronoUnit.MILLIS))
                .option(
                        PoolingConnectionFactoryProvider.MAX_ACQUIRE_TIME,
                        Duration.of(0, ChronoUnit.MILLIS))
                .option(
                        PoolingConnectionFactoryProvider.MAX_CREATE_CONNECTION_TIME,
                        Duration.of(0, ChronoUnit.MILLIS))
        ...
        LOGGER.info(
                "Connection pool created for database server {} on host {}",
                configuration.getProtocol(),
                configuration.getHost());
        ...
        new R2DBCConnectionMetrics(Metrics.getDefaultRegistry(), tags).register(connectionPool);
    ...
    public ConnectionFactory factory() {
        ...
        if (uri != null) {
            ...
            if (driver == null || host == null) {
                LOGGER.error(
                        "Missing one of connection parameters 'driver', 'host' or 'port' for {} database",
                        prefix);
                throw new IllegalArgumentException(
                        "Missing properties for '" + prefix + "' database");
            }
            ...
            ConnectionFactoryOptions.Builder builder =
                    ConnectionFactoryOptions.builder()
                            ...
                            .option(DATABASE, db)
                            .option(
                                    PoolingConnectionFactoryProvider.ACQUIRE_RETRY,
                                    Integer.parseInt(
                                            environment.getProperty(prefix + "acquireRetry", "1")))
                            .option(
                                    PoolingConnectionFactoryProvider.INITIAL_SIZE,
                                    Integer.parseInt(
                                            environment.getProperty(prefix + "initialSize", "0")))
                            .option(
                                    PoolingConnectionFactoryProvider.MAX_SIZE,
                                    Integer.parseInt(
                                            environment.getProperty(prefix + "maxSize", "10")))
                            .option(
                                    PoolingConnectionFactoryProvider.MAX_IDLE_TIME,
                                    Duration.of(
                                            Long.parseLong(
                                                    environment.getProperty(
                                                            prefix + "maxIdleTime", "30000")),
                                            ChronoUnit.MILLIS))
                            .option(
                                    PoolingConnectionFactoryProvider.MAX_LIFE_TIME,
                                    Duration.of(
                            ...
                            .option(
                                    ...
                                    Duration.of(
                                            Long.parseLong(
                                                    environment.getProperty(
                                                            prefix + "maxCreateConnectionTime",
                                                            "0")),
                                            ChronoUnit.MILLIS));
            ...
            if (validationQuery != null) {
                builder.option(PoolingConnectionFactoryProvider.VALIDATION_QUERY, validationQuery)
                        .option(
                                PoolingConnectionFactoryProvider.VALIDATION_DEPTH,
                                ValidationDepth.REMOTE);
            } else {
                builder.option(
                        PoolingConnectionFactoryProvider.VALIDATION_DEPTH, ValidationDepth.LOCAL);
            }
        ...
        if (connectionPool instanceof ConnectionPool) {
            Tags tags =
                    Tags.of(
                            ...
                            Tag.of(
                                    TAG_SERVER,
                                    Strings.isNullOrEmpty(getJdbcPort())
                                            ? getJdbcHostname()
                                            : getJdbcHostname() + ":" + getJdbcPort()));
