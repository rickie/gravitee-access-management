 ...
 */
@Repository
public interface SpringApplicationRepository
        extends RxJava2CrudRepository<JdbcApplication, String> {
    ...
    @Query(
            "SELECT a.* FROM applications a INNER JOIN application_factors f ON a.id = f.application_id where f.factor = :factor")
    Flowable<JdbcApplication> findAllByFactor(@Param("factor") String factor);

    @Query(
            "SELECT a.* FROM applications a INNER JOIN application_grants g ON a.id = g.application_id where g.grant_type = :grant and a.domain = :domain")
    Flowable<JdbcApplication> findAllByDomainAndGrant(
            @Param("domain") String domain, @Param("grant") String grant);
