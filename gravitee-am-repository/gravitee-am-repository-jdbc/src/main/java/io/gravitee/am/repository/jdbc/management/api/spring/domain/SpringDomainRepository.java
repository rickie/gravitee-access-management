...
public interface SpringDomainRepository extends RxJava2CrudRepository<JdbcDomain, String> {
    ...
    Flowable<JdbcDomain> findAllByReferenceId(
            @Param("refId") String refId, @Param("refType") String refType);

    @Query(
            "select * from domains d where d.reference_id = :refId and d.reference_type = :refType and d.hrid = :hrid")
    Maybe<JdbcDomain> findByHrid(
