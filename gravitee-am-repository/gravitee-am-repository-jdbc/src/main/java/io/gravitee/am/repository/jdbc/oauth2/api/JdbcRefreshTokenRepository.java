 ...
 */
@Repository
public class JdbcRefreshTokenRepository extends AbstractJdbcRepository
        implements RefreshTokenRepository {

    @Autowired private SpringRefreshTokenRepository refreshTokenRepository;
    ...
    public Single<RefreshToken> create(RefreshToken refreshToken) {
        refreshToken.setId(
                refreshToken.getId() == null ? RandomString.generate() : refreshToken.getId());
        LOGGER.debug("Create refreshToken with id {}", refreshToken.getId());
        return monoToSingle(template.insert(toJdbcEntity(refreshToken)))
                .map(this::toEntity)
                .doOnError(
                        (error) ->
                                LOGGER.error(
                                        "Unable to create refreshToken with id {}",
                                        refreshToken.getId(),
                                        error));
    }
    ...
    public Completable delete(String token) {
        ...
        return monoToCompletable(
                        template.delete(JdbcRefreshToken.class)
                                .matching(Query.query(where("token").is(token)))
                                .all())
    ...
    public Completable deleteByUserId(String userId) {
        ...
        return monoToCompletable(
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete refresh token with subject {}",
                                        userId,
                                        error));
    }
    ...
    public Completable deleteByDomainIdClientIdAndUserId(
        ...
        return monoToCompletable(
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete refresh token with domain {}, client {} and subject {}",
                                        domainId,
                                        clientId,
                                        userId,
                                        error));
    ...
    public Completable deleteByDomainIdAndUserId(String domainId, String userId) {
        ...
        return monoToCompletable(
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete refresh token with domain {} and subject {}",
                                        domainId,
                                        userId,
                                        error));
    }
