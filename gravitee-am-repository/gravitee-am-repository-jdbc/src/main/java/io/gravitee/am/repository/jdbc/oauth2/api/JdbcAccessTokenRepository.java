 ...
 */
@Repository
public class JdbcAccessTokenRepository extends AbstractJdbcRepository
        implements AccessTokenRepository {
    @Autowired private SpringAccessTokenRepository accessTokenRepository;
    ...
    public Maybe<AccessToken> findByToken(String token) {
        ...
        return accessTokenRepository
                .findByToken(token, LocalDateTime.now(UTC))
                .map(this::toEntity)
    ...
    public Single<AccessToken> create(AccessToken accessToken) {
        accessToken.setId(
                accessToken.getId() == null ? RandomString.generate() : accessToken.getId());
        LOGGER.debug("Create accessToken with id {}", accessToken.getId());
        return monoToSingle(template.insert(toJdbcEntity(accessToken)))
                .map(this::toEntity)
                .doOnError(
                        (error) ->
                                LOGGER.error(
                                        "Unable to create accessToken with id {}",
                                        accessToken.getId(),
                                        error));
    }
        ...
        return Completable.fromMaybe(
                findByToken(token)
                        .flatMap(
                                accessToken ->
                                        monoToMaybe(
                                                        template.delete(JdbcAccessToken.class)
                                                                .matching(
                                                                        Query.query(
                                                                                where("token")
                                                                                        .is(token)))
    ...
    public Observable<AccessToken> findByClientIdAndSubject(String clientId, String subject) {
        ...
        return accessTokenRepository
                .findByClientIdAndSubject(clientId, subject, LocalDateTime.now(UTC))
                .map(this::toEntity)
                .toObservable()
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve access tokens with client {} and subject {}",
                                        clientId,
                                        subject,
                                        error));
    }
    ...
    public Observable<AccessToken> findByClientId(String clientId) {
        ...
        return accessTokenRepository
                .findByClientId(clientId, LocalDateTime.now(UTC))
                .map(this::toEntity)
                .toObservable()
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve access tokens with client {}",
                                        clientId,
                                        error));
    }
    ...
    public Observable<AccessToken> findByAuthorizationCode(String authorizationCode) {
        ...
        return accessTokenRepository
                .findByAuthorizationCode(authorizationCode, LocalDateTime.now(UTC))
                .map(this::toEntity)
                .toObservable()
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve access tokens with authorization code {}",
                                        authorizationCode,
                                        error));
    }
    ...
    public Completable deleteByUserId(String userId) {
        ...
        return monoToCompletable(
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete access tokens with subject {}",
                                        userId,
                                        error));
    }
    ...
    public Completable deleteByDomainIdClientIdAndUserId(
        ...
        return monoToCompletable(
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete access token with domain {}, client {} and subject {}",
                                        domainId,
                                        clientId,
                                        userId,
                                        error));
    ...
    public Completable deleteByDomainIdAndUserId(String domainId, String userId) {
        ...
        return monoToCompletable(
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete access tokens with domain {} and subject {}",
                                        domainId,
                                        userId,
                                        error));
    }
    ...
    public Completable purgeExpiredData() {
        ...
        return monoToCompletable(
                        template.delete(JdbcAccessToken.class)
                                .matching(Query.query(where("expire_at").lessThan(now)))
                                .all())
