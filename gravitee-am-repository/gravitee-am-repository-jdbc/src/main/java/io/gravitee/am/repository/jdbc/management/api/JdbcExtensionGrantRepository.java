 ...
 */
@Repository
public class JdbcExtensionGrantRepository extends AbstractJdbcRepository
        implements ExtensionGrantRepository {

    @Autowired private SpringExtensionGrantRepository extensionGrantRepository;
    ...
    public Flowable<ExtensionGrant> findByDomain(String domain) {
        ...
        return extensionGrantRepository.findByDomain(domain).map(this::toEntity);
    }

    @Override
    public Maybe<ExtensionGrant> findByDomainAndName(String domain, String name) {
        LOGGER.debug("findByDomainAndName({}, {})", domain, name);
        return extensionGrantRepository.findByDomainAndName(domain, name).map(this::toEntity);
    }

    @Override
    public Maybe<ExtensionGrant> findById(String id) {
        LOGGER.debug("findByDomainAndName({}, {})", id);
        return extensionGrantRepository.findById(id).map(this::toEntity);
    }
    ...
    public Single<ExtensionGrant> update(ExtensionGrant item) {
        ...
        return this.extensionGrantRepository.save(toJdbcEntity(item)).map(this::toEntity);
    }
