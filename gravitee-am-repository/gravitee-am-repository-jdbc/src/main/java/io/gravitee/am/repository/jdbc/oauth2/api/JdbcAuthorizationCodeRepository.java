 ...
 */
@Repository
public class JdbcAuthorizationCodeRepository extends AbstractJdbcRepository
        implements AuthorizationCodeRepository, InitializingBean {
    ...
    @Autowired private SpringAuthorizationCodeRepository authorizationCodeRepository;
    ...
    public Single<AuthorizationCode> create(AuthorizationCode authorizationCode) {
        authorizationCode.setId(
                authorizationCode.getId() == null
                        ? RandomString.generate()
                        : authorizationCode.getId());
        LOGGER.debug(
                "Create authorizationCode with id {} and code {}",
                authorizationCode.getId(),
        ...
        insertSpec =
                addQuotedField(
                        insertSpec, COL_CLIENT_ID, authorizationCode.getClientId(), String.class);
        insertSpec =
                addQuotedField(insertSpec, COL_CODE, authorizationCode.getCode(), String.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_REDIRECT_URI,
        ...
        insertSpec =
                addQuotedField(
                        insertSpec, COL_SUBJECT, authorizationCode.getSubject(), String.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_TRANSACTION_ID,
                        authorizationCode.getTransactionId(),
                        String.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_CONTEXT_VERSION,
                        authorizationCode.getContextVersion(),
                        int.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_CREATED_AT,
                        dateConverter.convertTo(authorizationCode.getCreatedAt(), null),
                        LocalDateTime.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_EXPIRE_AT,
                        dateConverter.convertTo(authorizationCode.getExpireAt(), null),
                        LocalDateTime.class);
        insertSpec =
                databaseDialectHelper.addJsonField(
                        insertSpec, COL_SCOPES, authorizationCode.getScopes());
        insertSpec =
                databaseDialectHelper.addJsonField(
                        insertSpec,
                        COL_REQUEST_PARAMETERS,
                        authorizationCode.getRequestParameters());
                ...
                .flatMap(
                        (i) ->
                                authorizationCodeRepository
                                        .findById(authorizationCode.getId())
                                        .map(this::toEntity)
                                        .toSingle())
                .doOnError(
                        (error) ->
                                LOGGER.error(
                                        "Unable to create authorizationCode with id {}",
                                        authorizationCode.getId(),
                                        error));
    }
    ...
    public Maybe<AuthorizationCode> delete(String id) {
        ...
        return authorizationCodeRepository
                ...
                .flatMap(
                        authCode ->
                                monoToMaybe(
                                                template.delete(JdbcAuthorizationCode.class)
    ...
    public Maybe<AuthorizationCode> findByCode(String code) {
        ...
        return authorizationCodeRepository
                .findByCode(code, LocalDateTime.now(UTC))
                .map(this::toEntity)
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve AuthorizationCode with code {}", code));
    }
    ...
    public Completable purgeExpiredData() {
        ...
        return monoToCompletable(
                        template.delete(JdbcAuthorizationCode.class)
                                .matching(Query.query(where(COL_EXPIRE_AT).lessThan(now)))
                                .all())
