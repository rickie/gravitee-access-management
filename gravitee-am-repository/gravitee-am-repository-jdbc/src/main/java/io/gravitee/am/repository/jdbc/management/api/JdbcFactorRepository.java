...
public class JdbcFactorRepository extends AbstractJdbcRepository implements FactorRepository {
    ...
    @Autowired private SpringFactorRepository factorRepository;
    ...
    public Flowable<Factor> findAll() {
        ...
        return factorRepository.findAll().map(this::toEntity);
    }

    @Override
    public Flowable<Factor> findByDomain(String domain) {
        LOGGER.debug("findByDomain({})", domain);
        return factorRepository.findByDomain(domain).map(this::toEntity);
    }

    @Override
    public Maybe<Factor> findById(String id) {
        LOGGER.debug("findById({})", id);
        return factorRepository.findById(id).map(this::toEntity);
    }
    ...
    public Single<Factor> update(Factor item) {
        ...
        return this.factorRepository.save(toJdbcEntity(item)).map(this::toEntity);
    }
