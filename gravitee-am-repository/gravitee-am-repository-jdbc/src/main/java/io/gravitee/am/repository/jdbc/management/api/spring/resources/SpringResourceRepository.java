...
public interface SpringResourceRepository extends RxJava2CrudRepository<JdbcResource, String> {
    ...
    @Query(
            "select * from uma_resource_set u where u.domain = :domain and u.client_id = :client and u.user_id = :uid")
    Flowable<JdbcResource> findByDomainAndClientAndUser(
            @Param("domain") String domain,
            @Param("client") String client,
            @Param("uid") String user);

    @Query(
            "select * from uma_resource_set u where u.domain = :domain and u.client_id = :client and u.id in (:ids)")
    Flowable<JdbcResource> findByDomainAndClientAndResources(
            @Param("domain") String domain,
            @Param("client") String client,
            @Param("ids") List<String> resources);

    @Query(
            "select * from uma_resource_set u where u.domain = :domain and u.client_id = :client and u.user_id = :uid and u.id = :rid")
    Maybe<JdbcResource> findByDomainAndClientAndUserIdAndResource(
            @Param("domain") String domain,
            @Param("client") String client,
            @Param("uid") String user,
            @Param("rid") String resource);
}
