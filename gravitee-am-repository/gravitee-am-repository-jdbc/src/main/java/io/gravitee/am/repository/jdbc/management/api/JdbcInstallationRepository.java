 ...
 */
@Component
public class JdbcInstallationRepository extends AbstractJdbcRepository
        implements InstallationRepository, InitializingBean {
    ...
    private static final List<String> columns =
            List.of(COL_ID, COL_CREATED_AT, COL_UPDATED_AT, COL_ADDITIONAL_INFORMATION);

    private String INSERT_STATEMENT;
    private String UPDATE_STATEMENT;

    @Autowired private SpringInstallationRepository installationRepository;
    ...
    public Maybe<Installation> find() {
        LOGGER.debug("find()");
        return this.installationRepository.findAll().firstElement().map(this::toEntity);
    }

    @Override
    public Maybe<Installation> findById(String id) {
        LOGGER.debug("findById({})", id);
        return this.installationRepository.findById(id).map(this::toEntity);
    }

    @Override
    public Single<Installation> create(Installation installation) {
        installation.setId(
                installation.getId() == null ? RandomString.generate() : installation.getId());
        ...
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_CREATED_AT,
                        dateConverter.convertTo(installation.getCreatedAt(), null),
                        LocalDateTime.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_UPDATED_AT,
                        dateConverter.convertTo(installation.getUpdatedAt(), null),
                        LocalDateTime.class);
        insertSpec =
                databaseDialectHelper.addJsonField(
                        insertSpec,
                        COL_ADDITIONAL_INFORMATION,
                        installation.getAdditionalInformation());
