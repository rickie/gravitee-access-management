 ...
 */
@Repository
public class JdbcAuthenticationFlowContextRepository extends AbstractJdbcRepository
        implements AuthenticationFlowContextRepository, InitializingBean {
    ...
    public Maybe<AuthenticationFlowContext> findById(String id) {
        ...
        return monoToMaybe(
                        template.select(JdbcAuthenticationFlowContext.class)
    ...
    public Single<AuthenticationFlowContext> create(AuthenticationFlowContext context) {
        ...
        insertSpec =
                addQuotedField(
                        insertSpec, COL_TRANSACTION_ID, context.getTransactionId(), String.class);
        insertSpec = addQuotedField(insertSpec, COL_VERSION, context.getVersion(), Integer.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_CREATED_AT,
                        dateConverter.convertTo(context.getCreatedAt(), null),
                        LocalDateTime.class);
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_EXPIRE_AT,
                        dateConverter.convertTo(context.getExpireAt(), null),
                        LocalDateTime.class);
        ...
        return monoToSingle(insertAction).flatMap((i) -> this.findById(id).toSingle());
    }
    ...
    public Completable delete(String transactionId) {
        ...
        return monoToCompletable(
                template.delete(JdbcAuthenticationFlowContext.class)
                        .matching(Query.query(where(COL_TRANSACTION_ID).is(transactionId)))
                        .all());
    }
                ...
                template.delete(JdbcAuthenticationFlowContext.class)
                        .matching(
                                Query.query(
                                        where(COL_TRANSACTION_ID)
                                                .is(transactionId)
                                                .and(where(COL_VERSION).is(version))))
                        .all());
    }
    ...
    public Completable purgeExpiredData() {
        ...
        return monoToCompletable(
                        template.delete(JdbcAuthenticationFlowContext.class)
                                ...
                                .all()
                                .then())
                .doOnError(error -> LOGGER.error("Unable to purge authentication contexts", error));
    }
}
