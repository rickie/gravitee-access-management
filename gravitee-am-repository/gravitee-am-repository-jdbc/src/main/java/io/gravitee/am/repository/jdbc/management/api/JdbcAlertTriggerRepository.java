 ...
 */
@Component
public class JdbcAlertTriggerRepository extends AbstractJdbcRepository
        implements AlertTriggerRepository {

    @Autowired private SpringAlertTriggerRepository alertTriggerRepository;
    ...
    private static final String INSERT_ALERT_NOTIFIER_STMT =
            "INSERT INTO alert_triggers_alert_notifiers("
                    + ALERT_TRIGGER_ID
                    + ", "
                    + ALERT_NOTIFIER_ID
                    + ") VALUES (:"
    ...
    public Maybe<AlertTrigger> findById(String id) {
        ...
        Maybe<List<String>> alertNotifierIds =
                alertTriggerAlertNotifierRepository
                        .findByAlertTriggerId(id)
                        .map(JdbcAlertTrigger.AlertNotifier::getAlertNotifierId)
                        .toList()
                        .toMaybe();

        return this.alertTriggerRepository
                .findById(id)
                .map(this::toEntity)
                .zipWith(
                        alertNotifierIds,
    ...
    public Single<AlertTrigger> create(AlertTrigger alertTrigger) {
        alertTrigger.setId(
                alertTrigger.getId() == null ? RandomString.generate() : alertTrigger.getId());
    ...
    public Flowable<AlertTrigger> findByCriteria(
        ...
        if (criteria.getAlertNotifierIds().isPresent()
            ...
            queryBuilder.append(
                    "INNER JOIN alert_triggers_alert_notifiers n ON t.id = n."
                            + ALERT_TRIGGER_ID
                            + " ");
        ...
        queryBuilder.append(
                "WHERE t.reference_id = :reference_id AND t.reference_type = :reference_type");

        if (!queryCriteria.isEmpty()) {
            queryBuilder.append(" AND (");
            queryBuilder.append(
                    queryCriteria.stream()
                            .collect(
                                    Collectors.joining(criteria.isLogicalOR() ? " OR " : " AND ")));
            queryBuilder.append(")");
        }
        ...
        org.springframework.r2dbc.core.DatabaseClient.GenericExecuteSpec execute =
                template.getDatabaseClient().sql(queryBuilder.toString());

        for (Map.Entry<String, Object> entry : params.entrySet()) {
            execute = execute.bind(entry.getKey(), entry.getValue());
        }

        return fluxToFlowable(execute.map(x -> x.get(0, String.class)).all())
                .flatMapMaybe(this::findById)
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve AlertTrigger with referenceId {}, referenceType {} and criteria {}",
                                        referenceId,
                                        referenceType,
                                        criteria,
                                        error));
        ...
        if (alertNotifiers != null && !alertNotifiers.isEmpty()) {
            return delete.thenMany(
                            Flux.fromIterable(alertNotifiers)
                                    .map(
                                            alertNotifierId -> {
                                                JdbcAlertTrigger.AlertNotifier dbAlertNotifier =
                                                        new JdbcAlertTrigger.AlertNotifier();
                                                dbAlertNotifier.setAlertNotifierId(alertNotifierId);
                                                dbAlertNotifier.setAlertTriggerId(
                                                        alertTrigger.getId());
    ...
    private Mono<Void> deleteAlertNotifiers(String alertTriggerId) {
        return template.delete(
                        Query.query(where(ALERT_TRIGGER_ID).is(alertTriggerId)),
                        JdbcAlertTrigger.AlertNotifier.class)
                .then();
    }
}
