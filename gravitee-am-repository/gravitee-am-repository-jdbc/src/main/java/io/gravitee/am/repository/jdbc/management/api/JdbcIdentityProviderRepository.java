 ...
 */
@Repository
public class JdbcIdentityProviderRepository extends AbstractJdbcRepository
        implements IdentityProviderRepository, InitializingBean {
    ...
    @Autowired private SpringIdentityProviderRepository identityProviderRepository;
    ...
    public Flowable<IdentityProvider> findAll(ReferenceType referenceType) {
        ...
        return this.identityProviderRepository.findAll(referenceType.name()).map(this::toEntity);
    }

    @Override
    public Flowable<IdentityProvider> findAll() {
        LOGGER.debug("findAll()");
        return this.identityProviderRepository.findAll().map(this::toEntity);
    }
    ...
    public Maybe<IdentityProvider> findById(
        ...
        return this.identityProviderRepository
                .findById(referenceType.name(), referenceId, identityProviderId)
                .map(this::toEntity);
    }
    ...
    public Maybe<IdentityProvider> findById(String id) {
        ...
        return this.identityProviderRepository.findById(id).map(this::toEntity);
    }
    ...
    public Single<IdentityProvider> create(IdentityProvider item) {
        ...
        insertSpec =
                databaseDialectHelper.addJsonField(
                        insertSpec,
                        COL_DOMAIN_WHITELIST,
                        ofNullable(item.getDomainWhitelist()).orElse(List.of()));
        insertSpec = databaseDialectHelper.addJsonField(insertSpec, COL_MAPPERS, item.getMappers());
        insertSpec =
                databaseDialectHelper.addJsonField(
                        insertSpec, COL_ROLE_MAPPER, item.getRoleMapper());
