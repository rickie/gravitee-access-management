 ...
 */
@Repository
public class JdbcPushedAuthorizationRequestRepository extends AbstractJdbcRepository
        implements PushedAuthorizationRequestRepository {

    @Autowired private SpringPushedAuthorizationRequestRepository parRepository;
    ...
    public Maybe<PushedAuthorizationRequest> findById(String id) {
        ...
        return parRepository
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve PushedAuthorizationRequest with id {}",
                                        id,
                                        error));
    }
    ...
    public Single<PushedAuthorizationRequest> create(PushedAuthorizationRequest par) {
        ...
        return monoToSingle(template.insert(toJdbcEntity(par)))
                .map(this::toEntity)
                .doOnError(
                        (error) ->
                                LOGGER.error(
                                        "Unable to create PushedAuthorizationRequest with id {}",
                                        par.getId(),
                                        error));
    }
    ...
    public Completable delete(String id) {
        ...
        return parRepository
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete PushedAuthorizationRequest with id {}",
                                        id,
                                        error));
    }
    ...
    public Completable purgeExpiredData() {
        ...
        return monoToCompletable(
                        template.delete(JdbcPushedAuthorizationRequest.class)
                                .matching(Query.query(where("expire_at").lessThan(now)))
                                .all())
                .doOnError(
                        error -> LOGGER.error("Unable to purge PushedAuthorizationRequest", error));
    }
}
