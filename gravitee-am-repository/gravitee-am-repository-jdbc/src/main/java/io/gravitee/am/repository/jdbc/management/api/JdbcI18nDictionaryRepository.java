...
@Repository
public class JdbcI18nDictionaryRepository extends AbstractJdbcRepository
        implements I18nDictionaryRepository, InitializingBean {
    ...
    @Autowired private SpringI18nDictionaryRepository repository;

    private static final List<String> columns =
            List.of(ID, REFERENCE_TYPE, REFERENCE_ID, NAME, LOCALE, CREATED_AT, UPDATED_AT);

    private static final List<String> entryColumns = List.of(ENTRY_ID, ENTRY_KEY, ENTRY_MESSAGE);
    ...
    public Single<I18nDictionary> create(I18nDictionary item) {
        ...
        return monoToSingle(
                        insertAction.as(
                                mono -> TransactionalOperator.create(tm).transactional(mono)))
    ...
    public Single<I18nDictionary> update(I18nDictionary item) {
        ...
        return monoToSingle(updateAction.as(trx::transactional))
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "unable to update i18n dictionary with id {}",
                                        item.getId(),
                                        error));
    }

    private Mono<Integer> deleteEntries(String dictionaryId) {
        return template.delete(JdbcI18nDictionary.Entry.class)
                .matching(Query.query(where(ENTRY_ID).is(dictionaryId)))
                .all();
    }
    ...
    private Mono<Integer> persistEntries(Mono<Integer> insertAction, I18nDictionary item) {
        ...
        if (!item.getEntries().isEmpty()) {
            ...
            entries =
                    Flux.fromIterable(jdbcEntries)
                            .concatMap(
                                    entry -> {
                                        DatabaseClient.GenericExecuteSpec insert =
                                                template.getDatabaseClient()
                                                        .sql(insertEntryStatement);
    ...
    public Completable delete(String id) {
        ...
        return monoToCompletable(
                template.delete(JdbcI18nDictionary.class)
    ...
    @Override
    public Maybe<I18nDictionary> findByName(
            ReferenceType referenceType, String referenceId, String name) {
        LOGGER.debug("findByName({}, {}, {})", referenceType, referenceId, name);
        return repository
                .findByName(referenceType.toString(), referenceId, name)
                .map(this::toEntity)
    ...
    private Single<I18nDictionary> completeDictionary(I18nDictionary dictionary) {
        return fluxToFlowable(
                        template.select(JdbcI18nDictionary.Entry.class)
                            ...
                            if (!entries.isEmpty()) {
                                entries.forEach(
                                        entry ->
                                                dictionary
                                                        .getEntries()
                                                        .put(entry.getKey(), entry.getMessage()));
                            }
                            return dictionary;
                        });
    }
