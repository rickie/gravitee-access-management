...
@Override
    public Mono<RequestObject> findById_migrated(String id) {
        ...
        return RxJava2Adapter.maybeToMono(requestObjectRepository.findById(id)).filter(bean -> bean.getExpireAt() == null || bean.getExpireAt().isAfter(now)).map(RxJavaReactorMigrationUtil.toJdkFunction(this::toEntity)).doOnError(error -> LOGGER.error("Unable to retrieve RequestObject with id {}", id, error));
    }
...
@Override
    public Mono<RequestObject> create_migrated(RequestObject requestObject) {
        ...
        return action.flatMap(i->RxJava2Adapter.maybeToMono(requestObjectRepository.findById(requestObject.getId())).map(RxJavaReactorMigrationUtil.toJdkFunction(this::toEntity)).single()).doOnError((error) -> LOGGER.error("Unable to create requestObject with id {}", requestObject.getId(), error));
    }
...
public Mono<Void> purgeExpiredData_migrated() {
        ...
        LocalDateTime now = LocalDateTime.now(UTC);
        return dbClient.delete().from(JdbcRequestObject.class).matching(where("expire_at").lessThan(now)).then().doOnError(error -> LOGGER.error("Unable to purge RequestObjects", error));
    }
}
