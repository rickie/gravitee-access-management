 ...
 */
@Repository
public class JdbcRequestObjectRepository extends AbstractJdbcRepository
        implements RequestObjectRepository {

    @Autowired private SpringRequestObjectRepository requestObjectRepository;
    ...
    public Maybe<RequestObject> findById(String id) {
        ...
        return requestObjectRepository
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve RequestObject with id {}", id, error));
    }

    @Override
    public Single<RequestObject> create(RequestObject requestObject) {
        requestObject.setId(
                requestObject.getId() == null ? RandomString.generate() : requestObject.getId());
    ...
    public Completable delete(String id) {
        ...
        return requestObjectRepository
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to delete RequestObject with id {}", id, error));
    }
    ...
    public Completable purgeExpiredData() {
        ...
        return monoToCompletable(
                        template.delete(JdbcRequestObject.class)
                                .matching(Query.query(where("expire_at").lessThan(now)))
                                .all())
                .doOnError(error -> LOGGER.error("Unable to purge RequestObjects", error));
    }
}
