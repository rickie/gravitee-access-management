 ...
 */
@Repository
public class JdbcFlowRepository extends AbstractJdbcRepository
        implements FlowRepository, InitializingBean {
    ...
    private static final List<String> stepsColumns =
            List.of(
                    COL_STEP_ID,
                    COL_STEP_STAGE,
    ...
    @Autowired protected SpringFlowRepository flowRepository;
    ...
    public Single<Flow> create(Flow item) {
        ...
        insertSpec =
                addQuotedField(insertSpec, COL_APPLICATION, item.getApplication(), String.class);
        insertSpec =
    ...
    private Mono<Integer> persistFlowSteps(Flow item, List<Step> steps, JdbcFlow.StepType type) {
        ...
        return Flux.fromIterable(jdbcPostSteps)
                .concatMap(
                        step -> {
                            DatabaseClient.GenericExecuteSpec insert =
                                    template.getDatabaseClient().sql(INSERT_STEP_STATEMENT);
                            insert =
                                    step.getFlowId() != null
                                            ? insert.bind(COL_STEP_ID, step.getFlowId())
                                            : insert.bindNull(COL_STEP_ID, String.class);
                            insert =
                                    step.getStage() != null
                                            ? insert.bind(COL_STEP_STAGE, step.getStage())
                                            : insert.bindNull(COL_STEP_STAGE, String.class);
                            insert =
                                    step.getName() != null
                                            ? insert.bind(COL_STEP_NAME, step.getName())
                                            : insert.bindNull(COL_STEP_NAME, String.class);
                            insert =
                                    step.getPolicy() != null
                                            ? insert.bind(COL_STEP_POLICY, step.getPolicy())
                                            : insert.bindNull(COL_STEP_POLICY, String.class);
                            insert =
                                    step.getDescription() != null
                                            ? insert.bind(
                                                    COL_STEP_DESCRIPTION, step.getDescription())
                                            : insert.bindNull(COL_STEP_DESCRIPTION, String.class);
                            insert =
                                    step.getConfiguration() != null
                                            ? insert.bind(
                                                    COL_STEP_CONFIGURATION, step.getConfiguration())
                                            : insert.bindNull(COL_STEP_CONFIGURATION, String.class);
                            ...
                            insert =
                                    step.getCondition() != null
                                            ? insert.bind(COL_STEP_CONDITION, step.getCondition())
                                            : insert.bindNull(COL_STEP_CONDITION, String.class);
    ...
    private Mono<Integer> deleteChildEntities(String flowId) {
        return template.delete(JdbcFlow.JdbcStep.class)
                .matching(Query.query(where(COL_STEP_ID).is(flowId)))
                .all();
    }
    ...
    public Completable delete(String id) {
        ...
        return monoToCompletable(
                template.delete(JdbcFlow.class)
    ...
    public Maybe<Flow> findById(ReferenceType referenceType, String referenceId, String id) {
        ...
        return flowRepository
                .findById(referenceType.name(), referenceId, id)
                .map(this::toEntity)
    ...
    public Flowable<Flow> findAll(ReferenceType referenceType, String referenceId) {
        ...
        return flowRepository
                .findAll(referenceType.name(), referenceId)
                .map(this::toEntity)
    ...
    public Flowable<Flow> findByApplication(
        ...
        return flowRepository
                .findByApplication(referenceType.name(), referenceId, application)
                .map(this::toEntity)
                .flatMap(flow -> completeFlow(flow).toFlowable());
    }
    ...
    protected Single<Flow> completeFlow(Flow flow) {
        return fluxToFlowable(
                        template.select(JdbcFlow.JdbcStep.class)
                                .matching(
                                        Query.query(where(COL_STEP_ID).is(flow.getId()))
                                                .sort(Sort.by(COL_STEP_STAGE_ORDER).ascending()))
                                .all())
                .toList()
                .map(
                        steps -> {
                            if (steps != null && !steps.isEmpty()) {
                                ...
                                for (JdbcFlow.JdbcStep jStep : steps) {
                                    ...
                                    } else if (jStep.getStage()
                                            .equals(JdbcFlow.StepType.post.name())) {
                                        postSteps.add(convertToStep(jStep));
                                    } else {
                                        LOGGER.debug(
                                                "Unknown step type '{}', ignore it!",
                                                jStep.getStage());
                                    }
                                }
                                if (!preSteps.isEmpty()) {
