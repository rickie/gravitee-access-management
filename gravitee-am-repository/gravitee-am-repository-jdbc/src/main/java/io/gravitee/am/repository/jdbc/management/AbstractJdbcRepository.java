...
public abstract class AbstractJdbcRepository {
    ...
    @Autowired protected R2dbcEntityTemplate template;

    @Autowired protected DatabaseDialectHelper databaseDialectHelper;

    @Autowired protected ReactiveTransactionManager tm;

    protected LocalDateConverter dateConverter = new LocalDateConverter();

    protected static final Mapper mapper =
            DozerBeanMapperBuilder.create().withMappingFiles(Arrays.asList("dozer.xml")).build();

    @Autowired protected MappingR2dbcConverter rowMapper;

    protected <T> GenericInsertSpec<Map<String, Object>> addQuotedField(
            GenericInsertSpec<Map<String, Object>> spec, String name, Object value, Class<T> type) {
        return value == null
                ? spec.nullValue(SqlIdentifier.quoted(name), type)
                : spec.value(SqlIdentifier.quoted(name), value);
    }
    ...
    protected <T> Map<SqlIdentifier, Object> addQuotedField(
            Map<SqlIdentifier, Object> spec, String name, Object value) {
        spec.put(
                SqlIdentifier.unquoted(databaseDialectHelper.toSql(SqlIdentifier.quoted(name))),
                value);
        return spec;
    }
