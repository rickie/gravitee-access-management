 ...
 */
@Repository
public class JdbcCertificateRepository extends AbstractJdbcRepository
        implements CertificateRepository, InitializingBean {
    ...
    @Autowired private SpringCertificateRepository certificateRepository;
    ...
    public Flowable<Certificate> findAll() {
        ...
        return this.certificateRepository.findAll().map(this::toEntity);
    }

    @Override
    public Flowable<Certificate> findByDomain(String domain) {
        LOGGER.debug("findByDomain({})", domain);
        return this.certificateRepository.findByDomain(domain).map(this::toEntity);
    }
    ...
    public Maybe<Certificate> findById(String id) {
        ...
        return this.certificateRepository
                ...
                .doOnError(
                        error ->
                                LOGGER.error(
                                        "Unable to retrieve Certificate with id {}", id, error));
    }
    ...
    public Single<Certificate> create(Certificate item) {
        ...
        insertSpec =
                databaseDialectHelper.addJsonField(insertSpec, COL_METADATA, item.getMetadata());
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_CREATED_AT,
        ...
        insertSpec =
                addQuotedField(
                        insertSpec,
                        COL_EXPIRES_AT,
                        dateConverter.convertTo(item.getExpiresAt(), null),
                        LocalDateTime.class);
        ...
        return monoToSingle(action)
                ...
                .doOnError(
                        (error) ->
                                LOGGER.error(
                                        "unable to create certificate with id {}",
                                        item.getId(),
                                        error));
    }
    ...
    public Single<Certificate> update(Certificate item) {
        ...
        return monoToSingle(updateAction)
                ...
                .doOnError(
                        (error) ->
                                LOGGER.error(
                                        "unable to update certificate with id {}",
                                        item.getId(),
                                        error));
    }
    ...
    public Completable updateExpirationDate(String certificateId, Date expiresAt) {
        LOGGER.debug(
                "update Certificate expiration date with id {} with value '{}'",
                certificateId,
                expiresAt);
        ...
        update =
                addQuotedField(
                        update,
                        COL_EXPIRES_AT,
                        dateConverter.convertTo(expiresAt, null),
                        LocalDateTime.class);
    ...
    public Completable delete(String id) {
        ...
        return this.certificateRepository
                ...
                .doOnError(
                        error ->
                                LOGGER.error("Unable to delete Certificate with id {}", id, error));
    }
}
