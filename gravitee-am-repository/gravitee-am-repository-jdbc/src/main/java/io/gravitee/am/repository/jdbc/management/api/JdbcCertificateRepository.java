...
@Override
    public Flux<Certificate> findByDomain_migrated(String domain) {
        ...
        return RxJava2Adapter.flowableToFlux(RxJava2Adapter.fluxToFlowable(this.certificateRepository.findByDomain_migrated(domain))).map(RxJavaReactorMigrationUtil.toJdkFunction(this::toEntity));
    }
...
@Override
    public Mono<Certificate> findById_migrated(String id) {
        ...
        return RxJava2Adapter.maybeToMono(this.certificateRepository.findById(id)).map(RxJavaReactorMigrationUtil.toJdkFunction(this::toEntity)).doOnError(RxJavaReactorMigrationUtil.toJdkConsumer(error -> LOGGER.error("Unable to retrieve Certificate with id {}", id, error)));
    }
...
@Override
    public Mono<Certificate> create_migrated(Certificate item) {
        ...
        return action.flatMap(i->RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(this.findById_migrated(item.getId()))).single()).doOnError(RxJavaReactorMigrationUtil.toJdkConsumer((error) -> LOGGER.error("unable to create certificate with id {}", item.getId(), error)));
    }
...
@Override
    public Mono<Certificate> update_migrated(Certificate item) {
        ...
        return action.flatMap(i->RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(this.findById_migrated(item.getId()))).single()).doOnError(RxJavaReactorMigrationUtil.toJdkConsumer((error) -> LOGGER.error("unable to update certificate with id {}", item.getId(), error)));
    }
...
@Override
    public Mono<Void> delete_migrated(String id) {
        ...
        return this.certificateRepository.deleteById(id).as(RxJava2Adapter::completableToMono).doOnError(RxJavaReactorMigrationUtil.toJdkConsumer(error -> LOGGER.error("Unable to delete Certificate with id {}", id, error)));
    }
}
