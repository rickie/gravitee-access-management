 ...
 */
@Component
public class MongoRequestObjectRepository extends AbstractOAuth2MongoRepository
    ...
    public void init() {
        requestObjectCollection =
                mongoOperations.getCollection("request_objects", RequestObjectMongo.class);
    ...
    public Maybe<RequestObject> findById(String id) {
        return Observable.fromPublisher(
                        requestObjectCollection.find(eq(FIELD_ID, id)).limit(1).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<RequestObject> create(RequestObject requestObject) {
        return Single.fromPublisher(requestObjectCollection.insertOne(convert(requestObject)))
    ...
    public Completable delete(String id) {
        return Completable.fromPublisher(
                requestObjectCollection.findOneAndDelete(eq(FIELD_ID, id)));
    }
