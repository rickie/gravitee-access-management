 ...
 */
@Component
public class MongoFormRepository extends AbstractManagementMongoRepository
    ...
    public Flowable<Form> findAll(ReferenceType referenceType) {
        return Flowable.fromPublisher(
                        formsCollection.find(eq(FIELD_REFERENCE_TYPE, referenceType.name())))
                .map(this::convert);
    }
    ...
    public Maybe<Form> findById(String id) {
        return Observable.fromPublisher(formsCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Form> create(Form item) {
        ...
        return Single.fromPublisher(formsCollection.insertOne(page))
                .flatMap(
                        success -> {
    ...
    public Single<Form> update(Form item) {
        FormMongo page = convert(item);
        return Single.fromPublisher(formsCollection.replaceOne(eq(FIELD_ID, page.getId()), page))
