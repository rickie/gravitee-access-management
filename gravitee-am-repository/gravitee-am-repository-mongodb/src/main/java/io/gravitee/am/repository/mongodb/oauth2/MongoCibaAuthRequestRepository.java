 ...
 */
@Component
public class MongoCibaAuthRequestRepository extends AbstractOAuth2MongoRepository
    ...
    @PostConstruct
    public void init() {
        cibaAuthRequestCollection =
                mongoOperations.getCollection("ciba_auth_requests", CibaAuthRequestMongo.class);
    ...
    public Maybe<CibaAuthRequest> findById(String id) {
        return Observable.fromPublisher(
                        cibaAuthRequestCollection.find(eq(FIELD_ID, id)).limit(1).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Maybe<CibaAuthRequest> findByExternalId(String externalId) {
        return Observable.fromPublisher(
                        cibaAuthRequestCollection
                                .find(eq(FIELD_EXTERNAL_ID, externalId))
                                .limit(1)
                                .first())
                .firstElement()
                .map(this::convert);
    ...
    public Single<CibaAuthRequest> create(CibaAuthRequest authreq) {
        ...
        return Single.fromPublisher(cibaAuthRequestCollection.insertOne(convert(authreq)))
    ...
    public Single<CibaAuthRequest> update(CibaAuthRequest authreq) {
        return Single.fromPublisher(
                        cibaAuthRequestCollection.replaceOne(
                                eq(FIELD_ID, authreq.getId()), convert(authreq)))
                .flatMap(success -> findById(authreq.getId()).toSingle());
    }
    ...
    public Single<CibaAuthRequest> updateStatus(String authReqId, String status) {
        return Single.fromPublisher(
                        cibaAuthRequestCollection.updateOne(
                                and(eq(FIELD_ID, authReqId)), Updates.set("status", status)))
    ...
    public Completable delete(String id) {
        return Completable.fromPublisher(
                cibaAuthRequestCollection.findOneAndDelete(eq(FIELD_ID, id)));
    }
