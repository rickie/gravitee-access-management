...
@Override
    public Mono<Page<Resource>> findByDomainAndClient_migrated(String domain, String client, int page, int size) {
        ...
        Single<List<Resource>> resourcesOperation = RxJava2Adapter.fluxToObservable(Flux.from(resourceCollection.find(and(eq(FIELD_DOMAIN, domain), eq(FIELD_CLIENT_ID, client))).sort(new BasicDBObject(FIELD_UPDATED_AT, -1)).skip(size * page).limit(size))).map(this::convert).toList();
...
@Override
    public Mono<Page<Resource>> findByDomain_migrated(String domain, int page, int size) {
        Single<Long> countOperation = RxJava2Adapter.fluxToObservable(Flux.from(resourceCollection.countDocuments(eq(FIELD_DOMAIN, domain)))).first(0l);
        Single<Set<Resource>> resourceSetOperation = RxJava2Adapter.fluxToObservable(Flux.from(resourceCollection.find(eq(FIELD_DOMAIN, domain)).sort(new BasicDBObject(FIELD_UPDATED_AT, -1)).skip(size * page).limit(size))).map(this::convert).collect(HashSet::new, Set::add);
