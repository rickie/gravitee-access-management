 ...
 */
@Component
public class MongoGroupRepository extends AbstractManagementMongoRepository
    ...
    public Flowable<Group> findByMember(String memberId) {
        return Flowable.fromPublisher(groupsCollection.find(eq(FIELD_MEMBERS, memberId)))
                .map(this::convert);
    }
    ...
    public Single<Page<Group>> findAll(
                ...
                Observable.fromPublisher(
                                groupsCollection.countDocuments(
                                        and(
                                                ...
                                                eq(FIELD_REFERENCE_ID, referenceId))))
                        .first(0l);
        Single<List<Group>> groupsOperation =
                Observable.fromPublisher(
                                groupsCollection
                                        .find(
                                                and(
        ...
        return Single.zip(
                countOperation,
                groupsOperation,
                (count, groups) -> new Page<>(groups, page, count));
    }
    ...
    public Maybe<Group> findById(String group) {
        return Observable.fromPublisher(groupsCollection.find(eq(FIELD_ID, group)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Group> create(Group item) {
        ...
        return Single.fromPublisher(groupsCollection.insertOne(group))
                .flatMap(
                        success -> {
    ...
    public Single<Group> update(Group item) {
        GroupMongo group = convert(item);
        return Single.fromPublisher(groupsCollection.replaceOne(eq(FIELD_ID, group.getId()), group))
