...
public class MongoTagRepository extends AbstractManagementMongoRepository implements TagRepository {
    ...
    public void init() {
        ...
        super.createIndex(
                tagsCollection, new Document(FIELD_ID, 1).append(FIELD_ORGANIZATION_ID, 1));
    }
    ...
    public Maybe<Tag> findById(String id) {
        return Observable.fromPublisher(tagsCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Flowable<Tag> findAll(String organizationId) {
        return Flowable.fromPublisher(
                        tagsCollection.find(eq(FIELD_ORGANIZATION_ID, organizationId)))
                .map(this::convert);
    }
    ...
    public Single<Tag> create(Tag item) {
        ...
        return Single.fromPublisher(tagsCollection.insertOne(tag))
                .flatMap(
                        success -> {
    ...
    public Single<Tag> update(Tag item) {
        TagMongo tag = convert(item);
        return Single.fromPublisher(tagsCollection.replaceOne(eq(FIELD_ID, tag.getId()), tag))
