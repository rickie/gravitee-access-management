 ...
 */
@Component
public class MongoThemeRepository extends AbstractManagementMongoRepository
    ...
    public Maybe<Theme> findById(String id) {
        return Observable.fromPublisher(themesCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Theme> create(Theme item) {
        ...
        return Single.fromPublisher(themesCollection.insertOne(theme))
                .flatMap(
                        success -> {
    ...
    public Single<Theme> update(Theme item) {
        ThemeMongo theme = convert(item);
        return Single.fromPublisher(themesCollection.replaceOne(eq(FIELD_ID, theme.getId()), theme))
