...
@Override
    public Mono<Credential> findById_migrated(String id) {
        return Flux.from(credentialsCollection.find(eq(FIELD_ID, id)).first()).next().map(RxJavaReactorMigrationUtil.toJdkFunction(this::convert));
    }
...
@Override
    public Mono<Credential> create_migrated(Credential item) {
        ...
        return Mono.from(credentialsCollection.insertOne(credential)).flatMap(success->findById_migrated(credential.getId()).single());
    }
...
@Override
    public Mono<Credential> update_migrated(Credential item) {
        CredentialMongo credential = convert(item);
        return Mono.from(credentialsCollection.replaceOne(eq(FIELD_ID, credential.getId()), credential)).flatMap(updateResult->findById_migrated(credential.getId()).single());
    }
