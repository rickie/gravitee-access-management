...
@Override
    public Mono<Factor> findById_migrated(String factorId) {
        return Flux.from(factorsCollection.find(eq(FIELD_ID, factorId)).first()).next().map(RxJavaReactorMigrationUtil.toJdkFunction(this::convert));
    }
...
@Override
    public Mono<Factor> create_migrated(Factor item) {
        ...
        return Mono.from(factorsCollection.insertOne(authenticator)).flatMap(success->findById_migrated(authenticator.getId()).single());
    }
...
@Override
    public Mono<Factor> update_migrated(Factor item) {
        FactorMongo authenticator = convert(item);
        return Mono.from(factorsCollection.replaceOne(eq(FIELD_ID, authenticator.getId()), authenticator)).flatMap(updateResult->findById_migrated(authenticator.getId()).single());
    }
