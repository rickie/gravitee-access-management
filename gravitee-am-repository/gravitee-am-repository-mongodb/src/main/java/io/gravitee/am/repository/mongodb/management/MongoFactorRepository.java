 ...
 */
@Component
public class MongoFactorRepository extends AbstractManagementMongoRepository
        implements FactorRepository {
    ...
    public void init() {
        ...
        super.createIndex(
                factorsCollection, new Document(FIELD_DOMAIN, 1).append(FIELD_FACTOR_TYPE, 1));
    }
    ...
    public Flowable<Factor> findByDomain(String domain) {
        return Flowable.fromPublisher(factorsCollection.find(eq(FIELD_DOMAIN, domain)))
                .map(this::convert);
    }

    @Override
    public Maybe<Factor> findById(String factorId) {
        return Observable.fromPublisher(factorsCollection.find(eq(FIELD_ID, factorId)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Factor> create(Factor item) {
        ...
        authenticator.setId(
                authenticator.getId() == null ? RandomString.generate() : authenticator.getId());
        return Single.fromPublisher(factorsCollection.insertOne(authenticator))
                .flatMap(
                        success -> {
                            item.setId(authenticator.getId());
    ...
    public Single<Factor> update(Factor item) {
        ...
        return Single.fromPublisher(
                        factorsCollection.replaceOne(
                                eq(FIELD_ID, authenticator.getId()), authenticator))
