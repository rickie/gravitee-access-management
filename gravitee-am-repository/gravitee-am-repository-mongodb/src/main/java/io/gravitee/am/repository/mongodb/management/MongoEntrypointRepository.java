...
@Override
    public Mono<Entrypoint> create_migrated(Entrypoint item) {
        ...
        return Mono.from(collection.insertOne(entrypoint)).flatMap(success->findById_migrated(entrypoint.getId()).single());
    }
...
@Override
    public Mono<Entrypoint> update_migrated(Entrypoint item) {
        EntrypointMongo entrypoint = convert(item);
        return Mono.from(collection.replaceOne(eq(FIELD_ID, entrypoint.getId()), entrypoint)).flatMap(updateResult->findById_migrated(entrypoint.getId()).single());
    }
