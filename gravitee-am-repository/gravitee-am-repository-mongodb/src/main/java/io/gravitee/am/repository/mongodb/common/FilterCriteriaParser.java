...
public final class FilterCriteriaParser {
    ...
    private static final List<String> specialCharsList =
            new ArrayList<>() {
                {
                    add("'");
                    add("\"");
        ...
        switch (operator) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Invalid operator [" + operator + "] found in the search query");
        }
    }
    ...
    private static String convertFilterName(String filterName) {
        ...
        if (specialCharsList.stream().anyMatch(s -> filterName.contains(s))) {
            throw new IllegalArgumentException(
                    "Invalid filter name [" + filterName + "] found in the the search query");
        }
    ...
    private static String convertFilterValue(
            FilterCriteria criteria, String filterName, String operator) {
        if (specialCharsList.stream().anyMatch(s -> criteria.getFilterValue().contains(s))) {
            throw new IllegalArgumentException(
                    "Invalid filter value ["
                            + criteria.getFilterValue()
                            + "] found in the the search query");
        }
    ...
    private static boolean isDateInput(String filterName) {
        return "createdAt".equals(filterName) || "updatedAt".equals(filterName);
    }
}
