...
@Override
    public Mono<Email> findById_migrated(ReferenceType referenceType, String referenceId, String id) {
        return Flux.from(emailsCollection.find(and(eq(FIELD_REFERENCE_TYPE, referenceType.name()), eq(FIELD_REFERENCE_ID, referenceId), eq(FIELD_ID, id))).first()).next().map(RxJavaReactorMigrationUtil.toJdkFunction(this::convert));
    }
...
@Override
    public Mono<Email> findById_migrated(String id) {
        return Flux.from(emailsCollection.find(eq(FIELD_ID, id)).first()).next().map(RxJavaReactorMigrationUtil.toJdkFunction(this::convert));
    }
...
@Override
    public Mono<Email> create_migrated(Email item) {
        ...
        return Mono.from(emailsCollection.insertOne(email)).flatMap(success->findById_migrated(email.getId()).single());
    }
...
@Override
    public Mono<Email> update_migrated(Email item) {
        EmailMongo email = convert(item);
        return Mono.from(emailsCollection.replaceOne(eq(FIELD_ID, email.getId()), email)).flatMap(updateResult->findById_migrated(email.getId()).single());
    }
