 ...
 */
@Component
public class MongoEmailRepository extends AbstractManagementMongoRepository
    ...
    public Maybe<Email> findById(String id) {
        return Observable.fromPublisher(emailsCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Email> create(Email item) {
        ...
        return Single.fromPublisher(emailsCollection.insertOne(email))
                .flatMap(
                        success -> {
                            item.setId(email.getId());
    ...
    public Single<Email> update(Email item) {
        EmailMongo email = convert(item);
        return Single.fromPublisher(emailsCollection.replaceOne(eq(FIELD_ID, email.getId()), email))
