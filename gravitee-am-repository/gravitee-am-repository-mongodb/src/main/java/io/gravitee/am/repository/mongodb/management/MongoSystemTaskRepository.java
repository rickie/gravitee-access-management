...
public class MongoSystemTaskRepository extends AbstractManagementMongoRepository
    ...
    public Maybe<SystemTask> findById(String id) {
        return Observable.fromPublisher(systemTaskCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(SystemTaskMongo::convert);
    }
    ...
    public Single<SystemTask> create(SystemTask item) {
        ...
        return Single.fromPublisher(systemTaskCollection.insertOne(task))
                .flatMap(
                        success -> {
    ...
    public Single<SystemTask> updateIf(SystemTask item, String operationId) {
        ...
        return Single.fromPublisher(
                        systemTaskCollection.replaceOne(
                                and(
                                        eq(FIELD_ID, task.getId()),
                                        eq(FIELD_OPERATION_ID, operationId)),
                                task))
                .flatMap(
                        updateResult ->
                                updateResult.getModifiedCount() == 1
                                        ? Single.just(item)
                                        : findById(task.getId()).toSingle());
    }
    ...
    public Flowable<SystemTask> findByType(String type) {
        return Flowable.fromPublisher(systemTaskCollection.find(eq(FIELD_TYPE, type)))
                .map(SystemTaskMongo::convert);
    }
}
