 ...
 */
@Component
public class MongoDeviceRepository extends AbstractManagementMongoRepository
    ...
    @PostConstruct
    public void init() {
        rememberDeviceMongoCollection =
                mongoOperations.getCollection(COLLECTION_NAME, DeviceMongo.class);
    ...
    public Maybe<Device> findByReferenceAndClientAndUserAndDeviceIdentifierAndDeviceId(
        ...
        var query =
                and(
                        ...
                        eq(FIELD_USER_ID, user),
                        eq(DEVICE_IDENTIFIER_ID, deviceIdentifierId),
                        eq(DEVICE_ID, deviceId));
        return Observable.fromPublisher(rememberDeviceMongoCollection.find(query).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Maybe<Device> findById(String deviceId) {
        return Observable.fromPublisher(
                        rememberDeviceMongoCollection.find(eq(FIELD_ID, deviceId)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Device> create(Device item) {
        ...
        return Single.fromPublisher(rememberDeviceMongoCollection.insertOne(entity))
                .flatMap(
                        success -> {
    ...
    public Single<Device> update(Device item) {
        ...
        return Single.fromPublisher(
                        rememberDeviceMongoCollection.replaceOne(
                ...
                .map(
                        device ->
                                new Device()
                                        ...
                                        .setReferenceType(
                                                ReferenceType.valueOf(device.getReferenceType()))
