 ...
 */
@Component
public class MongoIdentityProviderRepository extends AbstractManagementMongoRepository
    ...
    public void init() {
        identitiesCollection =
                mongoOperations.getCollection("identities", IdentityProviderMongo.class);
    ...
    public Flowable<IdentityProvider> findAll(ReferenceType referenceType) {
        return Flowable.fromPublisher(
                        identitiesCollection.find(eq(FIELD_REFERENCE_TYPE, referenceType.name())))
                .map(this::convert);
    }
    ...
    public Maybe<IdentityProvider> findById(String identityProviderId) {
        return Observable.fromPublisher(
                        identitiesCollection.find(eq(FIELD_ID, identityProviderId)).first())
                .firstElement()
                .map(this::convert);
    }

    @Override
    public Maybe<IdentityProvider> findById(
    ...
    public Single<IdentityProvider> create(IdentityProvider item) {
        ...
        if (optionalIdp.isPresent()) {
            ...
            identityProvider.setId(
                    identityProvider.getId() == null
                            ? RandomString.generate()
                            : identityProvider.getId());
            return Single.fromPublisher(identitiesCollection.insertOne(identityProvider))
                    .flatMap(
                            success -> {
    ...
    public Single<IdentityProvider> update(IdentityProvider item) {
        ...
        if (optionalIdp.isPresent()) {
            ...
            return Single.fromPublisher(
                            identitiesCollection.replaceOne(
                                    eq(FIELD_ID, identityProvider.getId()), identityProvider))
    ...
    private IdentityProvider convert(IdentityProviderMongo identityProviderMongo) {
        ...
        if (identityProviderMongo.getRoleMapper() != null) {
            Map<String, String[]> roleMapper =
                    new HashMap<>(identityProviderMongo.getRoleMapper().size());
        ...
        identityProvider.setDomainWhitelist(
                ofNullable(identityProviderMongo.getDomainWhitelist())
                        .orElse(new BsonArray())
                        .stream()
        ...
        return ofNullable(identityProvider)
                ...
                .map(
                        idp -> {
                            ...
                            identityProviderMongo.setConfiguration(
                                    identityProvider.getConfiguration());
                            identityProviderMongo.setReferenceType(
                            ...
                            var mappers =
                                    new Document(
                                            (Map)
                                                    ofNullable(identityProvider.getMappers())
                                                            .filter(Objects::nonNull)
                                                            .orElse(Map.of()));
                            var roleMapper =
                                    new Document(
                                            convert(
                                                    ofNullable(identityProvider.getRoleMapper())
                                                            .filter(Objects::nonNull)
                                                            .orElse(Map.of())));
                            ...
                            identityProviderMongo.setDomainWhitelist(
                                    ofNullable(identityProvider.getDomainWhitelist())
                                            .orElse(List.of())
                                            .stream()
                                            .map(BsonString::new)
