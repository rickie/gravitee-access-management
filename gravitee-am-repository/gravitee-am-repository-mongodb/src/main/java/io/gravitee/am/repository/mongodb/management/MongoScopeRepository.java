 ...
 */
@Component
public class MongoScopeRepository extends AbstractManagementMongoRepository
    ...
    public Maybe<Scope> findById(String id) {
        return Observable.fromPublisher(scopesCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Scope> create(Scope item) {
        ...
        return Single.fromPublisher(scopesCollection.insertOne(scope))
                .flatMap(
                        success -> {
    ...
    public Single<Scope> update(Scope item) {
        ScopeMongo scope = convert(item);
        return Single.fromPublisher(scopesCollection.replaceOne(eq(FIELD_ID, scope.getId()), scope))
    ...
    public Single<Page<Scope>> findByDomain(String domain, int page, int size) {
        ...
        Single<Long> countOperation =
                Observable.fromPublisher(scopesCollection.countDocuments(mongoQuery)).first(0l);
        Single<List<Scope>> scopesOperation =
                Observable.fromPublisher(
                                scopesCollection.find(mongoQuery).skip(size * page).limit(size))
                        .map(this::convert)
                        .toList();
        return Single.zip(
                countOperation,
                scopesOperation,
                (count, scope) -> new Page<Scope>(scope, page, count));
    }
    ...
    public Single<Page<Scope>> search(String domain, String query, int page, int size) {
        ...
        Single<List<Scope>> scopesOperation =
                Observable.fromPublisher(
                                scopesCollection
                                        .find(mongoQuery)
                                        .sort(new BasicDBObject(FIELD_KEY, 1))
                                        .skip(size * page)
                                        .limit(size))
                        .map(this::convert)
                        .toList();
        return Single.zip(
                countOperation,
                scopesOperation,
                (count, scopes) -> new Page<>(scopes, page, count));
    }
    ...
    public Maybe<Scope> findByDomainAndKey(String domain, String key) {
        return Observable.fromPublisher(
                        scopesCollection
                                .find(and(eq(FIELD_DOMAIN, domain), eq(FIELD_KEY, key)))
                                .first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Flowable<Scope> findByDomainAndKeys(String domain, List<String> keys) {
        return Flowable.fromPublisher(
                        scopesCollection.find(and(eq(FIELD_DOMAIN, domain), in(FIELD_KEY, keys))))
                .map(this::convert);
    }
