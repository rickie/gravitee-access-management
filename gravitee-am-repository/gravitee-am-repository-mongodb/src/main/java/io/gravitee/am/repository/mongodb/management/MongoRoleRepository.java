 ...
 */
@Component
public class MongoRoleRepository extends AbstractManagementMongoRepository
                ...
                Observable.fromPublisher(
                                rolesCollection.countDocuments(
                                        and(
                                                ...
                                                eq(FIELD_REFERENCE_ID, referenceId))))
                        .first(0l);
        Single<List<Role>> rolesOperation =
                Observable.fromPublisher(
                                rolesCollection
                                        .find(
                                                and(
                        ...
                        .toList();
        return Single.zip(
                countOperation, rolesOperation, (count, roles) -> new Page<>(roles, page, count));
    }
    ...
    public Single<Page<Role>> search(
        ...
        Single<Long> countOperation =
                Observable.fromPublisher(rolesCollection.countDocuments(mongoQuery)).first(0l);
        Single<List<Role>> rolesOperation =
                Observable.fromPublisher(
                                rolesCollection.find(mongoQuery).skip(size * page).limit(size))
                        .map(this::convert)
                        .toList();
        return Single.zip(
                countOperation, rolesOperation, (count, roles) -> new Page<>(roles, 0, count));
    }
    ...
    public Maybe<Role> findById(String role) {
        return Observable.fromPublisher(rolesCollection.find(eq(FIELD_ID, role)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Role> create(Role item) {
        ...
        return Single.fromPublisher(rolesCollection.insertOne(role))
                .flatMap(
                        success -> {
    ...
    public Single<Role> update(Role item) {
        RoleMongo role = convert(item);
        return Single.fromPublisher(rolesCollection.replaceOne(eq(FIELD_ID, role.getId()), role))
    ...
    private Role convert(RoleMongo roleMongo) {
        ...
        role.setReferenceType(
                roleMongo.getReferenceType() == null
                        ? null
                        : ReferenceType.valueOf(roleMongo.getReferenceType()));
        role.setReferenceId(roleMongo.getReferenceId());
        role.setAssignableType(
                roleMongo.getAssignableType() == null
                        ? null
                        : ReferenceType.valueOf(roleMongo.getAssignableType()));
    ...
    private RoleMongo convert(Role role) {
        ...
        roleMongo.setReferenceType(
                role.getReferenceType() == null ? null : role.getReferenceType().name());
        roleMongo.setReferenceId(role.getReferenceId());
        roleMongo.setAssignableType(
                role.getAssignableType() == null ? null : role.getAssignableType().name());
        ...
        roleMongo.setPermissionAcls(
                role.getPermissionAcls() == null
                        ? null
                        : role.getPermissionAcls().entrySet().stream()
                                .collect(
                                        Collectors.toMap(
                                                o -> o.getKey().name(), Map.Entry::getValue)));
