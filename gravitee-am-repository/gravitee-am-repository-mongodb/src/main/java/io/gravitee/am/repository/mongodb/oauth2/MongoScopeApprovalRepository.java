 ...
 */
@Component
public class MongoScopeApprovalRepository extends AbstractOAuth2MongoRepository
    ...
    public Flowable<ScopeApproval> findByDomainAndUserAndClient(
        ...
        return Flowable.fromPublisher(
                        scopeApprovalsCollection.find(
                                and(
                                        eq(FIELD_DOMAIN, domain),
                                        eq(FIELD_CLIENT_ID, clientId),
    ...
    public Maybe<ScopeApproval> findById(String id) {
        return Observable.fromPublisher(scopeApprovalsCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<ScopeApproval> create(ScopeApproval scopeApproval) {
        ...
        scopeApprovalMongo.setId(
                scopeApprovalMongo.getId() == null
                        ? RandomString.generate()
                        : scopeApprovalMongo.getId());
        return Single.fromPublisher(scopeApprovalsCollection.insertOne(scopeApprovalMongo))
                .flatMap(success -> Single.just(scopeApproval));
    }
    ...
    public Single<ScopeApproval> update(ScopeApproval scopeApproval) {
        ...
        return Single.fromPublisher(
                        scopeApprovalsCollection.replaceOne(
                                ...
                                scopeApprovalMongo))
                .flatMap(updateResult -> Single.just(scopeApproval));
    }
    ...
    public Completable deleteByDomainAndScopeKey(String domain, String scope) {
        return Completable.fromPublisher(
                scopeApprovalsCollection.deleteMany(
    ...
    private Single<ScopeApproval> _findById(String id) {
        return Single.fromPublisher(scopeApprovalsCollection.find(eq(FIELD_ID, id)).first())
                .map(this::convert);
    }
    ...
    private ScopeApproval convert(ScopeApprovalMongo scopeApprovalMongo) {
        ...
        scopeApproval.setStatus(
                ScopeApproval.ApprovalStatus.valueOf(scopeApprovalMongo.getStatus().toUpperCase()));
