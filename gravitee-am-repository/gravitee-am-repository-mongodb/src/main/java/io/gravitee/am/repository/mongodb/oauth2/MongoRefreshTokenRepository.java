 ...
 */
@Component
public class MongoRefreshTokenRepository extends AbstractOAuth2MongoRepository
    ...
    public void init() {
        refreshTokenCollection =
                mongoOperations.getCollection("refresh_tokens", RefreshTokenMongo.class);
    ...
    private Maybe<RefreshToken> findById(String id) {
        return Observable.fromPublisher(refreshTokenCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Maybe<RefreshToken> findByToken(String token) {
        return Observable.fromPublisher(refreshTokenCollection.find(eq(FIELD_TOKEN, token)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<RefreshToken> create(RefreshToken refreshToken) {
        ...
        return Single.fromPublisher(refreshTokenCollection.insertOne(convert(refreshToken)))
    ...
    public Completable deleteByUserId(String userId) {
        return Completable.fromPublisher(
                refreshTokenCollection.deleteMany(eq(FIELD_SUBJECT, userId)));
    }
    ...
    private List<WriteModel<RefreshTokenMongo>> convert(List<RefreshToken> refreshTokens) {
        return refreshTokens.stream()
                .map(refreshToken -> new InsertOneModel<>(convert(refreshToken)))
