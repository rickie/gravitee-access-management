...
@Override
    public Mono<Flow> create_migrated(Flow item) {
        ...
        return RxJava2Adapter.singleToMono(Single.fromPublisher(flowsCollection.insertOne(flow))).flatMap(success->findById_migrated(flow.getId()).single());
    }
...
@Override
    public Mono<Flow> update_migrated(Flow item) {
        FlowMongo flow = convert(item);
        return RxJava2Adapter.singleToMono(Single.fromPublisher(flowsCollection.replaceOne(eq(FIELD_ID, flow.getId()), flow))).flatMap(updateResult->findById_migrated(flow.getId()).single());
    }
