 ...
 */
@Component
public class MongoFlowRepository extends AbstractManagementMongoRepository
    ...
    public Maybe<Flow> findById(String id) {
        return Observable.fromPublisher(flowsCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Flow> create(Flow item) {
        ...
        return Single.fromPublisher(flowsCollection.insertOne(flow))
                .flatMap(
                        success -> {
    ...
    public Single<Flow> update(Flow item) {
        FlowMongo flow = convert(item);
        return Single.fromPublisher(flowsCollection.replaceOne(eq(FIELD_ID, flow.getId()), flow))
    ...
    private FlowMongo convert(Flow flow) {
        ...
        flowMongo.setReferenceType(
                flow.getReferenceType() == null ? null : flow.getReferenceType().toString());
    ...
    private Flow convert(FlowMongo flowMongo) {
        ...
        flow.setReferenceType(
                flowMongo.getReferenceType() == null
                        ? null
                        : ReferenceType.valueOf(flowMongo.getReferenceType()));
