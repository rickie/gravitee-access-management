 ...
 */
@Repository
public class MongoUserActivityRepository extends AbstractManagementMongoRepository
    ...
    public void init() {
        userActivityCollection =
                mongoOperations.getCollection("user_activities", UserActivityMongo.class);
        ...
        super.createIndex(
                userActivityCollection,
                new Document(FIELD_EXPIRE_AT, 1),
                new IndexOptions().expireAfter(0L, TimeUnit.SECONDS));
    }

    @Override
    public Maybe<UserActivity> findById(String id) {
        return Observable.fromPublisher(userActivityCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }

    @Override
    public Flowable<UserActivity> findByReferenceAndTypeAndKeyAndLimit(
            ReferenceType referenceType, String referenceId, Type type, String key, int limit) {
    ...
    public Single<UserActivity> create(UserActivity item) {
        ...
        userActivity.setId(
                userActivity.getId() == null ? RandomString.generate() : userActivity.getId());
        return Single.fromPublisher(userActivityCollection.insertOne(userActivity))
                .flatMap(
                        success -> {
    ...
    public Single<UserActivity> update(UserActivity item) {
        ...
        return Single.fromPublisher(
                        userActivityCollection.replaceOne(
                                eq(FIELD_ID, userActivity.getId()), userActivity))
    ...
    @Override
    public Completable deleteByReferenceAndKey(
            ReferenceType referenceType, String referenceId, String key) {
        return Completable.fromPublisher(
                userActivityCollection.deleteMany(
                        and(
                                eq(FIELD_REFERENCE_TYPE, referenceType.name()),
                                eq(FIELD_REFERENCE_ID, referenceId),
                                eq(FIELD_USER_ACTIVITY_KEY, key))));
