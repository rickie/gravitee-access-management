 ...
 */
@Component
public class MongoPushedAuthorizationRequestRepository extends AbstractOAuth2MongoRepository
    ...
    public void init() {
        parCollection =
                mongoOperations.getCollection(
                        "pushed_authorization_requests", PushedAuthorizationRequestMongo.class);
    ...
    public Maybe<PushedAuthorizationRequest> findById(String id) {
        return Observable.fromPublisher(parCollection.find(eq(FIELD_ID, id)).limit(1).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<PushedAuthorizationRequest> create(PushedAuthorizationRequest item) {
        PushedAuthorizationRequestMongo par = convert(item);
        par.setId(item.getId() == null ? RandomString.generate() : item.getId());
        return Single.fromPublisher(parCollection.insertOne(par))
                .flatMap(
                        success -> {
                            item.setId(par.getId());
