 ...
 */
@Component
public class MongoAuthenticationFlowContextRepository extends AbstractManagementMongoRepository
        implements AuthenticationFlowContextRepository {
    ...
    @PostConstruct
    public void init() {
        authContextCollection =
                mongoOperations.getCollection(
                        "auth_flow_ctx", AuthenticationFlowContextMongo.class);
        super.init(authContextCollection);
        super.createIndex(
                authContextCollection,
                new Document(FIELD_TRANSACTION_ID, 1).append(FIELD_VERSION, -1));
        super.createIndex(
                authContextCollection,
    ...
    public Maybe<AuthenticationFlowContext> findById(String id) {
        return Observable.fromPublisher(authContextCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Maybe<AuthenticationFlowContext> findLastByTransactionId(String transactionId) {
        return Observable.fromPublisher(
                        authContextCollection
                                .find(
                                        and(
                                                eq(FIELD_TRANSACTION_ID, transactionId),
                                                gt(FIELD_RESET_TIME, new Date())))
                                .sort(new BasicDBObject(FIELD_VERSION, -1))
                                .first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Completable delete(String transactionId) {
        return Completable.fromPublisher(
                authContextCollection.deleteMany(eq(FIELD_TRANSACTION_ID, transactionId)));
    }

    @Override
    public Completable delete(String transactionId, int version) {
        return Completable.fromPublisher(
                authContextCollection.deleteOne(
                        and(eq(FIELD_TRANSACTION_ID, transactionId), eq(FIELD_VERSION, version))));
    }
