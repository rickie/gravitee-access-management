 ...
 */
@Component
public class MongoMembershipRepository extends AbstractManagementMongoRepository
    ...
    public void init() {
        ...
        super.createIndex(
                membershipsCollection,
                new Document(FIELD_REFERENCE_ID, 1).append(FIELD_MEMBER_ID, 1));
        super.createIndex(
                membershipsCollection,
                new Document(FIELD_MEMBER_ID, 1).append(FIELD_MEMBER_TYPE, 1));
    }
    ...
    public Flowable<Membership> findByCriteria(
        ...
        if (criteria.getGroupIds().isPresent()) {
            eqGroupId =
                    and(
                            eq(FIELD_MEMBER_TYPE, MemberType.GROUP.name()),
                            in(FIELD_MEMBER_ID, criteria.getGroupIds().get()));
        }

        if (criteria.getUserId().isPresent()) {
            eqUserId =
                    and(
                            eq(FIELD_MEMBER_TYPE, MemberType.USER.name()),
                            eq(FIELD_MEMBER_ID, criteria.getUserId().get()));
        }
        ...
        return toBsonFilter(criteria.isLogicalOR(), eqGroupId, eqUserId)
                ...
                .flatMapPublisher(
                        filter -> Flowable.fromPublisher(membershipsCollection.find(filter)))
                .map(this::convert);
    }
    ...
    public Maybe<Membership> findById(String id) {
        return Observable.fromPublisher(membershipsCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Membership> create(Membership item) {
        ...
        return Single.fromPublisher(membershipsCollection.insertOne(membership))
    ...
    public Single<Membership> update(Membership item) {
        ...
        return Single.fromPublisher(
                        membershipsCollection.replaceOne(
                                eq(FIELD_ID, membership.getId()), membership))
                .map(success -> convert(membership));
    }
