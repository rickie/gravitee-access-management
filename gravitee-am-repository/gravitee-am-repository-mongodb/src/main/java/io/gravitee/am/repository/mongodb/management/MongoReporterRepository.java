 ...
 */
@Component
public class MongoReporterRepository extends AbstractManagementMongoRepository
        implements ReporterRepository {
    ...
    public Flowable<Reporter> findByDomain(String domain) {
        return Flowable.fromPublisher(reportersCollection.find(eq(FIELD_DOMAIN, domain)))
                .map(this::convert);
    }

    @Override
    public Maybe<Reporter> findById(String id) {
        return Observable.fromPublisher(reportersCollection.find(eq(FIELD_ID, id)).first())
                .firstElement()
                .map(this::convert);
    }
    ...
    public Single<Reporter> create(Reporter item) {
        ...
        return Single.fromPublisher(reportersCollection.insertOne(reporter))
                .flatMap(
                        success -> {
    ...
    public Single<Reporter> update(Reporter item) {
        ...
        return Single.fromPublisher(
                        reportersCollection.replaceOne(eq(FIELD_ID, reporter.getId()), reporter))
