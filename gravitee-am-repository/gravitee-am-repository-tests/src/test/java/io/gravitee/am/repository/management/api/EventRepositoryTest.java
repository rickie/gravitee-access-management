...
public class EventRepositoryTest extends AbstractManagementTest {
    ...
    public void testFindByTimeFrame() throws TechnicalException {
        long from = 1571214259000l;
        long to = 1571214281000l;
        // create event
        ...
        eventBefore.setCreatedAt(
                new Date(Instant.ofEpochMilli(from).minusSeconds(30).getEpochSecond()));
        ...
        eventAfter.setCreatedAt(
                new Date(Instant.ofEpochMilli(to).plusSeconds(30).getEpochSecond()));
    ...
    public void testFindById() throws TechnicalException {
        ...
        event.setPayload(
                new Payload(
                        payload)); // duplicate the payload to avoid inner transformation that make
        // test failing
        ...
        testObserver.assertValue(
                e -> e.getPayload().keySet().size() == event.getPayload().keySet().size());
        testObserver.assertValue(
                e -> e.getPayload().keySet().containsAll(event.getPayload().keySet()));
        ...
        testObserver.assertValue(
                e -> e.getPayload().getReferenceId().equals(payload.getReferenceId()));
        testObserver.assertValue(
                e -> e.getPayload().getReferenceType().equals(payload.getReferenceType()));
    }
