...
public class AuthenticationFlowContextRepositoryTest extends AbstractManagementTest {
    ...
    @Autowired private AuthenticationFlowContextRepository authenticationFlowContextRepository;
    ...
    public void shouldNotFindSession() {
        TestSubscriber<AuthenticationFlowContext> observer =
                authenticationFlowContextRepository.findByTransactionId("unknown-sessions").test();
    ...
    public void shouldNotFindLastSession() {
        TestObserver<AuthenticationFlowContext> observer =
                authenticationFlowContextRepository
                        .findLastByTransactionId("unknown-sessions")
                        .test();
        observer.awaitTerminalEvent();
    ...
    public void shouldCreate() {
        ...
        TestObserver<AuthenticationFlowContext> observer =
                authenticationFlowContextRepository.create(entity).test();
        observer.awaitTerminalEvent();
    ...
    private void assertSameContext(
            AuthenticationFlowContext entity, TestObserver<AuthenticationFlowContext> observer) {
        observer.assertValue(
                ctx ->
                        ctx.getTransactionId() != null
                                && ctx.getTransactionId().equals(entity.getTransactionId()));
        observer.assertValue(ctx -> ctx.getVersion() == entity.getVersion());
        observer.assertValue(
                ctx -> ctx.getData() != null && ctx.getData().size() == entity.getData().size());
        observer.assertValue(
                ctx ->
                        ctx.getData().get("Key") != null
                                && ctx.getData().get("Key").equals(entity.getData().get("Key")));
    }
    ...
    public void shouldDelete() {
        ...
        TestSubscriber<AuthenticationFlowContext> testList =
                authenticationFlowContextRepository.findByTransactionId(TRANSACTION_ID).test();
        testList.awaitTerminalEvent();
        testList.assertNoErrors();
        testList.assertValueCount(2);

        TestObserver<Void> testObserver =
                authenticationFlowContextRepository.delete(TRANSACTION_ID).test();
    ...
    public void shouldDeleteSingle() {
        ...
        TestObserver<Void> testObserver =
                authenticationFlowContextRepository.delete(TRANSACTION_ID, 1).test();
        testObserver.awaitTerminalEvent();
        testObserver.assertNoErrors();
        ...
        AuthenticationFlowContext readValue =
                authenticationFlowContextRepository
                        .findByTransactionId(TRANSACTION_ID)
                        .blockingFirst();
        assertNotNull(readValue);
        assertEquals(
                "Expected version 2 because version 1 should be deleted",
                2,
    ...
    public void shouldFind() {
        ...
        TestObserver<AuthenticationFlowContext> testObserver =
                authenticationFlowContextRepository.findLastByTransactionId(TRANSACTION_ID).test();
        testObserver.awaitTerminalEvent();
        testObserver.assertNoErrors();
    ...
    public void shouldNotFind_NullTransactionId() {
        TestSubscriber<AuthenticationFlowContext> testList =
                authenticationFlowContextRepository.findByTransactionId(null).test();
        testList.awaitTerminalEvent();
        testList.assertNoValues();
        testList.assertNoErrors();
    }
    ...
    public void shouldNotFindExpiredData() {
        AuthenticationFlowContext entity =
                generateAuthContext(Instant.now().minus(10, ChronoUnit.MINUTES), 1);
        authenticationFlowContextRepository.create(entity).blockingGet();
        entity = generateAuthContext(Instant.now(), 2);
        authenticationFlowContextRepository.create(entity).blockingGet();
