...
public class IdentityProviderRepositoryTest extends AbstractManagementTest {
    ...
    public void testFindByDomain() throws TechnicalException {
        ...
        // fetch idps
        TestObserver<List<IdentityProvider>> testObserver =
                identityProviderRepository
                        .findAll(ReferenceType.DOMAIN, "testDomain")
                        .toList()
                        .test();
    ...
    private IdentityProvider buildIdentityProvider() {
        ...
        identityProvider.setDomainWhitelist(
                List.of("gmail.com", "hotmail.com", "customdomain.com"));
    ...
    public void testFindById() throws TechnicalException {
        // create idp
        IdentityProvider identityProvider = buildIdentityProvider();
        IdentityProvider identityProviderCreated =
                identityProviderRepository.create(identityProvider).blockingGet();

        // fetch idp
        TestObserver<IdentityProvider> testObserver =
                identityProviderRepository.findById(identityProviderCreated.getId()).test();
        testObserver.awaitTerminalEvent();
    ...
    private void assertEqualsTo(
            IdentityProvider identityProvider, TestObserver<IdentityProvider> testObserver) {
        ...
        testObserver.assertValue(
                idp -> idp.getConfiguration().equals(identityProvider.getConfiguration()));
        testObserver.assertValue(
                idp -> idp.getReferenceId().equals(identityProvider.getReferenceId()));
        testObserver.assertValue(
                idp -> idp.getReferenceType().equals(identityProvider.getReferenceType()));
        testObserver.assertValue(
                idp ->
                        idp.getMappers()
                                .entrySet()
                                .equals(identityProvider.getMappers().entrySet()));
        testObserver.assertValue(
                idp ->
                        idp.getRoleMapper()
                                .keySet()
                                .equals(identityProvider.getRoleMapper().keySet()));
        testObserver.assertValue(
                idp ->
                        idp.getRoleMapper().values().stream()
    ...
    public void testCreate() throws TechnicalException {
        ...
        identityProvider.setRoleMapper(
                Collections.singletonMap("username=johndoe", new String[] {"dev", "admin"}));
        TestObserver<IdentityProvider> testObserver =
                identityProviderRepository.create(identityProvider).test();
        testObserver.awaitTerminalEvent();
    ...
    public void testUpdate() throws TechnicalException {
        ...
        TestObserver<IdentityProvider> testObserver =
                identityProviderRepository.update(updatedIdentityProvider).test();
        testObserver.awaitTerminalEvent();
    ...
    public void testDelete() throws TechnicalException {
        ...
        TestObserver testObserver1 =
                identityProviderRepository.delete(identityProviderCreated.getId()).test();
