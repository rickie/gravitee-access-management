...
public class DeviceIdentifierPluginServiceImpl implements DeviceIdentifierPluginService {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(DeviceIdentifierPluginServiceImpl.class);
    ...
    public Maybe<DeviceIdentifierPlugin> findById(String deviceIdentifierId) {
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        var deviceIdentifierPlugin =
                                deviceIdentifierPluginManager.findById(deviceIdentifierId);
                        Optional.ofNullable(deviceIdentifierPlugin)
                                .ifPresentOrElse(
                                        plugin -> emitter.onSuccess(convert(plugin)),
                                        emitter::onComplete);
                    } catch (Exception ex) {
                        LOGGER.error(
                                "An error occurs while trying to get device identifier plugin : {}",
                                deviceIdentifierId,
                                ex);
                        emitter.onError(
                                new TechnicalManagementException(
                                        "An error occurs while trying to get factor plugin : "
                                                + deviceIdentifierId,
                                        ex));
                    }
                });
                ...
                emitter -> {
                    try {
                        ...
                        Optional.ofNullable(schema)
                                .ifPresentOrElse(emitter::onSuccess, emitter::onComplete);
                    } catch (Exception e) {
                        LOGGER.error(
                                "An error occurs while trying to get schema for device identifier plugin {}",
                                factorId,
                                e);
                        emitter.onError(
                                new TechnicalManagementException(
