...
public class ApplicationIdentityProviderUpgrader extends SystemTaskUpgrader {
    ...
    private final Logger logger =
            LoggerFactory.getLogger(ApplicationIdentityProviderUpgrader.class);
    ...
    @Lazy @Autowired private ApplicationService applicationRepository;

    @Lazy @Autowired private IdentityProviderRepository identityProviderRepository;

    @Override
    protected Single<Boolean> processUpgrade(
            String instanceOperationId, SystemTask task, String conditionalOperationId) {
                            ...
                            if (updatedTask.getOperationId().equals(instanceOperationId)) {
                                ...
                                return Single.error(
                                        new IllegalStateException(
                                                "Task "
                                                        + getTaskId()
                                                        + " already processed by another instance : trigger a retry"));
                            }
                        });
    }
    ...
    private Single<Boolean> migrateApplicationIdentityProviders(SystemTask task) {
        return applicationRepository
                ...
                .filter(
                        application ->
                                application.getIdentityProviders() != null
                ...
                .andThen(
                        updateSystemTask(task, SystemTaskStatus.SUCCESS, task.getOperationId())
                                ...
                                .onErrorResumeNext(
                                        (err) -> {
                                            logger.error(
                                                    "Unable to update application identityProviders options task: {}",
                                                    err.getMessage(),
                                                    err);
                ...
                .onErrorResumeNext(
                        (err) -> {
                            logger.error(
                                    "Unable to migrate application identityProvider options for applications: {}",
                                    err.getMessage(),
                                    err);
                            return Single.just(false);
                        });
    ...
    private Single<Application> addRuleInApplicationIdentityProvider(Application application) {
        ...
        return identityProviderRepository
                .findAll(DOMAIN, application.getDomain())
                ...
                .flatMap(
                        identities -> {
                            ...
                            for (ApplicationIdentityProvider appIdp :
                                    application.getIdentityProviders()) {
                                        ...
                                        var selectionRule =
                                                whitelist.stream()
                                                        .map(
                                                                pattern ->
                                                                        format(
                                                                                "#request.params['username'] matches '.+@%s$'",
                                                                                pattern))
                                                        .collect(joining(" || "));
                                        appIdp.setSelectionRule("{" + selectionRule + "}");
                                        logger.debug(
                                                "Rule '{}' added to application '{}' for identityProvider '{}'",
                ...
                .onErrorResumeNext(
                        err -> {
                            logger.error(
                                    "Unable to update application identityProvider options for applications: {}",
                                    err.getMessage(),
                                    err);
                            return Single.just(application);
                        });
    ...
    private Single<SystemTask> updateSystemTask(
            SystemTask task, SystemTaskStatus status, String operationId) {
