        ...
        return auditService
                ...
                .map(
                        values -> {
                            Timestamp timestamp =
                                    new Timestamp(
                                            query.getFrom(), query.getTo(), query.getInterval());
                            List<Bucket> buckets =
                                    values.entrySet().stream()
                                            .map(
                                                    entry -> {
                                                        ...
                                                        bucket.setData(
                                                                (List<Long>) entry.getValue());
                                                        return bucket;
                                                    })
                                            .collect(Collectors.toList());
                            AnalyticsHistogramResponse analyticsHistogramResponse =
                                    new AnalyticsHistogramResponse();
            ...
            case Field.APPLICATION:
                ...
                return executeGroupBy(query.getDomain(), queryBuilder.build(), query.getType())
                        .flatMap(
                                analyticsResponse ->
                                        fetchMetadata(
                                                (AnalyticsGroupByResponse) analyticsResponse));
            case Field.USER_STATUS:
            case Field.USER_REGISTRATION:
                return userService
                        .statistics(query)
                        .map(value -> new AnalyticsGroupByResponse(value));
            default:
                return executeGroupBy(query.getDomain(), queryBuilder.build(), query.getType());
        }
    }

    private Single<AnalyticsResponse> fetchMetadata(
            AnalyticsGroupByResponse analyticsGroupByResponse) {
        ...
        switch (query.getField()) {
            case Field.APPLICATION:
                return applicationService
                        .countByDomain(query.getDomain())
                        .map(value -> new AnalyticsCountResponse(value));
            case Field.USER:
                return userService
                        .countByDomain(query.getDomain())
                        .map(value -> new AnalyticsCountResponse(value));
                        ...
                        .map(
                                values ->
                                        values.values().isEmpty()
                                                ? new AnalyticsCountResponse(0l)
                                                : new AnalyticsCountResponse(
                                                        (Long) values.values().iterator().next()));
        }
    }

    private Single<AnalyticsResponse> executeGroupBy(
            String domain, AuditReportableCriteria criteria, Type type) {
        return auditService
                .aggregate(domain, criteria, type)
                .map(values -> new AnalyticsGroupByResponse(values));
    }
