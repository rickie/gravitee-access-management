    ...
    public Single<GoodbyeReply> handle(GoodbyeCommand command) {
        return installationService
                .addAdditionalInformation(
                        Collections.singletonMap(COCKPIT_INSTALLATION_STATUS, DELETED_STATUS))
                .flatMap(
                        installation ->
                                Single.just(
                                        new GoodbyeReply(command.getId(), CommandStatus.SUCCEEDED)))
                .doOnSuccess(reply -> logger.info("Installation has been removed."))
                .doOnError(
                        error -> logger.error("Error occurred when deleting installation.", error))
