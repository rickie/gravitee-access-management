 ...
 */
@Component
public class CertificateServiceProxyImpl extends AbstractSensitiveProxy
        implements CertificateServiceProxy {
    ...
    public Single<Certificate> create(
        ...
        return certificateService
                .create(domain, newCertificate, principal, isSystem)
                .flatMap(this::filterSensitiveData)
                .doOnSuccess(
                        certificate ->
                                auditService.report(
    ...
    public Single<Certificate> update(
        ...
        return certificateService
                ...
                .flatMap(
                        oldCertificate ->
                                filterSensitiveData(oldCertificate)
                                        .flatMap(
                                                safeOldCert ->
                                                        updateSensitiveData(
                                                                        updateCertificate,
    ...
    private Single<Certificate> filterSensitiveData(Certificate cert) {
        return certificatePluginService
                .getSchema(cert.getType())
                .switchIfEmpty(
                        Single.error(new CertificatePluginSchemaNotFoundException(cert.getType())))
                .map(
                        schema -> {
    ...
    private Single<UpdateCertificate> updateSensitiveData(
            UpdateCertificate updateCertificate, Certificate oldCertificate) {
        return certificatePluginService
                .getSchema(oldCertificate.getType())
                .switchIfEmpty(
                        Single.error(
                                new CertificatePluginSchemaNotFoundException(
                                        oldCertificate.getType())))
                .map(
                        schema -> {
                            var updateConfig =
                                    objectMapper.readTree(updateCertificate.getConfiguration());
                            var oldConfig =
                                    objectMapper.readTree(oldCertificate.getConfiguration());
