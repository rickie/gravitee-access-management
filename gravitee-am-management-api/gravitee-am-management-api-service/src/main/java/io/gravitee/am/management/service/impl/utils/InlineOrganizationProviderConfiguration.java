...
public class InlineOrganizationProviderConfiguration extends OrganizationProviderConfiguration {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(InlineOrganizationProviderConfiguration.class);

    public static final String MEMORY_TYPE = "memory";

    private static final List<String> authorizedRoles =
            Arrays.asList(
                    ORGANIZATION_OWNER.name(),
                    ORGANIZATION_USER.name(),
                    ORGANIZATION_PRIMARY_OWNER.name());
    ...
    public InlineOrganizationProviderConfiguration(
        ...
        while (found) {
            ...
            if (found) {
                ...
                if (StringUtils.isEmpty(user.getPassword())
                    ...
                    LOGGER.warn(
                            "User definition ignored for '{}': missing role or password", username);
                } else if (!authorizedRoles.contains(user.getRole())) {
                    LOGGER.warn(
                            "User definition ignored for '{}': invalid role. (expected: \"ORGANIZATION_OWNER\", \"ORGANIZATION_USER\", \"ORGANIZATION_PRIMARY_OWNER\")",
                            username);
                } else {
    ...
    public IdentityProvider buildIdentityProvider() {
        ...
        provider.setType(
                "inline-am-idp"); // use the inline provider implementation as InMemory provider
