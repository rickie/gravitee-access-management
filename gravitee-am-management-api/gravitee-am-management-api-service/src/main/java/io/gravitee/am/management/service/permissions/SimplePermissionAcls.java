...
class SimplePermissionAcls implements PermissionAcls {
    ...
    public SimplePermissionAcls(
            ReferenceType referenceType, String referenceId, Permission permission, Set<Acl> acls) {
        ...
        if (!permission.isRelevantWith(referenceType)) {
            throw new IllegalArgumentException(
                    String.format(
                            "The permission [%s] is not relevant with type [%s]. ",
                            permission, referenceType));
        }
        ...
        return permissions.keySet().stream()
                ...
                .flatMap(
                        membership ->
                                permissions
                                        .get(membership)
                                        .getOrDefault(permission, emptySet())
                                        .stream())
