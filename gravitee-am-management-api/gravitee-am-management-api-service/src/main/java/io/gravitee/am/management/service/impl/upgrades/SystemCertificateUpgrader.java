    ...
    private Single<Boolean> flagSystemCertificates(SystemTask task) {
        return domainService
                ...
                .flatMap(
                        domain ->
                                certificateRepository
                                        ...
                                        .filter(
                                                cert ->
                                                        // look for the certificates named "Default"
                                                        // and with a creation & update date close
                                                        // to the domain creation
                ...
                .andThen(
                        updateSystemTask(task, SystemTaskStatus.SUCCESS, task.getOperationId())
                                ...
                                .onErrorResumeNext(
                                        (err) -> {
                                            logger.error(
                                                    "Unable to update status for system certificates task: {}",
                                                    err.getMessage());
                ...
                .onErrorResumeNext(
                        (err) -> {
                            logger.error(
                                    "Unable to migrate system certificates: {}", err.getMessage());
                            return Single.just(false);
                        });
    }
