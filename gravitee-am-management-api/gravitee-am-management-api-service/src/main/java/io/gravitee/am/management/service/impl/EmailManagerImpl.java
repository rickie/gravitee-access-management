 ...
 */
@Component
public class EmailManagerImpl extends AbstractService<EmailManager>
        implements EmailManager, EventListener<EmailEvent, Payload> {
    ...
    @Autowired private TemplateLoader templateLoader;
    ...
    public void onEvent(Event<EmailEvent, Payload> event) {
        switch (event.type()) {
            ...
            default:
                logger.debug(
                        "{} event received for EmailTemplate {}, ignore it as it will be loaded on demand",
                        event.type(),
                        event.content().getId());
        }
    }
    ...
    @Override
    public Maybe<Email> getEmail(
            io.gravitee.am.model.Template templateDef,
            User user,
            String defaultSubject,
            int defaultExpiresAfter) {
        return getEmail0(
                templateDef,
    ...
    public Maybe<Email> getEmail(
            ...
            int defaultExpiresAfter) {
        return getEmail0(
                template, referenceType, referenceId, user, defaultSubject, defaultExpiresAfter);
    }

    private Maybe<Email> getEmail0(
            Template template,
            ReferenceType referenceType,
            String referenceId,
            User user,
                ...
                .map(
                        customEmail -> {
                            ...
                            if (localEmailTemplate != null
                                ...
                                return create(
                                        templateName,
                                        localEmailTemplate.getFrom(),
                                        localEmailTemplate.getFromName(),
                                        localEmailTemplate.getSubject(),
                                        localEmailTemplate.getExpiresAfter());
                            }
    ...
    private void removeEmail(String email) {
        ...
        Optional<Email> emailOptional =
                emailTemplates.values().stream()
                        .filter(email1 -> email.equals(email1.getId()))
                        .findFirst();
        if (emailOptional.isPresent()) {
            ...
            ((StringTemplateLoader) templateLoader)
                    .removeTemplate(getTemplateName(emailToRemove) + TEMPLATE_SUFFIX);
        }
    }
    ...
    private void reloadTemplate(String templateName, String content) {
        ((StringTemplateLoader) templateLoader)
                .putTemplate(templateName, content, System.currentTimeMillis());
    ...
    private Maybe<Email> innerGetEmail(
            io.gravitee.am.model.Template templateDef,
        ...
        if (user.getClient() == null) {
            return emailTemplateService
                    .findByTemplate(refType, referenceId, templateDef.template())
                    .filter(Email::isEnabled);
        }
        return emailTemplateService
                .findByClientAndTemplate(
                        refType, referenceId, user.getClient(), templateDef.template())
                .filter(Email::isEnabled)
                .switchIfEmpty(
                        Maybe.defer(
                                () ->
                                        emailTemplateService.findByTemplate(
                                                refType, referenceId, templateDef.template())))
                .filter(Email::isEnabled);
    }
