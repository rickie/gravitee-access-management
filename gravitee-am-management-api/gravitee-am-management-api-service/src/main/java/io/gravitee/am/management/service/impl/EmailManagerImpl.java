...
public class EmailManagerImpl extends AbstractService<EmailManager> implements EmailManager, EventListener<EmailEvent, Payload> {
    ...
    private void deployEmail(String emailId) {
        ...
        emailTemplateService.findById_migrated(emailId).subscribe(this::loadEmail, error -> logger.error("Unable to deploy email {}", emailId, error), () -> logger.error("No email found with id {}", emailId));
    }
