...
public class IdentityProviderPluginServiceImpl implements IdentityProviderPluginService {
    ...
    public Single<List<IdentityProviderPlugin>> findAll(Boolean external, List<String> expand) {
        ...
        return Observable.fromIterable(identityProviderPluginManager.getAllEntries().entrySet())
                ...
                .onErrorResumeNext(
                        ex -> {
                            LOGGER.error(
                                    "An error occurs while trying to list all identity provider plugins",
                                    ex);
                            return Single.error(
    ...
    public Maybe<IdentityProviderPlugin> findById(String identityProviderId) {
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        Plugin identityProvider =
                                identityProviderPluginManager.findById(identityProviderId);
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get identity provider plugin : {}",
                                identityProviderId,
                                ex);
                        emitter.onError(
                                new TechnicalManagementException(
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get schema for identity provider plugin {}",
                                identityProviderId,
                                e);
                        emitter.onError(
                                new TechnicalManagementException(
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get icon for identity provider plugin {}",
                                identityProviderId,
                                e);
                        emitter.onError(
                                new TechnicalManagementException(
    ...
    private IdentityProviderPlugin convert(Plugin identityProviderPlugin, List<String> expand) {
        ...
        if (expand != null) {
            ...
            if (expand.contains(IdentityProviderPluginService.EXPAND_DISPLAY_NAME)) {
                plugin.setDisplayName(
                        identityProviderPlugin
                                .manifest()
                                .properties()
                                .get(IdentityProviderPluginService.EXPAND_DISPLAY_NAME));
            }
            if (expand.contains(IdentityProviderPluginService.EXPAND_LABELS)) {
                String tags =
                        identityProviderPlugin
                                .manifest()
                                .properties()
                                .get(IdentityProviderPluginService.EXPAND_LABELS);
                if (tags != null) {
