...
/**
 * This class is used to read tasks from the SystemTask that may be scheduled. The goal is to ensure
 * that scheduled task are executed if the Management instance restart before the task execution.
 *
 * @author Eric LELEU (eric.leleu at graviteesource.com)
 * @author GraviteeSource Team
 */
@Component
public class TasksLoader extends AbstractService<TasksLoader>
        implements LifecycleComponent<TasksLoader> {

    private final Logger logger = LoggerFactory.getLogger(TaskManager.class);

    @Autowired private ObjectMapper mapper;
    @Autowired private TaskScheduler scheduler;
    @Autowired @Lazy private SystemTaskRepository taskRepository;
    @Autowired @Lazy private ApplicationService applicationService;
    @Autowired @Lazy private CertificateRepository certificateRepository;
    ...
    protected void doStart() throws Exception {
        ...
        this.taskRepository
                .findByType(TaskType.SIMPLE.name())
                // currently only one kind of Simple tasks, so we simply filter on this value for
                // safety
                .filter(
                        systemTask ->
                ...
                .subscribe(
                        task -> {
                            logger.debug(
                                    "Reschedule {} task of type {} with definition {}",
                                    task.type(),
                                    task.kind(),
                                    task.getDefinition());
