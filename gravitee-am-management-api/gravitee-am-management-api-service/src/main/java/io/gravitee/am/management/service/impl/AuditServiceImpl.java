...
public class AuditServiceImpl implements AuditService {
    ...
    public Single<Audit> findById(ReferenceType referenceType, String referenceId, String auditId) {
        try {
            return RxJava2Adapter.monoToSingle(RxJava2Adapter.maybeToMono(getReporter(referenceType, referenceId).findById(referenceType, referenceId, auditId)).switchIfEmpty(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(Mono.error(new AuditNotFoundException(auditId))))));
        } catch (Exception ex) {
