 ...
 */
@Component
public class ServiceResourceServiceProxyImpl extends AbstractSensitiveProxy
        implements ServiceResourceServiceProxy {

    @Autowired private ResourcePluginService resourcePluginService;

    @Autowired private ServiceResourceService serviceResourceService;
    ...
    public Single<ServiceResource> update(
            String domain, String id, UpdateServiceResource updateServiceResource, User principal) {
        return serviceResourceService
                .findById(id)
                .switchIfEmpty(Single.error(new ServiceResourceNotFoundException(id)))
                .flatMap(
                        oldResource ->
                                filterSensitiveData(oldResource)
                                        .flatMap(
                                                safeOldResource ->
                                                        updateSensitiveData(
    ...
    private Single<ServiceResource> filterSensitiveData(ServiceResource serviceResource) {
        return resourcePluginService
                .getSchema(serviceResource.getType())
    ...
    private Single<UpdateServiceResource> updateSensitiveData(
            UpdateServiceResource updateServiceResource, ServiceResource oldServiceResource) {
        return resourcePluginService
                .getSchema(oldServiceResource.getType())
                .switchIfEmpty(
                        Single.error(
                                new ResourcePluginNotFoundException(oldServiceResource.getType())))
                .map(
                        schema -> {
                            var updateConfig =
                                    objectMapper.readTree(updateServiceResource.getConfiguration());
                            var oldConfig =
                                    objectMapper.readTree(oldServiceResource.getConfiguration());
                            var schemaConfig = objectMapper.readTree(schema);
                            super.updateSensitiveData(
                                    updateConfig,
                                    oldConfig,
                                    schemaConfig,
                                    updateServiceResource::setConfiguration);
