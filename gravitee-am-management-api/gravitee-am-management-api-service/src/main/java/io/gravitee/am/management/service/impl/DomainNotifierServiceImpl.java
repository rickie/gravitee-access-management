...
public class DomainNotifierServiceImpl implements DomainNotifierService, InitializingBean {
    ...
    @Autowired private EmailNotifierConfiguration emailConfiguration;
    ...
    public void afterPropertiesSet() throws Exception {
        final String expiryThresholds =
                env.getProperty(
                        ...
                        String.class,
                        DEFAULT_CERTIFICATE_EXPIRY_THRESHOLDS);
        this.certificateExpiryThresholds =
                List.of(expiryThresholds.trim().split(",")).stream()
                        .map(String::trim)
                        .map(Integer::valueOf)
                    ...
                    .subscribe(
                            definition ->
                                    notifierService.register(
                                            definition,
                                            new CertificateNotificationCondition(
                                                    this.certificateExpiryThresholds),
                                            new CertificateResendNotificationCondition(
                                                    this.certificateExpiryThresholds)));
        }
    }
    ...
    public Completable deleteCertificateExpirationAcknowledgement(String certificateId) {
        if (this.certificateNotificationEnabled) {
            LOGGER.debug(
                    "Remove All NotificationAcknowledge for the certificate {}", certificateId);
    ...
    private Flowable<User> retrieveDomainOwners(Domain domain) {
        return findEnvironment(domain)
                .flatMapPublisher(
                        env ->
                                Maybe.concat(
                                                roleService.findSystemRole(
                                                        SystemRole.DOMAIN_PRIMARY_OWNER,
                                                        ReferenceType.DOMAIN),
                                                roleService.findDefaultRole(
                                                        env.getOrganizationId(),
                                                        DefaultRole.DOMAIN_OWNER,
                                                        ReferenceType.DOMAIN))
                                        .map(role -> role.getId())
                                        .flatMap(
                                                roleId -> {
                        ...
                        page -> {
                            if (page.getData().size() < 10) {
                                ...
                                return Flowable.concat(
                                        Flowable.fromIterable(page.getData()),
                                        readUsersFromGroup(membership, pageIndex + 1, size));
                            }
                        });
    }

    private Maybe<NotificationDefinition> buildEmailNotificationDefinition(
            Certificate certificate, Domain domain, User user) {
        if (emailNotifierEnabled && !Strings.isNullOrEmpty(user.getEmail())) {
            Map<String, Object> data =
                    new NotificationDefinitionUtils.ParametersBuilder()
                            .withDomain(domain)
                            .withUser(user)
            ...
            return emailService
                    .getFinalEmail(domain, null, Template.CERTIFICATE_EXPIRATION, user, data)
                    .map(
                            email -> {
                                EmailNotifierConfiguration notifierConfig =
                                        new EmailNotifierConfiguration(this.emailConfiguration);
            ...
            LOGGER.debug(
                    "Ignore email notification for certificate {}, email is disabled or email address is missing",
                    certificate.getId());
        }
        return Maybe.empty();
    }

    private Maybe<NotificationDefinition> buildUINotificationDefinition(
            Certificate certificate, Domain domain, User user) {
