 ...
 */
@Component
public class AuthenticationDeviceNotifierPluginServiceImpl
        implements AuthenticationDeviceNotifierPluginService {

    private final Logger LOGGER =
            LoggerFactory.getLogger(AuthenticationDeviceNotifierPluginServiceImpl.class);

    @Autowired private AuthenticationDeviceNotifierPluginManager pluginManager;
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get authentication device notifier plugin : {}",
                                pluginId,
                                ex);
                        emitter.onError(
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get schema for authentication device notifier plugin {}",
                                pluginId,
                                e);
                        emitter.onError(
                                new TechnicalManagementException(
    ...
    private AuthenticationDeviceNotifierPlugin convert(
            Plugin authDeviceNotifierPlugin, List<String> expand) {
