...
public class UserNotificationServiceImpl implements UserNotificationService {
    ...
    @Autowired @Lazy private UserNotificationRepository notificationRepository;

    @Autowired private ObjectMapper mapper;

    @Autowired @Lazy private ManagementUITemplateProvider uiTemplateProvider;
    ...
    public CompletableFuture<Void> send(Notification notification, Map<String, Object> param) {
        ...
        if (audience == null || domain == null) {
            ...
            logger.warn(
                    "Receive notification to store in database without user or domain, ignore it.");
            future.complete(null);

        } else {

            try {
                ManagementUINotifierConfiguration notifierConfiguration =
                        mapper.readValue(
                                notification.getConfiguration(),
                                ManagementUINotifierConfiguration.class);
                ...
                logger.debug(
                        "Receive notification to store in database for user '{}'",
                        audience.getId());
                ...
                notificationRepository
                        ...
                        .subscribe(
                                createdNotif -> {
                                    ...
                                    future.complete(
                                            null); // CompletableStage use the Void type. So it
                                    // requires null to be mapped properly in the
                                    // NotificationTrigger
                                },
                ...
                logger.warn(
                        "Unable to deserialize ManagementUI Notifier configuration : {}",
                        e.getMessage());
    ...
    @Override
    public Flowable<UserNotification> listAllNotifications(
            User user, UserNotificationStatus status) {
    ...
    public Completable markAsRead(User user, String notificationId) {
        return notificationRepository.updateNotificationStatus(
                notificationId, UserNotificationStatus.READ);
    }
}
