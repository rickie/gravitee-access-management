...
public class OrganizationCommandHandler
    ...
    public Single<OrganizationReply> handle(OrganizationCommand command) {
        ...
        return organizationService
                .createOrUpdate(organizationPayload.getId(), newOrganization, null)
                .map(
                        organization ->
                                new OrganizationReply(command.getId(), CommandStatus.SUCCEEDED))
                .doOnSuccess(
                        reply ->
                                logger.info(
                                        "Organization [{}] handled with id [{}].",
                ...
                .doOnError(
                        error ->
                                logger.error(
                                        ...
                                        error))
                .onErrorReturn(
                        throwable -> new OrganizationReply(command.getId(), CommandStatus.ERROR));
    }
}
