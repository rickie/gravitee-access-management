...
public class ExtensionGrantPluginServiceImpl implements ExtensionGrantPluginService {
    ...
    public Single<Set<ExtensionGrantPlugin>> findAll() {
        ...
        return Single.create(
                emitter -> {
                    try {
                        emitter.onSuccess(
                                extensionGrantPluginManager.getAll().stream()
                                        .map(this::convert)
                                        .collect(Collectors.toSet()));
                    } catch (Exception ex) {
                        LOGGER.error(
                                "An error occurs while trying to list all extension grant plugins",
                                ex);
                        emitter.onError(
    ...
    public Maybe<ExtensionGrantPlugin> findById(String extensionGrantPluginId) {
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        Plugin extensionGrant =
                                extensionGrantPluginManager.findById(extensionGrantPluginId);
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get extension grant plugin : {}",
                                extensionGrantPluginId,
                                ex);
                        emitter.onError(
                                new TechnicalManagementException(
                                        "An error occurs while trying to get extension grant plugin : "
    ...
    public Maybe<String> getSchema(String extensionGrantPluginId) {
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        String schema =
                                extensionGrantPluginManager.getSchema(extensionGrantPluginId);
                        if (schema != null) {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get schema for extension grant plugin {}",
                                extensionGrantPluginId,
                                e);
                        emitter.onError(
                                new TechnicalManagementException(
