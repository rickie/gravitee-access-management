...
@Deprecated  
default Single<Set<ExtensionGrantPlugin>> findAll() {
    return RxJava2Adapter.monoToSingle(findAll_migrated());
}
default Mono<Set<ExtensionGrantPlugin>> findAll_migrated() {
    return RxJava2Adapter.singleToMono(findAll());
}

      @InlineMe(replacement = "RxJava2Adapter.monoToMaybe(this.findById_migrated(id))", imports = "reactor.adapter.rxjava.RxJava2Adapter")
@Deprecated  
default Maybe<ExtensionGrantPlugin> findById(String id) {
    return RxJava2Adapter.monoToMaybe(findById_migrated(id));
}
default Mono<ExtensionGrantPlugin> findById_migrated(String id) {
