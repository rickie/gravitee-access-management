    ...
    public Single<InstallationReply> handle(InstallationCommand command) {
        ...
        return installationService
                ...
                .doOnSuccess(
                        additionalInfos ->
                                additionalInfos.put(
                                        Installation.COCKPIT_INSTALLATION_STATUS,
                                        installationPayload.getStatus()))
                .flatMap(installationService::setAdditionalInformation)
                .map(
                        installation ->
                                new InstallationReply(command.getId(), CommandStatus.SUCCEEDED))
                .doOnSuccess(
                        installation ->
                                logger.info(
                                        "Installation status is [{}].",
                                        installationPayload.getStatus()))
                .doOnError(
                        error ->
                                logger.info(
                                        "Error occurred when updating installation status.", error))
                .onErrorReturn(
                        throwable -> new InstallationReply(command.getId(), CommandStatus.ERROR));
    }
}
