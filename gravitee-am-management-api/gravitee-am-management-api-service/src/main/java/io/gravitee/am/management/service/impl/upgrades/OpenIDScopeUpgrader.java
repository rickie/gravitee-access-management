...
public class OpenIDScopeUpgrader implements Upgrader, Ordered {
    ...
    private Single<Scope> createSystemScope(
            String domain, io.gravitee.am.common.oidc.Scope systemScope) {
        return scopeService
                ...
                .flatMapSingle(
                        optScope -> {
                            if (!optScope.isPresent()) {
                                logger.info(
                                        "Create a new system scope key[{}] for domain[{}]",
                                        systemScope.getKey(),
                                        domain);
                                ...
                                scope.setName(
                                        existingScope.getName() != null
                                                ...
                                                : systemScope.getLabel());
                                scope.setDescription(
                                        existingScope.getDescription() != null
                                                ? existingScope.getDescription()
                                                : systemScope.getDescription());
                                scope.setClaims(systemScope.getClaims());
    ...
    /**
     * Update System scope if it is not currently set as system or if discovery property does not
     * match.
     *
     * @param optScope
     * @param systemScope
     * @return
     */
    private boolean shouldUpdateSystemScope(
            Optional<Scope> optScope, io.gravitee.am.common.oidc.Scope systemScope) {
        // If not currently set as system or if discovery property does not match
        return !optScope.get().isSystem()
                || optScope.get().isDiscovery() != systemScope.isDiscovery();
    }
