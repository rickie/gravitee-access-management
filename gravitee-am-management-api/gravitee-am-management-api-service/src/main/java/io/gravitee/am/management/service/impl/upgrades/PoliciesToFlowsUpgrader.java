...
public class PoliciesToFlowsUpgrader implements Upgrader, Ordered {
    ...
    @Lazy @Autowired private PolicyRepository policyRepository;

    @Autowired private FlowService flowService;

    @Autowired private PolicyPluginService policyPluginService;
        ...
        policyRepository
                ...
                .flatMapCompletable(
                        collectionExists -> {
                            if (collectionExists) {
                                LOGGER.info(
                                        "Policies collection exists, upgrading policies to flows");
                ...
                .subscribe(
                        ...
                        error ->
                                LOGGER.error(
                                        "An error occurs while updating policies to flows", error));
    ...
    private Completable migrateToFlows(List<Policy> policies, String domain) {
        ...
        Map<ExtensionPoint, List<Policy>> policiesPerExtPoint =
                policies.stream().collect(Collectors.groupingBy(Policy::getExtensionPoint));
        Map<Type, Flow> flows =
                flowService.defaultFlows(ReferenceType.DOMAIN, domain).stream()
                        .collect(Collectors.toMap(Flow::getType, identity()));
        ...
        for (Map.Entry<ExtensionPoint, List<Policy>> epPolicies : policiesPerExtPoint.entrySet()) {
            ...
            switch (epPolicies.getKey()) {
                case ROOT:
                    flows.get(Type.ROOT)
                            .setPre(
                                    epPolicies.getValue().stream()
                                            .map(this::createStep)
                                            .collect(Collectors.toList()));
                    break;
                case PRE_CONSENT:
                    flows.get(Type.CONSENT)
                            .setPre(
                                    epPolicies.getValue().stream()
                                            .map(this::createStep)
                                            .collect(Collectors.toList()));
                ...
                default:
                    LOGGER.info(
                            "ExtensionPoint '{}' shouldn't be present before version 3.5, ignore it",
                            epPolicies.getKey());
            }
        }
        ...
        return Observable.fromIterable(flows.values())
                .flatMapCompletable(
                        flow ->
                                flowService
                                        .create(ReferenceType.DOMAIN, domain, flow)
                                        .toCompletable())
                .doOnComplete(() -> LOGGER.info("Policies migrated to flows for domain {}", domain))
                .doOnError(
                        (error) ->
                                LOGGER.info(
                                        "Error during policies migration for domain {}",
                                        domain,
                                        error));
    }
