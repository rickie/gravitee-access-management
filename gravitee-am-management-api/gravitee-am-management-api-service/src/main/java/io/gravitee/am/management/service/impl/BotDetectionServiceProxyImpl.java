 ...
 */
@Component
public class BotDetectionServiceProxyImpl extends AbstractSensitiveProxy
        implements BotDetectionServiceProxy {

    @Autowired private BotDetectionService botDetectionService;

    @Autowired private BotDetectionPluginService botDetectionPluginService;
    ...
    public Maybe<BotDetection> findById(String id) {
        return botDetectionService
                .findById(id)
                .flatMap(reporter -> filterSensitiveData(reporter).toMaybe());
    }
    ...
    public Single<BotDetection> create(
            String domain, NewBotDetection botDetection, User principal) {
        return botDetectionService
                .create(domain, botDetection, principal)
    ...
    public Single<BotDetection> update(
        ...
        return botDetectionService
                ...
                .flatMap(
                        oldBotDetection ->
                                filterSensitiveData(oldBotDetection)
                                        .flatMap(
                                                safeOldBoDetection ->
                                                        updateSensitiveData(
                                                                        updateBotDetection,
    ...
    private Single<BotDetection> filterSensitiveData(BotDetection botDetection) {
        return botDetectionPluginService
                .getSchema(botDetection.getType())
    ...
    private Single<UpdateBotDetection> updateSensitiveData(
            UpdateBotDetection updateBotDetection, BotDetection oldBotDetection) {
        return botDetectionPluginService
                .getSchema(oldBotDetection.getType())
                .switchIfEmpty(
                        Single.error(
                                new BotDetectionPluginSchemaNotFoundException(
                                        oldBotDetection.getType())))
                .map(
                        schema -> {
                            var updateConfig =
                                    objectMapper.readTree(updateBotDetection.getConfiguration());
                            var oldConfig =
                                    objectMapper.readTree(oldBotDetection.getConfiguration());
                            var schemaConfig = objectMapper.readTree(schema);
                            super.updateSensitiveData(
                                    updateConfig,
                                    oldConfig,
                                    schemaConfig,
                                    updateBotDetection::setConfiguration);
