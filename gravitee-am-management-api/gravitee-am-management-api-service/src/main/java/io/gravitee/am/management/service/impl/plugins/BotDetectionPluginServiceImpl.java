...
public class BotDetectionPluginServiceImpl implements BotDetectionPluginService {
    ...
    public Single<List<BotDetectionPlugin>> findAll() {
        ...
        return Observable.fromIterable(pluginManager.getAll()).map(this::convert).toList();
    }
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get bot detection plugin : {}",
                                pluginId,
                                ex);
                        emitter.onError(
        ...
        return Maybe.create(
                emitter -> {
                    try {
                        ...
                        LOGGER.error(
                                "An error occurs while trying to get schema for bot detection plugin {}",
                                pluginId,
                                e);
                        emitter.onError(
                                new TechnicalManagementException(
