    ...
    private Single<Boolean> migrateScopeSettings(SystemTask task) {
        return applicationService
                        ...
                        app -> {
                            ...
                            if (app.getSettings() != null && app.getSettings().getOauth() != null) {
                                ...
                                if (oauthSettings.getScopes() != null
                                        && !oauthSettings.getScopes().isEmpty()) {
                                    logger.debug(
                                            "Process scope options for application '{}'",
                                            app.getId());
                                    for (String scope : oauthSettings.getScopes()) {
                                        ApplicationScopeSettings setting =
                                                new ApplicationScopeSettings();
                                        setting.setScope(scope);
                                        setting.setDefaultScope(
                                                oauthSettings.getDefaultScopes() != null
                                                        && oauthSettings
                                                                .getDefaultScopes()
                                                                .contains(scope));
                                        if (oauthSettings.getScopeApprovals() != null
                                                && oauthSettings
                                                        .getScopeApprovals()
                                                        .containsKey(scope)) {
                                            setting.setScopeApproval(
                                                    oauthSettings.getScopeApprovals().get(scope));
                                        }
                                    ...
                                    logger.debug(
                                            "Update settings for application '{}'", app.getId());
                                    return applicationService.update(app);
                                } else {
                                    logger.debug(
                                            "No scope to process for application '{}'",
                                            app.getId());
                                }
                            } else {
                                logger.debug(
                                        "No scope to process for application '{}'", app.getId());
                            }
                ...
                .andThen(
                        updateSystemTask(task, SystemTaskStatus.SUCCESS, task.getOperationId())
                                ...
                                .onErrorResumeNext(
                                        (err) -> {
                                            logger.error(
                                                    "Unable to update status for migrate scope options task: {}",
                                                    err.getMessage());
                ...
                .onErrorResumeNext(
                        (err) -> {
                            logger.error(
                                    "Unable to migrate scope options for applications: {}",
                                    err.getMessage());
                            return Single.just(false);
                        });
    }
