...
public class ResolvePropertyCommandHandler implements TriggerProvider.OnCommandResultListener {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ResolvePropertyCommandHandler.class);
    ...
    public ResolvePropertyCommandHandler(
            DomainService domainService, ApplicationService applicationService) {
    ...
    public <T> void doOnCommand(Command command, Handler<T> resultHandler) {
        ...
        if (command instanceof ResolvePropertyCommand) {
            resolveProperties((ResolvePropertyCommand) command)
                    .subscribe(
                            result -> resultHandler.handle((T) result),
                            error -> resultHandler.handle(null));
        } else {
    ...
    private Single<Map<String, Map<String, Object>>> resolveProperties(
            ResolvePropertyCommand command) {
        ...
        if (commandProperties != null) {
            commandProperties.forEach(
                    (key, value) -> {
                        if (RESOLVE_DOMAIN_PROPERTIES_KEY.equals(key)) {
                            obs.add(
                                    resolveDomainProperties(value)
                                            .doOnSuccess(
                                                    domainProperties ->
                                                            values.put(key, domainProperties)));
                        } else if (RESOLVE_APPLICATION_PROPERTIES_KEY.equals(key)) {
                            obs.add(
                                    resolveApplicationProperties(value)
                                            .doOnSuccess(
                                                    appProperties ->
                                                            values.put(key, appProperties)));
                        }
                    });
        }

        return Single.merge(obs).ignoreElements().andThen(Single.just(values));
    }
