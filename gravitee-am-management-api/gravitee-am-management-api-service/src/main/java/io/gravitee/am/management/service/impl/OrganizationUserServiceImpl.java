...
public class OrganizationUserServiceImpl
        extends AbstractUserService<io.gravitee.am.service.OrganizationUserService>
        implements OrganizationUserService {

    public static final BCryptPasswordEncoder PWD_ENCODER = new BCryptPasswordEncoder();

    @Autowired private io.gravitee.am.service.OrganizationUserService userService;
    ...
    @Override
    public Single<User> createOrUpdate(
            ReferenceType referenceType, String referenceId, NewUser newUser) {

        return userService
                .findByExternalIdAndSource(
                        referenceType, referenceId, newUser.getExternalId(), newUser.getSource())
                .switchIfEmpty(
                        Maybe.defer(
                                () ->
    ...
    public Single<User> createGraviteeUser(
            Organization organization,
            NewUser newUser,
            io.gravitee.am.identityprovider.api.User principal) {
        // Organization user are linked to the Gravitee Idp only
        if (!Strings.isNullOrEmpty(newUser.getSource())
                && !IDP_GRAVITEE.equals(newUser.getSource())) {
            return Single.error(
                    new UserInvalidException(
                            "Invalid identity provider for ['" + newUser.getUsername() + "']"));
        }
                                ...
                                return identityProviderManager
                                        ...
                                        .flatMapSingle(
                                                userProvider -> {
                                                    ...
                                                    // - perform first validation of user to avoid
                                                    // error status 500 when the IDP is based on
                                                    ...
                                                    final User userToPersist =
                                                            transform(
                                                                    ...
                                                                    ReferenceType.ORGANIZATION,
                                                                    organization.getId());
                                                    userToPersist.setReferenceId(
                                                            organization.getId());
                                                    userToPersist.setReferenceType(
                                                            ReferenceType.ORGANIZATION);

                                                    return userValidator
                                                            .validate(userToPersist)
                                                            .doOnError(
                                                                    throwable ->
                                                                            auditService.report(
                                                                                    AuditBuilder
                                                                    ...
                                                                    userProvider
                                                                            ...
                                                                            .map(
                                                                                    idpUser -> {
                                                                                        // Excepted
                                                                                        // for
                                                                                        // GraviteeIDP that manage Organization Users
                                                                                        // AM
                                                                                        // 'users'
                                                                                        // collection is not made for authentication (but only management stuff)
                                                                                        userToPersist
                                                                                                .setPassword(
                                                                                                        PWD_ENCODER
                                                                                                                .encode(
    ...
    public Completable resetPassword(
        ...
        if (password == null || !passwordService.isValid(password)) {
            return Completable.error(
                    InvalidPasswordException.of(
                            "Field [password] is invalid", "invalid_password_value"));
        }

        if (!IDP_GRAVITEE.equals(user.getSource())) {
            return Completable.error(
                    new InvalidUserException("Unsupported source for this action"));
        }
    ...
    @Override
    public Single<User> updateLogoutDate(
