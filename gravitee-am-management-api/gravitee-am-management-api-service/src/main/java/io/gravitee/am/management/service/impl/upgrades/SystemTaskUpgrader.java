...
public abstract class SystemTaskUpgrader implements Upgrader, Ordered {
    ...
    @Lazy @Autowired protected SystemTaskRepository systemTaskRepository;
                                    ...
                                    switch (SystemTaskStatus.valueOf(task.getStatus())) {
                                        case INITIALIZED:
                                            return processUpgrade(
                                                    instanceOperationId, task, instanceOperationId);
                                        case FAILURE:
                                            // In Failure case, we use the operationId of the read
                                            // task otherwise update will always fail
                                            // force the task.operationId to assign the task to the
                                            // instance
                                            String previousOperationId = task.getOperationId();
                                            task.setOperationId(instanceOperationId);
                                            return processUpgrade(
                                                    instanceOperationId, task, previousOperationId);
                                        case ONGOING:
    ...
    protected abstract Single<Boolean> processUpgrade(
            String instanceOperationId, SystemTask task, String previousOperationId);
    ...
    protected Single<SystemTask> createSystemTask(String operationId) {
        ...
        return systemTaskRepository
                .create(systemTask)
                .onErrorResumeNext(
                        err -> {
                            logger.warn(
                                    "SystemTask {} can't be created due to '{}'",
                                    taskId,
                                    err.getMessage());
                            // if the creation fails, try to find the task, this will allow to
                            // manage the retry properly
                    ...
                    (throwable) -> {
                        if (++retryCount < maxRetries) {
                            ...
                            return Flowable.timer(
                                    (long) retryDelayMillis * (retryCount + 1),
