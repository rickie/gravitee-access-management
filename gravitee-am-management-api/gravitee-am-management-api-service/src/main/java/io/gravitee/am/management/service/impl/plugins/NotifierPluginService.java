...
public class NotifierPluginService {
    ...
    public static final String DEFAULT_NOTIFIER_MESSAGE =
            "An alert '${alert.name}' has been fired.\\n"
                    + "\\n"
    ...
    public NotifierPluginService(
            @Lazy NotifierPluginManager notifierPluginManager, ObjectMapper objectMapper) {
                        ...
                        throwable -> {
                            return Flowable.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to get notifier plugins",
                                            throwable));
                        });
    }
        ...
        return Maybe.fromCallable(() -> notifierPluginManager.findById(notifierId))
                ...
                .onErrorResumeNext(
                        throwable -> {
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to get notifier plugin "
                                                    + notifierId,
                                            throwable));
                        })
                .switchIfEmpty(
                        Single.defer(
                                () ->
                                        Single.error(
                                                new NotifierPluginNotFoundException(notifierId))));
    }
        ...
        return Maybe.fromCallable(() -> notifierPluginManager.getSchema(notifierId))
                ...
                .onErrorResumeNext(
                        throwable -> {
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to get schema for notifier plugin "
                                                    + notifierId,
                                            throwable));
                        })
                .switchIfEmpty(
                        Single.defer(
                                () ->
                                        Single.error(
                                                new NotifierPluginSchemaNotFoundException(
                                                        notifierId))));
    }
        ...
        return Maybe.fromCallable(() -> notifierPluginManager.getIcon(notifierId))
                .onErrorResumeNext(
                        throwable -> {
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to get incon for notifier plugin "
                                                    + notifierId,
                                            throwable));
                        });
    }
        ...
        return Maybe.fromCallable(() -> notifierPluginManager.getDocumentation(notifierId))
                .onErrorResumeNext(
                        throwable -> {
                            return Maybe.error(
                                    new TechnicalManagementException(
                                            "An error occurs while trying to get documentation for notifier plugin "
                                                    + notifierId,
                                            throwable));
                        });
    }
        ...
        if (expand != null) {
            ...
            if (expandList.contains(EXPAND_ICON)) {
                return this.getIcon(notifierPlugin.getId())
                        ...
                        .ignoreElement()
                        .andThen(Single.just(notifierPlugin));
            }
        }
