...
public class ApplicationIdentityProviderUpgraderTest {
    ...
    private ApplicationIdentityProviderUpgrader upgrader =
            new ApplicationIdentityProviderUpgrader();
    ...
    public void shouldMigrate() {
        ...
        when(identityProviderRepository.findAll(eq(ReferenceType.DOMAIN), eq(domain1)))
                .thenReturn(Flowable.just(idp1, idp2, idp3));
        ...
        verify(applicationService, atMost(1))
                .update(
                        argThat(
                                app ->
                                        app.getIdentityProviders().stream()
                                                .anyMatch(
                                                        appIdp ->
                                                                appIdp.getSelectionRule() != null
                                                                        && Objects.equals(
                                                                                appIdp
                                                                                        .getSelectionRule(),
