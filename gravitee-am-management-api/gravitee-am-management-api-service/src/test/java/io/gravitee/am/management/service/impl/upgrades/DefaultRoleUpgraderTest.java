...
public class DefaultRoleUpgraderTest {
    ...
    public void shouldCreateSystemRoles() {
        when(roleService.createOrUpdateSystemRoles_migrated()).thenReturn(Mono.empty());
        cut.upgrade();
    }

    @Test
    public void shouldCreateSystemRoles_technicalError() {
        when(roleService.createOrUpdateSystemRoles_migrated()).thenReturn(Mono.error(TechnicalException::new));
