...
public class DefaultIdentityProviderUpgraderTest {
    ...
    private DefaultIdentityProviderUpgrader defaultIdentityProviderUpgrader =
            new DefaultIdentityProviderUpgrader();
    ...
    public void shouldUpdateDefaultIdp() {
        ...
        when(identityProviderManager.createProviderConfiguration(anyString(), isNull()))
                .thenReturn("new-config-created");
        when(identityProviderService.update(
                        anyString(), anyString(), any(UpdateIdentityProvider.class), eq(true)))
                .thenReturn(changeIdpConfig(true));
        ...
        verify(identityProviderService, times(1))
                .update(
                        anyString(),
                        anyString(),
                        argThat(upIdp -> upIdp.getConfiguration().equals("new-config-created")),
                        anyBoolean());
    }
    ...
    public void shouldNotUpdateDefaultIdp() {
        ...
        when(identityProviderService.findAll())
                .thenReturn(Flowable.just(nonSystemIdentityProvider));
