...
public class DefaultOrganizationUpgraderTest {
    ...
    @Mock private MembershipHelper membershipHelper;
    ...
    @Before
    public void before() {
        when(environment.getProperty("security.defaultAdmin", boolean.class, true))
                .thenReturn(true);
        cut =
    ...
    public void shouldCreateDefaultOrganization() {
        ...
        when(organizationService.update(
                        eq(Organization.DEFAULT), any(PatchOrganization.class), isNull()))
                        ...
                        argThat(
                                user ->
                                        !user.isInternal()
                                                ...
                                                && user.getReferenceId()
                                                        .equals(Organization.DEFAULT))))
                .thenReturn(Single.just(adminUser));
        when(domainService.findById("admin")).thenReturn(Maybe.empty());
        doNothing()
                .when(membershipHelper)
                .setOrganizationPrimaryOwnerRole(
                        argThat(user -> user.getId().equals(adminUser.getId())));
        when(organizationService.findById(Organization.DEFAULT))
                .thenReturn(Single.just(organization));
    ...
    public void shouldCreateAdminUser_noAdminUser() {
        ...
        when(identityProviderService.findAll(ReferenceType.ORGANIZATION, Organization.DEFAULT))
                .thenReturn(Flowable.just(idp));
        when(userService.findByUsernameAndSource(
                        ReferenceType.ORGANIZATION, Organization.DEFAULT, "admin", idp.getId()))
    ...
    public void shouldCreateSystemRoles_setOwnerRoleToExistingUsers() {
        ...
        List<User> users =
                Stream.iterate(0, i -> i++).limit(10).map(i -> user).collect(Collectors.toList());
        ...
        when(userService.findAll(
                        eq(ReferenceType.ORGANIZATION), eq(Organization.DEFAULT), eq(0), anyInt()))
                .thenReturn(Single.just(new Page<>(users, 0, totalUsers)));

        when(userService.findAll(
                        eq(ReferenceType.ORGANIZATION), eq(Organization.DEFAULT), eq(1), anyInt()))
                .thenReturn(Single.just(new Page<>(users, 1, totalUsers)));

        when(userService.findAll(
                        eq(ReferenceType.ORGANIZATION), eq(Organization.DEFAULT), eq(2), anyInt()))
