...
public class EmailManagerTest {
    ...
    @InjectMocks private EmailManager emailManager = new EmailManagerImpl();

    @Mock private StringTemplateLoader templateLoader;
    ...
    public void shouldGetTemplate() {
        when(emailTemplateService.findByTemplate(
                        eq(ReferenceType.DOMAIN),
                        eq(REFERENCE_ID),
                        eq(Template.REGISTRATION_CONFIRMATION.template())))
                .thenReturn(Maybe.empty());
        when(emailTemplateService.findByClientAndTemplate(
                        eq(ReferenceType.DOMAIN),
                        eq(REFERENCE_ID),
                        eq(CLIENT),
                        eq(Template.REGISTRATION_CONFIRMATION.template())))
        ...
        Email email =
                emailManager
                        .getEmail(Template.REGISTRATION_CONFIRMATION, user, "subject", 1000)
                        .blockingGet();
    ...
    public void shouldGetTemplate_domainTemplate() {
        ...
        when(emailTemplateService.findByTemplate(
                        ...
                        eq(REFERENCE_ID),
                        eq(Template.REGISTRATION_CONFIRMATION.template())))
                .thenReturn(Maybe.just(domainEmail));
        when(emailTemplateService.findByClientAndTemplate(
                        eq(ReferenceType.DOMAIN),
                        eq(REFERENCE_ID),
                        eq(CLIENT),
    ...
    public void shouldGetTemplate_clientTemplate() {
        ...
        Email email =
                emailManager.getEmail(Template.RESET_PASSWORD, user, "subject", 1000).blockingGet();
