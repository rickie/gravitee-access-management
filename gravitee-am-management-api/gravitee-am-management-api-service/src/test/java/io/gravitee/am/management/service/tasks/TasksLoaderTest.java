...
public class TasksLoaderTest {
    ...
    @InjectMocks private TasksLoader tasksLoader;
    ...
    public void shouldSchedule_SimpleTask() throws Exception {
        ...
        doReturn(Flowable.fromIterable(tasks))
                .when(taskRepository)
                .findByType(TaskType.SIMPLE.name());

        tasksLoader.doStart();

        verify(scheduler, times(numberOfSimpleTasks))
                .schedule(
                        argThat(task -> task instanceof AssignSystemCertificate),
                        any(Instant.class));
    }
}
