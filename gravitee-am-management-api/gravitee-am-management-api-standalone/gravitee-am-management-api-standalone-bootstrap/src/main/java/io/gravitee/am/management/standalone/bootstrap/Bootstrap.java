...
public class Bootstrap {
    ...
    private static final String CONTAINER_CLASS =
            "io.gravitee.am.management.standalone.container.ManagementContainer";
    ...
    /**
     * Set the <code>gravitee.home</code> System property to the current working directory if it has
     * not been set.
     */
    ...
    private void checkInstallRoot(File graviteeHomeDir) {
        ...
        if (!graviteeHomeDir.isDirectory()) {
            throw new RuntimeException(
                    "Invalid Gravitee.io Node home directory. Not a directory: "
        ...
        File[] files =
                graviteeLibDir.listFiles(
                        pathname ->
                                pathname.getName()
                                        .startsWith(
                                                "gravitee-am-management-api-standalone-bootstrap"));

        if (files == null || files.length == 0 || files.length > 1) {
            throw new RuntimeException(
                    "Invalid Gravitee.io Node home directory. No bootstrapable jar can be found in "
