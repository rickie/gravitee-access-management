...
public class ScheduledPurgeService extends AbstractService implements Runnable {
    ...
    @Autowired @Lazy private PurgeManager purgeManager;
    ...
    /**
     * Cleaning expired data from Relational Database when Gravitee node is starting. This cleaning
     * phase can be done by all nodes or only one depending on the enabled flag.
     */
    private void doPurgeExpiredData() {
        logger.debug(
                "Cleaning expired data #{} started at {}",
                counter.incrementAndGet(),
                Instant.now().toString());
        this.purgeManager.purge(
                Arrays.asList(exclude.split(",")).stream()
                        .map(String::trim)
                        .filter(value -> !value.isEmpty())
                        .map(TableName::valueOf)
                        .collect(Collectors.toList()));
        logger.debug(
                "Cleaning expired data #{} ended at {}", counter.get(), Instant.now().toString());
    }
