...
public class CertificatesResource extends AbstractResource {
    ...
    @Autowired private CertificateServiceProxy certificateService;
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_CERTIFICATE[LIST] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                ...
                response = CertificateEntity.class,
        ...
        checkAnyPermission(
                ...
                .andThen(
                        domainService
                                ...
                                .map(
                                        cert ->
                                                this.filterCertificateInfos(
                                                        cert, certificateExpiryThresholds.get(0)))
    ...
    private void processExpiryThresholds() {
        ...
        if (this.certificateExpiryThresholds == null) {
            this.certificateExpiryThresholds =
                    Stream.of(expiryThresholds.trim().split(","))
                            .map(String::trim)
                            .map(Integer::valueOf)
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_CERTIFICATE[CREATE] permission on the specified domain "
                                ...
                                .flatMapSingle(
                                        schema ->
                                                certificateService.create(
                                                        domain, newCertificate, authenticatedUser))
                                .map(
                                        certificate ->
                                                Response.created(
                                                                URI.create(
        ...
        checkAnyPermission(
                ...
                .andThen(
                        domainService
                                ...
                                .switchIfEmpty(Maybe.error(new DomainNotFoundException(domain)))
                                .flatMapSingle(
                                        schema -> certificateService.rotate(domain, principal))
                                .map(
                                        certificate ->
                                                Response.created(
                                                                URI.create(
    ...
    private CertificateEntity filterCertificateInfos(
            Certificate certificate, int certificateExpiryThreshold) {
        ...
        if (certificate.getExpiresAt() != null) {
            ...
            if (certificate.getExpiresAt().getTime() <= now.toEpochMilli()) {
                filteredCertificate.setStatus(CertificateStatus.EXPIRED);
                    ...
                    < now.plus(certificateExpiryThreshold, ChronoUnit.DAYS).toEpochMilli()) {
