...
public class BotDetectionPluginResource {
    ...
    public void get(@PathParam("botDetection") String botDetectionId,
        ...
        RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(pluginService.findById(botDetectionId)).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.error(new BotDetectionPluginNotFoundException(botDetectionId))))))).map(RxJavaReactorMigrationUtil.toJdkFunction(policyPlugin -> Response.ok(policyPlugin).build())))
    ...
    public void getSchema(@PathParam("botDetection") String botDetection,
        ...
        RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(pluginService.findById(botDetection)
                ...
                .flatMap(irrelevant -> pluginService.getSchema(botDetection))).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.error(new BotDetectionPluginSchemaNotFoundException(botDetection))))))).map(RxJavaReactorMigrationUtil.toJdkFunction(policyPluginSchema -> Response.ok(policyPluginSchema).build())))
                .subscribe(response::resume, response::resume);
    }
}