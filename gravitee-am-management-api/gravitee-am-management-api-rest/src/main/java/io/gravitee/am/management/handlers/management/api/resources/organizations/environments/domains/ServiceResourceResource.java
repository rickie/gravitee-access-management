...
public class ServiceResourceResource extends AbstractResource {
    ...
    public void get(
        ...
        checkAnyPermission(organizationId, environmentId, domain, Permission.DOMAIN_RESOURCE, Acl.READ).as(RxJava2Adapter::completableToMono).then(RxJava2Adapter.maybeToMono(domainService.findById(domain)
                        .switchIfEmpty(Maybe.error(new DomainNotFoundException(domain)))).flatMap(z->resourceService.findById(resource).as(RxJava2Adapter::maybeToMono)).switchIfEmpty(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(Mono.error(new FactorNotFoundException(resource))))).map(RxJavaReactorMigrationUtil.toJdkFunction(res1 -> {
