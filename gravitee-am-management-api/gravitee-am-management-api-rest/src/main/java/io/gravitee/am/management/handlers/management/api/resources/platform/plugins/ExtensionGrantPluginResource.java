...
public class ExtensionGrantPluginResource {
    ...
    public void getSchema(@PathParam("extensionGrant") String extensionGrantId,
        ...
        RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(extensionGrantPluginService.findById(extensionGrantId)
                .switchIfEmpty(Maybe.error(new ExtensionGrantPluginNotFoundException(extensionGrantId)))
                .flatMap(irrelevant -> extensionGrantPluginService.getSchema(extensionGrantId))).switchIfEmpty(RxJava2Adapter.maybeToMono(Maybe.wrap(Maybe.error(new ExtensionGrantPluginSchemaNotFoundException(extensionGrantId))))))).map(RxJavaReactorMigrationUtil.toJdkFunction(extensionGrantPluginSchema -> Response.ok(extensionGrantPluginSchema).build())))
                .subscribe(response::resume, response::resume);
    }
}