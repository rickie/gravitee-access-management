...
public class ServiceResourcesResource extends AbstractResource {
    ...
    @ApiOperation(
            value = "List registered resources for a security domain",
            nickname = "listResources",
            notes =
                    "User must have the DOMAIN_RESOURCE[LIST] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "List registered resources for a security domain",
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_RESOURCE[CREATE] permission on the specified domain "
    ...
    public void create(
            ...
            @PathParam("environmentId") String environmentId,
            @PathParam("domain") String domain,
            @ApiParam(name = "resource", required = true) @Valid @NotNull
                    final NewServiceResource newResource,
            @Suspended final AsyncResponse response) {
                                ...
                                .flatMapSingle(
                                        __ ->
                                                resourceService.create(
                                                        domain, newResource, authenticatedUser))
                                .map(
                                        resource ->
                                                Response.created(
                                                                URI.create(
