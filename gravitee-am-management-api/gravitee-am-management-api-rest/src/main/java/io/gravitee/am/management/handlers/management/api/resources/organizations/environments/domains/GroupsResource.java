...
public class GroupsResource extends AbstractResource {
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_GROUP[LIST] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "List groups for a security domain",
                response = GroupPage.class),
        ...
        checkAnyPermission(organizationId, environmentId, domain, Permission.DOMAIN_GROUP, Acl.LIST)
                .andThen(
                        domainService
                                ...
                                .flatMapSingle(
                                        irrelevant ->
                                                groupService.findByDomain(
                                                        domain,
                                                        page,
                                                        Integer.min(
                                                                size, MAX_GROUPS_SIZE_PER_PAGE)))
                                .map(
                                        groupPage ->
                                                new Page<>(
                                                        groupPage.getData().stream()
                                                                .map(this::filterGroupInfos)
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_GROUP[CREATE] permission on the specified domain "
    ...
    public void create(
        ...
        checkAnyPermission(
                        organizationId, environmentId, domain, Permission.DOMAIN_GROUP, Acl.CREATE)
                .andThen(
                        domainService
                                .findById(domain)
                                .switchIfEmpty(Maybe.error(new DomainNotFoundException(domain)))
                                .flatMapSingle(
                                        irrelevant ->
                                                groupService.create(
                                                        domain, newGroup, authenticatedUser))
                                .map(
                                        group ->
                                                Response.created(
                                                                URI.create(
