...
public class UserConsentsResource extends AbstractResource {
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "User consents successfully fetched",
                response = ScopeApprovalEntity.class),
                                ...
                                .flatMapPublisher(
                                        __ -> {
                                            if (clientId == null || clientId.isEmpty()) {
                                                return scopeApprovalService.findByDomainAndUser(
                                                        domain, user);
                                            }
                ...
                .andThen(
                        domainService
                                ...
                                .flatMapCompletable(
                                        __ -> {
                                            if (clientId == null || clientId.isEmpty()) {
                                                return scopeApprovalService.revokeByUser(
                                                        domain, user, authenticatedUser);
                                            }
                                            return scopeApprovalService.revokeByUserAndClient(
                                                    domain, user, clientId, authenticatedUser);
                                        }))
    ...
    private Single<ApplicationEntity> getClient(String domain, String clientId) {
        return applicationService
                ...
                .defaultIfEmpty(
                        new ApplicationEntity("unknown-id", clientId, "unknown-client-name"))
                .toSingle()
                .cache();
    }
    ...
    private Single<ScopeEntity> getScope(String domain, String scopeKey) {
        return scopeService
                ...
                .switchIfEmpty(
                        scopeService
                                .findByDomainAndKey(domain, getScopeBase(scopeKey))
                                .map(
                                        entity -> {
                                            // set the right scopeKey since the one returned by the
                                            // service contains the scope definition without
                                            // parameter
