...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public User onAuthenticationSuccess(Authentication auth) {
        ...
        Map<String, String> details =
                auth.getDetails() == null
                        ? new HashMap<>()
                        : new HashMap<>((Map<String, String>) auth.getDetails());
                                ...
                                existingUser -> {
                                    ...
                                    if (existingUser.getAdditionalInformation() != null) {
                                        existingUser
                                                ...
                                                .putAll(principal.getAdditionalInformation());
                                    } else {
                                        existingUser.setAdditionalInformation(
                                                new HashMap<>(
                                                        principal.getAdditionalInformation()));
                                    }
                                    return userService
                                            .update(existingUser)
                                            .flatMap(
                                                    user ->
                                                            updateRoles(principal, existingUser)
                                ...
                                user ->
                                        auditService.report(
                                                AuditBuilder.builder(
                                                                AuthenticationAuditBuilder.class)
        ...
        principal
                .getAdditionalInformation()
                .put(StandardClaims.PREFERRED_USERNAME, endUser.getUsername());
        ...
        principal
                .getAdditionalInformation()
                .computeIfAbsent(StandardClaims.EMAIL, val -> endUser.getEmail());
        principal
                .getAdditionalInformation()
                .computeIfAbsent(StandardClaims.NAME, val -> endUser.getDisplayName());

        // set roles
        Set<String> roles =
                endUser.getRoles() != null ? new HashSet<>(endUser.getRoles()) : new HashSet<>();
    ...
    private Completable updateRoles(User principal, io.gravitee.am.model.User existingUser) {
        ...
        return membershipService
                .findByMember(existingUser.getId(), MemberType.USER)
