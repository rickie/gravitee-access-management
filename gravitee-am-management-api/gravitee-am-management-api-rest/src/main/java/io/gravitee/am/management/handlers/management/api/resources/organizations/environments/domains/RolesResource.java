...
public class RolesResource extends AbstractResource {
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_ROLE[LIST] permission on the specified domain "
    ...
    public void list(
        ...
        checkAnyPermission(organizationId, environmentId, domain, Permission.DOMAIN_ROLE, Acl.LIST)
                ...
                .map(
                        pagedRoles -> {
                            List<Role> roles =
                                    pagedRoles.getData().stream()
                                            .map(this::filterRoleInfos)
                                            .sorted(Comparator.comparing(Role::getName))
                                            .collect(Collectors.toList());
                            return new RolePage(
                                    roles, pagedRoles.getCurrentPage(), pagedRoles.getTotalCount());
                        })
    ...
    @ApiOperation(
            ...
            value = "Create a role",
            notes =
                    "User must have the DOMAIN_ROLE[CREATE] permission on the specified domain "
    ...
    private Single<Page<Role>> searchRoles(String domain, String query, int page, int size) {
        ...
        return roleService.searchByDomain(
                domain, query, page, Math.min(MAX_ROLES_SIZE_PER_PAGE, size));
    }
