...
public class BotDetectionResource extends AbstractResource {
    ...
    @Autowired private BotDetectionServiceProxy botDetectionService;
    ...
    @ApiOperation(
            value = "Get a bot detection",
            notes =
                    "User must have the DOMAIN_BOT_DETECTION[READ] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "Bot detection successfully fetched",
    ...
    public void get(
        ...
        checkAnyPermission(
                        ...
                        environmentId,
                        domain,
                        Permission.DOMAIN_BOT_DETECTION,
                        Acl.READ)
                .andThen(
                        domainService
                                .findById(domain)
                                ...
                                .switchIfEmpty(
                                        Maybe.error(
                                                new BotDetectionNotFoundException(botDetectionId)))
                                .map(
                                        botDetection -> {
                                            if (!botDetection
                                                            .getReferenceId()
                                                ...
                                                throw new BadRequestException(
                                                        "BotDetection does not belong to domain");
                                            }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(
            value = "Update a bot detection",
            notes =
                    "User must have the DOMAIN_BOT_DETECTION[UPDATE] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 201,
                message = "Bot detection successfully updated",
                response = BotDetection.class),
        @ApiResponse(code = 500, message = "Internal server error")
    })
    public void update(
    ...
    @DELETE
    @ApiOperation(
            value = "Delete a bot detection",
            notes =
                    "User must have the DOMAIN_BOT_DETECTION[DELETE] permission on the specified domain "
