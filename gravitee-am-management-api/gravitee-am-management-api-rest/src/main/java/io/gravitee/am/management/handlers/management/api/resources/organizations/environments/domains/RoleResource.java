...
public class RoleResource extends AbstractResource {
    ...
    @ApiOperation(
            ...
            value = "Get a role",
            notes =
                    "User must have the DOMAIN_ROLE[READ] permission on the specified domain "
                                        ...
                                        role1 -> {
                                            if (role1.getReferenceType() == ReferenceType.DOMAIN
                                                ...
                                                throw new BadRequestException(
                                                        "Role does not belong to domain");
                                            }
    ...
    @ApiOperation(
            ...
            value = "Update a role",
            notes =
                    "User must have the DOMAIN_ROLE[UPDATE] permission on the specified domain "
    ...
    public void update(
        ...
        checkAnyPermission(
                        organizationId, environmentId, domain, Permission.DOMAIN_ROLE, Acl.UPDATE)
                .andThen(
                        domainService
                                .findById(domain)
                                .switchIfEmpty(Maybe.error(new DomainNotFoundException(domain)))
    ...
    @ApiOperation(
            ...
            value = "Delete a role",
            notes =
                    "User must have the DOMAIN_ROLE[DELETE] permission on the specified domain "
    ...
    public void delete(
        ...
        checkAnyPermission(
                        organizationId, environmentId, domain, Permission.DOMAIN_ROLE, Acl.DELETE)
     ...
     * AM permission are now structured using Permission and Acls while domain role permission are
     * stil simple strings (oauth scopes). Internaly, role structure has now 2 distinct attributes:
     *
     * <ul>
     *   <li>permissions: holds the AM role permissions
     *   <li>oauthScopes: holds the domain role permissions
     * </ul>
     *
     * This will be removed when we deal with this issue:
     * https://github.com/gravitee-io/issues/issues/3323
     */
    ...
    /**
     * Special converter that just fill permissions from Role.oauthScope attribute for compatibility
     * with v2.
     */
