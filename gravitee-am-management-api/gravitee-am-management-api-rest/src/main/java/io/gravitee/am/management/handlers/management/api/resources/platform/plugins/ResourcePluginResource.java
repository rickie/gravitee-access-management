...
public class ResourcePluginResource {
    ...
    public void get(@PathParam("resource") String resourceId,
        ...
        RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(resourcePluginService.findById_migrated(resourceId))).switchIfEmpty(Mono.error(new ResourcePluginNotFoundException(resourceId))).map(RxJavaReactorMigrationUtil.toJdkFunction(resourcePlugin -> Response.ok(resourcePlugin).build())))
    ...
    public void getSchema(@PathParam("resource") String resourceId,
        ...
        RxJava2Adapter.monoToMaybe(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(resourcePluginService.findById_migrated(resourceId))).switchIfEmpty(Mono.error(new ResourcePluginNotFoundException(resourceId))).flatMap(z->RxJava2Adapter.monoToMaybe(resourcePluginService.getSchema_migrated(resourceId)).as(RxJava2Adapter::maybeToMono)).switchIfEmpty(Mono.error(new ResourcePluginNotFoundException(resourceId))).map(RxJavaReactorMigrationUtil.toJdkFunction(policyPluginSchema -> Response.ok(policyPluginSchema).build())))
                .subscribe(response::resume, response::resume);
    }
}