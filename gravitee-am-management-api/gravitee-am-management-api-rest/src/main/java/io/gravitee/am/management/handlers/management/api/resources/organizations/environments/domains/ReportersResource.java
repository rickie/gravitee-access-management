...
public class ReportersResource extends AbstractResource {
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_REPORTER[LIST] permission on the specified domain "
    ...
    public void list(
        ...
        checkAnyPermission(
                        organizationId, environmentId, domain, Permission.DOMAIN_REPORTER, Acl.LIST)
                .andThen(
                        domainService
                                .findById(domain)
                                ...
                                .flatMapSingle(
                                        irrelevant ->
                                                reporterService.findByDomain(domain).toList()))
                .flatMap(
                        reporters ->
                                hasAnyPermission(
                                                authenticatedUser,
    ...
    @ApiOperation(
            value = "Create a reporter for a security domain",
            notes =
                    "User must have the DOMAIN_REPORTER[CREATE] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 201,
                message = "Reporter created for a security domain",
                response = Reporter.class),
        @ApiResponse(code = 500, message = "Internal server error")
    })
    public void create(
