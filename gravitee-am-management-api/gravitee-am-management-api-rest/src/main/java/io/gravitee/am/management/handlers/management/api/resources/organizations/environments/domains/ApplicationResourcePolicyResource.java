...
public class ApplicationResourcePolicyResource extends AbstractResource {
    ...
    public void get(
        ...
        checkAnyPermission(organizationId, environmentId, domain, application, Permission.APPLICATION_RESOURCE, Acl.READ).as(RxJava2Adapter::completableToMono).then(RxJava2Adapter.maybeToMono(domainService.findById(domain)
                        ...
                        .switchIfEmpty(Maybe.error(new ApplicationNotFoundException(application)))).flatMap(z->resourceService.findAccessPolicy(policy).as(RxJava2Adapter::maybeToMono))).as(RxJava2Adapter::monoToMaybe)
