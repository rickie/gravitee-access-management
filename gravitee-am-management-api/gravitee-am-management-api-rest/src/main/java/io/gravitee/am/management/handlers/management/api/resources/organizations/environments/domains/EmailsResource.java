...
public class EmailsResource extends AbstractResource {
    ...
    @ApiOperation(
            value = "Find a email",
            notes =
                    "User must have the DOMAIN_EMAIL_TEMPLATE[READ] permission on the specified domain "
    ...
    public void get(
        ...
        checkAnyPermission(
                        ...
                        environmentId,
                        domain,
                        Permission.DOMAIN_EMAIL_TEMPLATE,
                        Acl.READ)
                .andThen(
                        emailTemplateService.findByDomainAndTemplate(
                                domain, emailTemplate.template()))
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_EMAIL_TEMPLATE[CREATE] permission on the specified domain "
    ...
    public void create(
            ...
            @PathParam("environmentId") String environmentId,
            @PathParam("domain") String domain,
            @ApiParam(name = "email", required = true) @Valid @NotNull NewEmail newEmail,
            @Suspended AsyncResponse response) {
                                ...
                                .flatMapSingle(
                                        __ ->
                                                emailTemplateService.create(
                                                        domain, newEmail, authenticatedUser))
                                .map(
                                        email ->
                                                Response.created(
                                                                URI.create(
