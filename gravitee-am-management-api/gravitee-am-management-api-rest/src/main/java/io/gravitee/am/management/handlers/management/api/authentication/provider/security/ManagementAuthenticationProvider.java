    ...
    public Authentication authenticate(Authentication authentication)
        ...
        if (organization == null) {
            throw new InternalAuthenticationServiceException(
                    "No organization found when trying to authenticate the end-user");
        }
        List<String> identities =
                organization.getIdentities() == null
                        ? new ArrayList<>()
                        : new ArrayList<>(organization.getIdentities());
        ...
        SimpleAuthenticationContext context = new SimpleAuthenticationContext();
        details.forEach((k, v) -> context.setAttribute(k, v));

        io.gravitee.am.identityprovider.api.Authentication provAuthentication =
                new EndUserAuthentication(
                        authentication.getName(), authentication.getCredentials(), context);

        while (iter.hasNext() && user == null) {
            String provider = iter.next();

            IdentityProvider identityProvider =
                    identityProviderManager.getIdentityProvider(provider);
            ...
            try {
                ...
                logger.info(
                        "Unable to authenticate user {} with provider {}",
        ...
        throw new BadCredentialsException(
                "No user found for providers "
                        + StringUtils.collectionToDelimitedString(identities, ","));
    }
