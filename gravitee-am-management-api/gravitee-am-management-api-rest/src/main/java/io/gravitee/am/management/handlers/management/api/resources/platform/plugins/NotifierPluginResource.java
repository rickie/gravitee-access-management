...
public class NotifierPluginResource {
    ...
    @Inject private NotifierPluginService notifierPluginService;

    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(
            value = "Get a notifier",
            notes = "There is no particular permission needed. User must be authenticated.")
    @ApiResponses({
        @ApiResponse(code = 200, message = "Notifier plugin", response = NotifierPlugin.class),
        @ApiResponse(
                code = 404,
                message = "Notifier plugin not found",
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(
            value = "Get a notifier plugin's schema",
            notes = "There is no particular permission needed. User must be authenticated.")
    @ApiResponses({
        @ApiResponse(code = 200, message = "Notifier plugin schema", response = String.class),
        @ApiResponse(
                code = 404,
                message = "Notifier plugin schema not found",
                response = ErrorEntity.class),
        @ApiResponse(code = 500, message = "Internal server error")
    })
