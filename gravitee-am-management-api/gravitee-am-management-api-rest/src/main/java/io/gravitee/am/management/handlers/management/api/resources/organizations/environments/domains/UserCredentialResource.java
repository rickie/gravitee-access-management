...
public class UserCredentialResource extends AbstractResource {
    ...
    public void revoke(
            ...
            @PathParam("credential") String credential,
            @Suspended final AsyncResponse response) {
        RxJava2Adapter.monoToCompletable(RxJava2Adapter.completableToMono(checkAnyPermission(organizationId, environmentId, domain, Permission.DOMAIN_USER, Acl.UPDATE)).then(RxJava2Adapter.maybeToMono(domainService.findById(domain)).switchIfEmpty(Mono.error(new DomainNotFoundException(domain))).flatMap(__->RxJava2Adapter.completableToMono(credentialService.delete(credential))).then()))
