...
public class IdentityProviderManagerImpl
        implements IdentityProviderManager,
                ...
                InMemoryIdentityProviderListener {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        try {
            identityProviderService
                    ...
                    .blockingForEach(this::updateAuthenticationProvider);
        ...
        identityProviderService
                ...
                .subscribe(
                        identityProvider -> {
                            ...
                            logger.info(
                                    "Identity provider {} {}d for organization {}",
                                    identityProviderId,
                                    eventType,
                                    organizationId);
                        },
                        error ->
                                logger.error(
                                        "Unable to {} identity provider for organization {}",
                                        eventType,
                                        organizationId,
                                        error),
                        () ->
    ...
    private void updateAuthenticationProvider(IdentityProvider identityProvider) {
        try {
            if (needDeployment(identityProvider)) {
                logger.info(
                        "\tInitializing identity provider: {} for organization {} [{}]",
                ...
                logger.info(
                        "\tIdentity provider already initialized: {} for organization {} [{}]",
                        identityProvider.getName(),
                        identityProvider.getReferenceId(),
                        identityProvider.getType());
            }
    ...
    private void removeIdentityProvider(String identityProviderId) {
        logger.info(
                "Received identity provider event, delete identity provider {}",
                identityProviderId);
    ...
    private void clearProvider(String identityProviderId) {
        ...
        if (authenticationProvider != null) {
            ...
            try {
                ...
                logger.error(
                        "An error occurs while stopping the identity provider : {}",
                        identityProviderId,
                        e);
            }
        }
