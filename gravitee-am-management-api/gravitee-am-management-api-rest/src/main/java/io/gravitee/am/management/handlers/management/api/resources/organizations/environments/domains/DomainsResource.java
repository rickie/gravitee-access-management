...
public class DomainsResource extends AbstractDomainResource {
    ...
    public void list(
        ...
        RxJava2Adapter.monoToSingle(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(checkAnyPermission_migrated(organizationId, environmentId, Permission.DOMAIN, Acl.LIST))).thenMany(query != null ? RxJava2Adapter.fluxToFlowable(domainService.search_migrated(organizationId, environmentId, query)) : RxJava2Adapter.fluxToFlowable(domainService.findAllByEnvironment_migrated(organizationId, environmentId))).flatMap(e->RxJava2Adapter.maybeToMono(Maybe.wrap(RxJavaReactorMigrationUtil.<Domain, MaybeSource<Domain>>toJdkFunction(domain -> RxJava2Adapter.monoToMaybe(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(hasPermission_migrated(authenticatedUser, or(of(ReferenceType.DOMAIN, domain.getId(), Permission.DOMAIN, Acl.READ),
    ...
    public void create(
        ...
        RxJava2Adapter.monoToSingle(RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(checkAnyPermission_migrated(organizationId, environmentId, Permission.DOMAIN, Acl.CREATE))).then(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(domainService.create_migrated(organizationId, environmentId, newDomain, authenticatedUser))).flatMap(domain->RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(identityProviderManager.create_migrated(domain.getId()))).map(RxJavaReactorMigrationUtil.toJdkFunction((io.gravitee.am.model.IdentityProvider __)->domain))).flatMap(domain->RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(reporterService.createDefault_migrated(domain.getId()))).map(RxJavaReactorMigrationUtil.toJdkFunction((io.gravitee.am.model.Reporter __)->domain)))))
    ...
    public void get(@PathParam("organizationId") String organizationId,
        ...
        RxJava2Adapter.monoToSingle(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(domainService.findByHrid_migrated(environmentId, hrid))).flatMap(domain->RxJava2Adapter.completableToMono(RxJava2Adapter.monoToCompletable(checkAnyPermission_migrated(authenticatedUser, organizationId, environmentId, domain.getId(), Permission.DOMAIN, Acl.READ))).then(Mono.defer(()->RxJava2Adapter.singleToMono(RxJava2Adapter.monoToSingle(findAllPermissions_migrated(authenticatedUser, organizationId, environmentId, domain.getId()))).map(RxJavaReactorMigrationUtil.toJdkFunction((java.util.Map<io.gravitee.am.model.ReferenceType, java.util.Map<io.gravitee.am.model.permissions.Permission, java.util.Set<io.gravitee.am.model.Acl>>> userPermissions)->filterDomainInfos(domain, userPermissions))))))).subscribe(response::resume, response::resume);
    }
