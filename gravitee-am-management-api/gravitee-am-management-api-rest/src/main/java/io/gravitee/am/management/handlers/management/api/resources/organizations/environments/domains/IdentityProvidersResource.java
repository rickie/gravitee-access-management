...
public class IdentityProvidersResource extends AbstractResource {
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_IDENTITY_PROVIDER[LIST] permission on the specified domain "
    ...
    public void list(
        ...
        checkAnyPermission(
                        ...
                        environmentId,
                        domain,
                        Permission.DOMAIN_IDENTITY_PROVIDER,
                        Acl.LIST)
                .andThen(
                        domainService
                                .findById(domain)
                                ...
                                .flatMapPublisher(
                                        __ -> identityProviderService.findByDomain(domain))
                                .flatMapMaybe(
                                        identityProvider -> {
                                            if (userProvider) {
                                                // if userProvider, we only want to manage IDP with
                                                // a UserProvider
                                                // as UserProvider may be disabled by configuration
                                                // we have to check the existence of the instance
                                                return identityProviderManager
                                                        .getUserProvider(identityProvider.getId())
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_IDENTITY_PROVIDER[CREATE] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 201,
                message = "Identity provider successfully created",
                response = IdentityProvider.class),
        @ApiResponse(code = 500, message = "Internal server error")
    })
    public void create(
