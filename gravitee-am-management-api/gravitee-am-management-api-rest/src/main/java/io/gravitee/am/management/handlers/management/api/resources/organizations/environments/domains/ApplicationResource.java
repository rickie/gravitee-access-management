...
public class ApplicationResource extends AbstractResource {
    ...
    public void update(
        ...
        RxJava2Adapter.monoToSingle(checkAnyPermission_migrated(organizationId, environmentId, domain, application, Permission.APPLICATION, Acl.UPDATE).then(applicationService.updateType_migrated(domain, application, patchApplicationType.getType(), authenticatedUser)))
                .subscribe(response::resume, response::resume);
    }
    ...
    public void delete(
        ...
        RxJava2Adapter.monoToCompletable(checkAnyPermission_migrated(organizationId, environmentId, domain, application, Permission.APPLICATION, Acl.DELETE).then(applicationService.delete_migrated(application, authenticatedUser)))
    ...
    public void renewClientSecret(
        ...
        RxJava2Adapter.monoToSingle(checkAnyPermission_migrated(organizationId, environmentId, domain, application, Permission.APPLICATION_OPENID, Acl.READ).then(RxJava2Adapter.singleToMono(RxJava2Adapter.monoToMaybe(domainService.findById_migrated(domain).switchIfEmpty(Mono.error(new DomainNotFoundException(domain))))
