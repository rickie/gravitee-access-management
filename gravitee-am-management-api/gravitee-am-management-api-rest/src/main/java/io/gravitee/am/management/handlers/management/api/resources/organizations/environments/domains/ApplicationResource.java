...
public class ApplicationResource extends AbstractResource {
    ...
    public void get(
        ...
        checkAnyPermission_migrated(organizationId, environmentId, domain, application, Permission.APPLICATION, Acl.READ).then(domainService.findById_migrated(domain).switchIfEmpty(Mono.error(new DomainNotFoundException(domain))).flatMap(z->applicationService.findById_migrated(application)).switchIfEmpty(Mono.error(new ApplicationNotFoundException(application))).flatMap(y->findAllPermissions_migrated(authenticatedUser, organizationId, environmentId, domain, application).map(RxJavaReactorMigrationUtil.toJdkFunction((Map<ReferenceType, Map<Permission, Set<Acl>>> userPermissions)->filterApplicationInfos(y, userPermissions))))).map(RxJavaReactorMigrationUtil.toJdkFunction(application1 -> {
