...
public class RememberDevicesPluginResource {
    ...
    public void list(@Suspended final AsyncResponse response) {
        ...
        rememberDevicePluginService.findAll()
                .map(authenticatorPlugins -> authenticatorPlugins.stream()
                        .sorted(Comparator.comparing(RememberDevicePlugin::getName))
                        .collect(Collectors.toList()))
                .subscribe(response::resume, response::resume);
    }

    @Path("{rememberDevice}")
    public RememberDevicePluginResource getFactorPluginResource() {
        return resourceContext.getResource(RememberDevicePluginResource.class);
    }
}