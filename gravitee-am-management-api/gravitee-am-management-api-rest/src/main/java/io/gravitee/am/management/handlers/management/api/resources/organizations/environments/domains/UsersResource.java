...
public class UsersResource extends AbstractUsersResource {
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_USER[LIST] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "List users for a security domain",
    ...
    @ApiOperation(
            ...
            notes =
                    "User must have the DOMAIN_USER[CREATE] permission on the specified domain "
    ...
    public void create(
        ...
        checkAnyPermission(
                        organizationId, environmentId, domainId, Permission.DOMAIN_USER, Acl.CREATE)
                .andThen(
                        domainService
                                .findById(domainId)
                                ...
                                .flatMapSingle(
                                        domain ->
                                                userService.create(
                                                        domain, newUser, authenticatedUser))
                                .map(
                                        user ->
                                                Response.created(
                                                                URI.create(
    ...
    private Single<User> filterUserInfos(Boolean hasPermission, User user) {
        ...
        if (hasPermission) {
            ...
            // Current user doesn't have read permission, select only few information and remove
            // default values that could be inexact.
