...
public class ScopeResource extends AbstractResource {
    ...
    public void get(
        ...
        checkAnyPermission(organizationId, environmentId, domain, Permission.DOMAIN_SCOPE, Acl.READ).as(RxJava2Adapter::completableToMono).then(RxJava2Adapter.maybeToMono(domainService.findById(domain)
                        .switchIfEmpty(Maybe.error(new DomainNotFoundException(domain)))).flatMap(z->scopeService.findById(scopeId).as(RxJava2Adapter::maybeToMono)).switchIfEmpty(RxJava2Adapter.maybeToMono(RxJava2Adapter.monoToMaybe(Mono.error(new ScopeNotFoundException(scopeId))))).map(RxJavaReactorMigrationUtil.toJdkFunction(scope -> {
