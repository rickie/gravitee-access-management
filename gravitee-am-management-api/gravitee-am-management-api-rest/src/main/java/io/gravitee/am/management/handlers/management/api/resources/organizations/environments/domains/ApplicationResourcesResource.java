...
public class ApplicationResourcesResource extends AbstractResource {
    ...
    @ApiOperation(
            value = "List resources for an application",
            notes =
                    "User must have APPLICATION_RESOURCE[LIST] permission on the specified application "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "List resources for an application",
    ...
    public void list(
        ...
        checkAnyPermission(
                        ...
                        domain,
                        application,
                        Permission.APPLICATION_RESOURCE,
                        Acl.LIST)
                .andThen(
                        domainService
                                .findById(domain)
                                ...
                                .flatMapSingle(
                                        application1 ->
                                                resourceService.findByDomainAndClient(
                                                        domain,
                                                        application1.getId(),
                                                        page,
                                                        Integer.min(
                                                                MAX_RESOURCES_SIZE_PER_PAGE, size)))
                                .flatMap(
                                        pagedResources ->
                                                Observable.fromIterable(pagedResources.getData())
                                                        .flatMapSingle(
                                                                r ->
                                                                        resourceService
