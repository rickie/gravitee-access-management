...
public class ApplicationResourcePoliciesResource extends AbstractResource {
    ...
    @ApiOperation(
            value = "Get resource access policies",
            notes =
                    "User must have APPLICATION_RESOURCE[READ] permission on the specified application "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "Get resource access policies",
                response = AccessPolicyListItem.class,
        ...
        checkAnyPermission(
                ...
                .andThen(
                        domainService
                                ...
                                .switchIfEmpty(
                                        Single.error(new ApplicationNotFoundException(application)))
                                .flatMap(
                                        application1 ->
                                                resourceService
                                                        .findAccessPoliciesByResources(
                                                                Collections.singletonList(resource))
                                                        .map(AccessPolicyListItem::new)
                                                        .toList()))
