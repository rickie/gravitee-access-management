...
public class AuthenticationDeviceNotifiersResource extends AbstractResource {
    ...
    @ApiOperation(
            value = "List registered Authentication Device Notifiers for a security domain",
            notes =
                    "User must have the DOMAIN_AUTHDEVICE_NOTIFIER[LIST] permission on the specified domain "
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = "List registered Authentication Device Notifiers for a security domain",
                                ...
                                .flatMapSingle(
                                        ___ ->
                                                authDeviceNotifierService
                                                        .findByDomain(domain)
                                                        .map(this::filterBotDetectionInfos)
                                                        .toList()))
    ...
    @ApiOperation(
            value = "Create an Authentication Device Notifier",
            notes =
                    "User must have the DOMAIN_AUTHDEVICE_NOTIFIER[CREATE] permission on the specified domain "
    ...
    public void create(
            ...
            @ApiParam(name = "notifier", required = true) @Valid @NotNull
                    final NewAuthenticationDeviceNotifier newDeviceNotifier,
            @Suspended final AsyncResponse response) {
        ...
        checkAnyPermission(
                ...
                .andThen(
                        domainService
                                ...
                                .flatMapSingle(
                                        __ ->
                                                authDeviceNotifierService.create(
                                                        domain,
                                                        newDeviceNotifier,
                                                        authenticatedUser))
                                .map(
    ...
    private AuthenticationDeviceNotifier filterBotDetectionInfos(
            AuthenticationDeviceNotifier deviceNotifier) {
