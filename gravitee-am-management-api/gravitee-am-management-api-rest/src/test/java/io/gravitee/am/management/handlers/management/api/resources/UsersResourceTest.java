...
public class UsersResourceTest extends JerseySpringTest {
    ...
    public void shouldGetOrganizationUsers() {
        ...
        Map<Permission, Set<Acl>> permissions =
                Maps.<Permission, Set<Acl>>builder()
                        ...
                        .build();
        when(permissionService.findAllPermissions(
                        any(), eq(ReferenceType.ORGANIZATION), eq(organizationId)))
                .thenReturn(Single.just(permissions));
        doReturn(Single.just(pagedUsers))
    ...
    public void shouldNotCreate_invalid_identity_provider() {
        ...
        doReturn(Maybe.just(mockDomain)).when(domainService).findById(domainId);
        doReturn(Single.error(new UserProviderNotFoundException(newUser.getSource())))
                .when(userService)
    ...
    public void shouldCreateOrganizationUser() {
        ...
        when(organizationService.findById(ORGANIZATION_DEFAULT))
