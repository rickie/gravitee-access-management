...
public class EmailsResourceTest extends JerseySpringTest {
    ...
    public void shouldGetEmail() {
        ...
        doReturn(RxJava2Adapter.monoToMaybe(Mono.just(mockEmail))).when(emailTemplateService).findByDomainAndTemplate(domainId, Template.LOGIN.template());
    ...
    public void shouldGetEmails_technicalManagementException() {
        final String domainId = "domain-1";
        doReturn(RxJava2Adapter.monoToMaybe(Mono.error(new TechnicalManagementException("error occurs")))).when(emailTemplateService).findByDomainAndTemplate(domainId, Template.LOGIN.template());
    ...
    public void shouldCreate() {
        ...
        doReturn(RxJava2Adapter.monoToSingle(Mono.just(new Email()))).when(emailTemplateService).create(eq(domainId), any(), any(User.class));
