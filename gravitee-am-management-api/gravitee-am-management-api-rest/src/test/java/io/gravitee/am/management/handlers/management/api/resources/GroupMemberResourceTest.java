...
public class GroupMemberResourceTest extends JerseySpringTest {
    ...
    public void shouldNotAddMember_groupNotFound() {
        ...
        doReturn(Mono.empty()).when(groupService).findById_migrated(mockGroup.getId());
    ...
    public void shouldNotAddMember_userNotFound() {
        ...
        doReturn(Mono.empty()).when(userService).findById_migrated("member-1");
    ...
    public void shouldAddMember() {
        ...
        doReturn(Mono.just(mockDomain)).when(domainService).findById_migrated(domainId);
        doReturn(Mono.just(mockGroup)).when(groupService).findById_migrated(mockGroup.getId());
        doReturn(Mono.just(mockGroup)).when(groupService).update_migrated(eq(mockDomain.getId()), eq(mockGroup.getId()), any(), any());
        doReturn(Mono.just(mockUser)).when(userService).findById_migrated(mockUser.getId());
