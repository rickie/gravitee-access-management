...
public class IdentityProvidersPluginResourceTest extends JerseySpringTest {
    ...
    public void shouldList() {
        ...
        doReturn(Mono.just(Collections.singletonList(identityProviderPlugin))).when(identityProviderPluginService).findAll_migrated(false, new ArrayList<>());
    ...
    public void shouldList_technicalManagementException() {
        doReturn(Mono.error(new TechnicalManagementException("Error occurs"))).when(identityProviderPluginService).findAll_migrated(false, new ArrayList<>());
