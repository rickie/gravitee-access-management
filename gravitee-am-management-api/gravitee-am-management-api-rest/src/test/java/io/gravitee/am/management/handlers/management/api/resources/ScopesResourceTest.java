...
public class ScopesResourceTest extends JerseySpringTest {
    ...
    public void shouldGetScopes() {
        ...
        doReturn(Mono.just(new Page<>(scopes,0, 2))).when(scopeService).findByDomain_migrated(domainId, 0, 50);
    ...
    public void shouldGetScopes_technicalManagementException() {
        final String domainId = "domain-1";
        doReturn(Mono.error(new TechnicalManagementException("error occurs"))).when(scopeService).findByDomain_migrated(domainId, 0, 50);
    ...
    public void shouldCreate() {
        ...
        doReturn(Mono.just(scope)).when(scopeService).create_migrated(eq(domainId), any(NewScope.class), any());
