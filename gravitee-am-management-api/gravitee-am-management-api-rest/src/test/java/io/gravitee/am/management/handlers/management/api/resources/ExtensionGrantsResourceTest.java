...
public class ExtensionGrantsResourceTest extends JerseySpringTest {
    ...
    public void shouldGetExtensionGrants() {
        ...
        doReturn(Flux.just(mockExtensionGrant, mockExtensionGrant2)).when(extensionGrantService).findByDomain_migrated(domainId);
    ...
    public void shouldGetExtensionGrants_technicalManagementException() {
        final String domainId = "domain-1";
        doReturn(Flux.error(new TechnicalManagementException("error occurs"))).when(extensionGrantService).findByDomain_migrated(domainId);
    ...
    public void shouldCreate() {
        ...
        doReturn(Mono.just(mockDomain)).when(domainService).findById_migrated(domainId);
        doReturn(Mono.just(extensionGrant)).when(extensionGrantService).create_migrated(eq(domainId), any(), any());
