...
public class FlowResourceTest extends JerseySpringTest {
    ...
    public void shouldGetFlow() {
        ...
        doReturn(Mono.just(mockFlow)).when(flowService).findById_migrated(ReferenceType.DOMAIN, DOMAIN_ID, FLOW_ID);
    ...
    public void shouldNotGetFlow_notFound() {
        doReturn(Mono.empty()).when(flowService).findById_migrated(ReferenceType.DOMAIN, DOMAIN_ID, FLOW_ID);
    ...
    public void shouldUpdateFlow() {
        ...
        doReturn(Mono.just(updatedFlow)).when(flowService).update_migrated(eq(ReferenceType.DOMAIN), eq(DOMAIN_ID), eq(FLOW_ID), any(Flow.class), any(User.class));
    ...
    public void shouldNotUpdateEntrypoint_notFound() {
        ...
        doReturn(Mono.error(new FlowNotFoundException(FLOW_ID))).when(flowService).update_migrated(eq(ReferenceType.DOMAIN), eq(DOMAIN_ID), eq(FLOW_ID), any(Flow.class), any(User.class));
