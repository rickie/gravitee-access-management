...
public class CertificatesResourceTest extends JerseySpringTest {
    ...
    public void shouldGetCertificates() {
        ...
        doReturn(Flux.just(mockCertificate, mockCertificate2)).when(certificateService).findByDomain_migrated(domainId);
    ...
    public void shouldGetCertificates_technicalManagementException() {
        final String domainId = "domain-1";
        doReturn(Flux.error(new TechnicalManagementException("error occurs"))).when(certificateService).findByDomain_migrated(domainId);
    ...
    public void shouldCreate() {
        ...
        doReturn(Mono.just(mockDomain)).when(domainService).findById_migrated(domainId);
        doReturn(Mono.just("certificate-schema")).when(certificatePluginService).getSchema_migrated(anyString());
        doReturn(Mono.just(certificate)).when(certificateService).create_migrated(eq(domainId), any(), any());
