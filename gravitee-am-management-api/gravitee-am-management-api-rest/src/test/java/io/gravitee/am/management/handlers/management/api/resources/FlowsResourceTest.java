...
public class FlowsResourceTest extends JerseySpringTest {
    ...
    public void shouldGetFlows() {
        doReturn(Flux.just(new Flow(), new Flow())).when(flowService).findAll_migrated(ReferenceType.DOMAIN, DOMAIN_ID, true);
    ...
    public void shouldGetFlows_technicalManagementException() {
        doReturn(Flux.error(new TechnicalManagementException("error occurs"))).when(flowService).findAll_migrated(ReferenceType.DOMAIN, DOMAIN_ID, true);
