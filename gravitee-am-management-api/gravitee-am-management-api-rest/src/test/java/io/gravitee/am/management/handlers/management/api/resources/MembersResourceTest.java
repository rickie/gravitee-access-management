...
public class MembersResourceTest extends JerseySpringTest {
    ...
    public void shouldAddMember() {
        ...
        doReturn(Mono.just(membership)).when(membershipService).addOrUpdate_migrated(eq(organization.getId()), any(Membership.class), any(User.class));
    ...
    public void shouldGetMembers() {
        ...
        doReturn(Mono.just(organization)).when(organizationService).findById_migrated(organization.getId());
        doReturn(Flux.just(Arrays.asList(membership))).when(membershipService).findByReference_migrated(organization.getId(), ReferenceType.ORGANIZATION);
        doReturn(Mono.just(new HashMap<>())).when(membershipService).getMetadata_migrated(anyList());
    ...
    public void shouldGetMembers_organizationNotFound() {
        ...
        doReturn(Mono.error(new OrganizationNotFoundException(organization.getId()))).when(organizationService).findById_migrated(organization.getId());
