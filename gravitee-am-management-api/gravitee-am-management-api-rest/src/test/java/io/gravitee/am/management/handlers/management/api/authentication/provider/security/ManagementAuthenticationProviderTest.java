...
public class ManagementAuthenticationProviderTest {
    ...
    @InjectMocks
    private ManagementAuthenticationProvider managementAuthenticationProvider =
    ...
    public void shouldAuthenticate_firstAuthProvider() {
        ...
        when(authenticationProvider.loadUserByUsername(
                        any(io.gravitee.am.identityprovider.api.Authentication.class)))
                .thenReturn(Maybe.just(new DefaultUser("username")));
        when(identityProviderManager.getIdentityProvider("idp1"))
                .thenReturn(new IdentityProvider());
        when(identityProviderManager.get("idp1")).thenReturn(authenticationProvider);

        Authentication authentication =
                managementAuthenticationProvider.authenticate(
    ...
    public void shouldAuthenticate_secondAuthProvider() {
        ...
        when(authenticationProvider2.loadUserByUsername(
                        any(io.gravitee.am.identityprovider.api.Authentication.class)))
                .thenReturn(Maybe.just(new DefaultUser("username")));
        when(identityProviderManager.getIdentityProvider("idp1"))
                .thenReturn(new IdentityProvider());
        when(identityProviderManager.getIdentityProvider("idp2"))
                .thenReturn(new IdentityProvider());
        when(identityProviderManager.get("idp1")).thenReturn(authenticationProvider);
