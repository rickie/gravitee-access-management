...
public class CibaNotifierWebSockerHandler implements Handler<ServerWebSocket> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(CibaNotifierWebSockerHandler.class);
    ...
    public void handle(ServerWebSocket ws) {
        ws.textMessageHandler(
                msg -> {
                    ...
                    } else if (Objects.equals(action, ACTION_VALIDATE)
                            || Objects.equals(action, ACTION_REJECT)) {
                        doCallback(jsonMsg);
                    }
                });
    }
    ...
    private void doCallback(JsonObject jsonMsg) {
        ...
        try {
            ...
            if (optCallback.isPresent()) {
                ...
                formData.set(
                        CALLBACK_VALIDATE,
                        Boolean.toString(Objects.equals(action, ACTION_VALIDATE)));
                ...
                this.webClient
                        ...
                        .sendForm(formData)
                        .onSuccess(
                                res -> LOGGER.info("Callback succeeded for tid {}", transactionId))
                        .onFailure(
                                err ->
                                        LOGGER.warn(
            ...
            LOGGER.warn(
                    "Unable to parse the state {} for transactionId {}", state, transactionId, ex);
        }
    }
}
