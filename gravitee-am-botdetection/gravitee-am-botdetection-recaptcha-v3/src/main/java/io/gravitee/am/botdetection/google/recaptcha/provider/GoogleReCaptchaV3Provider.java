...
public class GoogleReCaptchaV3Provider implements BotDetectionProvider {
    ...
    @Autowired private GoogleReCaptchaV3Configuration configuration;
        ...
        return client.post(URI.create(configuration.getServiceUrl()).toString())
                .rxSendForm(
                        MultiMap.caseInsensitiveMultiMap()
                                .set("secret", configuration.getSecretKey())
                                .set("response", token))
                .map(
                        buffer -> {
                            if (buffer.statusCode() != 200) {
                                LOGGER.error(
                                        "An error occurred when trying to validate ReCaptcha token. (status={}/message={})",
                                        buffer.statusCode(),
                ...
                .onErrorResumeNext(
                        throwable -> {
                            LOGGER.error(
                                    "An error occurred when trying to validate ReCaptcha token.",
                                    throwable);
                            return Single.just(false);
                        });
    }
