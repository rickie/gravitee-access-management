...
public enum Permission {
    ...
    DOMAIN_EMAIL_TEMPLATE(
            ReferenceType.ORGANIZATION, ReferenceType.ENVIRONMENT, ReferenceType.DOMAIN),
        ...
        if (permissions != null) {
            permissions.forEach(
                    (key, value) ->
                            value.forEach(
                                    acl ->
                                            flattenedPermissions.add(
                                                    key.name().toLowerCase()
                                                            + "_"
                                                            + acl.name().toLowerCase())));
        }
                    ...
                    .forEach(
                            flatPermission -> {
                                ...
                                Permission permission =
                                        Permission.valueOf(flatPermission.substring(0, i));
